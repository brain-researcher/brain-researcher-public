...
class TestSPARQLEndpoint:
    ...
    def sparql_endpoint(self, mock_neo4j_db):
        ...
        with patch(
            "brain_researcher.services.br_kg.sparql.endpoint.SPARQLToCypherTranslator"
        ):
            with patch(
                "brain_researcher.services.br_kg.sparql.endpoint.FederationQueryHandler"
            ):
                endpoint = SPARQLEndpoint(
                    neo4j_db=mock_neo4j_db,
                    base_uri="https://test.br_kg.org/",
    ...
    def test_blueprint_creation(self, sparql_endpoint):
        ...
        [rule.rule for rule in blueprint.deferred_functions]
    ...
    def test_get_request_parsing(self, app, sparql_endpoint):
        ...
        with app.test_client() as client:
            ...
            sparql_endpoint._execute_sparql_query = Mock(
                return_value={
                    "head": {"vars": ["subject"]},
                    "results": {"bindings": []},
                }
    ...
    def test_post_request_parsing_sparql_query(self, app, sparql_endpoint):
        ...
        with app.test_client() as client:
            ...
            response = client.post(
                "/sparql", data=query, content_type="application/sparql-query"
            )
    ...
    def test_unsupported_content_type(self, app):
        ...
        with app.test_client() as client:
            response = client.post(
                "/sparql", data="SELECT * WHERE { ?s ?p ?o }", content_type="text/plain"
            )
    ...
    def test_query_caching(self, sparql_endpoint):
        ...
        # Setup
        sparql_endpoint.translator.translate_query = Mock(
            return_value=("MATCH (n) RETURN n", {})
        )
        sparql_endpoint._execute_cypher_query = Mock(
            return_value=[{"n": {"id": "test"}}]
        )
        ...
        with patch(
            ...
            result1 = sparql_endpoint._execute_sparql_query(
                "SELECT * WHERE { ?s ?p ?o }"
            )

            # Second execution - should hit cache
            result2 = sparql_endpoint._execute_sparql_query(
                "SELECT * WHERE { ?s ?p ?o }"
            )
    ...
    def test_format_select_result(self, sparql_endpoint):
        ...
        sparql_endpoint._extract_variables = Mock(
            return_value=["subject", "predicate", "object"]
        )
    ...
    def test_format_construct_result(self, sparql_endpoint):
        ...
        mock_parsed_query = Mock()
        result = sparql_endpoint._format_construct_result(
            neo4j_result, mock_parsed_query
        )
                    ...
                    "triples": [
                        {
                            "subject": {
                                "type": "uri",
                                "value": "https://example.org/s1",
                            },
                            "predicate": {
                                "type": "uri",
                                "value": "https://example.org/p1",
                            },
    ...
    def test_federation_query_detection(self, sparql_endpoint):
        ...
        # Mock parsed query
        mock_parsed_query = Mock()
        mock_parsed_query.__str__ = Mock(
            return_value="SELECT * WHERE { SERVICE <http://dbpedia.org/sparql> { ?s ?p ?o } }"
        )
    ...
    def test_cache_key_generation(self, sparql_endpoint):
        ...
        key3 = sparql_endpoint._get_cache_key(
            "Different query", default_graphs, named_graphs
        )
        assert key1 != key3
    ...
    def test_performance_metrics_update(self, sparql_endpoint):
        ...
        expected_avg = ((initial_avg * initial_total) + execution_time) / (
            initial_total + 1
        )
        assert (
            abs(sparql_endpoint.query_metrics["avg_execution_time"] - expected_avg)
            < 0.001
        )
    ...
    def test_get_query_type(self, sparql_endpoint):
        ...
        describe_query = Mock()
        describe_query.__str__ = Mock(
            return_value="DESCRIBE <http://example.org/resource>"
        )
...
class TestSPARQLEndpointEdgeCases:
    ...
    def test_none_values_in_result(self, minimal_endpoint):
        ...
        mock_parsed_query = Mock()
        minimal_endpoint._extract_variables = Mock(
            return_value=["subject", "predicate", "object"]
        )
    ...
    def test_large_result_set_handling(self, minimal_endpoint):
        ...
        large_result = [
            {
                "subject": f"https://example.org/s{i}",
