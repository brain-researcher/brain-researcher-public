...
from brain_researcher.services.br_kg import entity_cache
...
def app_module(monkeypatch):
    ...
    # The entity cache substrate lives in ``entity_cache`` (a singleton that is
    # NOT popped on app reimport), so its module-level cache state would leak
    # across tests. Reset it here to give each test fresh caches, matching the
    # pre-carve behaviour where these globals lived on the reimported app module.
    entity_cache._TASK_TREE_CACHE.clear()
    entity_cache._DISEASE_ENTITY_CACHE.clear()
    entity_cache._TASK_ENTITY_CACHE.clear()
    entity_cache._TASK_ENTITY_SINGLEFLIGHT_LOCKS.clear()
    entity_cache._TASK_ENTITY_REDIS_INITIALIZED = False
    entity_cache._TASK_ENTITY_REDIS_CLIENT = None
    br_kg_app = importlib.import_module("brain_researcher.services.br_kg.app")
    monkeypatch.setattr(br_kg_app, "performance_monitor", _DummyPerfMonitor())
    monkeypatch.setattr(br_kg_app, "neo4j_db", _LensStubNeo4j())
    monkeypatch.setattr(br_kg_app, "BR_KG_LENSES_V1", True)
    return br_kg_app
...
def test_task_family_profile_calibrated_defaults(monkeypatch):
    ...
    monkeypatch.setenv("BR_KG_TASK_FAMILY_PROFILE", "calibrated_v1")
    monkeypatch.delenv("BR_KG_TASK_FAMILY_FUZZY_THRESHOLD", raising=False)
    monkeypatch.delenv(
    ...
    monkeypatch.delenv("BR_KG_TASK_FAMILY_AMBIGUITY_MARGIN", raising=False)
    monkeypatch.setattr(neo4j_utils, "require_neo4j_db", lambda **_kwargs: object())
    sys.modules.pop("brain_researcher.services.br_kg.app", None)
    app = importlib.import_module("brain_researcher.services.br_kg.app")

    assert app.BR_KG_TASK_FAMILY_PROFILE == "calibrated_v1"
    assert app.BR_KG_TASK_FAMILY_FUZZY_THRESHOLD == pytest.approx(0.82)
    assert app.BR_KG_TASK_FAMILY_AGGRESSIVE_PRIMARY_THRESHOLD == pytest.approx(0.68)
    assert app.BR_KG_TASK_FAMILY_AGGRESSIVE_SECONDARY_THRESHOLD == pytest.approx(0.60)
    assert app.BR_KG_TASK_FAMILY_AMBIGUITY_MARGIN == pytest.approx(0.03)
...
def test_task_summary_cache_headers(monkeypatch, app_module):
    entity_cache._TASK_ENTITY_CACHE.clear()
    entity_cache._TASK_ENTITY_REDIS_INITIALIZED = True
    entity_cache._TASK_ENTITY_REDIS_CLIENT = None
    monkeypatch.setattr(entity_cache, "BR_KG_TASK_ENTITY_REDIS_URL", "")
...
def test_lens_task_tree_uses_cache_for_repeated_requests(monkeypatch, app_module):
    ...
    monkeypatch.setattr(entity_cache, "BR_KG_TASK_TREE_CACHE_TTL_SECONDS", 300.0)
    entity_cache._TASK_TREE_CACHE.clear()
    client = app_module.app.test_client()
...
def test_lens_flag_disabled_returns_404(monkeypatch, app_module):
    monkeypatch.setattr(app_module, "BR_KG_LENSES_V1", False)
    client = app_module.app.test_client()
