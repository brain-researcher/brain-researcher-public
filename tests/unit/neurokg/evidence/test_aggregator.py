...
from brain_researcher.services.neurokg.evidence.aggregator import (
    gather_evidence,
...
class TestGatherEvidence:
    ...
    @pytest.mark.asyncio
    async def test_gather_single_source(self, mock_neurokg_connector):
        """Test gathering from a single source."""
        bundle = await gather_evidence(
            "working memory",
            connectors=[mock_neurokg_connector],
        )

        assert isinstance(bundle, EvidenceBundle)
        assert bundle.query == "working memory"
        assert EvidenceSource.NEUROKG in bundle.sources_queried
        assert len(bundle.items) == 1

    @pytest.mark.asyncio
    async def test_gather_multiple_sources(
        self, mock_neurokg_connector, mock_dataset_connector
    ):
        """Test gathering from multiple sources in parallel."""
        bundle = await gather_evidence(
            "motor task",
            connectors=[mock_neurokg_connector, mock_dataset_connector],
        )
    ...
    async def test_gather_with_source_filter(
        ...
        bundle = await gather_evidence(
            "test query",
            sources=[EvidenceSource.NEUROKG],
            connectors=[mock_neurokg_connector, mock_dataset_connector],
        )

        # Only BR-KG should be queried
        assert bundle.sources_queried == [EvidenceSource.NEUROKG]

    @pytest.mark.asyncio
    async def test_gather_with_limit(self, mock_neurokg_connector, sample_evidence_items):
        """Test limiting results per source."""
        # Create connector with multiple items
        from tests.unit.neurokg.evidence.conftest import MockConnector

        connector = MockConnector(EvidenceSource.NEUROKG, sample_evidence_items * 3)
    ...
    async def test_gather_skips_unavailable(
        self, mock_neurokg_connector, mock_unavailable_connector
    ):
        """Test that unavailable connectors are skipped."""
        bundle = await gather_evidence(
            "test",
            connectors=[mock_neurokg_connector, mock_unavailable_connector],
        )
    ...
    @pytest.mark.asyncio
    async def test_gather_records_query_time(self, mock_neurokg_connector):
    ...
    async def test_gather_partial_failure(
        self, mock_neurokg_connector, mock_failing_connector
    ):
        """Test that partial failures don't prevent successful results."""
        bundle = await gather_evidence(
            "test",
            connectors=[mock_neurokg_connector, mock_failing_connector],
        )
...
class TestInferSourcesFromQur:
    ...
    def test_always_includes_neurokg(self):
        ...
        assert EvidenceSource.NEUROKG in sources
