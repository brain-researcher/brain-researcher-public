...
class TestGNNModels:
    ...
    def mock_torch(self):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.TORCH_AVAILABLE', True):
            ...
            with patch.multiple(
                'brain_researcher.services.neurokg.ml.gnn_models',
                torch=mock_torch,
                nn=mock_nn,
                F=mock_f,
                Adam=Mock,
    ...
    def test_gcn_initialization(self, mock_torch):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.GCNConv', MockModule):
    ...
    def test_graphsage_initialization(self, mock_torch):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.SAGEConv', MockModule):
    ...
    def test_gat_initialization(self, mock_torch):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.GATConv', MockModule):
...
class TestGNNPredictor:
    ...
    def mock_torch_available(self):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.TORCH_AVAILABLE', True):
            with patch('brain_researcher.services.neurokg.ml.gnn_models.torch') as mock_torch:
                mock_torch.device.return_value = "cpu"
                mock_torch.cuda.is_available.return_value = False
                mock_torch.zeros.return_value = MockTensor(np.zeros((10, 5)))
                mock_torch.tensor.return_value = MockTensor()
    ...
    def test_build_gcn_model(self, mock_torch_available):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.GraphConvolutionalNetwork', MockModule):
            with patch('brain_researcher.services.neurokg.ml.gnn_models.Adam', Mock):
    ...
    def test_build_graphsage_model(self, mock_torch_available):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.GraphSAGE', MockModule):
            with patch('brain_researcher.services.neurokg.ml.gnn_models.Adam', Mock):
    ...
    def test_build_gat_model(self, mock_torch_available):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.GraphAttentionNetwork', MockModule):
            with patch('brain_researcher.services.neurokg.ml.gnn_models.Adam', Mock):
    ...
    def test_prepare_graph_data_with_features(self, mock_torch_available):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.Data', Mock) as mock_data:
            with patch('brain_researcher.services.neurokg.ml.gnn_models.torch') as mock_torch:
    ...
    def test_prepare_graph_data_without_features(self, mock_torch_available):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.Data', Mock):
            with patch('brain_researcher.services.neurokg.ml.gnn_models.torch') as mock_torch:
    ...
    def test_node_classification_training(self, mock_torch_available):
        ...
        with patch.object(predictor, 'prepare_graph_data') as mock_prep:
            ...
            with patch('brain_researcher.services.neurokg.ml.gnn_models.torch') as mock_torch:
                with patch('brain_researcher.services.neurokg.ml.gnn_models.F') as mock_f:
...
class TestTorchUnavailable:
    ...
    def test_import_error_on_model_creation(self):
        ...
        with patch('brain_researcher.services.neurokg.ml.gnn_models.TORCH_AVAILABLE', False):
