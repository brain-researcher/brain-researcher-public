...
class TestBaseGraphEmbedder:
    ...
    def mock_deps(self):
        """Mock dependencies."""
        with patch('brain_researcher.services.neurokg.ml.graph_embeddings.DEPS_AVAILABLE', True):
            with patch('brain_researcher.services.neurokg.ml.graph_embeddings.nx', MockNetworkX):
                yield
    ...
    def test_initialization_without_deps(self):
        ...
        with patch('brain_researcher.services.neurokg.ml.graph_embeddings.DEPS_AVAILABLE', False):
            config = EmbeddingConfig(embedding_type=EmbeddingType.NODE2VEC)
...
class TestNode2VecEmbedder:
    ...
    def mock_deps(self):
        ...
        with patch('brain_researcher.services.neurokg.ml.graph_embeddings.DEPS_AVAILABLE', True):
            with patch('brain_researcher.services.neurokg.ml.graph_embeddings.nx', MockNetworkX):
                with patch('brain_researcher.services.neurokg.ml.graph_embeddings.Word2Vec', MockWord2Vec):
                    yield
...
class TestGraphEmbedder:
    ...
    def test_visualization_without_matplotlib(self, mock_deps):
        ...
        with patch('brain_researcher.services.neurokg.ml.graph_embeddings.plt', None):
            embedder = GraphEmbedder(EmbeddingType.NODE2VEC)
    ...
    def test_visualization_empty_embeddings(self, mock_deps):
        ...
        with patch('brain_researcher.services.neurokg.ml.graph_embeddings.plt', Mock()):
            embedder = GraphEmbedder(EmbeddingType.NODE2VEC)
    ...
    def test_visualization_with_embeddings(self, mock_deps):
        ...
        with patch('brain_researcher.services.neurokg.ml.graph_embeddings.plt') as mock_plt:
            with patch('brain_researcher.services.neurokg.ml.graph_embeddings.TSNE') as mock_tsne:
                embedder = GraphEmbedder(EmbeddingType.NODE2VEC)
