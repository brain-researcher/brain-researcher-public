...
def test_publication_anchor_lookup_terms_include_publication_identifiers() -> None:
    entity = query_service.KGNodeSummary(
        ...
        node_type="Publication",
        properties={"doi": "10.1101/2025.07.21.665938", "title": "Rapid decoding paper"},
    )
...
def test_normalize_graph_node_preserves_inner_properties_shape() -> None:
    ...
    normalized = query_service._normalize_graph_node(payload, default_type="Publication")
...
def test_search_tools_structured_exception_sets_fallback_reason(monkeypatch):
    ...
    assert captured["fallback_reason"] == "neurokg_error:RuntimeError"
...
def test_verify_hypothesis_candidate_lane_mode_strict_filters_candidate_rows(
    ...
    monkeypatch.setattr(query_service, "search_nodes", lambda *args, **kwargs: [concept])
    monkeypatch.setattr(query_service, "node_details", lambda *_a, **_k: None)
    ...
    assert [
        item["claim"]["kg_id"] for item in strict["supporting_evidence"] if item["claim"]
    ] == ["claim:benchmark"]
    assert any(
...
def test_verify_hypothesis_candidate_lane_mode_strict_falls_back_to_benchmark_union(
    ...
    assert result["supporting_evidence"]
    assert any("candidate-lane evidence row" in warning for warning in result["warnings"])
...
def test_multi_hop_traverse_passes_per_query_timeout(monkeypatch):
    ...
    monkeypatch.setattr(
        "brain_researcher.services.neurokg.traversal.multi_hop_queries.MultiHopQueryEngine",
        FakeEngine,
    )
