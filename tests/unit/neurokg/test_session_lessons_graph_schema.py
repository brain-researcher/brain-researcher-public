...
def _digest() -> dict[str, object]:
    return {
        ...
        "snapshot": {
            ...
            "next_command": "pytest -q tests/unit/neurokg/test_session_lessons_graph_schema.py",
        },
        "done_items": [
...
def test_session_graph_payload_has_no_dangling_edges() -> None:
    ...
    session_node = next(node for node in graph["nodes"] if "AgentSession" in node["labels"])
