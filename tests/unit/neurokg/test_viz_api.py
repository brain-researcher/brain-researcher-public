...
def _load_viz_api(monkeypatch, *, template_root: Path, dataset_root: Path, job_root: Path):
    monkeypatch.setenv("BR_NEUROKG_VIZ_TEMPLATE_ROOTS", str(template_root))
    monkeypatch.setenv("BR_NEUROKG_VIZ_DATASET_ROOTS", str(dataset_root))
    monkeypatch.setenv("BR_NEUROKG_VIZ_JOB_ROOTS", str(job_root))
    monkeypatch.setenv("BR_NEUROKG_VIZ_DEFAULT_TEMPLATE", "mni152")
    monkeypatch.setenv("BR_NEUROKG_VIZ_DEFAULT_DATASET", "openneuro/ds-test")

    module = importlib.import_module("brain_researcher.services.neurokg.viz_api")
