...
class TestIstioServiceDiscovery:
    ...
    def service_registry(self, mock_k8s_client):
        ...
        from brain_researcher.infrastructure.istio.service_registry import IstioServiceRegistry
    ...
    def test_register_service(self, service_registry):
        ...
        assert "neurokg-service" in service_registry.services
    
    def test_discover_services(self, service_registry):
        """Test service discovery through Istio."""
        # Register multiple services
        services = [
            {"name": "neurokg-service", "port": 5001, "labels": {"app": "neurokg"}},
        ...
        discovered = service_registry.discover_services(label_selector="app=neurokg")
        
        assert len(discovered) == 1
        assert discovered[0]["name"] == "neurokg-service"
...
class TestIstioTrafficManagement:
    ...
    def traffic_manager(self, mock_k8s_client, mock_istio_config):
        ...
        from brain_researcher.infrastructure.istio.traffic_manager import IstioTrafficManager
    ...
    def test_create_virtual_service(self, traffic_manager):
        ...
        result = traffic_manager.create_virtual_service("neurokg-vs", vs_config)
        
        assert result is True
        assert "neurokg-vs" in traffic_manager.config.virtual_services
    ...
    def test_configure_load_balancing(self, traffic_manager):
        ...
        result = traffic_manager.create_destination_rule("neurokg-dr", dr_config)
        
        assert result is True
        assert "neurokg-dr" in traffic_manager.config.destination_rules
        ...
        split_config = {
            ...
            "spec": {
                ...
                "http": [{
                    "route": [
                        {
                            "destination": {"host": "neurokg-service", "subset": "v1"},
                            "weight": 80
                        },
                        {
                            "destination": {"host": "neurokg-service", "subset": "v2"},
                            "weight": 20
                        }
                    ]
                }]
...
class TestIstioSecurityPolicies:
    ...
    def test_create_authorization_policy(self, security_manager):
        ...
        policy_config = {
            ...
            "spec": {
                "selector": {
                    "matchLabels": {"app": "neurokg"}
                },
                "rules": [{
                    "from": [{
                        "source": {
                            "principals": ["cluster.local/ns/brain-researcher/sa/agent-service"]
                        }
                    }],
                    "to": [{
                        "operation": {
        ...
        result = security_manager.create_authorization_policy("neurokg-access", policy_config)
    ...
    def test_jwt_validation(self, security_manager):
        ...
        result = security_manager.configure_jwt_validation("jwt-auth", request_auth_config)
...
class TestIstioObservability:
    ...
    def test_configure_telemetry(self, observability_manager):
        ...
        result = observability_manager.configure_telemetry("metrics-config", telemetry_config)
    ...
    def test_metrics_collection(self, mock_get, observability_manager):
        ...
        metrics = observability_manager.get_service_metrics("neurokg-service")
...
class TestIstioBridge:
    ...
    def test_circuit_breaker_configuration(self, istio_bridge):
        ...
        result = istio_bridge.configure_circuit_breaker("neurokg-service", cb_config)
    ...
    def test_retry_policy_configuration(self, istio_bridge):
        ...
        result = istio_bridge.configure_retry_policy("neurokg-service", retry_config)
    ...
    def test_timeout_configuration(self, istio_bridge):
        ...
        result = istio_bridge.configure_timeout("neurokg-service", timeout_config)
...
class TestIstioConfiguration:
    ...
    def test_configuration_validation(self):
        ...
        from brain_researcher.infrastructure.istio.config_validator import IstioConfigValidator
    ...
    def test_configuration_templating(self):
        ...
        from brain_researcher.infrastructure.istio.config_templates import IstioConfigTemplates
...
class TestIstioIntegration:
    ...
    @pytest.mark.skipif(
        not pytest.importorskip("kubernetes"),
