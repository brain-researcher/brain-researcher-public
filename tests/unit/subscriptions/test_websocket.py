...
from brain_researcher.services.neurokg.subscriptions.websocket import (
    EventType,
    SubscriptionFilter,
    GraphEvent,
    Subscription,
...
class TestSubscription:
    ...
    def test_matches_event_by_properties(self):
        ...
        event1 = GraphEvent(
            ...
            data={
                'node_id': 'A',
                'properties': {'status': 'active', 'priority': 'high', 'other': 'value'}
            }
        )
        assert sub.matches_event(event1) is True
...
class TestConnectionManager:
    ...
    async def test_distribute_event(self, manager):
        ...
        sub2 = await manager.subscribe('client2', filter2)
...
class TestWebSocketIntegration:
    ...
    async def test_subscription_workflow(self):
        ...
        sub_id = await manager.subscribe('test_client', filter)
