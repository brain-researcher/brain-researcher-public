...
class TestVectorSearchEngine:
    ...
    def engine(self):
        ...
        with patch(
            "brain_researcher.services.br_kg.search.vector_search.TRANSFORMERS_AVAILABLE",
            False,
        ):
            return VectorSearchEngine(store_type=VectorStoreType.FAISS, dimension=128)
    ...
    def test_search_vector_mode(self, engine):
        ...
        # Index documents
        documents = [
            {"id": f"doc{i}", "content": f"Document {i} content"} for i in range(20)
        ]
        engine.index_documents(documents)

        # Search
        results = engine.search(query="test query", k=5, mode=SearchMode.VECTOR)
    ...
    def test_search_hybrid_mode(self, engine):
        ...
        # Hybrid search
        results = engine.search(query="neural networks", k=3, mode=SearchMode.HYBRID)
    ...
    def test_search_with_filters(self, engine):
        ...
        # Search with filter
        results = engine.search(query="analysis", k=10, filters={"category": "imaging"})
    ...
    def test_search_with_rerank(self, engine):
        ...
        results = engine.search(query="test", k=5, rerank=True)
    ...
    def test_update_document(self, engine):
        ...
        engine.update_document(
            "doc1", content="Updated content", metadata={"version": 2}
        )
        ...
        if results:
            assert (
                "Updated" in results[0].content
                or results[0].metadata.get("version") == 2
            )
...
class TestPineconeIntegration:
    ...
    @patch(
        "brain_researcher.services.br_kg.search.vector_search.PINECONE_AVAILABLE", True
    )
    @patch("brain_researcher.services.br_kg.search.vector_search.pinecone", create=True)
