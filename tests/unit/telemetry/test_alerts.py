...
class TestAlertThreshold:
    ...
    def test_threshold_greater_than(self):
        ...
        assert threshold.evaluate(2500.0)
        assert not threshold.evaluate(1500.0)
        assert not threshold.evaluate(2000.0)
    ...
    def test_threshold_less_than(self):
        ...
        assert not threshold.evaluate(98.0)
    ...
    def test_threshold_with_duration(self):
        ...
        assert threshold.evaluate(6.0, duration_met=True)
        assert not threshold.evaluate(6.0, duration_met=False)
...
class TestNotificationManager:
    ...
    async def test_send_webhook_notification(self, notification_config):
        ...
        with patch("requests.post") as mock_post:
            ...
            call_args = mock_post.call_args
            assert not call_args.kwargs["json"]["is_resolution"]
...
class TestAlertManager:
    ...
    def test_should_re_alert(self, alert_manager):
        ...
        assert alert_manager._should_re_alert(alert1, rule)
        ...
        assert alert_manager._should_re_alert(alert2, rule)
        ...
        assert not alert_manager._should_re_alert(alert3, rule)
