...
from brain_researcher.services.neurokg.versioning.snapshot import (
    GraphSnapshot,
    GraphVersioning,
    SnapshotType,
    ChangeType
...
class TestGraphVersioning:
    ...
    def test_time_travel_query(self, versioning, sample_graph):
        ...
        snapshot2_id = versioning.create_snapshot(sample_graph)
    ...
    def test_compare_versions(self, versioning):
        ...
        assert ('B', 'C') in diff['edges']['added'] or ('C', 'B') in diff['edges']['added']
        assert diff['summary']['total_changes'] == 3  # 1 node added, 1 modified, 1 edge added
    ...
    def test_merge_branches(self, versioning):
        ...
        base = versioning.create_snapshot(base_graph)
    ...
    def test_calculate_diff(self, versioning):
        ...
        node_added = next(c for c in changes if c['type'] == ChangeType.NODE_ADDED.value)
