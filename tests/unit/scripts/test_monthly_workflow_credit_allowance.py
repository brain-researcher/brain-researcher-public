...
from scripts.billing import monthly_workflow_credit_allowance as allowance_script


@pytest.mark.asyncio
async def test_monthly_workflow_credit_allowance_dry_run_reports_separate_accounts(
    ...
    assert (
        store.get_balance("default", "user_with_spend")["balance_milli"]
        == 6_000
    )
    assert store.get_balance("default", "user_empty")["balance_milli"] == 0


@pytest.mark.asyncio
async def test_monthly_workflow_credit_allowance_apply_is_idempotent(
    tmp_path, monkeypatch
):
    store = CreditsStore(str(tmp_path / "credits.sqlite"))
    ...
    assert first["results"][0]["bucket"] == WORKFLOW_RUNTIME_BUCKET
    assert first["results"][0]["currency"] == WORKFLOW_RUNTIME_CURRENCY
    assert second["results"][0]["idempotent"] is True
    assert store.get_balance("default", "user_active")["balance_milli"] == 10_000


def test_monthly_workflow_credit_allowance_cli_defaults_to_dry_run_json(
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "monthly_workflow_credit_allowance.py",
            "--month",
            "2026-05",
            "--workspace-id",
            "default",
    ...
    assert store.get_balance("default", "user_cli")["balance_milli"] == 0
