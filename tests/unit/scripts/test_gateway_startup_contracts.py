...
def test_start_services_uses_active_five_service_matrix() -> None:
    ...
    assert 'start_service "orchestrator" 3001 "http://127.0.0.1:3001/health" 45 \\' in script
...
def test_ci_docker_matrix_does_not_build_legacy_api_gateway_image() -> None:
    ...
    assert "service: [orchestrator, agent, neurokg, web-ui]" in workflow
    assert "service: [orchestrator, agent, neurokg, web-ui, api-gateway]" not in workflow
...
def test_blue_green_rollout_omits_legacy_api_gateway_service() -> None:
    ...
    assert 'SERVICES=("orchestrator" "neurokg" "agent" "web-ui")' in script
...
def test_sync_web_is_explicitly_legacy_and_uses_web_ui_paths() -> None:
    ...
    assert 'DEST_DIR="${BR_SYNC_WEB_DEST:-$ROOT_DIR/artifacts/web-ui-static-export}"' in script
    assert 'SRC_DIR="$ROOT_DIR/apps/web"' not in script
    assert 'DEST_DIR="$ROOT_DIR/brain_researcher/services/neurokg/web_public"' not in script
