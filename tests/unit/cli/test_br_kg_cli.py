"""CLI tests for br_kg ingest commands."""

from pathlib import Path

from typer.testing import CliRunner

from brain_researcher.cli.commands import br_kg_ingest
...
def test_br_kg_ingest_tools_catalog_help(tmp_path):
    result = runner.invoke(app, ["br-kg-ingest", "tools-catalog", "--help"])
    assert result.exit_code == 0
    assert "tools-catalog" in result.output


def test_br_kg_ingest_tools_catalog_help_nested(tmp_path):
    result = runner.invoke(app, ["br-kg", "ingest", "tools-catalog", "--help"])
    assert result.exit_code == 0
    assert "tools-catalog" in result.output


def test_br_kg_ingest_tools_catalog_ingests_with_stub(tmp_path, monkeypatch):
...
tools:
  - id: demo.tool
    ...
    monkeypatch.setattr(br_kg_ingest, "Neo4jGraphDB", StubDB)
    monkeypatch.setattr(br_kg_ingest.tools_catalog_loader, "ingest", stub_ingest)
...
def test_br_kg_ingest_tools_catalog_dry_run(tmp_path):
    cat = tmp_path / "caps.yaml"
    cat.write_text(
        """
tools:
...
def test_br_kg_ingest_allen_ccfv3_dry_run(monkeypatch):
    calls = {}
    ...
    monkeypatch.setattr(br_kg_ingest, "AllenBrainUnifiedLoader", StubAllenLoader)
    monkeypatch.setattr(br_kg_ingest, "Neo4jGraphDB", StubDB)

    result = runner.invoke(app, ["br-kg-ingest", "allen-ccfv3", "--dry-run"])
...
def test_br_kg_ingest_allen_ccfv3_ingests_with_stub(monkeypatch):
    calls = {}
