...
class MockPushNotificationManager:
    def __init__(self, options=None):
        self.vapid_public_key = options.get('vapidPublicKey', 'test-key') if options else 'test-key'
        self.subscription_endpoint = options.get('subscriptionEndpoint', '/api/push/subscribe') if options else '/api/push/subscribe'
...
class TestPushNotificationManager:
    ...
    def test_initialization(self):
        ...
        assert self.manager.vapid_public_key == 'BNJxw7sCGkGLOUP2cawBaBXRuWZ8-MCkqRyTWJfgRtPw'
    ...
    async def test_subscription_creation(self):
        ...
        with patch('builtins.fetch', new_callable=AsyncMock, create=True) as mock_fetch:
            ...
            assert self.manager.subscription['endpoint'] == 'https://fcm.googleapis.com/endpoint/123'
    ...
    async def test_subscription_backend_sync(self):
        ...
        with patch('builtins.fetch', new_callable=AsyncMock, create=True) as mock_fetch:
            ...
            await mock_fetch('/api/push/subscribe', {
                'method': 'POST',
    ...
    async def test_test_notification(self):
        ...
        with patch('builtins.Notification', create=True) as mock_notification_constructor:
            ...
            if self.manager.get_permission_status() == 'granted':
                notification = mock_notification_constructor('Brain Researcher Test', {
...
class TestPushNotificationUtils:
    ...
    def test_mobile_device_detection(self):
        ...
        def is_mobile_device(user_agent):
            return any(mobile in user_agent for mobile in ['iPhone', 'Android', 'iPad', 'Mobile'])
    ...
    def test_brain_notification_formatting(self):
        ...
        def format_brain_notification_body(notification_type, data):
            ...
            return templates.get(notification_type, data.get('body', 'Brain Researcher notification'))
        
        # Test analysis complete with significant findings
        data1 = {
            'analysisName': 'fMRI Connectivity',
        ...
        data2 = {
            'analysisName': 'Structural Analysis',
            'significantFindings': False
        }
        ...
        data4 = {
            ...
            'projectName': 'Multi-site fMRI Study'
        }
...
class TestPushNotificationIntegration:
    ...
    async def test_analysis_complete_notification_flow(self):
        ...
        with patch('builtins.fetch', new_callable=AsyncMock, create=True) as mock_fetch:
            ...
            mock_response = Mock()
            mock_response.ok = True
            mock_response.json = AsyncMock(return_value={'sent': True, 'messageId': 'msg_123'})
            mock_fetch.return_value = mock_response
            
            # Simulate sending notification
            await mock_fetch('/api/push/send', {
                'method': 'POST',
                'headers': {'Content-Type': 'application/json'},
        ...
        with patch('builtins.fetch', new_callable=AsyncMock, create=True) as mock_fetch:
            ...
            alert_scenarios = [
                ...
                },
                {
                    ...
                    'affectedServices': ['neurokg', 'finder']
                },
                {
    ...
    def test_notification_action_handling(self):
        ...
        for action_name, action_data in notification_actions.items():
            result = handle_notification_action(action_data['action'], action_data['data'])
    ...
    async def test_notification_scheduling(self):
        ...
        def should_deliver_immediately(notification_type, current_hour, user_preferences):
        ...
        test_cases = [
            ...
            {'type': 'analysis-complete', 'hour': 14, 'preferences': {}, 'expected': True},
            # During quiet hours - should not deliver
            {'type': 'analysis-complete', 'hour': 23, 'preferences': {'quietHoursStart': 22, 'quietHoursEnd': 8}, 'expected': False},
            ...
            {'type': 'data-update', 'hour': 6, 'preferences': {'quietHoursStart': 22, 'quietHoursEnd': 8}, 'expected': False},
        ]
        
        for case in test_cases:
            result = should_deliver_immediately(case['type'], case['hour'], case['preferences'])
    ...
    def test_notification_payload_validation(self):
        ...
        def validate_notification_payload(payload):
            ...
            # Validate type
            valid_types = ['analysis-complete', 'data-update', 'system-alert', 'default']
            ...
            if 'actions' in payload:
                for action in payload['actions']:
                    if not isinstance(action, dict) or 'action' not in action or 'title' not in action:
...
class TestPushNotificationMetrics:
    ...
    def test_engagement_analysis(self):
        ...
        def calculate_engagement_metrics(data):
            ...
            click_rate = clicked_notifications / total_notifications if total_notifications > 0 else 0
            
            # Calculate average time to click for clicked notifications
            clicked_times = [n['time_to_click'] for n in data if n['clicked'] and 'time_to_click' in n]
            avg_time_to_click = sum(clicked_times) / len(clicked_times) if clicked_times else 0
            ...
            for notif_type in type_engagement:
                ...
                type_engagement[notif_type]['click_rate'] = clicked / total if total > 0 else 0
    ...
    def test_subscription_health_monitoring(self):
        ...
        def check_subscription_health(subscriptions):
            ...
            if health_report['total_subscriptions'] > 0:
                health_report['health_score'] = health_report['active_subscriptions'] / health_report['total_subscriptions']
        ...
        subscriptions = [
            {
                'id': 'sub_1',
                'platform': 'chrome',
                'last_successful_delivery': (datetime.now() - timedelta(days=1)).isoformat()
            },
            {
                'id': 'sub_2',
                'platform': 'firefox',
                'last_successful_delivery': (datetime.now() - timedelta(days=15)).isoformat()
            },
            {
                'id': 'sub_3',
                'platform': 'safari',
                'last_successful_delivery': (datetime.now() - timedelta(days=45)).isoformat()
            }
        ]
