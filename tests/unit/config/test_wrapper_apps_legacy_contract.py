...
FORBIDDEN_SUBSTRINGS = {
    "apps/README.md": (
        ...
        "`neurokg/`: legacy module-wrapper shim for the BR-KG service",
        ...
        "`neurokg/`: BR-KG service wrapper and docs",
...
REMOVED_WRAPPER_FILES = (
    ...
    "apps/mcp/README.md",
    "apps/mcp/main.py",
    "apps/neurokg/README.md",
    "apps/neurokg/main.py",
...
def test_apps_tree_only_advertises_web_ui_as_active_app() -> None:
    for relpath, required in REQUIRED_SUBSTRINGS.items():
        ...
        for needle in required:
            assert needle in text, f"Missing expected wrapper-app text in {relpath}: {needle}"
...
def test_wrapper_apps_do_not_read_as_active_entrypoints() -> None:
    for relpath, forbidden in FORBIDDEN_SUBSTRINGS.items():
        ...
        for needle in forbidden:
            assert needle not in text, f"Found stale wrapper-app text in {relpath}: {needle}"


def test_wrapper_app_files_are_removed_from_tracked_surface() -> None:
    for relpath in REMOVED_WRAPPER_FILES:
        assert not (REPO_ROOT / relpath).exists(), f"Legacy wrapper file still exists: {relpath}"


def test_wrapper_app_dirs_are_removed_from_apps_tree() -> None:
    for relpath in REMOVED_WRAPPER_DIRS:
        assert not (REPO_ROOT / relpath).exists(), f"Legacy wrapper dir still exists: {relpath}"
