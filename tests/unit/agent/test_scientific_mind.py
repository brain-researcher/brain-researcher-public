...
class TestScientificMind:
    ...
    def test_confidence_scorer(self):
        ...
        res_high = tool.run(
            evidence_count=5, statistical_validation=True, contradictions=0
        )
        assert res_high["data"]["score"] > 0.8

        # Low confidence (failed validation)
        res_low = tool.run(
            evidence_count=5, statistical_validation=False, contradictions=0
        )
    ...
    def test_add_finding_mock(self):
        ...
        res = tool.run(description="Test finding", source_tool="pytest")
