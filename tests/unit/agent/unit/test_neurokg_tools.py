...
class TestFindRelatedConceptsTool:
    ...
    def test_skip_local_fallback_when_env_enabled(self, mock_get, mock_post):
        ...
        with patch.dict("os.environ", {"NEUROKG_FIND_RELATED_SKIP_LOCAL_FALLBACK": "1"}):
            tool = FindRelatedConceptsTool(query_service=fake_service)
            result = tool.run(concept="motor cortex", depth=1, limit=5)
...
class TestContrastToActivationMapTool:
    ...
    @patch(
        "brain_researcher.services.neurokg.niclip.contrast_text_orchestrator."
...
class TestTaskMappingTool:
    ...
    @patch("brain_researcher.services.neurokg.utils.task_matcher.TaskMatcher")
    ...
    def test_fallback_mapping(self):
        ...
        with patch.object(
            ...
            with patch(
                "brain_researcher.services.neurokg.utils.task_matcher.TaskMatcher",
...
class TestNeuroKGTools:
    """Test NeuroKGTools collection class."""
    ...
    def test_custom_api_url(self):
        ...
        tools = NeuroKGTools(api_url=custom_url)
...
class TestCrossToolIntegration:
    ...
    @patch("requests.get")
    @patch(
        "brain_researcher.services.neurokg.utils.niclip_concept_extractor.NiCLIPConceptExtractor"
    )
