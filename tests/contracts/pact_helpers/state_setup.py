...
class StateSetupManager:
    ...
    def register_state(self, state_name: str, setup_func: Callable, cleanup_func: Callable = None):
...
class OrchestratorStateSetup:
    ...
    def get_state_manager() -> StateSetupManager:
        ...
        manager.register_state(
            "no jobs exist",
            OrchestratorStateSetup._setup_no_jobs_exist
        )
        ...
        manager.register_state(
            "no threads exist",
            OrchestratorStateSetup._setup_no_threads_exist
        )
        
        # Dataset-related states
        manager.register_state(
            "datasets are available",
            OrchestratorStateSetup._setup_datasets_available
        )
        
        # User-related states
        manager.register_state(
            "user is authenticated",
            OrchestratorStateSetup._setup_user_authenticated
        )
        
        # Service health states
        manager.register_state(
            "all services are healthy",
            OrchestratorStateSetup._setup_services_healthy
        )
    ...
    @staticmethod
    async def _setup_completed_job_exists(job_id: str = "job_completed123") -> Dict[str, Any]:
    ...
    @staticmethod
    async def _setup_user_authenticated(user_id: str = "user_test123") -> Dict[str, Any]:
    ...
    async def _setup_services_healthy():
        ...
        health_data = {
            ...
            "services": {
                ...
                "neurokg": {"name": "neurokg-service", "status": "healthy"}
            }
        }
...
class AgentStateSetup:
    ...
    def get_state_manager() -> StateSetupManager:
        ...
        manager.register_state(
            ...
            AgentStateSetup._setup_agent_ready
        )
        
        manager.register_state(
            "agent is busy with other jobs",
            AgentStateSetup._setup_agent_busy
        )
        
        manager.register_state(
            "neurokg service is available",
            AgentStateSetup._setup_neurokg_available
        )
    ...
    @staticmethod
    async def _setup_neurokg_available():
        """Set up BR-KG available state."""
        logger.info("BR-KG service is available")
        return {"neurokg_status": "available"}


class NeuroKGStateSetup:
    ...
    def get_state_manager() -> StateSetupManager:
        ...
        manager.register_state(
            ...
            NeuroKGStateSetup._setup_kg_with_data
        )
        
        manager.register_state(
            "knowledge graph is empty", 
            NeuroKGStateSetup._setup_empty_kg
        )
