...
class TestAPIGatewayToOrchestratorContract:
    ...
    async def test_gateway_rate_limiting_contract(self, pact_client):
        ...
        async with pact_client as pact:
            ...
            (pact
             .given("orchestrator is rate limiting requests")
...
class TestAPIGatewayToNeuroKGContract:
    ...
    def pact_client(self):
        ...
        return PactClient(gateway_config, neurokg_config)
    
    @pytest.mark.asyncio
    async def test_gateway_neurokg_health_contract(self, pact_client):
        """Test BR-KG health check routing through gateway."""
        async with pact_client as pact:
            (pact
             .given("neurokg service is running")
             .upon_receiving("a neurokg health check from API gateway")
             .with_request(
                 method="GET",
                 path="/health",
                 headers={"X-Forwarded-By": "api-gateway"}
    ...
    async def test_gateway_public_dataset_search_contract(self, pact_client):
        ...
        async with pact_client as pact:
            ...
            # Execute the request
            response = await pact.execute_request(
                "POST", "/api/public/datasets/search",
                headers={
                    "Content-Type": "application/json",
                    "X-Forwarded-By": "api-gateway"
                },
    ...
    async def test_gateway_circuit_breaker_contract(self, pact_client):
        ...
        async with pact_client as pact:
            ...
            (pact
             .given("neurokg service is temporarily down")
             .upon_receiving("a request when neurokg circuit breaker is open")
             .with_request(
                 method="POST",
...
class TestAPIGatewayAuthenticationContract:
    ...
    async def test_gateway_invalid_token_contract(self, pact_client):
        ...
        async with pact_client as pact:
            (pact
             .given("user has invalid JWT token")
