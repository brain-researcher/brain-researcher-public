...
class TestOrchestratorToBRKGContract:
    ...
    def pact_client(self):
        ...
        return PactClient(orchestrator_config, br_kg_config)

    @pytest.mark.asyncio
    async def test_br_kg_health_check_contract(self, pact_client):
        """Test BR-KG health check endpoint contract."""
        async with pact_client as pact:
            (
                pact.given("br_kg service is running")
                .upon_receiving("a request for br_kg health status")
    ...
    async def test_get_dataset_details_contract(self, pact_client):
        ...
        async with pact_client as pact:
            (
                pact.given("dataset exists in knowledge graph")
                .upon_receiving("a request for dataset details")
                .with_request(method="GET", path=f"/api/datasets/{dataset_id}")
                .will_respond_with(
                    status=200,
                    headers={"Content-Type": "application/json"},
                    body={
    ...
    async def test_get_task_concept_relationships_contract(self, pact_client):
        ...
        async with pact_client as pact:
            (
                pact.given("knowledge graph has task-concept relationships")
                .upon_receiving("a request for task-concept relationships")
                .with_request(
                    method="GET", path="/api/tasks/motor%20execution/concepts"
                )
                .will_respond_with(
                    status=200,
                    headers={"Content-Type": "application/json"},
            ...
            # Execute the request
            response = await pact.execute_request(
                "GET", "/api/tasks/motor%20execution/concepts"
            )
    ...
    async def test_dataset_not_found_contract(self, pact_client):
        ...
        async with pact_client as pact:
            (
                pact.given("dataset does not exist")
    ...
    async def test_knowledge_graph_statistics_contract(self, pact_client):
        ...
        async with pact_client as pact:
            (
                pact.given("knowledge graph has data")
                .upon_receiving("a request for knowledge graph statistics")
                .with_request(method="GET", path="/api/stats")
                .will_respond_with(
                    status=200,
                    headers={"Content-Type": "application/json"},
                    body={
