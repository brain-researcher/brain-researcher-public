...
class TestOrchestratorProvider:
    ...
    async def test_job_exists_provider_state(self, state_manager):
        ...
        # Set up state
        job_data = await state_manager.setup_state("a job exists", {"job_id": "job_test123"})
    ...
    async def test_completed_job_provider_state(self, state_manager):
        ...
        # Set up state
        job_data = await state_manager.setup_state("a completed job exists", 
                                                  {"job_id": "job_completed123"})
        ...
        # Clean up state
        await state_manager.cleanup_state("a completed job exists", 
                                        {"job_id": "job_completed123"})
    ...
    async def test_thread_creation_provider_state(self, state_manager):
        ...
        # Set up state
        auth_data = await state_manager.setup_state("user is authenticated", 
    ...
    async def test_thread_exists_provider_state(self, state_manager):
        ...
        # Set up state
        thread_data = await state_manager.setup_state("a thread exists", 
        ...
        # Clean up state
        await state_manager.cleanup_state("a thread exists", 
                                        {"thread_id": "thread_test123"})
    ...
    async def test_agent_unavailable_provider_state(self, state_manager):
        ...
        assert health_data["services"]["neurokg"]["status"] == "healthy"
    ...
    def test_contract_backward_compatibility(self):
        ...
        comparison = VerificationHelper.compare_pact_versions(previous_pact, current_pact)
    ...
    def test_response_schema_validation(self):
        ...
        job_response_schema = {
            "type": "object",
            "required": ["job_id", "estimated_duration", "queue_position", "status_url", "stream_url"],
            "properties": {
