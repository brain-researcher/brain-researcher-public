...
try:
    ...
    from ..pact_helpers.state_setup import BRKGStateSetup
...
class TestBRKGProvider:
    """Provider verification tests for BR-KG service."""

    @pytest.fixture
    def state_manager(self):
        """Get state setup manager for BR-KG."""
        return BRKGStateSetup.get_state_manager()

    @pytest.fixture
    def br_kg_config(self):
        """Get BR-KG service configuration."""
        return get_service_config("br_kg")

    @pytest.fixture
    def verifier(self, br_kg_config):
        """Create Pact verifier for BR-KG."""
        return Verifier(
            provider="br_kg-service",
            provider_base_url=br_kg_config.base_url,
            pact_dir=str(pact_config.pact_dir),
            provider_states_setup_url=f"{br_kg_config.base_url}/pact/provider-states",
    ...
    def test_verify_orchestrator_consumer_contract(self, verifier, state_manager):
        ...
        pact_file = pact_config.pact_dir / "orchestrator-br_kg.json"
    ...
    def test_verify_agent_consumer_contract(self, verifier, state_manager):
        ...
        pact_file = pact_config.pact_dir / "agent-br_kg.json"
    ...
    async def test_health_endpoint_response(self, state_manager):
        ...
        health_response = {
            "status": "healthy",
            "database": {"connected": True, "nodes": 10000, "relationships": 50000},
            "search_indices": {
                "concepts": "ready",
                "datasets": "ready",
                "tasks": "ready",
    ...
    async def test_dataset_search_response(self, state_manager):
        ...
        search_response = {
            ...
            "facets": {
                "modality": [{"value": "fMRI", "count": 1}],
                "source": [{"value": "OpenNeuro", "count": 1}],
                "n_subjects_range": [{"range": "10-50", "count": 1}],
            },
        }
    ...
    async def test_concept_search_response(self, state_manager):
        ...
        search_response = {
            "concepts": [
                {
                    ...
                    "coordinates": {"mni": [-42, -24, 58], "atlas": "MNI152"},
                    "related_tasks": [
                        {
                            "task_id": "task_motor",
            ...
            "total_count": 1,
            "query_metadata": {"processing_time_ms": 25, "semantic_similarity": 0.92},
        }
    ...
    async def test_task_concept_relationships_response(self, state_manager):
        ...
        # Set up state
        await state_manager.setup_state(
            "knowledge graph has task-concept relationships"
        )
    ...
    async def test_knowledge_graph_statistics_response(self, state_manager):
        ...
        stats_response = {
            ...
            "indices": {
                "concepts": {"size": 8000, "last_updated": "2025-01-01T00:00:00Z"},
                "datasets": {"size": 2500, "last_updated": "2025-01-01T00:00:00Z"},
            },
            "data_freshness": {
    ...
    def test_provider_state_coverage(self, state_manager):
        ...
        pact_files = list(pact_config.pact_dir.glob("*-br_kg.json"))
