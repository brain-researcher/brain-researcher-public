...
# ============================================================================
# Notification Contract Tests (UI-026)
# ============================================================================
...
class TestNotificationEndpoints:
    ...
    def test_notifications_list_contract(self):
        """Test notifications listing endpoint."""
    ...
    def test_notifications_pagination(self):
        ...
        response = client.get("/notifications?limit=5&unread_only=true", headers=headers)
        assert response.status_code == 200
        
        data = response.json()
        assert len(data["notifications"]) <= 5
    
    def test_mark_notifications_read_contract(self):
        """Test mark notifications as read endpoint."""
        headers = self.get_auth_headers()
        ...
        if notifications:
            ...
            response = client.post("/notifications/mark-read", 
                                 json=mark_read_data, headers=headers)
            assert response.status_code == 200
...
class TestServiceIntegration:
    ...
    @pytest.fixture
    def mock_neurokg_service(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "main_enhanced.EnhancedNeuroKGClient.search_datasets",
            mock_search_datasets
        )
    
    def test_graceful_degradation(self, mock_agent_service, mock_neurokg_service):
