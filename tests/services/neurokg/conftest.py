...
def _neurokg_test_database():
    ...
    target_db = os.getenv("NEO4J_DATABASE") or "neurokg_test"
    ...
    try:
        if target_db != "neo4j":
            if not _ensure_database(driver, target_db):
                if os.getenv("NEUROKG_TEST_ALLOW_DEFAULT_DB") == "1":
                    target_db = "neo4j"
                    os.environ["NEO4J_DATABASE"] = target_db
                else:
                    pytest.skip(
                        "Neo4j does not allow creating test DB. "
                        "Set NEO4J_DATABASE=neo4j and NEUROKG_TEST_ALLOW_DEFAULT_DB=1 "
