...
from brain_researcher.services.br_kg.performance_monitor import (
    IndexRecommendation,
    PerformanceMonitor,
    QueryProfile,
    profile_query,
...
class TestPerformanceMonitor:
    ...
    def test_performance_metrics(self, monitor):
        ...
        if len(p95_metrics) > 0 and len(metrics) > 0:
            assert (
                p95_metrics["query_execution_time_test"]
                >= metrics["query_execution_time_test"]
            )
    ...
    def test_query_pattern_frequency(self, monitor):
        ...
        for i in range(10):
            with monitor.profile_query(
                f"SELECT * FROM nodes WHERE id = {i}", "sql"
            ) as profile:
                time.sleep(0.01)
                profile.rows_returned = 1
