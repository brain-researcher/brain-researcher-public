    ...
    def test_query_graphql_syntax(self):
        ...
        for query_id, query in QUERIES.items():
            ...
            for param in query.parameters:
                ...
                assert (
                    f"${param}" in query.query
                ), f"Parameter ${param} not in query {query_id}"
...
class TestPersistedQueryIntegration:
    ...
    def setup(self, tmp_path, monkeypatch):
        ...
        db.create_node(
            "Concept",
            {"id": "trm_memory", "name": "memory", "definition": "Cognitive process"},
        )
        db.create_node(
            "Task",
            {
        ...
        db.create_node(
            "Region",
            {"id": "reg_pfc", "name": "prefrontal cortex", "abbreviation": "PFC"},
        )
        db.create_node(
            "Publication",
            {
        ...
        db.create_relationship(
            ...
            "tsk_nback",
            "MEASURES",
            {"confidence": 0.9, "source": "CogAtlas"},
        )
        db.create_relationship(
            "tsk_nback",
            "reg_pfc",
            "ACTIVATES",
            {"confidence": 0.85, "source": "PubMed"},
        )
