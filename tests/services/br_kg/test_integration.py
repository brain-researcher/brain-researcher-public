...
from brain_researcher.services.br_kg.bulk_loader import LoaderConfig, NDJSONBulkLoader
...
class TestFullIntegration:
    ...
    def setup_complete(self, tmp_path, monkeypatch):
        ...
        test_data = [
            ...
            },
            {
                ...
                "id": "t_stroop",
                "name": "Stroop",
                "description": "Interference task",
            },
            {
                "type": "Task",
                "id": "t_gonogo",
                "name": "Go/No-Go",
                "description": "Inhibition task",
            },
            # Regions
            {
                "type": "Region",
    ...
    def test_error_handling_integration(self, setup_complete):
        ...
        with open(mixed_file, "w") as f:
            # Valid
            f.write(
                json.dumps({"type": "Concept", "id": "valid", "name": "Valid"}) + "\n"
            )
            ...
            # Valid
            f.write(
                json.dumps({"type": "Task", "id": "valid_task", "name": "Valid Task"})
                + "\n"
            )
