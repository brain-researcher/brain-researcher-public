...
def test_old_vs_new_syntax():
    ...
    ), "Old style items should be 'integer'"
    assert (
        new_year_range["items"]["type"] == "integer"
    ), "New style items should be 'integer'"
    print("✓ Both have items type 'integer'")

    # Check constraints
    assert old_year_range.get("minItems") == 2, "Old style should have minItems=2"
    assert new_year_range.get("minItems") == 2, "New style should have minItems=2"
    assert old_year_range.get("maxItems") == 2, "Old style should have maxItems=2"
    assert new_year_range.get("maxItems") == 2, "New style should have maxItems=2"
    print("✓ Both have minItems=2 and maxItems=2")

    # Check nullable
    assert old_year_range.get("nullable"), "Old style should be nullable"
    assert new_year_range.get("nullable"), "New style should be nullable"
    ...
    assert old_keywords["type"] == "array", "Old keywords should be array"
    assert new_keywords["type"] == "array", "New keywords should be array"
    assert (
        old_keywords["items"]["type"] == "string"
    ), "Old keywords items should be string"
    assert (
        new_keywords["items"]["type"] == "string"
    ), "New keywords items should be string"
    assert old_keywords.get("nullable"), "Old keywords should be nullable"
    assert new_keywords.get("nullable"), "New keywords should be nullable"
