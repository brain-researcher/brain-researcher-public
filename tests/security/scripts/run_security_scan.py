...
class SecurityScanner:
    ...
    def run_bandit_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            subprocess.run(['pip', 'install', 'bandit[toml]'],
            ...
            cmd = [
                'bandit',
                '-r', str(self.project_root / 'brain_researcher'),
                '-f', 'json',
                '-o', str(output_file),
                '--config', str(bandit_config) if bandit_config.exists() else '',
                '--skip', 'B101,B601',  # Skip assert_used and shell=True for scientific tools
                '--exclude', 'tests,external,node_modules,__pycache__'
            ]
            ...
            if output_file.exists():
                ...
                return {
                    'status': 'completed',
                    'high_severity_count': len([issue for issue in bandit_data.get('results', [])
                                               if issue.get('issue_severity') == 'HIGH']),
                    'medium_severity_count': len([issue for issue in bandit_data.get('results', [])
    ...
    def run_semgrep_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                'semgrep',
                '--config', str(semgrep_config) if semgrep_config.exists() else 'auto',
                '--json',
        ...
        try:
            ...
            if output_file.exists():
                ...
                return {
                    ...
                    'high_severity_count': len([v for v in vulnerabilities
                ...
                if result.returncode != 0 and "No known security vulnerabilities found" in result.stdout:
                    return {
                        'status': 'completed',
                ...
                for pattern_name, pattern in secret_patterns:
                    ...
                    for match in matches:
                        ...
                        issues.append({
                            ...
                            'severity': 'HIGH' if pattern_name in ['API Key', 'Password'] else 'MEDIUM'
                        })
            except Exception as e:
                continue
    ...
    def run_owasp_zap_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if zap_result.returncode != 0:
                return {
                    'status': 'skipped',
                    'reason': 'OWASP ZAP not installed'
                }

            # Start ZAP in daemon mode
            zap_config = self.project_root / 'tests/security/owasp_zap/zap_automation.yaml'
            ...
            if zap_config.exists():
                ...
                cmd = [
                    ...
                    '-J', str(output_dir / 'zap_baseline_report.json'),
                    '-r', str(output_dir / 'zap_baseline_report.html')
                ]
    ...
    def generate_summary_report(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'total_scan_duration': str(datetime.now() - datetime.fromisoformat(self.results['scan_start'])),
            'findings_summary': {},
            'recommendations': [],
            'risk_level': 'LOW'
        }
        ...
        if deps.get('status') == 'completed':
            ...
            total_medium_issues += deps.get('vulnerability_count', 0) - deps.get('high_severity_count', 0)
        ...
        if total_high_issues > 0:
            recommendations.append("URGENT: Address high-severity security findings immediately")
        ...
        if sast.get('custom_checks', {}).get('issues_found', 0) > 0:
            recommendations.append("Review participant data handling for proper anonymization")
    ...
    def run_full_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Security scan completed. Results saved to {self.output_dir}")
            return self.results
...
def main():
    ...
    parser.add_argument('--scan-type', choices=['sast', 'dast', 'deps', 'all'], default='all',
    ...
    if 'summary' in results:
        ...
        print(f"High Severity Issues: {summary.get('findings_summary', {}).get('high_severity', 0)}")
        print(f"Medium Severity Issues: {summary.get('findings_summary', {}).get('medium_severity', 0)}")
