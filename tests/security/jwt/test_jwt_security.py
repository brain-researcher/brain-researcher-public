...
class TestJWTTokenValidation:
    ...
    def create_test_jwt(self, claims: Dict[str, Any], secret: str, algorithm: str = 'HS256') -> str:
    ...
    def test_expired_token_rejection(self):
        ...
        expired_token = self.create_test_jwt(expired_claims, self.test_secrets['strong_secret'])
        ...
        for service, base_url in self.base_urls.items():
            ...
            for endpoint in protected_endpoints:
                ...
                try:
                    ...
                    assert any(word in response_text for word in ['expired', 'invalid', 'unauthorized']), \
            ...
            try:
                if algorithm == 'none':
                    ...
                    token_parts = [
                        ...
                        jwt.utils.base64url_encode(json.dumps(self.test_claims).encode()).decode(),
    ...
    def test_token_tampering_detection(self):
        ...
        # Create valid token
        valid_token = self.create_test_jwt(self.test_claims, self.test_secrets['strong_secret'])
        ...
        tampering_tests = [
            ...
            ('signature', token_parts[0], token_parts[1], token_parts[2][:-5] + 'XXXXX'),
            ...
            # Extra parts
            ('extra_parts', token_parts[0], token_parts[1], token_parts[2], 'extra.part')
        ]
                ...
                for service, base_url in self.base_urls.items():
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            print(f"Warning: {service} may accept JWT with weak secret: {weak_secret}")
    ...
    def test_claim_validation(self):
        ...
        invalid_claim_tests = [
            ...
            # Future issued at
            {'sub': 'test_user', 'iat': int(time.time()) + 3600, 'exp': int(time.time()) + 7200},

            # Malicious claims
            {'sub': '<script>alert("xss")</script>', 'iat': int(time.time()), 'exp': int(time.time()) + 3600},
            {'sub': '"; DROP TABLE users; --', 'iat': int(time.time()), 'exp': int(time.time()) + 3600},

            # Invalid role claims
            {'sub': 'test', 'role': 'admin', 'iat': int(time.time()), 'exp': int(time.time()) + 3600},
            {'sub': 'test', 'role': '../../../etc/passwd', 'iat': int(time.time()), 'exp': int(time.time()) + 3600},
        ]

        for invalid_claims in invalid_claim_tests:
            try:
                invalid_token = self.create_test_jwt(invalid_claims, self.test_secrets['strong_secret'])
                ...
                for service, base_url in self.base_urls.items():
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            print(f"Info: {service} may not validate JWT claims: {invalid_claims}")
                        else:
                            assert response.status_code in [401, 403], \
    ...
    def test_jwt_timing_attacks(self):
        ...
        for service, base_url in self.base_urls.items():
            ...
            try:
                ...
                timing_results.append((service, avg_valid_time, avg_invalid_time, time_difference_ratio))
    ...
    def test_secure_token_transmission(self):
        ...
        for service, base_url in services.items():
            ...
            if not base_url.startswith('https://'):
                print(f"Info: {service} should use HTTPS for JWT token transmission in production")
            ...
            try:
                ...
                if 'jwt' in set_cookie_headers.lower() or 'token' in set_cookie_headers.lower():
                    assert 'Secure' in set_cookie_headers, f"{service} JWT cookies should have Secure flag"
                    assert 'HttpOnly' in set_cookie_headers, f"{service} JWT cookies should have HttpOnly flag"
                    assert 'SameSite' in set_cookie_headers, f"{service} JWT cookies should have SameSite attribute"
