...
def test_agent_service(port=8000):
    ...
    try:
        ...
        print(f"  BR_KG_API_URL=http://localhost:5005 python -m brain_researcher.services.agent.web_service --port {port}")
        return
    ...
    try:
        response = requests.post(
            f"{base_url}/chat",
            json={"message": complex_query, "thread_id": f"test_complex_{int(time.time())}"},
            timeout=30
        )
