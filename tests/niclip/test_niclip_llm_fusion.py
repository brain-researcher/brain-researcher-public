    ...
    try:
        ...
        for i, construct in enumerate(fused['constructs'][:5]):
            ...
            if 'conflict' in construct['evidence']:
                print(f"      ⚠️  Conflict score: {construct['evidence']['conflict_score']}")
        ...
        assert all(0 <= c['confidence'] <= 1 for c in fused['constructs']), "Invalid confidence"
...
def test_task_adaptive_weighting():
    ...
    try:
        ...
        print(f"   {'Task Type':<20} {'Category':<12} {'NiCLIP Weight':<15} {'Final Conf':<10}")
        print(f"   {'-'*60}")

        for task_name, expected_category, expected_niclip_weight in task_tests:
            fused = fusion.fuse_annotations(
                f'{task_name}_contrast',
                task_name,
                llm_result,
                niclip_result
            )
            ...
            print(f"   {task_name:<20} {actual_category:<12} {expected_niclip_weight:<15.1f} {construct['confidence']:<10.3f}")

            # Validate
            assert actual_category == expected_category, f"Wrong category for {task_name}"
...
def test_active_learning_identification():
    ...
    try:
        ...
        # High conflict case
        llm_high = {'constructs': [{'id': 'trm_1', 'name': 'attention', 'llm_confidence': 0.95, 'direction': '+1'}]}
        niclip_high = {'constructs': [{'id': 'trm_1', 'name': 'attention', 'niclip_confidence': 0.2, 'spatial_confidence': 0.25}]}
        results.append(fusion.fuse_annotations('high_conflict', 'task1', llm_high, niclip_high))

        # Medium conflict case
        llm_med = {'constructs': [{'id': 'trm_2', 'name': 'memory', 'llm_confidence': 0.7, 'direction': '+1'}]}
        niclip_med = {'constructs': [{'id': 'trm_2', 'name': 'memory', 'niclip_confidence': 0.5, 'spatial_confidence': 0.45}]}
        results.append(fusion.fuse_annotations('med_conflict', 'task2', llm_med, niclip_med))

        # Low conflict case
        llm_low = {'constructs': [{'id': 'trm_3', 'name': 'vision', 'llm_confidence': 0.8, 'direction': '+1'}]}
        niclip_low = {'constructs': [{'id': 'trm_3', 'name': 'vision', 'niclip_confidence': 0.75, 'spatial_confidence': 0.78}]}
        results.append(fusion.fuse_annotations('low_conflict', 'task3', llm_low, niclip_low))
        ...
        if len(conflicts) > 1:
            assert conflicts[0]['conflict_score'] >= conflicts[-1]['conflict_score'], "Not sorted by conflict"
