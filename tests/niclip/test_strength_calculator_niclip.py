...
def test_niclip_strength():
    ...
    for concept, region in test_cases:
        ...
        if "niclip_details" in all_results:
            ...
            print(
                f"  Cognitive Process: {all_results['niclip_details'].get('cognitive_process')}"
            )
...
def test_niclip_vs_traditional():
    ...
    # Coordinate strength
    s_coord, coord_details = calculator.strength_from_coordinates(
        concept, region, foci_df
    )
    ...
    improvement = (
        ((comp_enhanced - comp_traditional) / comp_traditional * 100)
        if comp_traditional > 0
        else 0
    )
...
def demonstrate_niclip_benefits():
    ...
    print("   → NiCLIP provides evidence when traditional methods fail")
