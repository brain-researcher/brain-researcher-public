"""Real-data smoke test for workflow_subtype_discovery on ds000114.

Builds a compact atlas-based connectivity feature matrix from real fMRIPrep
preproc BOLD, then runs the declarative subtype discovery workflow
...
def test_workflow_subtype_discovery_ds000114_smoke(tmp_path: Path):
    fmriprep_root = Path(
        os.environ.get(
    ...
    out_dir = tmp_path / "subtype_discovery"
