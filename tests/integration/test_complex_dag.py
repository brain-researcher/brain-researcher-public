...
pytest.skip(
    "complex DAG integration skipped (requires live services)", allow_module_level=True
)
"""
...
"""
    ...
    @pytest.mark.asyncio
    async def test_complete_neuroimaging_pipeline(
        self, neuroimaging_pipeline_dag, temp_data_dir
    ):
        ...
        async def mock_execute_tool(tool_spec):
            ...
            if tool_name == "bids_validator":
                ...
                return {
                    "status": "success",
                    "success": True,
                    "output_dir": str(temp_data_dir / "fmriprep"),
                }
            elif tool_name == "mriqc":
                return {
                    "status": "success",
                    "qc_metrics": {"framewise_displacement": 0.15},
                }
                ...
                return {
                    "status": "success",
                    "contrasts": ["task_vs_baseline", "negative_activation"],
                }
            elif tool_name == "nilearn_second_level_glm":
                return {
                    "status": "success",
                    "group_maps": ["group_task_vs_baseline.nii.gz"],
                }
            elif tool_name == "fsl_randomise":
                return {
                    "status": "success",
                    "corrected_p": f"corrected_p_{tool_spec['parameters'].get('output_suffix', '')}.nii.gz",
                }
            else:
    ...
    @pytest.mark.asyncio
    @pytest.mark.slow
    async def test_resource_intensive_workflow(
        self, resource_intensive_dag, temp_data_dir
    ):
...
nodes:
  ...
  niclip_embedding:
    type: tool
    dependencies: [br_kg_query]
    tool: niclip_encode_text
    parameters:
      text: ${br_kg_query_result.concepts}
...
"""
        ...
        async def mock_execute_tool(tool_spec):
            ...
            if tool_name == "br_kg_concept_query":
                return {
                    "status": "success",
                    "concepts": [
                        "working memory",
        ...
        assert "br_kg_query" in execution.completed_nodes
