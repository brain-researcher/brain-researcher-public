...
# Test markers
pytestmark = pytest.mark.skip("istio security policies require end_to_end marker; skipped in dev env")
...
def security_test_environment():
    ...
    return {
        ...
        "services": {
            "neurokg": {"port": 5000, "security_level": "strict"},
        ...
        "test_users": {
            "admin": {"roles": ["admin", "user"], "permissions": ["read", "write", "delete"]},
...
async def secure_http_session():
    ...
    connector = aiohttp.TCPConnector(
        ssl=False,  # For testing, we might use HTTP
        limit=100,
        limit_per_host=30
    )
...
class TestMutualTLS:
    ...
    def test_peer_authentication_configuration(self, security_test_environment):
        ...
        from brain_researcher.infrastructure.istio.security_manager import IstioSecurityManager
    ...
    @pytest.mark.asyncio
    async def test_mtls_enforcement(self, security_test_environment, secure_http_session):
        """Test mTLS enforcement behavior."""
        base_url = f"http://neurokg-service.{security_test_environment['namespace']}.svc.cluster.local:5000"
...
class TestAuthorizationPolicies:
    ...
    def test_basic_authorization_policy(self, security_test_environment):
        ...
        auth_policy = security_manager.generate_authorization_policy(
            name="neurokg-access-policy",
            selector={"matchLabels": {"app": "neurokg"}},
            rules=[{
                "from": [{
                    "source": {
                        "principals": [
    ...
    def test_role_based_access_control(self, security_test_environment):
        ...
        assert admin_policy["spec"]["rules"][0]["when"][0]["key"] == "request.auth.claims[roles]"
    ...
    def test_path_based_authorization(self, security_test_environment):
        ...
        assert "/public/*" in path_policy["spec"]["rules"][0]["to"][0]["operation"]["paths"]
    
    @pytest.mark.asyncio
    async def test_authorization_enforcement(self, security_test_environment, secure_http_session, jwt_token_generator):
        ...
        try:
            async with secure_http_session.get(f"{base_url}/api/v1/admin/users") as response:
...
class TestJWTValidation:
    ...
    def test_request_authentication_configuration(self, security_test_environment):
        ...
        assert jwt_config["spec"]["jwtRules"][0]["issuer"] == "https://auth.brain-researcher.io"
        assert "brain-researcher-api" in jwt_config["spec"]["jwtRules"][0]["audiences"]
    
    @pytest.mark.asyncio
    async def test_jwt_validation_behavior(self, security_test_environment, secure_http_session, jwt_token_generator):
    ...
    def test_jwt_claim_extraction(self, security_test_environment, jwt_token_generator):
        ...
        assert claim_policy["spec"]["rules"][0]["when"][0]["key"] == "request.auth.claims[sub]"
...
class TestServiceAccountTokens:
    ...
    def test_service_account_configuration(self, security_test_environment):
        ...
        # Policy allowing orchestrator to access neurokg
        ...
        assert expected_principal in sa_policy["spec"]["rules"][0]["from"][0]["source"]["principals"]
    
    @pytest.mark.asyncio
    async def test_service_account_authentication(self, security_test_environment, secure_http_session):
...
class TestNetworkPolicies:
    ...
    def test_network_policy_to_authorization_policy_migration(self, security_test_environment):
        ...
        auth_policy = migrator.convert_network_policy_to_authorization_policy(network_policy)
        
        assert auth_policy["kind"] == "AuthorizationPolicy"
        assert auth_policy["spec"]["selector"]["matchLabels"]["app"] == "neurokg"
    ...
    def test_deny_all_policy(self, security_test_environment):
        ...
        deny_all_policy = security_manager.generate_deny_all_policy(
            name="deny-all",
            selector={"matchLabels": {"app": "sensitive-service"}}
        )
...
class TestSecurityAuditing:
    ...
    def test_access_log_configuration(self, security_test_environment):
        ...
        assert "auth_principal" in telemetry_config["spec"]["accessLogging"][0]["providers"][0]["otel"]["format"]["labels"]
...
class TestSecurityPolicyValidation:
    ...
    def test_policy_conflict_detection(self, security_test_environment):
        ...
        from brain_researcher.infrastructure.istio.policy_validator import SecurityPolicyValidator
    ...
    def test_policy_completeness_check(self, security_test_environment):
        ...
        policies = [
            {
                "kind": "AuthorizationPolicy",
                "metadata": {"name": "neurokg-policy"},
                "spec": {"selector": {"matchLabels": {"app": "neurokg"}}}
            }
...
class TestSecurityPerformance:
    ...
    @pytest.mark.asyncio
    async def test_authentication_overhead(self, security_test_environment, secure_http_session, jwt_token_generator):
    ...
    @pytest.mark.asyncio
    async def test_authorization_policy_overhead(self, security_test_environment, secure_http_session):
...
class TestSecurityE2E:
    ...
    @pytest.mark.asyncio
    async def test_full_security_workflow(self, security_test_environment, secure_http_session, jwt_token_generator):
        ...
        try:
            ...
            async with secure_http_session.get(
                f"{orchestrator_url}/api/v1/research/datasets",
                headers=headers
            ) as response:
                assert response.status in [200, 404]  # Success or not found, but not auth error
            
            # Should NOT be able to access admin endpoints
            async with secure_http_session.get(
                f"{orchestrator_url}/api/v1/admin/users",
                headers=headers
            ) as response:
    ...
    @pytest.mark.asyncio
    async def test_cross_service_security(self, security_test_environment, secure_http_session):
        """Test security enforcement across service boundaries."""
        
        # Test: web-ui -> orchestrator -> neurokg (with proper service accounts)
        ...
        try:
            async with secure_http_session.post(
                ...
                if response.status in [401, 403]:
                    ...
                    assert "auth" in error_data.get("error", "").lower() or \
                           "permission" in error_data.get("error", "").lower()
