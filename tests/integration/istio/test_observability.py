...
def observability_environment():
    ...
    return {
        ...
        "services": {
            "br_kg": {"port": 5000, "traces": True, "metrics": True},
...
class TestIstioMetrics:
    ...
    @pytest.mark.asyncio
    async def test_prometheus_metrics_availability(
        self, observability_environment, observability_http_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_service_specific_metrics(
        self, observability_environment, observability_http_session
    ):
    ...
    def test_custom_metrics_configuration(self, observability_environment):
        ...
        assert (
            custom_metrics_config["spec"]["metrics"][0]["providers"][0]["prometheus"][
                "configOverride"
            ]["metric_name"]
            == "brain_researcher_requests"
        )
...
class TestDistributedTracing:
    ...
    @pytest.mark.asyncio
    async def test_jaeger_traces_availability(
        self, observability_environment, observability_http_session
    ):
        ...
        try:
            ...
            async with observability_http_session.get(services_url) as response:
                if response.status == 200:
                    ...
                    brain_researcher_services = [
                        s
                        for s in services
                        if any(exp in s for exp in expected_services)
                    ]
    ...
    @pytest.mark.asyncio
    async def test_trace_generation_and_retrieval(
        self, observability_environment, observability_http_session
    ):
        """Test trace generation and retrieval."""
        # Generate some traffic to create traces
        services_base_urls = {
            "br_kg": f"http://br_kg-service.{observability_environment['namespace']}.svc.cluster.local:5000",
        ...
        try:
            ...
            params = {"service": "orchestrator-service", "limit": 10, "lookback": "5m"}

            async with observability_http_session.get(
                traces_url, params=params
            ) as response:
                if response.status == 200:
    ...
    async def test_trace_sampling_configuration(
        ...
        base_url = f"http://br_kg-service.{observability_environment['namespace']}.svc.cluster.local:5000"
...
class TestAccessLogging:
    ...
    def test_access_log_configuration(self, observability_environment):
        ...
        assert (
            access_log_config["spec"]["accessLogging"][0]["providers"][0]["name"]
            == "otel"
        )
    ...
    @pytest.mark.asyncio
    async def test_access_log_generation(
        self, observability_environment, observability_http_session
    ):
        ...
        for req in test_requests:
            try:
                ...
                if req["method"] == "POST":
                    ...
                    async with observability_http_session.get(
                        f"{base_url}{req['path']}", headers=headers
                    ) as response:
                        pass
            except aiohttp.ClientError:
...
class TestServiceMeshObservability:
    ...
    @pytest.mark.asyncio
    async def test_service_topology_discovery(
        self, observability_environment, observability_http_session
    ):
            ...
            async with observability_http_session.get(
                ...
                if response.status == 200:
                    ...
                    expected_connections = {
                        ...
                        "orchestrator-service": ["br_kg-service", "agent-service"],
                        "agent-service": ["br_kg-service"],
                    }
            ...
            slos=[
                {
                    ...
                    "query": 'sum(rate(istio_requests_total{destination_service_name="br_kg-service",response_code!~"5.*"}[5m])) / sum(rate(istio_requests_total{destination_service_name="br_kg-service"}[5m]))',
                },
                {
                    "name": "latency_p99",
                    "target": 1000,  # <1000ms P99 latency
                    "query": 'histogram_quantile(0.99, sum(rate(istio_request_duration_milliseconds_bucket{destination_service_name="br_kg-service"}[5m])) by (le))',
                },
                {
                    "name": "error_rate",
                ...
                if response.status == 200:
                    ...
                    for group in rule_groups:
                        for rule in group.get("rules", []):
                            if (
                                "istio" in rule.get("alert", "").lower()
                                or "istio" in rule.get("expr", "").lower()
                            ):
...
class TestObservabilityPerformance:
    ...
    @pytest.mark.asyncio
    async def test_tracing_sampling_impact(
        self, observability_environment, observability_http_session
    ):
...
class TestObservabilityDashboards:
    ...
    @pytest.mark.asyncio
    async def test_grafana_dashboard_access(
        self, observability_environment, observability_http_session
    ):
    ...
    def test_dashboard_configuration(self, observability_environment):
        ...
        from brain_researcher.infrastructure.istio.dashboard_manager import (
...
class TestObservabilityIntegration:
    ...
    async def test_end_to_end_observability(
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            try:
                if scenario["method"] == "POST":
                    ...
                    async with observability_http_session.get(
                        f"{orchestrator_url}{scenario['path']}", headers=headers
                    ) as response:
                        pass
            except aiohttp.ClientError:
        ...
        try:
            query_url = f"{prometheus_url}/api/v1/query"
            params = {
                "query": 'sum(rate(istio_requests_total{destination_service_name="orchestrator-service"}[5m]))'
            }
