...
# Test markers
pytestmark = pytest.mark.skip("istio traffic management tests skipped (end_to_end marker not configured)")
...
class TestIstioVirtualServices:
    ...
    async def test_basic_routing(self, istio_environment, http_session):
        ...
        base_url = f"http://neurokg-service.{istio_environment['namespace']}.svc.cluster.local:5000"
            ...
            routes=[
                {
                    ...
                    "route": [{"destination": {"host": "neurokg-service", "subset": "v1"}}]
                },
                {
                    "match": [{"uri": {"prefix": "/api/v2"}}],
                    "route": [{"destination": {"host": "neurokg-service", "subset": "v2"}}]
                }
            ]
        )
...
class TestIstioDestinationRules:
    ...
    def test_load_balancer_configuration(self, istio_environment):
        ...
        assert dr_config["spec"]["trafficPolicy"]["loadBalancer"]["simple"] == "LEAST_CONN"
    ...
    def test_circuit_breaker_configuration(self, istio_environment):
        ...
        assert dr_config["spec"]["trafficPolicy"]["connectionPool"]["tcp"]["maxConnections"] == 10
...
class TestIstioTrafficSplitting:
    ...
    def test_canary_traffic_configuration(self, istio_environment):
        ...
        from brain_researcher.infrastructure.istio.canary_manager import IstioCanaryManager
    ...
    async def test_gradual_traffic_shift(self, istio_environment, http_session):
        ...
        for step_weight in traffic_steps:
            ...
            current_config = canary_manager.get_current_traffic_config("neurokg-service")
            
            if current_config:
                canary_route = next(
                    (r for r in current_config["spec"]["http"][0]["route"]
...
class TestIstioFaultInjection:
    ...
    def test_delay_fault_configuration(self, istio_environment):
        ...
        from brain_researcher.infrastructure.istio.fault_injector import IstioFaultInjector
    ...
    async def test_fault_injection_behavior(self, istio_environment, http_session):
        ...
        error_rate = len([s for s in status_codes if s >= 400 or s == 0]) / len(status_codes)
...
class TestIstioGateways:
    ...
    def test_gateway_configuration(self, istio_environment):
        ...
        from brain_researcher.infrastructure.istio.gateway_manager import IstioGatewayManager
        ...
        gateway_config = gateway_manager.generate_gateway_config(
            ...
            tls_config={
                "mode": "SIMPLE",
                "credentialName": "brain-researcher-tls"
            }
        )
        
        assert gateway_config["kind"] == "Gateway"
        assert gateway_config["spec"]["servers"][0]["hosts"] == ["brain-researcher.example.com"]
        assert gateway_config["spec"]["servers"][0]["tls"]["mode"] == "SIMPLE"
    
    @pytest.mark.asyncio
    async def test_external_access_through_gateway(self, istio_environment, http_session):
...
class TestIstioRetryPolicies:
    ...
    def test_retry_policy_configuration(self, istio_environment):
        ...
        from brain_researcher.infrastructure.istio.retry_manager import IstioRetryManager
        
        with patch('kubernetes.client'):
            retry_manager = IstioRetryManager(
                namespace=istio_environment['namespace']
            )
...
class TestIstioTimeouts:
    ...
    def test_timeout_configuration(self, istio_environment):
        ...
        from brain_researcher.infrastructure.istio.timeout_manager import IstioTimeoutManager
...
class TestIstioPerformance:
    ...
    async def test_throughput_characteristics(self, istio_environment, http_session):
        ...
        tasks = [
            http_session.get(f"{base_url}/health") 
...
class TestIstioTrafficManagementE2E:
    ...
    async def test_full_request_flow(self, istio_environment, http_session):
        ...
        try:
            async with http_session.get(
                f"{gateway_url}/api/v1/neurokg/health",
