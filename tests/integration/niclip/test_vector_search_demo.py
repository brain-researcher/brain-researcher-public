...
BASE_URL = os.environ.get("BR_KG_API_URL")
if not BASE_URL:
    base_root = os.environ.get("BR_KG_URL", "http://127.0.0.1:5000").rstrip("/")
...
def test_vector_search():
    ...
    for query in queries:
        ...
        response = requests.post(
            f"{BASE_URL}/vector/search", json={"query": query, "k": 5, "threshold": 0.3}
        )

        if response.status_code == 200:
            data = response.json()
            console.print(
                f"Found {data['count']} results in {data['search_time_ms']}ms"
            )
...
def test_hybrid_search():
    ...
    response = requests.post(
        ...
        json={"query": query, "k": 5, "vector_weight": 0.7, "text_weight": 0.3},
    )
    ...
    if response.status_code == 200:
        ...
        console.print(
            f"Weights: Vector={data['search_config']['vector_weight']}, Text={data['search_config']['text_weight']}"
        )
...
def test_similar_nodes():
    ...
    response = requests.post(f"{BASE_URL}/search", json={"query": "memory", "limit": 1})
    ...
    if response.status_code == 200:
        ...
        if search_results:
            ...
            response = requests.get(
                f"{BASE_URL}/vector/similar/{node_type}/{node_id}", params={"k": 5}
            )
            ...
            if response.status_code == 200:
                ...
                console.print(
                    f"[red]Error finding similar nodes: {response.text}[/red]"
                )
    else:
    ...
    if response.status_code == 200:
        ...
        if stats["indices"]:
            ...
            for node_type, details in stats["indices"].items():
                table.add_row(
                    node_type, str(details["num_vectors"]), details["index_type"]
                )
