...
class MockSubscriptionSystem:
    ...
    async def subscribe(self, connection_id: str, query: str, variables: Dict[str, Any] = None):
...
class AgentIntegrationTester:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        results = {
            ...
            "streaming_integration": await self.test_streaming_integration(redis_client),
            "deduplication_integration": await self.test_deduplication_integration(redis_client),
        ...
        self.logger.info(f"Integration tests completed: {summary['tests_passed']}/{summary['tests_run']} passed")
        
        return summary
        
    async def test_subscription_integration(self, subscription_system, redis_client) -> Dict[str, Any]:
        ...
        try:
            ...
            from brain_researcher.services.neurokg.subscriptions.subscription_system import EventType
            ...
            self.logger.error(f"Subscription integration test failed: {e}", exc_info=True)
    ...
    async def test_streaming_integration(self, redis_client) -> Dict[str, Any]:
        ...
        try:
            from brain_researcher.services.agent.streaming_integration import (
                AgentStreamingManager, AgentStreamType
            )
    ...
    async def test_deduplication_integration(self, redis_client) -> Dict[str, Any]:
        ...
        try:
            from brain_researcher.services.agent.deduplication_integration import (
                AgentDataDeduplication, AgentDeduplicationConfig
            )
            from brain_researcher.core.ingestion.deduplication.data_deduplication import DataDeduplication
            ...
            config = AgentDeduplicationConfig(auto_deduplicate=True, similarity_threshold=0.8)
            ...
            test_results = [
                ...
                {"id": "result_2", "title": "Test Analysis 1", "score": 0.85},  # Duplicate
            ...
            recent_queries = [
                ...
                {"query": "How does fMRI work?", "timestamp": datetime.now().isoformat()}
            ]
            ...
            self.logger.error(f"Deduplication integration test failed: {e}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    async def test_plugin_integration(self) -> Dict[str, Any]:
        ...
        try:
            from brain_researcher.services.agent.plugin_integration import (
                AgentPluginManager, AgentPluginConfig
            )
            ...
            template = manager.generate_plugin_template(
                "TestAPI", "api", None
            )
    ...
    async def test_notification_system(self, redis_client) -> Dict[str, Any]:
        ...
        try:
            from brain_researcher.services.agent.notification_system import (
                AgentNotificationSystem, NotificationChannel, NotificationPriority
            )
    ...
    async def test_tool_registry_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Tool registry integration test failed: {e}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    async def test_error_handling_integration(self, redis_client) -> Dict[str, Any]:
        ...
        try:
            from brain_researcher.services.agent.error_integration import (
                IntegrationErrorManager, IntegrationType, get_integration_logger
            )
            ...
            subscription_logger = manager.get_logger("subscription_test", IntegrationType.SUBSCRIPTION)
            streaming_logger = manager.get_logger("streaming_test", IntegrationType.STREAMING)
            ...
            self.logger.error(f"Error handling integration test failed: {e}", exc_info=True)
            return {"success": False, "error": str(e)}
