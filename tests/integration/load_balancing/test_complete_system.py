...
class SystemIntegrationTest:
    ...
    def _setup_monitoring(self):
        ...
        monitoring_thread = threading.Thread(
            target=self._collect_metrics_continuously, daemon=True
        )
        self.active_threads.append(monitoring_thread)
        monitoring_thread.start()

        # Start load balancer monitoring
        lb_thread = threading.Thread(target=self._monitor_load_balancer, daemon=True)
    ...
    def _get_active_connections(self) -> int:
        ...
        try:
            response = requests.get(
                TEST_CONFIG["haproxy_stats_url"] + ";csv", timeout=5
            )
    ...
    def _estimate_throughput(self) -> float:
        ...
        if len(recent_metrics) >= 2:
            ...
            if time_diff > 0:
                connection_changes = abs(
                    recent_metrics[-1].active_connections
                    - recent_metrics[0].active_connections
                )
...
class TestCompleteSystemIntegration:
    ...
    def test_system_health_check(self, system_test):
        ...
        for endpoint in health_endpoints:
            try:
                response = requests.get(
                    f"{TEST_CONFIG['base_url']}{endpoint}", timeout=10
                )
                assert (
    ...
    def test_load_balancer_distribution(self, system_test):
        ...
        if len(server_responses) > 1:
            ...
            assert (
                max_deviation < 0.3
            ), f"Load balancer distribution unfair: {server_responses}"
    ...
    def test_auto_scaling_behavior(self, system_test):
        ...
        print(
            f"Initial CPU: {initial_metrics.cpu_usage}%, Memory: {initial_metrics.memory_usage}%"
        )
        ...
        while (time.time() - start_time) < TEST_CONFIG["auto_scaling_timeout"]:
            ...
            if (
                current_metrics.cpu_usage > 80
                or current_metrics.memory_usage > 85
                or current_metrics.response_time > 2000
            ):

                print(
                    f"High resource usage detected - CPU: {current_metrics.cpu_usage}%, "
        ...
        print(
            f"Final CPU: {final_metrics.cpu_usage}%, Memory: {final_metrics.memory_usage}%"
        )

        # Auto-scaling should have either prevented excessive resource usage
        # or the system should have handled the load gracefully
        assert (
            scaling_detected or final_metrics.response_time < 5000
        ), "Auto-scaling failed to handle load"
        ...
        try:
            ...
            for service in services:
                if "brain-researcher" in service.name:
                    ...
                    service.reload()
                    replicas = (
                        service.attrs["Spec"]["Mode"]
                        .get("Replicated", {})
                        .get("Replicas", 1)
                    )
                    if replicas > 1:
                        print(
                            f"Scaling detected: {service.name} has {replicas} replicas"
                        )
                        return True
        except Exception as e:
    ...
    def test_failover_recovery(self, system_test):
        ...
        assert (
            success_rate > 0.8
        ), f"Success rate too low during failover: {success_rate}"
    ...
    def test_connection_pooling_efficiency(self, system_test):
        ...
        def database_operation():
            ...
            try:
                response = requests.get(
                    f"{TEST_CONFIG['base_url']}/api/datasets", timeout=10
                )
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            ...
            successful_ops = sum(
                1 for future in as_completed(futures, timeout=60) if future.result()
            )

        success_rate = successful_ops / 100
        assert (
            success_rate > 0.95
        ), f"Connection pooling efficiency too low: {success_rate}"
    ...
    def test_websocket_load_balancing(self, system_test):
        ...
        ), f"Too few WebSocket connections succeeded: {successful_connections}"

        print(
            f"✓ WebSocket load balancing test completed - {successful_connections} connections"
        )
    ...
    def test_blue_green_deployment_readiness(self, system_test):
        ...
        health_response = requests.get(
            f"{TEST_CONFIG['base_url']}/health/ready", timeout=5
        )
        ...
        assert health_response.status_code in [
            200,
            404,
        ], "Readiness endpoint should exist or return 404"
    ...
    def test_performance_under_sustained_load(self, system_test):
        ...
        def sustained_load_generator():
            ...
            while (time.time() - start_time) < 180:  # 3 minutes
                try:
                    response = requests.get(
                        f"{TEST_CONFIG['base_url']}/api/datasets", timeout=5
                    )
                    request_count += 1
        ...
        if system_test.metrics_history:
            avg_response_time = sum(
                m.response_time for m in system_test.metrics_history
            ) / len(system_test.metrics_history)
            max_response_time = max(
                m.response_time for m in system_test.metrics_history
            )
            avg_error_rate = sum(
                m.error_rate for m in system_test.metrics_history
            ) / len(system_test.metrics_history)

            print(
                f"Performance metrics - Avg RT: {avg_response_time:.1f}ms, "
            ...
            ), f"Maximum response time too high: {max_response_time}ms"
    ...
    def test_system_recovery_after_stress(self, system_test):
        ...
        assert (
            recovery_metrics.response_time < initial_metrics.response_time * 2
...
def generate_integration_test_report(
    system_test: SystemIntegrationTest,
) -> Dict[str, Any]:
    ...
    # Analyze metrics
    if system_test.metrics_history:
        avg_cpu = sum(m.cpu_usage for m in system_test.metrics_history) / len(
            system_test.metrics_history
        )
        max_cpu = max(m.cpu_usage for m in system_test.metrics_history)
        avg_memory = sum(m.memory_usage for m in system_test.metrics_history) / len(
            system_test.metrics_history
        )
    ...
    if system_test.load_balancer_stats:
        avg_active_backends = sum(
            s.active_backends for s in system_test.load_balancer_stats
        ) / len(system_test.load_balancer_stats)
        total_health_checks = sum(
            s.health_checks_passed + s.health_checks_failed
            for s in system_test.load_balancer_stats
        )
        health_check_success_rate = sum(
            s.health_checks_passed for s in system_test.load_balancer_stats
...
def integration_test_report(system_test):
    ...
    print(
        f"Performance: Avg CPU {report['performance_metrics']['cpu_usage']['average']:.1f}%, "
