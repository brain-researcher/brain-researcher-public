...
async def telemetry_system(mock_redis, telemetry_config, sentry_config):
    ...
    system = TelemetrySystem(
        config=telemetry_config, redis_client=mock_redis, sentry_config=sentry_config
    )
...
class TestTelemetrySystemInitialization:
    ...
    def test_service_integration_creation(self, telemetry_system):
        ...
        br_kg_integration = telemetry_system.get_service_integration(ServiceType.BR_KG)
        ui_integration = telemetry_system.get_service_integration(ServiceType.WEB_UI)

        assert agent_integration is not None
        assert br_kg_integration is not None
        assert ui_integration is not None

        # Should cache integrations
        assert (
            telemetry_system.get_service_integration(ServiceType.AGENT)
            is agent_integration
        )
...
class TestTelemetryEventFlow:
    ...
    async def test_error_tracking_flow(self, telemetry_system):
        ...
        # Track an error
        error_event_id = br_kg_integration.track_error(
...
class TestAlertSystem:
    ...
    async def test_alert_triggering_flow(self, telemetry_system):
        ...
        with patch.object(
            telemetry_system.alert_manager, "notification_manager"
    ...
    async def test_alert_resolution_flow(self, telemetry_system):
        ...
        with patch.object(
            ...
            resolved_alerts = [
                alert
                for alert in telemetry_system.alert_manager.active_alerts.values()
...
class TestPerformanceAndScaling:
    ...
    async def test_high_volume_event_processing(self, telemetry_system):
        ...
        # Get service integration
        service_integration = telemetry_system.get_service_integration(
            ServiceType.AGENT
        )
    ...
    async def test_concurrent_service_integrations(self, telemetry_system):
        ...
        async def br_kg_task():
            for i in range(20):
                br_kg_integration.track_graph_query(
                    query_type="cypher",
        ...
        start_time = time.time()
        await asyncio.gather(agent_task(), br_kg_task(), ui_task())
    ...
    async def test_system_resource_usage(self, telemetry_system):
        ...
        for i in range(500):
            service_integration.track_feature_usage(
                feature_name="resource_test", action="access", success=True
            )
...
class TestSystemResilience:
    ...
    async def test_redis_connection_failure_handling(
        self, telemetry_config, sentry_config
    ):
        """Test handling of Redis connection failures."""
        # Create a system with a failing Redis mock
        failing_redis = MagicMock()
        failing_redis.ping.side_effect = redis.ConnectionError(
    ...
    async def test_component_failure_isolation(self, telemetry_system):
        ...
        with patch.object(
            telemetry_system.alert_manager, "_evaluate_all_rules"
        ) as mock_evaluate:
            ...
            event_id = service_integration.track_feature_usage(
                feature_name="resilience_test", action="access", success=True
            )
    ...
    async def test_graceful_shutdown(self, telemetry_system):
        ...
        for i in range(10):
            service_integration.track_feature_usage(
                feature_name="shutdown_test", action="access", success=True
            )
