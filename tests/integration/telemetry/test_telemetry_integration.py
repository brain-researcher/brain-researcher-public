...
pytest.skip(
    "telemetry integration skipped (resilience marker not configured)",
...
class TestTelemetryIntegration:
    ...
    async def test_error_triggers_sentry_and_alert(self, telemetry_system):
        ...
        redis.get.side_effect = [
            json.dumps(
                {"service": ServiceType.AGENT, "status_code": 500, "error": True}
            ),
            json.dumps(
                {"service": ServiceType.AGENT, "status_code": 500, "error": True}
            ),
        ...
        with patch(
            ...
            sentry.capture_exception(
                exception=error, service=ServiceType.AGENT, tags={"severity": "high"}
            )
    ...
    async def test_telemetry_event_processing(self, telemetry_system):
        ...
        events = [
            TelemetryEvent(
                ...
                data={"endpoint": "/query", "duration_ms": 150, "status_code": 200},
            ),
            TelemetryEvent(
                id="evt_2",
                service=ServiceType.BR_KG,
                ...
                timestamp=datetime.utcnow(),
                data={"error": "Connection timeout", "severity": "warning"},
            ),
        ]
    ...
    async def test_pii_filtering_in_alerts(self, telemetry_system):
        ...
        with patch(
            "brain_researcher.services.telemetry.sentry_integration.sentry_sdk.capture_message"
        ) as mock_capture:
    ...
    async def test_concurrent_service_monitoring(self, telemetry_system):
        ...
        for alert in active_alerts:
            ...
            assert (
                service_index % 2 == 0
            )  # Only even-indexed services should have alerts
    ...
    async def test_high_volume_event_processing(self, telemetry_system):
        ...
        for i in range(1000):
            event = TelemetryEvent(
                ...
                timestamp=datetime.utcnow(),
                data={"index": i, "duration_ms": 100 + (i % 100)},
            )
            events.append(event)
    ...
    async def test_sentry_failure_handling(self, telemetry_system):
        ...
        with patch(
            ...
            event_id = sentry.capture_exception(
                exception=ValueError("Test error"), tags={"test": "true"}
            )
