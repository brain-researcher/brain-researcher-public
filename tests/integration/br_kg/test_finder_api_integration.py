...
BASE_URL = os.getenv("BR_KG_URL", "http://localhost:5000")
...
class TestFinderAPIIntegration:
    ...
    def test_suggest_filters_basic(self):
        ...
        response = requests.post(
            f"{BASE_URL}/kg/suggestFilters", json={"text": "task fMRI motor studies"}
        )
    ...
    def test_suggest_filters_complex(self):
        ...
        for query_test in queries:
            response = requests.post(
                f"{BASE_URL}/kg/suggestFilters", json={"text": query_test["text"]}
            )
            assert response.status_code == 200
            ...
            for expected_facet in query_test["expected_facets"]:
                assert (
                    expected_facet in facets
                ), f"Missing {expected_facet} in query: {query_test['text']}"

            # Check operators if specified
            if "expected_ops" in query_test:
                for facet, op in query_test["expected_ops"].items():
                    filter_item = next(
                        (f for f in filters if f["facet"] == facet), None
                    )
    ...
    @pytest.mark.skipif(not os.getenv("NEO4J_URI"), reason="Neo4j not configured")
    ...
    def test_empty_query(self):
        ...
        response = requests.post(f"{BASE_URL}/kg/suggestFilters", json={"text": ""})
        assert response.status_code == 200
        data = response.json()
        assert data["filters"] == []
    ...
    def test_invalid_request(self):
        ...
        response = requests.post(
            f"{BASE_URL}/kg/suggestFilters", json={}  # Missing 'text' field
        )
