...
class FinderAPIDemo:
    ...
    def demo_suggest_filters(self):
        ...
        for query in queries:
            ...
            response = requests.post(
                f"{self.base_url}/kg/suggestFilters", json={"text": query}
            )
    ...
    def demo_facets(self):
        ...
        test_cases = [
            {"name": "No filters (all data)", "filters": []},
            {
                "name": "Filter by fMRI",
        ...
        for test in test_cases:
            ...
            response = requests.post(
                f"{self.base_url}/kg/facets", json={"filters": test["filters"]}
            )

            if response.status_code == 200:
                facets = response.json()
                print("   Facet counts:")
                for facet_name, values in list(facets.items())[
                    :3
                ]:  # Show first 3 facets
                    print(f"     {facet_name}:")
        ...
        for search in searches:
            ...
            if response.status_code == 200:
                ...
                for item in results["items"][:3]:
                    readiness_icon = {"green": "🟢", "yellow": "🟡", "red": "🔴"}.get(
                        item.get("readiness", "red"), "⚪"
                    )
