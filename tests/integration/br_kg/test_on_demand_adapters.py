...
def test_register_neuroquery_nimare(loader):
    ...
    records = loader.ondemand.fetch(
        "neuroquery",
        task_ids=["task:nback"],
        region_ids=["schaefer2018_200_17n_2mm:L_DLPFC"],
    )
    ...
    nimare_records = loader.ondemand.fetch(
        "nimare",
        task_ids=["task:gng"],
        region_ids=["harvard_oxford_sub25:Left Caudate"],
    )
...
def test_register_neuroscout_allen(loader):
    ...
    feature_records = loader.ondemand.fetch(
        "neuroscout",
        contrast_ids=["contrast:movie-vs-rest"],
        feature_names=["luminance"],
    )
    assert feature_records[0]["value"] == 0.72

    expression_records = loader.ondemand.fetch(
        "allen_hba",
        region_ids=["harvard_oxford_cort25:Anterior Cingulate"],
        gene_symbols=["COMT"],
    )
