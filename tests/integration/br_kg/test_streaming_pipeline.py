...
class TestStreamingPipelineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_kafka_to_stream_processor_integration(
        self, kafka_consumer, stream_processor
    ):
    ...
    async def test_end_to_end_pipeline(
        self, cdc_processor, kafka_producer, kafka_consumer, stream_processor
    ):
        ...
        rules = [
            ...
            AggregationRule(
                "events_by_type", AggregationType.COUNT, group_by=["change_type"]
            ),
            AggregationRule(
                "distinct_entities",
                AggregationRule.DISTINCT_COUNT,
    ...
    async def test_pipeline_error_handling(
        self, cdc_processor, kafka_producer, stream_processor
    ):
    ...
    async def test_pipeline_windowing_accuracy(self, stream_processor):
        ...
        rules = [
            ...
            AggregationRule(
                "avg_value", AggregationType.AVERAGE, field_path="new_properties.value"
            ),
        ]
        ...
        for window in completed_windows:
            ...
            if (
                "avg_value" in window.aggregations
                and window.aggregations["avg_value"] is not None
            ):
    ...
    async def test_pipeline_state_persistence(self, stream_processor, tmp_path):
        ...
        new_processor = StreamProcessor(
            window_size=timedelta(seconds=5), window_type=WindowType.TUMBLING
        )
    ...
    @pytest.mark.asyncio
    async def test_pipeline_monitoring_metrics(
        self, cdc_processor, kafka_producer, stream_processor
    ):
        ...
        test_events = [
            GraphChangeEvent(
                ...
                (
                    ChangeType.NODE_CREATED
                    if i % 2 == 0
                    else ChangeType.RELATIONSHIP_CREATED
                ),
                datetime.now(),
                f"entity-{i}",
        ...
        monitoring_summary = {
            ...
            "total_events_processed": (
                cdc_stats["events_processed"] + processor_stats["events_processed"]
            ),
            "components": {
