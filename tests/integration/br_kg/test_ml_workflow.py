        ...
        self.mock_nodes = [
            {
                "id": i,
                "labels": ["Concept"],
                "properties": {"name": f"concept_{i}", "embedding": [0.1 * i] * 64},
            }
            for i in range(100)
        ]
...
class TestMLWorkflowIntegration:
    ...
    async def test_complete_embedding_to_gnn_pipeline(
        self, mock_neo4j_driver, temp_model_dir, embedding_config, model_config
    ):
        ...
        with patch.object(
            embeddings, "_load_graph_from_neo4j", return_value=mock_graph
        ):
            # Generate Node2Vec embeddings
            node2vec = Node2Vec(embedding_config)
            embedding_result = await embeddings.generate_embeddings(
                driver=mock_neo4j_driver, method="node2vec", config=embedding_config
            )
        ...
        node_features = torch.tensor(
            [embedding_result["embeddings"][str(i)] for i in range(100)]
        )
        ...
        training_result = await gnn_predictor.train_model(
            model=gcn_model, data=graph_data, config=model_config
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_multi_model_comparison_workflow(
        self,
        mock_neo4j_driver,
        temp_model_dir,
        embedding_config,
        ...
        for model_name, model in models_to_test:
            # Train each model
            training_result = await gnn_predictor.train_model(
                model=model, data=mock_graph_data, config=model_config
            )
            ...
            with torch.no_grad():
                ...
                test_acc = (
                    (test_predictions.argmax(dim=1) == test_labels).float().mean()
                )
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_embedding_method_comparison_workflow(
        self, mock_neo4j_driver, embedding_config
    ):
        ...
        with patch.object(
            ...
            for method in embedding_methods:
                result = await embeddings.generate_embeddings(
                    driver=mock_neo4j_driver, method=method, config=embedding_config
                )

                # Verify embedding quality
                embedding_matrix = np.array(
                    [result["embeddings"][str(i)] for i in range(50)]
                )
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_end_to_end_knowledge_graph_ml_workflow(
        self, mock_neo4j_driver, temp_model_dir, embedding_config, model_config
    ):
        ...
        with patch.object(embeddings, "_load_graph_from_neo4j") as mock_load:
            ...
            node_features = torch.tensor(
                [embedding_result["embeddings"][str(i)] for i in range(50)]
            )
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_ml_workflow_error_handling(
        self, mock_neo4j_driver, embedding_config, model_config
    ):
        ...
        with pytest.raises(RuntimeError):  # PyTorch will raise dimension mismatch
            await gnn_predictor.train_model(
                model=model, data=invalid_data, config=model_config
            )
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_concurrent_ml_workflows(
        self, mock_neo4j_driver, embedding_config, model_config
    ):
        ...
        async def run_gnn_workflow(model_class):
            ...
            result = await gnn_predictor.train_model(
                model=model, data=data, config=model_config
            )
            return result
        ...
        gnn_tasks = [run_gnn_workflow(GCN), run_gnn_workflow(GraphSAGE)]

        # Execute all tasks concurrently
        results = await asyncio.gather(
            *embedding_tasks, *gnn_tasks, return_exceptions=True
        )
...
async def test_ml_workflow_monitoring_and_metrics():
    ...
    metrics["memory_usage"].append(
        torch.cuda.memory_allocated() if torch.cuda.is_available() else 1024
    )
