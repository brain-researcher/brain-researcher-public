...
def sample_demo_request():
    ...
    return {
        ...
        "parameters": {"custom_threshold": 0.001, "email_notifications": True},
    }
    ...
    def test_scenario_data_completeness(self):
        ...
        for scenario_id, scenario in DEMO_SCENARIOS.items():
            for field in required_fields:
                assert hasattr(
                    scenario, field
                ), f"Scenario {scenario_id} missing {field}"
...
class TestDemoExecution:
    ...
    async def test_demo_execution_basic(self, demo_executor):
        ...
        demo_id = await demo_executor.execute_demo(
            demo_id="test_demo_001", scenario_id="glm_motor_task", user_id="test_user"
        )
    ...
    async def test_demo_execution_all_scenarios(self, demo_executor):
        ...
        for i, scenario_id in enumerate(scenarios_to_test):
            ...
            result_id = await demo_executor.execute_demo(
                demo_id=demo_id, scenario_id=scenario_id
            )
    ...
    async def test_demo_caching(self, demo_executor):
        ...
        assert (
            demo_executor.get_demo_progress(demo_id_1)["status"] == JobStatus.COMPLETED
        )
        assert (
            demo_executor.get_demo_progress(demo_id_2)["status"] == JobStatus.COMPLETED
        )
    ...
    async def test_demo_error_handling(self, demo_executor):
        ...
        with pytest.raises(ValueError, match="Unknown demo scenario"):
            await demo_executor.execute_demo(
                demo_id="error_test_001", scenario_id="invalid_scenario"
            )
...
class TestOrchestratorEndpoints:
    ...
    async def test_start_demo_endpoint(self, mock_http_client):
        ...
        response = await mock_http_client.post("/api/demos/start", json=request_data)

        # Verify call was made correctly
        mock_http_client.post.assert_called_once_with(
            "/api/demos/start", json=request_data
        )
...
class TestServiceConnectivity:
    ...
    @pytest.mark.asyncio
    async def test_br_kg_connectivity(self, mock_http_client):
        ...
        assert health_data["service"] == "br_kg"
...
class TestResponseFormats:
    ...
    def test_error_response_format(self):
        ...
        error_response = {
            "error": {
                ...
                "details": {"service": "br_kg", "retry_after": 30},
    ...
    def test_success_response_format(self):
        ...
        success_response = {
            "data": {"demo_id": "abc123", "status": "completed"},
            "meta": {"execution_time": 85.4, "cache_used": True},
        }
    ...
    def test_paginated_response_format(self):
        ...
        paginated_response = {
            ...
            "meta": {"total_count": 25, "page": 1, "per_page": 10, "has_next": True},
        }
        ...
        for i in range(3):
            task = asyncio.create_task(
                demo_executor.execute_demo(
                    f"concurrent_test_{i}", "knowledge_graph_query"  # Fastest scenario
                )
            )
...
class TestFailureScenarios:
    ...
    async def test_invalid_demo_parameters(self, demo_executor):
        ...
        with pytest.raises(ValueError):
            await demo_executor.execute_demo("invalid_test", "nonexistent_scenario")
    ...
    async def test_partial_demo_failure(self, demo_executor):
        ...
        with patch.object(
            demo_executor, "_generate_demo_result", failing_generate_result
        ):
