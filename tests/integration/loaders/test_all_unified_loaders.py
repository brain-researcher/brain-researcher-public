...
from brain_researcher.core.ingestion.loaders.cognitive_atlas_unified import CognitiveAtlasUnifiedLoader
...
from brain_researcher.core.ingestion.loaders.neurovault_unified import NeuroVaultUnifiedLoader
from brain_researcher.core.ingestion.loaders.openneuro_unified import OpenNeuroUnifiedLoader
from brain_researcher.core.ingestion.loaders.wikidata_unified import WikiDataUnifiedLoader
...
class UnifiedLoaderTester:
    ...
    def test_cognitive_atlas(self) -> Dict[str, Any]:
        ...
        try:
            ...
            uses_niclip = hasattr(loader, 'niclip_path') and loader.niclip_path is not None
            if uses_niclip:
                assert any("concept_class" in c for c in concepts), "Missing concept classes from NICLIP"
    ...
    def test_neurovault(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get images count
            images = loader.search_images(collection_id=collections[0]["id"]) if collections else []
    ...
    def test_niclip_embeddings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Text embeddings
            try:
                text_emb = loader.get_text_embeddings(model="BrainGPT-7B-v0.0", section="abstract")
    ...
    def test_master_loader(self) -> Dict[str, Any]:
        ...
        try:
            ...
            db_path = "data/test_db/test_neurokg_graph.db"
            ...
            test_sources = ["cognitive_atlas"] if self.quick_mode else ["cognitive_atlas", "wikidata"]
            ...
            result = {
                ...
                "total_relationships": results["statistics"].get("total_relationships", 0),
                "time": f"{elapsed:.2f}s"
            }
...
def main():
    ...
    parser.add_argument(
        "--loader",
        type=str,
        choices=["cognitive_atlas", "pubmed", "neurosynth", "neurovault", 
                 "openneuro", "wikidata", "niclip", "master"],
