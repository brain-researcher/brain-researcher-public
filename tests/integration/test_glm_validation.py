...
from brain_researcher.services.neurokg.etl.mappers.glm_direction_validator import get_glm_validator
...
def test_fusion_with_glm():
    ...
    if glm_val.get('validation_available'):
        ...
        for align in glm_val.get('alignments', [])[:3]:
            ...
            print(f"      Predicted: {'+' if align['predicted_direction'] > 0 else '-'}")
    ...
    for i, construct in enumerate(result['constructs'][:3]):
        ...
        if 'glm_alignment' in construct:
            ...
            print(f"      GLM: {match_symbol} β={glm_align['beta_value']:.3f}, align={glm_align['score']:.2%}")
