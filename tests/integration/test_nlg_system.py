...
from brain_researcher.services.agent.language_templates import (
    ExplanationLevel,
    Language,
)
...
class TestNLGSystemIntegration:
        ...
        return {
            ...
            "first_level": {
                ...
                "contrasts": [
                    {
                        "name": "2back_vs_0back",
                        "description": "Working memory vs control",
                    },
                    {
                        "name": "parametric_load",
                        "description": "Linear increase with load",
                    },
                ],
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_explanation_generation(
        self, nlg_engine, sample_user_profiles, complex_fmri_results
    ):
        ...
        response = await nlg_engine.generate_response_async(
            content=complex_fmri_results, context=context
        )
    ...
    @pytest.mark.asyncio
    async def test_multi_user_explanation_adaptation(
        self, nlg_engine, sample_user_profiles, complex_fmri_results
    ):
        ...
        clinical_count = sum(
            ...
            if term in clinician_response.primary_text.lower()
        )
    ...
    async def test_multi_language_workflow(
        ...
        spanish_response = await nlg_engine.generate_response_async(
            content=complex_fmri_results, context=context, language=Language.SPANISH
        )
        ...
        terms_preserved = sum(
            1 for term in technical_terms if term in spanish_response.primary_text
        )
        ...
        assert (
            abs(len(spanish_response.primary_text) - len(english_response.primary_text))
            / len(english_response.primary_text)
            < 0.3
        )

    @pytest.mark.asyncio
    async def test_conversational_context_evolution(
        self, nlg_engine, sample_user_profiles, complex_fmri_results
    ):
        ...
        response1 = await nlg_engine.generate_response_async(
            content=complex_fmri_results, context=context1
        )
        ...
        response2 = await nlg_engine.generate_response_async(
            content=follow_up_question, context=context2
        )

        # Second response should reference previous context
        assert response2.adaptation_applied
        assert (
            "dementia" in response2.primary_text.lower()
            or "cognitive decline" in response2.primary_text.lower()
        )
    ...
    async def test_error_handling_and_recovery(self, nlg_engine, sample_user_profiles):
        ...
        incomplete_results = {
            ...
            "partial_results": {"completed_subjects": 20, "total_subjects": 45},
        }
        ...
        response = await nlg_engine.generate_response_async(
            content=incomplete_results, context=context
        )

        assert response.primary_text is not None
        assert len(response.primary_text) > 0
        assert (
            "error" in response.primary_text.lower()
            or "failed" in response.primary_text.lower()
        )
        ...
        response_minimal = await nlg_engine.generate_response_async(
            content=minimal_data, context=context_minimal
        )
    ...
    async def test_cross_service_integration(self, nlg_engine, sample_user_profiles):
                ...
                "similar_studies": [
                    {
                        "study_id": "study_001",
                        "similarity": 0.87,
                        "title": "Working memory in young adults",
                    },
                    {
                        "study_id": "study_002",
                        "similarity": 0.82,
                        "title": "Age-related changes in prefrontal function",
                    },
                ]
            },
            "meta_analysis": {
        ...
        context = ResponseContext(
            ...
            session_context={
                "integration_services": ["br_kg", "niclip", "meta_analysis"],
                "cross_reference": True,
            },
        )

        response = await nlg_engine.generate_response_async(
            content=cross_service_results, context=context
        )
    ...
    async def test_real_time_adaptation(
        ...
        assert len(adapted_response.clarification_options) >= len(
            initial_response.clarification_options
        )
    ...
    async def test_visualization_integration(
        ...
        response = await nlg_engine.generate_response_async(
            content=visualization_context, context=context
        )
    ...
    @pytest.mark.asyncio
    async def test_citation_and_reference_integration(
        self, nlg_engine, sample_user_profiles, complex_fmri_results
    ):
        ...
        response = await nlg_engine.generate_response_async(
            content=referenced_results, context=context
        )
        ...
        assert any(
            "Smith" in citation or "fMRIPrep" in citation
            for citation in response.citations
        )

        # Should include reference to related work
        assert any(
            "Johnson" in citation or "working memory" in citation.lower()
            for citation in response.citations
        )
...
class TestNLGPerformanceIntegration:
    ...
    async def test_memory_usage_under_load(self, nlg_engine, sample_user_profiles):
        ...
        for i in range(20):
            user_profile = list(sample_user_profiles.values())[
                i % len(sample_user_profiles)
            ]
            ...
            response = await nlg_engine.generate_response_async(
                content=test_data, context=context
            )
    ...
    @pytest.mark.asyncio
    async def test_explanation_quality_consistency(
        self, nlg_engine, sample_user_profiles, complex_fmri_results
    ):
...
class TestNLGErrorHandlingIntegration:
    ...
    async def test_malformed_input_handling(self, nlg_engine, sample_user_profiles):
        ...
        for malformed_input in malformed_inputs:
            ...
            # Should not crash
            response = await nlg_engine.generate_response_async(
                content=malformed_input, context=context
            )
