...
from brain_researcher.services.br_kg.etl.mappers.niclip_spatial_mapper_enhanced import (
    get_enhanced_mapper,
)
...
def test_enhanced_mapper():
    ...
    for test in test_cases:
        ...
        results = mapper.coordinate_to_concepts(
            test["coordinates"], radius=10.0, top_k=5, min_percentile=50.0
        )
            ...
            if concepts:
                ...
                for i, concept in enumerate(concepts):
                    ...
                    matches_expected = any(
                        exp in concept_lower for exp in test["expected_concepts"]
                    )
                    if matches_expected:
                    ...
                    if concept.get("contributing_parcels"):
                        ...
                        for parcel in concept["contributing_parcels"][:2]:
                            print(
                                f"            - {parcel['parcel']} (weight: {parcel['weight']:.3f})"
                            )
            else:
