...
def example_direct_tool_usage():
    ...
    result = task_tool._run(task_name="n-back", include_synonyms=True)
    ...
    result = coord_tool._run(
        coordinates=[[-42, -22, 54], [0, -2, 48]], radius=10.0, top_k=3  # Motor and SMA
    )
    ...
    result = concept_tool._run(concept="working memory", depth=2, limit=5)
    print(
        f"   Related to 'working memory': {[r['concept'] for r in result.data['related_concepts'][:3]]}..."
    )
    ...
    result = graph_tool._run(query_type="neighbors", start_node="executive function")
    if result.status == "success":
        print(
            f"   Neighbors of 'executive function': {result.data['n_neighbors']} nodes"
        )
    else:
        print(f"   Error: {result.error}")
