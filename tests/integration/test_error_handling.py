...
class TestErrorHandling:
    ...
    def test_retryable_errors(self, error_codes):
        ...
        non_retryable = [k for k, v in error_codes.items() if not v.get('retryable', False)]
...
class TestServiceErrorHandler:
    ...
    async def test_circuit_breaker_opening(self, error_handler):
        ...
        with pytest.raises(ServiceUnavailableError):
            await error_handler.execute_with_circuit_breaker(
                failing_operation,
                operation_id="circuit_open_test"
            )
    ...
    async def test_graceful_degradation(self, error_handler):
        ...
        # Force circuit open
        error_handler.metrics.circuit_state = error_handler.metrics.circuit_state.__class__("open")
    ...
    def test_error_classification(self, error_handler):
        ...
        assert error_handler._classify_failure(connection_error).value == "connection_error"
        assert error_handler._classify_failure(validation_error).value == "validation_error"
...
class TestErrorHandlerRegistry:
    ...
    def test_metrics_aggregation(self, registry):
        ...
        handler2 = registry.register_handler("service2", ServiceType.NEUROKG)
...
class TestIntegration:
    ...
    async def test_recovery_scenario(self):
        ...
        result = await handler.execute_with_circuit_breaker(
            failing_op,
            operation_id="cache_test"  # Same ID as cached response
        )
