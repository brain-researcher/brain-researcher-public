...
class TestServiceHealthChecks:
    ...
    @pytest.mark.asyncio
    async def test_br_kg_health(self, http_client):
        """Test BR-KG service health endpoint."""
        try:
            response = await http_client.get(f"{SERVICE_ENDPOINTS['br_kg']}/health")
            ...
            if response.status_code == 200:
                ...
                assert health_data["service"].startswith("br_kg")

                # Check database connectivity metadata when provided
                if health_data["status"] == "healthy" and "database" in health_data:
                    assert health_data["database"] in ["connected", "available"]

            else:
                pytest.skip(
                    f"BR-KG service unavailable (status: {response.status_code})"
                )
    ...
    async def test_orchestrator_health(self, http_client):
        ...
        try:
            response = await http_client.get(
                f"{SERVICE_ENDPOINTS['orchestrator']}/health"
            )
            ...
            if response.status_code == 200:
                ...
                pytest.skip(
                    f"Orchestrator service unavailable (status: {response.status_code})"
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "langgraph_status" in health_data:
                    assert health_data["langgraph_status"] in [
                        "ready",
                        "initializing",
                        "error",
                    ]
                ...
                pytest.skip(
                    f"Agent service unavailable (status: {response.status_code})"
                )
    ...
    async def test_web_ui_health(self, http_client):
        ...
        try:
            ...
            assert response.status_code in [
                ...
                404,
                500,
            ]  # 404/500 might indicate dev mode
...
class TestServiceInterconnections:
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_to_br_kg_communication(self, http_client):
        ...
        try:
            ...
            response = await http_client.post(
                f"{SERVICE_ENDPOINTS['orchestrator']}/api/br-kg/query",
                json=query_request,
                timeout=10.0,
            )
    ...
    async def test_orchestrator_to_agent_communication(self, http_client):
        ...
        try:
            ...
            analysis_request = {
                ...
                "context": {"dataset": "test_dataset", "modality": "fMRI"},
            }
            ...
            if response.status_code == 200:
                ...
                assert (
                    "job_id" in result or "session_id" in result or "task_id" in result
                )
    ...
    @pytest.mark.asyncio
    async def test_agent_to_br_kg_communication(self, http_client):
        """Test that Agent service can query BR-KG."""
        try:
            # Test agent using BR-KG tools
            tool_request = {
                "tool_name": "coordinate_to_concept",
                "parameters": {"coordinates": [42, -22, 62], "radius_mm": 10},
            }
...
class TestWebSocketConnections:
    ...
    async def test_orchestrator_websocket(self):
        ...
        try:
            websocket = await _connect_websocket(
                WEBSOCKET_ENDPOINTS["orchestrator_ws"], timeout=5.0
            )
            try:
                # Send test message
                test_message = {
    ...
    async def test_agent_websocket(self):
        ...
        try:
            websocket = await _connect_websocket(
                WEBSOCKET_ENDPOINTS["agent_ws"], timeout=5.0
            )
            try:
                # Send test message
                test_message = {
...
class TestRedisConnections:
    ...
    async def test_redis_demo_caching(self, redis_client):
        ...
        await redis_client.setex(cache_key, 3600, json.dumps(demo_result))  # 1 hour TTL
    ...
    async def test_redis_session_storage(self, redis_client):
        ...
        await redis_client.setex(
            session_key, 1800, json.dumps(session_data)  # 30 minutes TTL
        )
...
class TestDatabaseConnections:
    ...
    @pytest.mark.asyncio
    async def test_br_kg_database_query(self, http_client):
        ...
        try:
            ...
            response = await http_client.post(
                f"{SERVICE_ENDPOINTS['br_kg']}/api/cypher",
                json=query_request,
                timeout=10.0,
            )
    ...
    @pytest.mark.asyncio
    async def test_br_kg_graph_traversal(self, http_client):
        ...
        try:
            ...
            response = await http_client.post(
                f"{SERVICE_ENDPOINTS['br_kg']}/api/graph/traverse",
...
class TestServiceFailureHandling:
    ...
    async def test_service_timeout_handling(self, http_client):
        ...
        async with httpx.AsyncClient(timeout=0.001) as fast_timeout_client:
            try:
                await fast_timeout_client.get(f"{SERVICE_ENDPOINTS['br_kg']}/health")
    ...
    async def test_service_unavailable_response(self, http_client):
        ...
        try:
            response = await http_client.get(
                f"{SERVICE_ENDPOINTS['orchestrator']}/api/br-kg/status"
            )
            ...
            if response.status_code in [503, 502]:
                ...
                assert (
                    "service_unavailable" in str(error_data).lower()
                    or "temporarily unavailable" in str(error_data).lower()
                )
    ...
    async def test_service_response_times(self, http_client):
        ...
        for service_name, endpoint in SERVICE_ENDPOINTS.items():
            ...
            try:
                ...
                response_times[service_name] = {"response_time": None, "error": str(e)}
...
class TestServiceAuthentication:
    ...
    async def test_api_key_authentication(self, http_client):
        ...
        response = await http_client.get(
            f"{SERVICE_ENDPOINTS['br_kg']}/api/protected/admin", timeout=5.0
        )
        ...
        response = await http_client.get(
            f"{SERVICE_ENDPOINTS['br_kg']}/api/protected/admin",
            headers=headers,
            timeout=5.0,
        )
