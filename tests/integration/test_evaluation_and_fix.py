...
from brain_researcher.services.neurokg.etl.evaluation.fusion_evaluator import FusionEvaluator
from brain_researcher.services.neurokg.etl.evaluation.misalignment_fixer import (
    MisalignmentFixer, create_improvement_config
)
from brain_researcher.services.neurokg.etl.mappers.niclip_llm_fusion import get_fusion_module
    ...
    for i, test in enumerate(test_cases):
        ...
        if result.status == "success":
            ...
            fusion_result = {
                ...
                'fusion_metrics': {},
                'glm_validation': {
                    'validation_available': False  # Mock for now
                }
            }
                ...
                for mapping in data['coordinate_mappings']:
                    for j, concept in enumerate(mapping.get('concepts', [])[:3]):
                        construct = {
                            ...
                            'evidence': {
                                'llm': {
                                    'confidence': 0.8 if test['expected'] == 'high' else 0.3,
                                    'direction': '+1'
                                },
                                'niclip': {
                                    'confidence': concept['score'],
            ...
            n_conflicts = 0 if test['expected'] == 'high' else (3 if test['expected'] == 'conflict' else 1)
            ...
            fusion_result['fusion_metrics'] = {
                ...
                'avg_confidence': np.mean([c['confidence'] for c in fusion_result['constructs']]) if fusion_result['constructs'] else 0
            }
