...
class TestOrchestratorService:
    ...
    async def test_authentication_flow(self, client):
        ...
        response = await client.post(
            f"{ORCHESTRATOR_URL}/auth/signup", json=signup_data
        )
        assert response.status_code == 200
    ...
    async def test_job_submission(self, client):
        ...
        job_request = {
            "dataset_id": "ds000114",
            "analysis_type": "glm",
            "parameters": {"smoothing_fwhm": 6, "tr": 2.0, "motion_correction": True},
        }
    ...
    async def test_batch_job_submission(self, client):
        ...
        response = await client.post(
            f"{ORCHESTRATOR_URL}/batch/jobs", json=batch_request
        )
        assert response.status_code == 200
    ...
    async def test_websocket_notifications(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            response = await client.post(
                f"{ORCHESTRATOR_URL}/auth/login", json=login_data
            )
        ...
        async with websockets.connect(uri) as websocket:
            ...
            await websocket.send(
                json.dumps({"type": "subscribe", "channels": ["jobs", "system"]})
            )
...
class TestBRKGService:
    ...
    async def test_graphql_query(self, client):
        ...
        response = await client.post(
            f"{BR_KG_URL}/graphql", json={"query": query, "variables": variables}
        )
        assert response.status_code == 200
        data = response.json()
        assert "data" in data
    ...
    async def test_vector_search(self, client):
        ...
        response = await client.post(f"{BR_KG_URL}/search/vector", json=search_request)
        assert response.status_code == 200
    ...
    async def test_hybrid_search(self, client):
        ...
        search_request = {
            ...
            "search_type": "hybrid",
            "filters": {"modality": "fMRI", "year_min": 2015},
        }
        response = await client.post(f"{BR_KG_URL}/search/hybrid", json=search_request)
        assert response.status_code == 200
        results = response.json()
    ...
    async def test_graph_traversal(self, client):
        ...
        response = await client.post(
            f"{BR_KG_URL}/traversal/multi-hop", json=traversal_request
        )
        assert response.status_code == 200
        results = response.json()
    ...
    async def test_temporal_query(self, client):
        ...
        response = await client.post(
            f"{BR_KG_URL}/temporal/query", json=temporal_request
        )
        assert response.status_code == 200
        results = response.json()
...
class TestAgentService:
    ...
    async def test_parameter_inference(self, client):
        ...
        request = {
            "tool": "fsl_feat",
            "context": {"dataset_id": "ds000114", "task": "finger_tapping", "tr": 2.0},
        }
        response = await client.post(
            f"{AGENT_URL}/tools/infer-parameters", json=request
        )
        assert response.status_code == 200
    ...
    async def test_error_recovery(self, client):
        ...
        request = {
            ...
            "context": {"tool": "fsl_feat", "dataset_size": "large"},
        }
...
class TestServiceCommunication:
    ...
    async def test_service_discovery(self, client):
        ...
        assert len(services["services"]) >= 3  # At least orchestrator, br_kg, agent
    ...
    async def test_request_transformation(self, client):
        ...
        request = {"legacy_field": "value", "old_format": {"nested": "data"}}
        response = await client.post(
    ...
    async def test_blue_green_deployment(self, client):
        ...
        response = await client.get(
            f"{GATEWAY_URL}/test/endpoint", headers={"X-Deployment-Target": "canary"}
        )
        assert response.status_code == 200
...
class TestEndToEndScenarios:
    ...
    async def test_complete_analysis_workflow(self, client):
        ...
        job_response = await client.post(
            f"{ORCHESTRATOR_URL}/run", json=job_request, headers=headers
        )
        ...
        for _ in range(max_attempts):
            status_response = await client.get(
                f"{ORCHESTRATOR_URL}/jobs/{job_id}", headers=headers
            )
        ...
        evidence_response = await client.get(
            f"{AGENT_URL}/evidence/{job_id}", headers=headers
        )
        ...
        kg_response = await client.post(
            f"{BR_KG_URL}/graphql", json={"query": query}, headers=headers
        )
    ...
    async def test_chat_driven_analysis(self, client):
        ...
        chat_response = await client.post(
            f"{AGENT_URL}/chat/start", json={"user_id": "test_user"}
        )
        ...
        confirm_response = await client.post(
            f"{AGENT_URL}/chat/{session_id}/confirm", json={"confirmed": True}
        )
        ...
        results_response = await client.get(f"{AGENT_URL}/chat/{session_id}/results")
