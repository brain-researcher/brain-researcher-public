...
class TestDatasetIntegration:
    ...
    def test_dataset_filtering(self, sample_datasets, dataset_filters):
        ...
        for dataset in sample_datasets:
            # Apply filters
            if dataset_filters.get('sources') and dataset['source'] not in dataset_filters['sources']:
                continue
            
            if dataset_filters.get('modalities'):
                if not any(mod in dataset['modality'] for mod in dataset_filters['modalities']):
                    continue
            ...
            if dataset_filters.get('quality_score_min'):
                if dataset.get('quality_score', 0) < dataset_filters['quality_score_min']:
                    continue
    ...
    async def test_related_datasets(self, sample_datasets):
        ...
        with patch('httpx.AsyncClient.get') as mock_get:
            ...
            mock_get.return_value.json = AsyncMock(return_value={'datasets': [sample_datasets[1]]})
            mock_get.return_value.status_code = 200
    ...
    def test_neurokg_integration(self):
        ...
        assert neurokg_response['status'] == 'connected'
        assert 'OpenNeuro' in neurokg_response['sources']
        assert neurokg_response['total_datasets'] == 150
