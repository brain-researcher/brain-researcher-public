#!/usr/bin/env python3
"""Generate a bounded plot probe from the production Brain Researcher NeuroKG.
...
- data/prod_neurokg_plot_data.json
...
ZONE = "us-west1-b"
VM = "brain-researcher-vm"
NAMESPACE = "brain-researcher-core"
NEUROKG_POD = "brain-researcher-neurokg-0"

HERE = Path(__file__).resolve().parent
DATA_DIR = HERE / "data"
FIG_DIR = HERE / "figures"
DATA_PATH = DATA_DIR / "prod_neurokg_plot_data.json"
...
def _remote_python(command_body: str, *, in_neurokg_pod: bool, timeout: int = 180) -> dict[str, Any]:
    ...
    if in_neurokg_pod:
        remote = (
            f"sudo k3s kubectl -n {NAMESPACE} exec {NEUROKG_POD} -- "
            f"{python_command}"
        )
    else:
...
'''
    return _remote_python(code, in_neurokg_pod=True, timeout=240)
...
print("BR_PLOT_DATA_END")
'''
    return _remote_python(code, in_neurokg_pod=False, timeout=180)
...
def plot_counts(data: dict[str, Any]) -> None:
    ...
    fig.suptitle("Prod NeuroKG is dominated by spatial, publication, task, and embedding surfaces", y=1.02)
...
def plot_schema_triple_heatmap(data: dict[str, Any]) -> dict[str, Any]:
    ...
    fig.suptitle("KG schema triple heatmap exposes dominant NeuroKG relation surfaces", y=1.02, fontsize=22)
    fig.text(
        0.50,
        0.035,
...
def plot_full_schema_triple_atlas(data: dict[str, Any]) -> dict[str, Any]:
    ...
    fig.suptitle("Full NeuroKG schema triple atlas", fontsize=24, weight="bold", y=0.982)
    fig.text(
        0.5,
        0.018,
        "Canonical label-set triples preserve edge counts exactly; source/target blocks reveal that NeuroKG is dominated by a few typed relation surfaces.",
        ha="center",
        fontsize=10.5,
        color="#444444",
    )
