...
OUT = ROOT / "report_outputs" / "bounded_autoresearch_full_report_with_figures"
CANON_TEX = ROOT / "BOUNDED_AUTORESEARCH_CASE_REPORT.tex"
CANON_PDF = ROOT / "BOUNDED_AUTORESEARCH_CASE_REPORT.pdf"
...
def figure(
    ...
    label: str = "",
    lof_caption: str | None = None,
    include_options: str | None = None,
    legend: str | None = None,
) -> str:
    """Insert a figure with an explicit textual label.

    `label` is the human-readable figure number (e.g. "Figure 1",
    "Supplementary Figure S2") rendered as a `\\caption*{...}` so that
    PDF caption text matches the markdown figure numbering exactly,
    independent of LaTeX chapter-based auto-numbering.

    `legend`, when provided, is rendered as a small italic block
    immediately below the caption inside the same float, so the legend
    sits with its figure rather than being collected in a separate
    standalone legend section.
    """
    if label:
        cap_body = rf"\textbf{{{label}.}} {caption}"
        lof_body = rf"{label}. {lof_caption or caption}"
    else:
        cap_body = caption
        lof_body = lof_caption or caption
    graphics_options = include_options or f"width={width}"
    legend_block = ""
    if legend:
        legend_block = (
            "\n\\par\\vspace{0.35em}\\noindent"
            "\\begin{minipage}{0.98\\linewidth}"
            "\\footnotesize\\textit{Legend.} " + legend +
            "\\end{minipage}"
        )
    return rf"""
\begin{{figure}}[htbp]
\centering
\includegraphics[{graphics_options}]{{{path}}}
\caption*{{{cap_body}{legend_block}}}
\addcontentsline{{lof}}{{figure}}{{{lof_body}}}
\end{{figure}}
"""
...
# Abbreviations
# ----------------------------------------------------------------------
SECTIONS["Abbreviations"] = r"""\begin{itemize}[leftmargin=1.6em,itemsep=0.2em]
  ...
  \item \textbf{KG} --- knowledge graph (BRKG); the prior layer that surfaces candidate connectivity statistics from the literature as logged hypotheses
...
# ----------------------------------------------------------------------
SECTIONS["4. Methodological claim and worked examples"] = r"""The autonomous loop above produces findings. This section names the campaign's primary contribution (M1, the loop-discipline claim) and a set of per-component worked examples (H1--H5, the brain-prediction claims) demonstrating how the loop produces locked, falsifiable downstream tests. The order matters: M1 is the contribution; H1--H5 are pieces of evidence the loop succeeded in producing.
...
# ----------------------------------------------------------------------
SECTIONS["5. Decisive next experiments"] = r"""
...
# References
# ----------------------------------------------------------------------
SECTIONS["References"] = r"""Cliff, O. M., Bryant, A. G., Lizier, J. T., Tsuchiya, N., \& Fulcher, B. D. (2023). Unifying pairwise interactions in complex dynamics. \emph{Nature Computational Science}, 3, 883--893. doi:10.1038/s43588-023-00519-x.
...
def main() -> None:
    ...
    tex_path = OUT / "bounded_autoresearch_full_report_with_figures.tex"
    for suffix in [".aux", ".fdb_latexmk", ".fls", ".lof", ".log", ".out", ".toc"]:
        stale = tex_path.with_suffix(suffix)
        if stale.exists():
            stale.unlink()
    render = render_report_tex(
        title="Calibrated rs-FC Prediction of HCP Behavioral Components",
        subtitle="A Bounded-Search Case Study with Frozen-Pipeline Inference",
        authors="Zijiao Chen",
        ...
        generated_note=(
            "Generated using the Brain Researcher LaTeX report template. "
            "This local render embeds the publication-plate and supplementary figures from the shared project filesystem."
        ),
    )
    if render.get("status") != "success":
        raise SystemExit(render)
    compile_result = compile_report_pdf(str(tex_path), str(OUT))
    if compile_result.get("status") != "success":
        raise SystemExit(compile_result)
    canonical_tex = tex_path.read_text().replace(f"{ROOT}/", "")
    CANON_TEX.write_text(canonical_tex)
    shutil.copy2(compile_result["outputs"]["pdf_path"], CANON_PDF)
    print(compile_result["outputs"]["pdf_path"])
    print(CANON_TEX)
    print(CANON_PDF)
