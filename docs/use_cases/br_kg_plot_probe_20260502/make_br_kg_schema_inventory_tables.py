#!/usr/bin/env python3
"""Create comprehensive BRKG schema inventory tables.
...
TRIPLES_CSV = TABLE_DIR / "br_kg_schema_triples_comprehensive.csv"
LABELSETS_CSV = TABLE_DIR / "br_kg_labelset_nodes_inventory.csv"
NODE_LABELS_CSV = TABLE_DIR / "br_kg_node_labels_inventory.csv"
RELATIONSHIPS_CSV = TABLE_DIR / "br_kg_relationship_types_inventory.csv"
SOURCE_TARGET_CSV = TABLE_DIR / "br_kg_source_target_surfaces.csv"
DATA_DICTIONARY_MD = TABLE_DIR / "br_kg_schema_inventory_data_dictionary.md"
HTML_PATH = TABLE_DIR / "br_kg_schema_inventory.html"
XLSX_PATH = TABLE_DIR / "br_kg_schema_inventory.xlsx"
SUMMARY_JSON = TABLE_DIR / "br_kg_schema_inventory_summary.json"
...
def build_tables() -> dict[str, pd.DataFrame]:
    ...
    schema["data_source"] = "prod_br_kg_neo4j_schema_export"
...
def write_data_dictionary(tables: dict[str, pd.DataFrame], *, wrote_xlsx: bool) -> None:
    lines = [
        "# BRKG Schema Inventory Data Dictionary",
        "",
        "These tables are generated from the full prod BRKG schema-triple export.",
    ...
    lines += [
        ...
        "",
        "Use `br_kg_schema_triples_comprehensive.csv` as the main comprehensive table. Key columns:",
        "",
...
<head>
  ...
  <title>BRKG Schema Inventory Tables</title>
  <style>
    :root {{
      --bg: #f7f9fc;
      --ink: #17202a;
...
<body>
  <header>
    <h1>BRKG Schema Inventory</h1>
    <p>Comprehensive tabular view of the prod BRKG schema: canonical schema triples, label-set nodes, single node labels, relationship types, and source-target surfaces.</p>
    <div class="stats">
