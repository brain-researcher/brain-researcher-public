# BR-KG Schema Inventory Data Dictionary

These tables are generated from the full prod BR-KG schema-triple export.
...
- `br_kg_schema_triples_comprehensive.csv`: one row per canonical schema triple.
- `br_kg_labelset_nodes_inventory.csv`: one row per canonical label-set node used in the schema graph.
- `br_kg_node_labels_inventory.csv`: one row per Neo4j node label.
- `br_kg_relationship_types_inventory.csv`: one row per Neo4j relationship type.
- `br_kg_source_target_surfaces.csv`: one row per source-label-set / target-label-set surface block.
- `br_kg_schema_inventory.html`: searchable browser version of the same tables.
