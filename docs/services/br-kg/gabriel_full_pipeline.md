...
- If Neo4j returns no publications (or is unavailable), optionally falls back to
  cached scholarly metadata under `data/br-kg/raw/scholarly_metadata`.
...
```text
data/br-kg/raw/gabriel/runs/<run_id>/
  manifest.json
  shards/
    shard_0000.jsonl
    shard_0001.jsonl
...
```bash
br gabriel ingest \
  --manifest data/br-kg/raw/gabriel/runs/<run_id>/manifest.json \
...
- Writes review queue items to:
  - `data/br-kg/raw/gabriel/runs/<run_id>/review_queue.jsonl`
- Writes loader checkpoint to:
  - `data/br-kg/raw/gabriel/runs/<run_id>/ingest_checkpoint.json`
...
```bash
br gabriel ingest-candidate-only \
  --queue data/br-kg/raw/gabriel/eval/<some_pack>/review_queue_candidate_only.jsonl \
...
```bash
br gabriel status --manifest data/br-kg/raw/gabriel/runs/<run_id>/manifest.json
```
...
latest manifest under `data/br-kg/raw/gabriel/runs`.
...
external/kg-gen/.venv/bin/python scripts/kggen_generate_from_manifest.py \
  --manifest data/br-kg/raw/gabriel/runs/<run_id>/manifest.json \
  --output data/br-kg/raw/kggen/real_from_manifest.jsonl \
  --max-papers 300 \
...
- It enriches title/abstract from `data/br-kg/raw/scholarly_metadata` when
  DOI-matched cache files exist.
- It writes a sibling summary file at
  `data/br-kg/raw/kggen/real_from_manifest.summary.json`.
...
br gabriel eval-kggen \
  --manifest data/br-kg/raw/gabriel/runs/<run_id>/manifest.json \
  --kggen-input data/br-kg/raw/kggen/candidates.jsonl \
  --output-dir data/br-kg/raw/gabriel/eval/kggen \
  --quality-profile balanced \
  --sample-size 300
```
...
br gabriel map-onvoc \
  --kggen-input data/br-kg/raw/kggen/real_from_manifest.jsonl \
  --output-dir data/br-kg/raw/gabriel/eval/kggen/onvoc \
  --min-score 0.82 \
  --margin-min 0.04 \
  --json
```
...
python scripts/calibrate_onvoc_thresholds.py \
  --mapping-rows data/br-kg/raw/gabriel/eval/kggen/onvoc/mapping_rows.jsonl \
...
python scripts/build/build_task_panel_ingest_package.py \
  --onvoc-dir data/br-kg/raw/gabriel/eval/kggen/onvoc \
  --output-dir data/br-kg/raw/gabriel/eval/kggen/task_panel_package \
...
```bash
br gabriel ingest \
  --manifest data/br-kg/raw/gabriel/eval/kggen/task_panel_package/manifest_task_panel.json \
...
python scripts/tools/etl/build_task_panel_exact_reroute_subset.py \
  --source-manifest data/br-kg/raw/gabriel/eval/kggen/task_panel_package/manifest_task_panel.json \
  --records-jsonl data/br-kg/raw/gabriel/eval/kggen/reroute_rows.jsonl \
  --output-dir data/br-kg/raw/gabriel/eval/kggen/reroute_subset \
...
python scripts/tools/etl/build_task_panel_exact_reroute_subset.py \
  --source-manifest data/br-kg/raw/gabriel/eval/kggen/task_panel_package/manifest_task_panel.json \
  --records-jsonl data/br-kg/raw/gabriel/eval/kggen/reroute_rows.jsonl \
  --output-dir data/br-kg/raw/gabriel/eval/kggen/reroute_subset_concept \
...
python scripts/tools/etl/migrate_task_panel_exact_ids.py \
  --manifest data/br-kg/raw/gabriel/eval/kggen/reroute_subset/manifest_task_panel.json
```
...
```bash
python scripts/tools/etl/migrate_task_panel_exact_ids.py \
  --manifest data/br-kg/raw/gabriel/eval/kggen/reroute_subset_concept/manifest_task_panel.json \
