...
- **Total Nodes:** 476,889
- **Total Edges:** 51,188
- **Database:** `br_kg_full.db` (424 MB)
- **Detailed Statistics:** See [`br_kg_graph_schema.md`](./br-kg_graph_schema.md)
...
The creation of edges is governed by matching logic and confidence scoring defined as tunable parameters in files under `configs/br-kg/`.
...
| **`IN_REGION`** | `StatsMap` → `BrainRegion` | Canonical spatial substrate path for voxel-level evidence. | `configs/br-kg/thresholds.yaml` (spatial) | ✅ Live canonical path |
| **`MEASURES`** | `Task` → `Concept` | The task is designed to measure the cognitive concept. | `configs/br-kg/edge_scoring.yaml` | ⚠️ Unknown |
| **`ACTIVATES`** | `Task`/`Concept` → `Region` \| `BrainRegion` | Semantic enrichment edge family; not part of substrate Gate A in the current contract revision. | `configs/br-kg/edge_scoring.yaml` | ⚠️ Partial, nonblocking |
| **`DERIVED_FROM`** | `StatisticalMap` → `Contrast` | The map was generated from the specified contrast. | Direct extraction | ⚠️ Partial |
| **`IMPLEMENTS_TASK`**| `Contrast` → `Task` | The contrast is an implementation of the task. | `configs/br-kg/edge_scoring.yaml` | ⚠️ Minimal |
...
| **`SAME_AS`** | `Node` → `Node` | Links two nodes of the same type that represent the same entity. | `configs/br-kg/edge_scoring.yaml` | ⚠️ Unknown |
| **`MAPS_TO`** | `Node` → `Node` | Links two nodes for cross-source or vocabulary mapping. | `configs/br-kg/mappings/` | ⚠️ Unknown |
