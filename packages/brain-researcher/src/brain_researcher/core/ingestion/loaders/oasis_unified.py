...
class OASISUnifiedLoader:
    ...
    def load_oasis2_longitudinal(self, demo_mode: bool = False) -> Dict[str, Any]:
        ...
        if demo_mode:
            ...
            for subject_id, group in df.groupby('Subject ID'):
                ...
                self.longitudinal_data[subject_id] = {
                    'visits': visits,
                    'n_visits': len(visits),
                    'follow_up_years': max(v['days_from_baseline'] for v in visits) / 365
                }
        ...
        logger.info(f"Loaded {len(longitudinal_subjects)} OASIS-2 longitudinal subjects")
    ...
    def load_oasis3_multimodal(self, oasis3_file: Optional[str] = None, demo_mode: bool = False) -> Dict[str, Any]:
    ...
    def _generate_oasis2_sample_data(self):
        ...
        for i in range(n_subjects):
            ...
            for v in range(n_visits):
                days_from_baseline = v * np.random.randint(180, 730)  # 6 months to 2 years
                age = baseline_age + days_from_baseline / 365
                
                # Progression
                if baseline_cdr > 0:
                    cdr_progression = min(3, baseline_cdr + v * 0.25 * np.random.random())
                else:
        ...
        for i in range(n_subjects):
            ...
            multimodal_data[subject_id] = {
                ...
                'clinical': {
                    ...
                    'mmse': np.random.randint(20, 26) if has_ad else np.random.randint(27, 31),
                    'moca': np.random.randint(18, 24) if has_ad else np.random.randint(25, 30),
                    'gds': np.random.randint(0, 10),  # Geriatric Depression Scale
                    'faq': np.random.uniform(0, 20) if has_ad else 0  # Functional Activities
                },
                'imaging': {
                    'has_t1': True,
                ...
                'cognitive_battery': {
                    ...
                    'visuospatial_composite': np.random.normal(-0.3 if has_ad else 0, 0.5)
                }
            }
            ...
            if subject_id in self.longitudinal_data:
                ...
                nodes.append({
                    ...
                    'properties': {
                        ...
                        'follow_up_years': self.longitudinal_data[subject_id]['follow_up_years'],
                        'converted': self.longitudinal_data[subject_id].get('converted', False)
                    }
                })
                edges.append({
                    'source': subject_node_id,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # Age distribution
        ages = [self.demographics.get(s, {}).get('age', 0) 
                for s in self.subjects if s in self.demographics]
        ...
        # Gender distribution
        genders = [self.demographics.get(s, {}).get('gender', 'U') 
                  for s in self.subjects if s in self.demographics]
