...
def compose_data_paths(project_root: Path | None = None, env: str = "dev") -> Dict[str, Any]:
    ...
    Args:
        ...
        env: Overlay name: one of dev|staging|prod|neurokg (for general keys)
    ...
    # API and cache
    for key in ("api_sources", "cache", "loader_defaults", "dataset_priority", "neurokg"):
        if key in overlay:
            out[key] = overlay[key]

    # Attach neurokg pipeline overlay under a namespaced key
    neurokg_overlay = _read_yaml(cfg_dir / "overlays" / "neurokg.yaml")
    if neurokg_overlay:
        out["_neurokg_overlay"] = neurokg_overlay
