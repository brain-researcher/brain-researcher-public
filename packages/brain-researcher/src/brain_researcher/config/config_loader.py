...
class DataPathConfig:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if composed:
                logger.info("Loaded dataset configuration from layered registry/overlays/local_mounts")
                return composed
        except Exception as e:
            logger.debug(f"Layered dataset config load failed, falling back to legacy file: {e}")
    ...
    def get_path(self, source: str, dataset: Optional[str] = None,
        ...
        for source_type in priority:
            ...
            if source_type == 'local':
                ...
                if self.is_oak_enabled():
                    alt_path = self.get_path(f'{source_type}.datasets.{dataset}', fallback=False)
                    if alt_path and alt_path.exists():
                        logger.info(f"Using fallback: {alt_path}")
                        return alt_path
    ...
    def get_loader_defaults(self) -> Dict[str, Any]:
        ...
        """
        return self._config.get('loader_defaults', {
            'max_workers': 4,
            'retry_attempts': 3,
            'retry_delay': 1.0,
            'use_cache': True,
...
def get_data_path(source: str, dataset: Optional[str] = None,
                  fallback: bool = True) -> Optional[Path]:
