...
def launch_kg_service(
    ...
    try:
        ...
        os.environ["FLASK_APP"] = "brain_researcher.services.neurokg.app:app"
        ...
        db_path = os.environ.get("NEUROKG_GLMFITLINS_DB_PATH")
        if not db_path:
            # Try to find default database
            default_db = get_data_root() / "neurokg" / "db" / "neurokg_glmfitlins.db"
            if default_db.exists():
                os.environ["NEUROKG_GLMFITLINS_DB_PATH"] = str(default_db)
                if verbose:
        ...
        from brain_researcher.services.neurokg.app import app
        ...
        console.print("  pip install -e '.[neurokg]'")
        sys.exit(1)
...
def launch_kg_service_production(
    ...
    try:
        ...
        cmd = [
            "gunicorn",
            "brain_researcher.services.neurokg.app:app",
            f"--bind={host}:{port}",
            f"--workers={workers}",
            "--worker-class=sync",
            "--timeout=120",
