...
def execute_tool(
    ...
    Args:
        tool_id: Tool name/ID from ToolSpec (e.g., "fsl.bet", "neurokg.client")
    ...
    Example:
        ...
        >>> result = execute_tool("neurokg.client", {"query": "motor cortex"})
...
def _execute_mcp_bridge(spec: ToolSpec, parameters: dict[str, Any]) -> ToolResult:
    """Execute read-only MCP namespace tools via in-process MCP server bridge."""
    ...
    try:
        ...
        return ToolResult(
            status="error",
            error=f"MCP bridge unavailable: {exc}",
            data=None,
    ...
    fn = getattr(mcp_server, method_name, None)
    if not callable(fn):
        return ToolResult(
            status="error",
            error=f"MCP bridge method not found: {method_name}",
            data=None,
            metadata={"tool_id": spec.name, "backend": "external_api"},
        )

    try:
        payload = fn(**parameters)
