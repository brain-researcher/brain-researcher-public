...
class ASLPerfusionArgs(BaseModel):
    ...
    m0_file: Optional[str] = Field(default=None, description="Optional M0 calibration image")
    ...
    post_labeling_delay: list[float] = Field(default_factory=lambda: [2.0], description="Post-labeling delays (s)")
    multi_delay: bool = Field(default=False, description="Whether multiple PLDs were acquired")
    delays: Optional[list[float]] = Field(default=None, description="Explicit multi-delay list")
    ...
    save_perfusion_weighted: bool = Field(default=True, description="Persist perfusion-weighted map")
