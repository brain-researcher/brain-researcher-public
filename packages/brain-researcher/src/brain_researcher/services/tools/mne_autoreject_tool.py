...
class AutorejectArgs(BaseModel):
    ...
    n_interpolate: Optional[List[int]] = Field(
        default=None,
        description="Number of channels to interpolate (auto if None)"
    )
    consensus: Optional[List[float]] = Field(
        default=None,
        description="Consensus parameter values to try (auto if None)"
    )
    cv: int = Field(
        default=5,
    ...
    use_local: bool = Field(
        default=True,
        description="Use local (channel-specific) rejection"
    )
    use_global: bool = Field(
        default=True,
        description="Use global rejection"
    )
    ...
    save_epochs: bool = Field(
        default=True,
        description="Save cleaned epochs"
    )
    save_report: bool = Field(
        default=True,
        description="Generate and save rejection report"
    )
        ...
        **kwargs
        ...
        try:
            ...
            if verbose and used_autoreject is False:
                logger.info("Autoreject package not available; fallback routine executed.")
