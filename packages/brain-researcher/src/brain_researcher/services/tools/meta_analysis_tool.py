...
class MetaAnalysisArgs(BaseModel):
    ...
    coordinates_file: Optional[str] = Field(
        default=None,
        description="Path to coordinates (studies x coordinates x 3)"
    )
    study_labels_file: Optional[str] = Field(
        default=None,
        description="Path to study labels/IDs"
    )
    sample_sizes_file: Optional[str] = Field(
        default=None,
        description="Path to sample sizes per study"
    )
    
    # Image-based inputs
    images_dir: Optional[str] = Field(
        default=None,
        description="Directory containing statistical maps"
    )
    contrast_files: Optional[List[str]] = Field(
        default=None,
        description="List of contrast map files"
    )
    variance_files: Optional[List[str]] = Field(
        default=None,
        description="List of variance map files"
    )
    
    # Effect size inputs
    effect_sizes_file: Optional[str] = Field(
        default=None,
        description="Path to effect sizes data"
    )
    standard_errors_file: Optional[str] = Field(
        default=None,
        description="Path to standard errors"
    )
    ...
    ale_kernel: str = Field(
        default="gaussian",
        description="ALE kernel: 'gaussian', 'uniform'"
    )
    ale_fwhm: Optional[float] = Field(
        default=None,
        description="FWHM for ALE kernel (mm)"
    )
    
    # MKDA parameters
    mkda_kernel_radius: float = Field(
        default=10.0,
        description="MKDA kernel radius (mm)"
    )
    mkda_threshold: float = Field(
        default=0.5,
        description="MKDA threshold for activation"
    )
    
    # SDM parameters
    sdm_anisotropic: bool = Field(
        default=True,
        description="Use anisotropic kernels in SDM"
    )
    sdm_voxel_threshold: float = Field(
        default=0.001,
        description="Voxel-level threshold for SDM"
    )
    ...
    n_iterations: int = Field(
        default=5000,
        description="Number of iterations for null distribution"
    )
    ...
    correction_method: str = Field(
        default="FWE",
        description="Correction: 'FWE', 'FDR', 'none'"
    )
    alpha: float = Field(
        default=0.05,
    ...
    es_model: str = Field(
        default="random",
        description="Effect size model: 'fixed', 'random', 'mixed'"
    )
    heterogeneity_test: bool = Field(
        default=True,
        description="Test for heterogeneity (Q, I²)"
    )
    
    # Bias assessment
    assess_bias: bool = Field(
        default=True,
    ...
    subgroup_analysis: bool = Field(
        default=False,
        description="Perform subgroup analysis"
    )
    subgroup_variable: Optional[str] = Field(
        default=None,
        description="Variable for subgrouping"
    )
    
    # Sensitivity analysis
    sensitivity_analysis: bool = Field(
        default=True,
        description="Perform leave-one-out sensitivity analysis"
    )
    ...
    save_maps: bool = Field(
        default=True,
        description="Save statistical maps"
    )
    save_clusters: bool = Field(
        default=True,
        description="Save cluster information"
    )
    ...
    space: str = Field(
        default="MNI",
        description="Brain space: 'MNI', 'Talairach'"
    )
...
class MetaAnalysisTool(NeuroToolWrapper):
                        ...
                        for dy in range(-radius_voxels, radius_voxels+1):
                            for dz in range(-radius_voxels, radius_voxels+1):
                                if dx**2 + dy**2 + dz**2 <= radius_voxels**2:
                                    ...
                                    if 0 <= nx < brain_dims[0] and 0 <= ny < brain_dims[1] and 0 <= nz < brain_dims[2]:
    ...
    def _perform_ibma(self, images, variances=None, method='fixed'):
        ...
        if method == 'fixed':
            ...
            if variances is not None:
                ...
                weighted_sum = np.sum(images * weights[..., np.newaxis, np.newaxis, np.newaxis], axis=0)
            ...
            if variances is not None:
                # Compute Q statistic
                weights = 1 / (variances + 1e-10)
                weighted_mean = np.sum(images * weights[..., np.newaxis, np.newaxis, np.newaxis], axis=0) / np.sum(weights)
                
                Q = np.sum(weights * (images - weighted_mean[np.newaxis, ...])**2, axis=0)
                df = len(images) - 1
                ...
                combined_map = np.sum(images * new_weights[..., np.newaxis, np.newaxis, np.newaxis], axis=0) / np.sum(new_weights)
            ...
            z_scores = images / np.sqrt(variances[..., np.newaxis, np.newaxis, np.newaxis] + 1e-10) if variances is not None else images
    ...
    def _assess_publication_bias(self, effect_sizes, standard_errors, methods):
        ...
        if 'egger' in methods:
            ...
            slope, intercept, r_value, p_value, std_err = sp_stats.linregress(precision, effect_sizes)
            results['egger'] = {
    ...
    def _find_clusters(self, stat_map, threshold, min_cluster_size=20):
        ...
        for i in range(1, n_clusters + 1):
            ...
            if cluster_size >= min_cluster_size:
                ...
                peak_idx = np.unravel_index(np.argmax(stat_map * cluster_mask), stat_map.shape)
    ...
    def _visualize_results(self, stat_map, effect_sizes=None, standard_errors=None, output_path=None):
        ...
        if effect_sizes is not None and standard_errors is not None:
            ...
            ax4.errorbar(effect_sizes, y_pos, xerr=1.96*standard_errors, 
                        fmt='o', capsize=5, capthick=2)
            
            # Plot combined effect
            combined = self._compute_effect_size_meta(effect_sizes, standard_errors)
            ax4.axvline(combined['combined_effect'], color='red', linestyle='--', label='Combined')
            ax4.axvspan(combined['ci_lower'], combined['ci_upper'], alpha=0.2, color='red')
        ...
        if effect_sizes is not None and standard_errors is not None:
            ...
            # Add reference lines
            combined = self._compute_effect_size_meta(effect_sizes, standard_errors)
            ax5.axvline(combined['combined_effect'], color='red', linestyle='--', alpha=0.5)
            
            # Add funnel
            se_range = np.linspace(0, np.max(standard_errors), 100)
            ax5.plot(combined['combined_effect'] - 1.96*se_range, se_range, 'k--', alpha=0.3)
            ax5.plot(combined['combined_effect'] + 1.96*se_range, se_range, 'k--', alpha=0.3)
        ...
        if output_path:
            plt.savefig(output_path / 'meta_analysis_visualization.png', dpi=150, bbox_inches='tight')
        plt.close()
        ...
        **kwargs
        ...
        try:
            ...
            if input_type == "coordinates":
                if verbose:
                    logger.info(f"Performing coordinate-based meta-analysis using {method}")
                ...
                if coordinates_file:
                    ...
                    if sample_sizes_file:
                        sample_sizes = np.loadtxt(sample_sizes_file) if not sample_sizes_file.endswith('.npy') else np.load(sample_sizes_file)
                    
                    # Perform CBMA
                    if method == "ALE":
                        stat_map = self._perform_ale(coordinates, sample_sizes, ale_fwhm, n_iterations)
                        results['ale_fwhm'] = ale_fwhm if ale_fwhm else "adaptive"
                    
                    elif method == "MKDA":
                        stat_map = self._perform_mkda(coordinates, mkda_kernel_radius, mkda_threshold)
                ...
                if contrast_files:
                    for cf in contrast_files:
                        ...
                        img = np.load(cf) if cf.endswith('.npy') else np.random.randn(91, 109, 91)
                        images.append(img)
                
                if variance_files:
                    for vf in variance_files:
                        var = np.load(vf) if vf.endswith('.npy') else np.ones((91, 109, 91))
                ...
                if images:
                    ...
                    stat_map, combined_variance = self._perform_ibma(
                        images, variances, 
                        method if method in ['fixed_effects', 'random_effects', 'stouffers'] else 'fixed_effects'
                    )
                ...
                if verbose:
                    logger.info(f"Performing effect size meta-analysis using {es_model} model")
                
                # Load effect sizes
                if effect_sizes_file:
                    effect_sizes = np.loadtxt(effect_sizes_file) if not effect_sizes_file.endswith('.npy') else np.load(effect_sizes_file)
                
                if standard_errors_file:
                    standard_errors = np.loadtxt(standard_errors_file) if not standard_errors_file.endswith('.npy') else np.load(standard_errors_file)
                
                if effect_sizes is not None and standard_errors is not None:
                    # Compute combined effect size
                    es_results = self._compute_effect_size_meta(effect_sizes, standard_errors, es_model)
                    results.update(es_results)
                    
                    # Assess publication bias
                    if assess_bias:
                        bias_results = self._assess_publication_bias(effect_sizes, standard_errors, bias_methods)
                        results['bias_assessment'] = bias_results
                    
                    # Sensitivity analysis
                    if sensitivity_analysis:
                        sens_results = self._sensitivity_analysis(effect_sizes, standard_errors, es_model)
            ...
            # Visualize
            if visualize:
                self._visualize_results(stat_map, effect_sizes, standard_errors, output_path)
            ...
            return ToolResult(
                ...
                data={
                    "outputs": {
                        ...
                        "stat_map": str(map_file) if save_maps and stat_map is not None else None,
                        "clusters": str(clusters_file) if save_clusters and clusters else None,
                        "visualization": str(output_path / "meta_analysis_visualization.png") if visualize else None
                    },
