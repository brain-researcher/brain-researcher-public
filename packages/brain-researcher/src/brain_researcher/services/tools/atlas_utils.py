...
def find_local_schaefer_atlas(
    ...
    *,
    ...
    relative_candidates = [
        ...
        Path("neurokg/raw/nilearn_atlases/schaefer_2018"),
    ]

    for tf_dir in _templateflow_search_dirs(search_roots, space=space):
        for path in sorted(tf_dir.glob("*.nii*")):
            if not _templateflow_schaefer_candidate(path) or not _is_nonempty_file(path):
                continue
            match = re.search(
...
def find_local_aal_atlas(roots: list[Path]) -> Path | None:
    ...
    relative_candidates = [
        ...
        Path("aal"),
        Path("atlases/aal"),
        Path("neurokg/raw/nilearn_atlases/aal_SPM12/aal/atlas"),
        Path("neurokg/raw/nilearn_atlases/aal_SPM12/aal"),
    ]
    for root in roots:
        for rel in relative_candidates:
            base = root / rel
...
def find_local_harvard_oxford_atlas(
    ...
    relative_candidates = [
        ...
        Path("neurokg/raw/nilearn_atlases/fsl/data/atlases/HarvardOxford"),
    ]
    for root in roots:
        for rel in relative_candidates:
            base = root / rel
...
def find_local_yeo_atlas(n_networks: int, roots: list[Path]) -> Path | None:
    ...
    relative_candidates = [
        ...
        Path("neurokg/raw/nilearn_atlases/yeo_2011/Yeo_JNeurophysiol11_MNI152"),
    ]
    for root in roots:
        for rel in relative_candidates:
            base = root / rel
...
def find_local_destrieux_atlas(
    ...
    *,
    ...
    relative_candidates = [
        ...
        Path("neurokg/raw/nilearn_atlases/destrieux_2009"),
    ]
    for root in roots:
        for rel in relative_candidates:
            base = root / rel
...
def find_local_basc_atlas(scale: int, roots: list[Path]) -> Path | None:
    ...
    relative_candidates = [
        ...
        Path(
            "neurokg/raw/nilearn_atlases/basc_multiscale_2015/"
...
def find_local_msdl_atlas(roots: list[Path]) -> Path | None:
    ...
    relative_candidates = [
        ...
        Path("neurokg/raw/nilearn_atlases/msdl_atlas/MSDL_rois"),
    ]
    for root in roots:
        for rel in relative_candidates:
            base = root / rel
...
def find_local_difumo_atlas(
    ...
    *,
    ...
    relative_candidates = [
        ...
        Path("difumo"),
        Path("atlases/difumo"),
        Path("neurokg/raw/nilearn_atlases/difumo"),
    ]
    for tf_dir in _templateflow_search_dirs(search_roots, space=space):
        for path in sorted(tf_dir.glob("*.nii*")):
    ...
    for root in search_roots:
        ...
        if dimension_text:
            deep_matches = [path for path in deep_matches if dimension_text in path.name]
        if deep_matches:
            return deep_matches[0]
    return None
