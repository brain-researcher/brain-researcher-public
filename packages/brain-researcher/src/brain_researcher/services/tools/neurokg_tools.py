...
_neurokg_health_cache: dict = {"status": None, "checked_at": 0, "ttl": 60}
...
def check_neurokg_health(api_url: str | None = None, force: bool = False) -> dict:
    ...
    now = time.time()
    url = api_url or os.environ.get("NEUROKG_API_URL", "http://localhost:5000")

    # Use cached result if recent and not forced
    if not force and _neurokg_health_cache["status"] is not None:
        if now - _neurokg_health_cache["checked_at"] < _neurokg_health_cache["ttl"]:
            return _neurokg_health_cache["status"]
    ...
    # Cache the result
    _neurokg_health_cache["status"] = result
    _neurokg_health_cache["checked_at"] = now
    return result
...
class FindRelatedConceptsTool(CachedToolWrapper):
    ...
    def __init__(
        ...
        self.backend = (
            backend or os.environ.get("NEUROKG_FIND_RELATED_BACKEND", "auto")
        ).strip().lower()
        self._last_backend_used: str | None = None
        self._last_backend_error: str | None = None
        self.runtime_rerank_mode = (
            runtime_rerank_mode
            or os.environ.get("NEUROKG_FIND_RELATED_RUNTIME_RERANK", "auto")
        ).strip().lower()
    ...
    def _should_skip_local_fallback(exc: requests.RequestException) -> bool:
        ...
        raw = os.environ.get("NEUROKG_FIND_RELATED_SKIP_LOCAL_FALLBACK", "")
...
class ContrastToActivationMapTool(NeuroToolWrapper):
    ...
    def _run(
        ...
        try:
            from brain_researcher.services.neurokg.niclip.contrast_text_orchestrator import (
...
class TaskMappingTool(CachedToolWrapper):
    ...
    def __init__(self, runtime_mapper: Any | None = None, runtime_mode: str | None = None):
        ...
        self.runtime_mode = (
            runtime_mode
            or os.environ.get("NEUROKG_TASK_MAPPING_RUNTIME", "auto")
        ).strip().lower()
        self._runtime_mapper = runtime_mapper
    ...
    def _load_vocab_loader_module() -> Any:
        ...
        for module_name in ("brain_researcher.services.neurokg.utils.vocab_loader",):
            try:
...
class NeuroKGTools:
    ...
    def __init__(self, api_url: str = None):
        ...
        enable_glm_priors = os.environ.get("NEUROKG_ENABLE_GLM_PRIORS", "0").lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def check_health(self, force: bool = False) -> dict:
        ...
        """
        return check_neurokg_health(self.api_url, force=force)
    ...
    def get_tool_by_name(self, name: str) -> NeuroToolWrapper | None:
        ...
        tool_map = {
            ...
            "neurokg.fetch_glm_priors": self.glm_priors,
        }
...
class GLMPriorsTool(CachedToolWrapper):
    ...
    def get_tool_name(self) -> str:
        return "neurokg.fetch_glm_priors"
    ...
    def _run(self, task: str, study_id: str | None = None, max_results: int = 200) -> ToolResult:
        ...
        if source_pref in {"kg", "neurokg", "hybrid"}:
            ...
            return ToolResult(
                ...
                data={
                    "outputs": {
                        ...
                        "source": kg_payload.get("source", "neurokg"),
