...
class LLMBudget:
    ...
    # USD limits
    daily_usd_limit: Optional[Decimal] = None
    monthly_usd_limit: Optional[Decimal] = None
...
class BudgetDecision:
    ...
    remaining_daily_usd: Optional[Decimal] = None
    remaining_monthly_usd: Optional[Decimal] = None
...
class BudgetStatus:
    ...
    daily_usd_remaining: Optional[Decimal] = None
    monthly_usd_remaining: Optional[Decimal] = None
...
class LLMBudgetManager:
    ...
    async def create_budget(self, budget: LLMBudget) -> bool:
        ...
        try:
            ...
            budget_data = {
                ...
                "daily_usd_limit": str(budget.daily_usd_limit) if budget.daily_usd_limit else None,
                "monthly_usd_limit": str(budget.monthly_usd_limit) if budget.monthly_usd_limit else None,
            ...
            self.redis.hset(budget_key, mapping={k: v for k, v in budget_data.items() if v is not None})
    ...
    async def _init_spending_counters(self, budget_id: str):
        ...
        # Daily counters
        self.redis.hset(f"llm_budget:{budget_id}:daily:{today}", mapping={
            "tokens": 0,
            "usd": "0.0"
        })

        # Monthly counters
        self.redis.hset(f"llm_budget:{budget_id}:monthly:{month}", mapping={
            "tokens": 0,
            "usd": "0.0"
        })
    ...
    async def post_invocation_record(
        ...
        try:
            ...
            if not allocation:
                logger.warning(f"Allocation {allocation_id} not found, creating new record")
                budget_id = "unknown"
            else:
    ...
    async def _update_spending_counters(self, budget_id: str, record: LLMSpendingRecord):
    ...
    async def get_budget_status(self, budget_id: str) -> BudgetStatus:
        ...
        try:
            ...
            status = BudgetStatus(
                ...
                daily_tokens_limit=int(daily_token_limit) if daily_token_limit else None,
                ...
                monthly_tokens_limit=int(monthly_token_limit) if monthly_token_limit else None,
                monthly_usd_spent=monthly_usd_spent,
                monthly_usd_limit=Decimal(monthly_usd_limit) if monthly_usd_limit else None
            )

            # Calculate remaining
            if status.daily_tokens_limit:
                status.daily_tokens_remaining = max(0, status.daily_tokens_limit - daily_tokens_used)

            if status.monthly_tokens_limit:
                status.monthly_tokens_remaining = max(0, status.monthly_tokens_limit - monthly_tokens_used)

            if status.daily_usd_limit:
                status.daily_usd_remaining = max(Decimal("0"), status.daily_usd_limit - daily_usd_spent)

            if status.monthly_usd_limit:
                status.monthly_usd_remaining = max(Decimal("0"), status.monthly_usd_limit - monthly_usd_spent)
    ...
    async def _check_alert_thresholds(
        ...
        try:
            ...
            for limit_type in ["daily_usd", "monthly_usd", "daily_tokens", "monthly_tokens"]:
                ...
                percentage = (float(used_value) / float(limit_value)) * 100 if limit_value > 0 else 0

                # Check thresholds
                for threshold in thresholds:
                    if percentage >= threshold:
                        # Check if we've already alerted for this threshold
                        alert_key = f"llm_budget:{budget_id}:alerts:{limit_type}:{threshold}"
