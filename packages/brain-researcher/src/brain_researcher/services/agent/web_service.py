...
def _first_present_text(*values: Any) -> Optional[str]:
    for value in values:
        if value is None:
            continue
...
def _request_header_value(*names: str) -> Optional[str]:
    for name in names:
...
def _payload_value(payload: Optional[dict[str, Any]], *keys: str) -> Optional[str]:
...
def _extract_llm_budget_id(payload: Optional[dict[str, Any]] = None) -> Optional[str]:
...
def _provider_for_preflight(model: str, provider_lock: Optional[str] = None) -> str:
...
def _record_llm_api_fee_debit_from_context(
    ...
    *,
    ...
    setattr(metadata, "api_fee_debit", debit_result.__dict__)
    return debit_result
...
def _env_tool_allowlist() -> Optional[List[str]]:
...
def _canonical_runtime_tool_id(tool_id: Optional[str]) -> Optional[str]:
...
def _expand_plan_surface_tool_ids(tool_ids: Optional[List[str]]) -> List[str]:
...
def _normalize_allowlist_mode(allowlist_mode: Optional[str]) -> str:
...
def _effective_tool_allowset() -> Optional[set[str]]:
...
def _is_tool_allowed_by_runtime_policy(tool_id: Optional[str]) -> bool:
...
def _allowlist_violation_response(
    ...
    *,
    ...
    denied_tool_id: Optional[str] = None,
    denied_family: Optional[str] = None,
    denial_stage: Optional[str] = None,
...
def _apply_agent_allowlist(plan_request: PlanRequest) -> Optional[Tuple[Response, int]]:
...
def _plan_surface_allowset(allowlist_mode: Optional[str]) -> Optional[set[str]]:
...
def _studio_plan_allowed_tool_id(
    tool_id: Any, allowset: Optional[set[str]]
) -> Optional[str]:
...
def _studio_plan_normalize_ops(
    ...
    *,
    plan_id: str,
) -> tuple[List[Dict[str, Any]], Optional[Tuple[Response, int]]]:
...
class PlanAllowlistError(ValueError):
    ...
    def __init__(self, disallowed: List[str], plan_id: Optional[str] = None):
...
def _get_relevant_tool_context(
    query: str, max_tools: int = 15, allowed_tools: Optional[List[str]] = None
) -> str:
    """
...
def _get_required_params(tool_name: str) -> Optional[set[str]]:
...
def _resolve_runtime_tool_instance(tool_id: Optional[str]):
...
def _plan_context_from_request(
    ...
    *,
    ...
    tool_candidate_diagnostics: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    context = {
...
def _build_plan_routing_diagnostics(
    *,
    ...
    preflight_tool_candidate_diagnostics: Optional[Dict[str, Any]] = None,
...
def _register_plan(plan: Plan, context: Optional[Dict[str, Any]] = None) -> None:
    ...
    if plan.dag.steps and isinstance(plan.candidates, list):
        ...
        for cand in plan.candidates:
            cand_tool = _canonical_runtime_tool_id(cand.get("tool_id") or cand.get("tool"))
    ...
    if plan.chosen_tool:
        plan.chosen_tool = _canonical_runtime_tool_id(plan.chosen_tool) or plan.chosen_tool
    ...
    def _generator():
        ...
        def _record_failure_memory(
            *,
            ...
            recovery_actions: Optional[List[Dict[str, Any]]],
        ) -> None:
                ...
                if not plan_memory:
                    ...
                    try:
                        ...
                        failure_id = plan_memory.record_failure(
                            ...
                            step_id=step_id,
                            tool_id=tool_key,
                            error_category=getattr(taxonomy, "category", None).value
                            if getattr(taxonomy, "category", None)
                            else None,
                            recovery_action=getattr(
                                taxonomy, "recovery_action", None
                            ).value
            ...
            if kg_write:
                ...
                try:
                    ...
                    record = FailureKGRecord(
                        ...
                        recovery_action=getattr(taxonomy, "recovery_action", None).value
                        if getattr(taxonomy, "recovery_action", None)
                        else None,
                        is_retryable=getattr(taxonomy, "is_retryable", None),
                        error_message=error_message,
        ...
        try:
            with parent_recorder:
                for step_idx, step in enumerate(steps):
                    ...
                    cache_key = None
                    try:
                        from brain_researcher.services.orchestrator.cache_key import (
                            build_cache_key,
                        )
                                ...
                                try:
                                    asyncio.run(
                                        cache_store.mark_completed(
                                            cache_key=cache_key,
                                            run_id=step_recorder.run_id
                                            if step_recorder
                                            else run_request.plan_id,
                                            run_dir=str(step_recorder.run_dir)
                                            if step_recorder
                                            else "",
...
from brain_researcher.services.agent.config import get_config

_config = get_config()
logger.info(f"Agent configuration loaded: {_config}")

from brain_researcher.services.agent.job_service import (
...
if _config.get("NEUROKG_API_URL"):
    os.environ["NEUROKG_API_URL"] = _config["NEUROKG_API_URL"]
...
def agent_plan_contract():
    ...
    try:
                    ...
                    if not chosen_tool:
                        ...
                        if plan_request.constraints:
                            if hasattr(plan_request.constraints, "warnings"):
                                warnings = list(
                                    getattr(plan_request.constraints, "warnings") or []
                                )
        ...
        if strict_plan_validation:
            ...
            if missing_tools:
                ...
                if not registry_tools and hasattr(agent.tool_registry, "tools"):
                    registry_tools = list(getattr(agent.tool_registry, "tools").keys())  # type: ignore[attr-defined]
        ...
        plan_routing_diagnostics["allowlist_mode"] = plan_payload.get(
            "allowlist_mode"
        ) or plan_request.allowlist_mode
...
@app.route("/agent/studio/plan", methods=["POST"])
def agent_studio_plan():
    ...
    if not prompt:
        return jsonify({"error": "missing_prompt", "message": "prompt is required"}), 400
    ...
    deterministic_plan = build_studio_plan(
        ...
        tool_candidate_diagnostics=getattr(
            bundle, "tool_candidate_diagnostics", None
        ),
    )
    assistant_message = (
    ...
    return jsonify({
        ...
        "source": "agent_typed",
        "tool_candidates": [
            {"tool_id": c.get("tool_id"), "source": c.get("source"), "score": c.get("score")}
...
@app.route("/agent/run_info", methods=["GET"])
def agent_run_info():
...
@app.route("/agent/run_plan", methods=["POST"])
def agent_run_plan_contract():
    ...
    if wants_sse:
        # If the orchestrator job store is unavailable in this process,
...
@app.route("/agent/plan/export/nipype", methods=["POST"])
...
@app.route("/agent/plan/export", methods=["POST"])
...
def neurodesk_execute_dispatch():
    ...
    if mode not in ("local", "k8s", "handoff"):
        return jsonify({"error": "invalid_mode", "allowed": ["local", "k8s", "handoff"]}), 400

    try:
        from brain_researcher.services.tools.neurodesk_compiler import NeurodeskDispatcher
        dispatcher = NeurodeskDispatcher(mode=mode, config={
...
def _infer_tool_family(tool_id: Optional[str]) -> Optional[str]:
    if not tool_id:
        return None
...
@app.route("/agent/debug/kg/tools", methods=["GET"])
def debug_kg_tools():
    """
        ...
        families_payload.append(
            {
                ...
                "runtime_kinds": sorted({t.runtime_kind for t in tools_list})
                if tools_list
                else [],
...
@app.route("/api/usage/records", methods=["GET"])
def usage_records():
    """
...
def _run_mcp_selftest_direct_response(query: str, ctx: dict | None = None):
    ...
    try:
        ...
        payload = mcp_server.system_self_test(
            mode=mode,
...
@app.route("/tools/execute_async", methods=["POST"])
@rate_limit(max_per_minute=20)
...
@app.route("/runs/execute_async", methods=["POST"])
@rate_limit(max_per_minute=20)
...
@app.route("/runs/<run_id>/metrics", methods=["GET"])
...
@app.route("/runs/<run_id>/bundle", methods=["GET"])
...
@app.route("/runs/<run_id>/scorecard", methods=["GET"])
...
@app.route("/tools/execute_async/<run_id>", methods=["GET"])
