...
class UnifiedPlanner:
    ...
    def _canonicalize_tool_id(tool_id: str | None) -> str | None:
        ...
        if tool_id_value:
            return resolve_primary_runtime_tool_id(str(tool_id_value)) or str(tool_id_value)
        ...
        *,
        ...
        if not base_candidates:
            ...
            routing_diagnostics = _build_routing_diagnostics(
                ...
                started_at=planner_started_at,
                source_counts={
                    "neurokg": len(candidates_payload),
                }
                if candidates_payload
                else {},
            )
            ...
            rescored.append(
                (
                    ...
                    cand,
                    {
                        ...
                        "failure_penalty": float(f"{failure_penalty:.6f}")
                        if failure_prior
                        else None,
                        "failed_on_count": int(fp.get("fail_count"))
