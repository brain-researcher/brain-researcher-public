...
from .policy_network import PolicyNetwork, QNetwork, EnsembleQNetwork
...
class CQLOptimizer:
    ...
    def __init__(
        ...
        if use_ensemble:
            self.q_network = EnsembleQNetwork(state_dim, action_dim, num_networks=3, hidden_dims=hidden_dims)
            self.target_q_network = EnsembleQNetwork(state_dim, action_dim, num_networks=3, hidden_dims=hidden_dims)
        else:
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(batches_per_epoch):
                ...
                # Train Q-function
                q_loss, cql_penalty = self._train_q_function(states, actions, rewards, next_states, dones, weights)
            ...
            if val_size > 0:
                ...
                self.training_stats["conservative_penalty"].append(val_stats["conservative_penalty"])
    ...
    def select_action(
        ...
        if use_policy:
            ...
            if self.use_ensemble:
                ...
                q_values = q_values[0] if len(q_values.shape) == 2 else q_values
                uncertainty = np.zeros_like(q_values)
        ...
        if return_info:
            ...
            if self.use_ensemble:
                ...
                q_values = q_values[0] if len(q_values.shape) == 2 else q_values
                q_uncertainty = q_uncertainty[0] if len(q_uncertainty.shape) == 2 else q_uncertainty
            else:
                q_values = self.q_network.get_q_values(state)
                q_values = q_values[0] if len(q_values.shape) == 2 else q_values
    ...
    def get_conservative_penalty(self, states: np.ndarray) -> float:
        ...
        if self.use_ensemble:
            ...
            q_values_data = np.array([self.q_network.get_q_values(state.reshape(1, -1))[0] for state in states])
    ...
    def evaluate_policy(
        ...
        return {
            ...
            "mean_conservative_gap": float(np.mean(conservative_penalties)) if conservative_penalties else 0.0,
    ...
    def _train_q_function(
        ...
        next_max_q = np.max(next_q_values, axis=1) if len(next_q_values.shape) == 2 else np.max(next_q_values)
        ...
        for i in range(batch_size):
            ...
            if self.use_ensemble:
                ...
                current_q_all = current_q_all[0] if len(current_q_all.shape) == 2 else current_q_all
            else:
            ...
            if self.use_ensemble:
                ...
                bootstrap_indices = np.random.choice(1, 1, replace=True)  # Single sample for now
                self.q_network.train_ensemble(state, q_target_vector.reshape(1, -1), bootstrap=False)
            else:
    ...
    def _adjust_alpha(self, current_penalty: float) -> None:
        ...
        logger.debug(f"Adjusted alpha to {self.alpha:.4f} (penalty: {current_penalty:.4f})")
