...
def _candidate_rank(
    tools: List["RoutingToolView"], runtime_id: Optional[str]
) -> Optional[int]:
    if not runtime_id:
        return None
...
class ToolPlan:
    ...
    leaf_runtime_id: Optional[str] = None
...
class ChatOrchestrator:
    ...
    def __init__(
        ...
        knowledge_aggregator: Optional["EvidenceAggregator"] = None,
        ...
        if enable_knowledge_layer:
            try:
                ...
                self._knowledge_memory_store: Optional["KnowledgeMemoryStore"] = (
                    KnowledgeMemoryStore()
                )
    ...
    def handle_chat(
        ...
        preflight_candidate_latency_ms: Optional[float] = None
    ...
    def _maybe_enrich_coding_plan(
        ...
        if plan.leaf_runtime_id != _CODE_AGENT_TOOL_ID and plan.tool != _CODE_AGENT_TOOL_ID:
            return plan
    ...
    def _apply_multiagent_plan_gate(
        ...
        *,
        ...
        ctx: Dict[str, Any],
        tools: List[RoutingToolView],
    ) -> tuple[ToolPlan, Optional[Dict[str, Any]]]:
    ...
    def _handle_mcp_selftest(
        ...
        *,
        ...
        try:
            ...
            logger.warning("MCP self-test import failed: %s", exc)
            return None
    ...
    def _match_declared_workflow(self, query: str) -> Optional[str]:
    ...
    def _detect_imaging_domain(self, query: str) -> Optional[str]:
    ...
    def _try_pipeline_execution(
        ...
        try:
            ...
            if preferred_workflow:
                ...
                validated = mcp_server.pipeline_plan_validate(plan)
                if not validated.get("ok"):
                    return self._build_pipeline_fallback_reply(
                        user_msg,
                        f"workflow_plan_invalid:{preferred_workflow}",
                    )
                submitted = mcp_server.pipeline_execute(plan, dry_run=preview)
