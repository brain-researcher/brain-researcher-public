...
class ComplexityResult:
    ...
    suggested_tool: Optional[str] = None  # For simple queries, which tool to use
...
class ComplexityGate:
    ...
    def assess(self, query: str, context: Optional[dict] = None) -> ComplexityResult:
        """
        ...
        for pattern, tool, reason in self.SIMPLE_PATTERNS:
            if re.match(pattern, query_lower, re.IGNORECASE):
                self._log_decision(query, "simple", 0.9, f"pattern_match: {reason}", context)
        ...
        if keyword_scores:
            ...
            if max_weight >= 0.7:
                self._log_decision(
                    query, "complex", max_weight,
                    f"keyword: '{max_keyword}' (weight={max_weight})", context
                )
                return ComplexityResult(
                    level="complex",
        ...
        for pattern in self.COMPLEX_DOMAIN_PATTERNS:
            if re.search(pattern, query_lower, re.IGNORECASE):
                self._log_decision(query, "complex", 0.85, f"domain_pattern: {pattern}", context)
                return ComplexityResult(
                    level="complex",
                    confidence=0.85,
        ...
        if self.plan_memory:
            try:
                ...
                if similar_plans:
                    ...
                    avg_steps = sum(p.get("step_count", 1) for p in similar_plans) / len(similar_plans)
        ...
        self._log_decision(
            query, "complex", 0.5,
            "default: no pattern match, no history",
            context
        )
        return ComplexityResult(
            level="complex",
...
def create_complexity_gate(plan_memory: Optional["PlanMemory"] = None) -> ComplexityGate:
    """
