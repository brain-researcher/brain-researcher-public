...
def _derive_testability(candidate: Mapping[str, Any]) -> float:
    ...
    seed_count = min(
        1.0,
        len(_non_empty_list(candidate.get("seed_kg_ids") or candidate.get("seeds"))) / 3.0,
    )
    return round(
        _clip01(
            (0.40 * minimal_test)
...
def _derive_impact_radius(candidate: Mapping[str, Any]) -> float:
    ...
    touched_domains = min(
        1.0,
        len(_non_empty_list(candidate.get("touched_domains") or candidate.get("domains")))
        / 3.0,
    )
...
def _derive_prior_art_obviousness(candidate: Mapping[str, Any]) -> float:
    ...
    result = max(0.0, (0.60 * bridge) - (0.20 * contradiction) - (0.15 * assumption) - (0.15 * transfer))
...
def score_wow_candidate(candidate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    transfer_signature = (
        str(candidate.get("transfer_signature") or "").strip() or None
    )
    ...
    wow_score = 0.0 if vetoed else round(counterintuitiveness * testability * impact_radius, 6)

    why_not_bridge = str(candidate.get("why_this_is_not_just_a_bridge") or "").strip()
    if not why_not_bridge:
        if broken_default_assumption:
            why_not_bridge = (
                f"This challenges the field default assumption: {broken_default_assumption}."
            )
        elif contradiction_signature:
            why_not_bridge = (
                f"This is driven by a contradiction pattern, not a missing bridge: {contradiction_signature}."
            )
        elif transfer_signature:
            why_not_bridge = (
                f"This proposes a method-family transfer beyond local bridge search: {transfer_signature}."
            )
        else:
...
def rank_wow_candidates(candidates: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
