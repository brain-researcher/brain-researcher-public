...
class DatasetResolution:
    ...
    resources: Optional["DatasetResources"] = None
    metadata: Dict[str, Any] = field(default_factory=dict)
...
def _default_kg_linker(text: str, entity_type: EntityType) -> Optional[KGNodeRef]:
...
def _service_kg_linker(text: str, entity_type: EntityType) -> Optional[KGNodeRef]:
...
def link_entities_to_kg(
    ...
    kg_linker: Optional[Callable[[str, EntityType], Optional[KGNodeRef]]] = None,
) -> List[KGNodeRef]:
...
def _path_from_mounts(dataset_id: str, mounts: Dict[str, Any]) -> Optional[Path]:
    # oak_mount datasets
...
def _public_s3_mount_roots(mounts: Dict[str, Any]) -> List[Path]:
    ...
    return _unique_existing_first(dataset_mount_candidates(local_cfg)["public_s3_mount"])
...
def _public_s3_alias_candidates(candidate: DatasetRecord) -> List[str]:
    ...
    values.append(PUBLIC_S3_ALIAS_MAP.get(_normalize_dataset_id(candidate.dataset_id), _normalize_dataset_id(candidate.dataset_id)))
...
def _looks_like_bids_root(path: Optional[Path]) -> bool:
...
def _openneuro_mount_roots(mounts: Dict[str, Any]) -> List[Path]:
    local_cfg = mounts.get("local", {}) if mounts else {}
    return _unique_existing_first(dataset_mount_candidates(local_cfg)["openneuro_mount"])
...
def _normalize_version_token(value: Optional[str]) -> Optional[str]:
    if not value:
        return None
    token = value.strip()
...
def _source_access_cache_key(dataset_id: str, requested_version: Optional[str]) -> str:
    version = _normalize_version_token(requested_version) or ""
    return f"{dataset_id.lower().strip()}::{version.lower()}"


def _cache_get_source_access(key: str) -> Optional[Dict[str, Any]]:
    now = time.time()
...
def _extract_s3_uri_from_url(raw: str) -> Optional[str]:
    text = (raw or "").strip()
    if not text:
        return None
...
def _collect_required_file_status(
    bids_path: Optional[Path], analysis_goal: str
) -> Dict[str, Any]:
...
def _run_bids_validator(bids_path: Optional[Path]) -> Dict[str, Any]:
...
def _attempt_openneuro_auto_heal(
    dataset_simple_id: str,
    current_bids_path: Optional[Path],
    missing_patterns: List[str],
) -> Dict[str, Any]:
    result: Dict[str, Any] = {
        "attempted": False,
...
def _participants_columns(bids_path: Optional[Path]) -> List[str]:
    if not bids_path:
        return []
...
def _semantic_requirements(
    analysis_goal: str, semantic_intent: Optional[str]
) -> List[str]:
...
def _resolve_catalog_candidate(
    ...
    dataset_ref: str,
) -> tuple[Optional[DatasetRecord], str, List[str]]:
...
def resolve_dataset_reference(
    ...
    candidate: Optional[DatasetRecord]
    if match_id:
    ...
    return DatasetResolution(
        ...
        local_path=resources.local_path if resources else _path_from_mounts(candidate.dataset_id, mounts),
        kg_node_id=kg_node,
...
def _fallback_dataset_resolution_from_kg(ds: Any) -> Optional[DatasetResolution]:
