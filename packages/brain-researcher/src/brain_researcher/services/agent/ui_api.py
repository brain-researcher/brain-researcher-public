...
def api_health_full():
    ...
    # BR-KG
    neurokg_url = os.getenv("NEUROKG_API_URL")
    if not neurokg_url:
        try:
            from brain_researcher.services.agent.config import resolve_neurokg_url

            neurokg_url = resolve_neurokg_url()
        except Exception as exc:  # pragma: no cover - fallback path
            _record("neurokg", "degraded", None, f"unresolved: {exc}")
            neurokg_url = None

    if neurokg_url:
        t0 = time.perf_counter()
        try:
            resp = req_lib.get(f"{neurokg_url}/health", timeout=1.5)
            latency = (time.perf_counter() - t0) * 1000
            if resp.ok:
                _record("neurokg", "ok", latency)
            else:
                _record("neurokg", "degraded", latency, f"status={resp.status_code}")
        except Exception as exc:
            latency = (time.perf_counter() - t0) * 1000
            _record("neurokg", "down", latency, f"error={exc}")
    ...
    if neurokg_url:
        t0 = time.perf_counter()
        try:
            stats_resp = req_lib.get(f"{neurokg_url}/health/stats", timeout=2.0)
            latency = (time.perf_counter() - t0) * 1000
            if stats_resp.ok:
...
# Upload directory (configurable via env). Fallback to cwd/.uploads if /tmp is not writable.
