...
from brain_researcher.services.agent.error_handling import ErrorHandler, ErrorCategory
...
class ToolExecutionRequest:
    ...
    mode: Optional[ExecutionMode] = None  # Auto-detect if not specified
    category: Optional[ToolCategory] = None  # Auto-detect if not specified
...
class ToolExecutionResult:
    ...
    result: Optional[Union[ToolResult, Dict[str, Any]]] = None
    command: Optional[str] = None  # Generated command if applicable
    ...
    error_category: Optional[str] = None  # infra|tool|data|stats|concept|user_input
...
class ToolExecutor:
    ...
    def __init__(
        ...
        neurodesk_tools: Optional[NeurodeskTools] = None,
        max_workers: int = 4,
        ...
        evidence_writer: Optional[ToolEvidenceWriter] = None,
        failure_writer: Optional[Neo4jFailureWriter] = None,
    ):
        """
    ...
    def _apply_multiagent_tool_gate(
        ...
        request: ToolExecutionRequest,
        *,
        tool: Optional[NeuroKGToolWrapper],
    ...
    def _build_provenance_metadata(
        self, recorder, extra: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def execute(self, request: ToolExecutionRequest) -> ToolExecutionResult:
        ...
        if os.getenv("BR_MCP_EXECUTION_GATE", "").strip().lower() in {
            ...
            try:
                ...
                issues = mcp_server._policy_check_tool(spec)
    ...
    def _attach_failure_taxonomy(
        ...
        *,
        ...
        try:
            returncode: Optional[int] = None
            stderr: Optional[str] = None
            error_message: Optional[str] = exec_result.error
            ...
            if isinstance(payload, dict):
                ...
                if hasattr(payload, "error"):
                    try:
                        error_message = error_message or getattr(payload, "error")
                    except Exception:
                        pass
                if hasattr(payload, "metadata"):
                    try:
                        md = getattr(payload, "metadata") or {}
    ...
    def _determine_backend(
        self, request: ToolExecutionRequest, tool: Optional[NeuroKGToolWrapper] = None
    ) -> ExecutionBackend:
    ...
    def _get_tool(self, tool_name: str) -> NeuroKGToolWrapper:
    ...
    def _execute_with_timeout(
        self, tool: NeuroKGToolWrapper, parameters: Dict[str, Any], timeout: float
    ...
    def _get_resource_usage(self, execution_id: Optional[str]) -> Dict[str, Any]:
    ...
    def _record_tool_evidence(
        ...
        try:
            if isinstance(result, dict):
                ...
                version = getattr(result, "metadata", {}) and getattr(
                    result, "metadata"
                ).get("version")
        except Exception:
            version = None
