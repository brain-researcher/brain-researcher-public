...
def run_checks(file_paths: Optional[List[str]] = None, test_command: Optional[str] = None) -> ExecutionResult:
    ...
    if file_paths:
        ...
        for p in plan.candidate_paths:
            ...
            if proc.returncode != 0:
                errors.append(proc.stderr or proc.stdout or f"py_compile failed for {p}")

        duration = int((time.time() - start) * 1000)
        if errors:
            return ExecutionResult(success=False, stdout="", stderr="\n".join(errors), exit_code=1, duration_ms=duration)
        return ExecutionResult(success=True, stdout="", stderr="", exit_code=0, duration_ms=duration)
