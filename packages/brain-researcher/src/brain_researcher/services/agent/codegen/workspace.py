...
from brain_researcher.services.agent.codegen.context import FileSnippet, ExecutionResult
...
class Workspace:
    ...
    def __init__(self, repo_root: Path, workdir: Optional[Path] = None):
    ...
    def run_checks(self, test_command: Optional[str] = None) -> ExecutionResult:
    ...
    def files_touched(self) -> List[str]:
        return sorted(self._touched) if self._touched else [str(p) for p in self._materialized]
