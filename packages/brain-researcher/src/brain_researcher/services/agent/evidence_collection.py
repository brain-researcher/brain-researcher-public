...
class Evidence:
    ...
    def to_citation(self) -> Optional[str]:
...
class EvidenceCollector:
    ...
    def collect(
        ...
        logger.debug(f"Collected evidence: {evidence.evidence_id} ({type.value} from {source})")
    ...
    def end_chain(self) -> Optional[EvidenceChain]:
        """End the current evidence chain."""
        if self.current_chain:
            chain = self.current_chain
            self.current_chain = None
            logger.info(f"Ended evidence chain: {chain.chain_id} ({len(chain.steps)} steps)")
            return chain
        return None
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                ...
                "evidence_types": {
                    type.value: len(self.by_type.get(type, []))
                    for type in EvidenceType
                },
    ...
    def export_jsonld(self, output_path: Optional[Path] = None) -> Path:
    ...
    def generate_run_card(self) -> Dict[str, Any]:
        ...
        params = [e.to_dict() for e in self.get_evidence_by_type(EvidenceType.PARAMETER)]
        pubs = [
            e.to_dict() for e in self.get_evidence_by_type(EvidenceType.PUBLICATION)
        ]
    ...
    def save_run_card(self, output_path: Optional[Path] = None) -> Path:
    ...
    def _summarize_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "result_count": len(response.get("results", [])) if "results" in response else None
        }
...
class EvidenceAPI:
    ...
    def counts(self) -> Dict[str, int]:
        return {
            t.value: len(self.collector.by_type.get(t, [])) for t in EvidenceType
        }
