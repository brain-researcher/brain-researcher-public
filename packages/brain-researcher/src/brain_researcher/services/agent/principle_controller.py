...
def _find_principle(principles: list[dict[str, Any]], principle_id: str) -> dict[str, Any] | None:
...
def _build_empty_state(
    *,
    ...
    return {
        ...
        "query_hash": hashlib.md5(str(query or "").strip().lower().encode()).hexdigest(),
...
def _select_active_principle_id(
    ...
    if preferred_id and preferred_id in posterior:
        top_score = max(float(value) for value in posterior.values()) if posterior else 0.0
...
def initialize_principle_state(
    *,
    ...
    if record is None:
        ...
        posterior = _normalize_distribution({p["principle_id"]: 1.0 for p in principles})
        active_principle_id = (
            taste_mode if taste_mode in posterior else _select_active_principle_id(posterior)
        )
...
def update_principle_state(
    *,
    ...
    anomaly_state = dict(state.get("anomaly_state") or {"counts": {}, "latest_flags": []})
    ...
    if active_principle_id:
        if anomaly_flags:
            log_weights[active_principle_id] = log_weights.get(
                active_principle_id, math.log(_EPSILON)
            ) - 0.18
        elif n_returned > 0:
            log_weights[active_principle_id] = log_weights.get(
                active_principle_id, math.log(_EPSILON)
    ...
    baseline_mass = max((_safe_float(value, 0.0) for value in posterior.values()), default=0.0)
    if baseline_mass <= 0:
