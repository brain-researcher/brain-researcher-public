...
class PlanRecord:
    ...
    linear_issue_id: Optional[str] = None  # Deprecated alias for compatibility
...
class PrincipleSessionRecord:
    ...
    taste_mode: str
    controller_mode: str
    active_principle_id: Optional[str]
...
class PlanMemory:
    ...
    def ensure_plan_record(
        ...
        *,
        ...
        if not isinstance(steps, list):
            dag = plan_payload.get("dag") if isinstance(plan_payload.get("dag"), dict) else {}
    ...
    def record_failure(
        ...
        *,
        ...
        taxonomy_json = json.dumps(error_taxonomy) if error_taxonomy is not None else None
        actions_json = json.dumps(recovery_actions) if recovery_actions is not None else None
        with self._get_connection() as conn:
            conn.execute(
                """
        ...
        for row in rows:
            failures.append(
                FailureRecord(
                    ...
                    is_retryable=bool(row["is_retryable"]) if row["is_retryable"] is not None else None,
                    error_message=row["error_message"],
    ...
    def get_principle_session(
        self, session_key: str
    ) -> Optional[PrincipleSessionRecord]:
    ...
    def append_principle_event(
        ...
        *,
        ...
        step_id: Optional[str] = None,
        active_principle_id: Optional[str] = None,
    ) -> str:
    ...
    def get_tracker_issue(self, plan_id: str) -> Optional[Dict[str, str]]:
    ...
    def get_linear_issue_id(self, plan_id: str) -> Optional[str]:
        """Deprecated compatibility wrapper."""
    ...
    def recall_similar(
        ...
        if workspace_id:
            visibility_sql += " OR (workspace_id = ? AND shared_level IN ('workspace', 'global'))"
    ...
    def get_plan(self, plan_id: str) -> Optional[PlanRecord]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM plan_records WHERE plan_id = ?",
                (plan_id,)
            )
            row = cursor.fetchone()
            if row:
    ...
    def share_plan(
        ...
        with self._get_connection() as conn:
            # Check ownership
            cursor = conn.execute(
                "SELECT user_id FROM plan_records WHERE plan_id = ?",
                (plan_id,)
            )
            row = cursor.fetchone()
...
def create_plan_memory(db_path: Optional[str] = None) -> PlanMemory:
    """
