...
from brain_researcher.services.neurokg.etl.loaders.session_snapshot_loader import (
...
def _get_latest_plan_job_store() -> Any | None:
    try:
        ...
        job_store = getattr(orchestrator_app.state, "job_store", None)
...
_MCP_SURFACE_NAME_ALIASES: dict[str, str] = {
    ...
    "neurokg.find_structural_leverage": "kg_find_structural_leverage",
    "neurokg.detect_contradiction_motifs": "kg_detect_contradiction_motifs",
    "neurokg.sample_ood_hypothesis": "kg_sample_ood_hypothesis",
    "neurokg.detect_topology_shifts": "kg_detect_topology_shifts",
}
...
def _run_roots_for_read() -> list[Path]:
    return list(get_mcp_run_roots_for_read(RUN_ROOT))
...
MCP_SETUP_PATH = (
    (os.getenv("BR_MCP_SETUP_PATH") or "/mcp/setup").strip() or "/mcp/setup"
)
...
def _ensure_dirs() -> None:
    ...
    _assert_run_root_writable(RUN_ROOT)
...
def _normalize_research_log_source(raw: Any) -> str:
    source = str(raw or "agent").strip().lower()
    if source not in {"agent", "user"}:
        raise ValueError("source must be one of: agent, user")
    return source
...
def _normalize_research_event_kind(raw: Any) -> str:
    ...
    if kind not in {"start", "note"}:
        raise ValueError("kind must be one of: start, note")
    return kind
...
def _research_session_index_path() -> Path:
    return RUN_ROOT / _RESEARCH_SESSION_INDEX_FILENAME
...
def _build_external_review_directive(
    ...
    *,
    ...
    directive = {
        ...
        "verdict_schema": ScientificReviewVerdict.model_json_schema(),
...
def _compat_alias_usage_path() -> Path:
    return RUN_ROOT / "compat_alias_usage.json"
...
def _prepare_spec_for_network_policy(
    ...
    *,
    ...
    domains = _resolve_local_loopback_domains(spec)
    if not domains:
        return spec
    prepared = _spec_with_local_loopback_caps(spec, domains)
    if patch_catalog:
        _patch_catalog_local_loopback_caps(spec.name, domains)
    return prepared


def _policy_check_tool(spec: ToolSpec) -> list[dict[str, str]]:
    spec = _prepare_spec_for_network_policy(spec, patch_catalog=False)
    issues: list[dict[str, str]] = []

    # Conservative defaults: don't run dangerous tools unless explicitly enabled.
    ...
    # Network gating: use tags/metadata when available, fallback to name heuristics.
    local_loopback_domains = _resolve_local_loopback_domains(spec)
    if (
        not ALLOW_NETWORK
        and _tool_requires_network(spec)
        and not local_loopback_domains
    ):
        issues.append(
            {
                "level": "error",
...
def _dependency_status_snapshot() -> dict[str, Any]:
    ...
    run_root_writable, run_root_error = _check_run_root_writable(RUN_ROOT)
...
def _startup_hard_health_checks() -> dict[str, Any]:
    ...
    if isinstance(run_root_status, dict) and not run_root_status.get("writable", False):
        raise RuntimeError(
            f"RUN_ROOT is not writable: {RUN_ROOT} "
...
def _stamp_claim_contract_review_verdict(
    ...
    *,
    ...
    is_blocked = claim_mode == "confirmatory" and bool(blocking)
    report_gate_status = (
...
def _run_post_execution_review_gate(run_id: str) -> None:
    ...
    On confirmatory runs with any block-action correctness finding, flips the
    run status to ``review_blocked``. Exploratory runs proceed but the warning
    findings are stamped into ``record.progress['review_caveats']`` so the
    report can render them as caveats.
    """
    ...
    with lock:
        ...
        if claim_mode == "confirmatory" and blocking:
...
def slurm_guide(
    ...
    Ships with ``sherlock_default`` profile; add your own at
    ...
    if include_workflows:
        ...
        for row in workflow_rows_by_id.values():
            ...
            if normalized_modalities:
                ...
                if not (row_modalities & normalized_modalities):
                    continue
    ...
    *,
    ...
    for purpose, tool_id in _infer_preflight_route_tool_ids(query):
        if any(
            ...
            and _record_dict(call.get("arguments")).get("tool_id") == tool_id
            for call in calls
        ):
            continue
        _append_preflight_next_call(
            ...
            rationale="Use BR's query-derived route shortlist to commit to a concrete scientific tool.",
        )
...
def plan_preflight(
    ...
    with semantic_matching_scope(semantic):
        ...
        ctx: dict[str, Any] = {"runtime_surface": "plan_preflight"}
        ...
        qsm_gate = tool_candidates.get("task_type_gate")
        tool_candidates = tool_candidates.get("tools", [])
        if qsm_gate is None:
...
def request_external_scientific_review_directive(
    ...
    the base four-axis schema. If a ``session_id`` (or ``client_session_id``)
    is provided, the directive issuance is also recorded via
    ``log_research_event`` so the submit path can cross-reference it later.
    """
...
def submit_external_scientific_review_verdict(
    ...
    scientific_review_directive`` -> external agent -> BR round-trip.
    """
...
def _write_session_digests_to_kg(digests: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    from brain_researcher.services.neurokg.db.schema import setup_schema
...
@mcp.tool()
def memory_write(card_type: str, card_data: dict[str, Any]) -> dict[str, Any]:
    """Validate and persist a derived memory card or relation event."""
    try:
        normalized_type = _first_text_value(card_type)
        if not normalized_type:
            raise ValueError("card_type is required")
        if not isinstance(card_data, dict):
            raise ValueError("card_data must be an object")
        return MemoryStore(run_root=RUN_ROOT).write(normalized_type, card_data)
    except Exception as exc:
        return {"ok": False, "error": "memory_write_failed", "message": str(exc)}
...
def memory_search(
    ...
    try:
        ...
        return MemoryStore(run_root=RUN_ROOT).search(
            normalized_query,
...
@mcp.tool()
def run_bundle_get(run_id: str) -> dict[str, Any]:
...
def run_list(limit: int = 20, cursor: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        primary_root = RUN_ROOT.expanduser().resolve()
...
def _compact_claim_memory_context(
    ...
    *,
    ...
    try:
        search_resp = MemoryStore(run_root=RUN_ROOT).search(
            search_query,
...
def _persist_verified_claim_memory(
    *,
    ...
    try:
        write_resp = MemoryStore(run_root=RUN_ROOT).write("claim_memory", payload)
        card_id = (
        ...
        if card_id and relation in _PERSISTABLE_CLAIM_RELATIONS:
            for raw_card in list(claim_memory_context.get("cards") or []):
                ...
                if not rhs_claim_id or rhs_claim_id == card_id:
                    continue
                event_resp = MemoryStore(run_root=RUN_ROOT).write(
...
_KG_PROBE_SUPPORTED_TYPES: tuple[str, ...] = (
    "structural_leverage",
...
def kg_hypothesis_candidate_cards(
    ...
    top_n: int = 5,
    top_k: int = 20,
    taste_mode: str = "balanced",
    controller_mode: str = "principle_v0",
    candidate_lane_mode: str = "broad",
    ...
    workflow_params = _compact_kwargs(
        {
            ...
            "top_k": top_k_i,
            "n_samples": top_n_i,
            "taste_mode": str(taste_mode or "balanced").strip().lower() or "balanced",
            "controller_mode": str(controller_mode or "principle_v0").strip()
            or "principle_v0",
            "candidate_lane_mode": str(candidate_lane_mode or "broad").strip()
            or "broad",
            "frontier_mode": (
                str(frontier_mode or "off").strip().lower()
                if str(frontier_mode or "off").strip().lower() != "off"
                else None
            ),
...
def kg_detect_topology_shifts(
    ...
    if normalized_mode not in {"detect", "apply"}:
        return {"ok": False, "error": "mode must be one of: detect, apply"}
...
def _dr_build_normalized_payload(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        # Small compatibility aliases for existing consumers.
        "text": summary,
        "documents": [
...
def google_deep_research_start(
    ...
    if stores:
        try:
            if (
                hasattr(genai, "types")
                and hasattr(genai.types, "Tool")
                and hasattr(genai.types, "FileSearch")
            ):
                tools = [
                    genai.types.Tool(
                ...
                tools = [{"type": "file_search", "file_search_store_names": stores}]
        except Exception:
            tools = [{"type": "file_search", "file_search_store_names": stores}]
...
def google_deep_research(
    ...
    def _extract_text_from_candidates(value: Any) -> str:
        candidates = _read_field(value, "candidates")
    ...
    try:
        ...
        if not normalized_text:
            normalized_text = _extract_text_from_candidates(resp)
        if not normalized_text:
            normalized_text = _extract_text_from_candidates(raw_response)
