...
the default ships a Stanford Sherlock ``your_pi_group`` profile for back-compat,
...
DEFAULT_PROFILE = os.environ.get("BR_SLURM_PROFILE", "sherlock_default")
...
# configs/slurm/profiles/sherlock_default.yaml.
_FALLBACK_PROFILES: dict[str, dict[str, Any]] = {
    "sherlock_default": {
        "account": "your_pi_group",
        "interactive_partition": "your_pi_group",
        "interactive_qos": "<pi_group>_interactive",
        "batch_partition": "your_pi_group",
        "batch_qos": "your_pi_group",
        "module_use": "/share/PI/<pi_group>/modules",
...
def _read_text_source(script_text: str | None, script_path: str | None) -> tuple[str | None, str | None]:
    if script_text:
...
def sherlock_get_guide(topic: GuideTopic, pi_group: str = "your_pi_group") -> dict[str, Any]:
    ...
    if topic == "login":
        return _ok(
            ...
            commands=[
                {"label": "ssh_login", "command": "ssh <sunet>@login.sherlock.stanford.edu"},
                {
                    "label": "ssh_alias",
                    "command": (
                        "Host sherlock\n"
    ...
    if topic == "storage":
        return _ok(
            ...
            routes=[
                {"filesystem": "$HOME", "use_for": "small code, configs, personal tools", "avoid_for": "large datasets"},
                {"filesystem": "$PI_HOME", "use_for": "shared non-purged project assets", "avoid_for": "large temporary analysis outputs"},
                {"filesystem": "$SCRATCH", "use_for": "large temporary files", "avoid_for": "durable storage"},
                {"filesystem": "$PI_SCRATCH", "use_for": "group temporary working area", "avoid_for": "durable storage"},
                {"filesystem": "$LOCAL_SCRATCH", "use_for": "per-job temporary files", "avoid_for": "anything to keep after the job"},
                {"filesystem": "$OAK/data", "use_for": "durable shared datasets", "avoid_for": "ephemeral analysis scratch"},
            ],
            commands=[
                {"label": "home_usage", "command": "df -h $HOME"},
                {"label": "scratch_usage", "command": "df -h $SCRATCH"},
                {"label": "user_quota", "command": "lfs quota -u <sunetid> -h /scratch/"},
                {"label": "group_quota", "command": f"lfs quota -g {pi_group} -h /scratch/"},
            ],
            notes=[
    ...
    if topic == "batch":
        return _ok(
            ...
            commands=[
                {"label": "submit", "command": "sbatch -o out.%j -e err.%j yourScript.sh arg1 arg2"},
    ...
    if topic == "transfer":
        return _ok(
            ...
            commands=[
                ...
                {"label": "sshfs_unmount", "command": "fusermount -u ~/sherlock_oak_mount"},
            ],
            notes=[
    ...
    if topic == "quota":
        return _ok(
            ...
            notes=["Do not report concrete quota values unless you have current command output."],
            warnings=[],
            sources=[SCRIPT_SOURCES[2]],
        )
    if topic == "acl":
            ...
            commands=[
                {
                    ...
                    "command": (
                        ...
                        "--user <sunetid> --group oak_<pi_group>"
                    ),
                }
            ],
            notes=[
    ...
    if topic == "logs":
        return _ok(
            ...
            commands=[
                ...
                {"label": "accounting", "command": "sacct -j <jobid> --format=JobID,State,ExitCode,Elapsed,MaxRSS"},
    ...
    if topic == "debugging":
        return _ok(
            ...
            commands=[
                {"label": "queue_reason", "command": "squeue -j <jobid> -o '%i %T %r %R'"},
                {"label": "accounting", "command": "sacct -j <jobid> --format=JobID,State,ExitCode,Elapsed,MaxRSS,NodeList"},
...
def sherlock_render_sbatch_script(
    ...
    *,
    ...
    if template_kind == "cpu_array":
        ...
        body_command = command or f'eval "$(sed -n "${{SLURM_ARRAY_TASK_ID}}p" {task_file_path})"'
    ...
    if template_kind.startswith("gpu") and "--gpus" not in script_text and "--gpus-per-node" not in script_text:
...
def sherlock_validate_sbatch_script(
    *,
    ...
    if ("gpus" in directives or "gpus-per-node" in directives) and "partition" not in directives:
        warnings.append("GPU job should set an explicit partition.")
    if "python " in text and "module load" not in text and "conda activate" not in text and "source " not in text:
        warnings.append("Python command found without an obvious module or environment activation block.")
    if directives.get("output", "").startswith("/") and not Path(directives["output"]).expanduser().parent.exists():
        warnings.append("Stdout path parent does not exist locally.")
    if directives.get("error", "").startswith("/") and not Path(directives["error"]).expanduser().parent.exists():
...
def sherlock_patch_sbatch_script(
    *,
    ...
    if "convert to array" in request and "array" not in _parse_sbatch_directives(updated):
    ...
    return _ok(
        ...
        validation=sherlock_validate_sbatch_script(script_text=updated, cluster_profile=cluster_profile),
        sources=[SCRIPT_SOURCES[0], SCRIPT_SOURCES[3]],
    )
    ...
    if include_sacct:
        ...
        if s["ok"] and s["stdout"].strip():
            ...
            if len(lines) >= 2:
                ...
                rows = [
                    dict(zip(header, row.split("|"), strict=False))
                    for row in lines[1:]
                ]
                result["sacct"] = rows
        elif not s["ok"]:
...
def sherlock_job_logs(
    *,
    ...
    if job_id and not log_path:
        inspection = sherlock_job_inspect(job_id, include_squeue=False, include_sacct=False, include_scontrol=True)
    ...
    if not any([resolved_stdout, resolved_stderr]):
        return _error("no_log_source_found", warnings=warnings, sources=[SCRIPT_SOURCES[3]])

    payload: dict[str, Any] = {"job_id": job_id, "warnings": warnings, "sources": [SCRIPT_SOURCES[3]]}
    ...
    def _read_log(path_str: str) -> str:
        ...
        if grep:
            filtered = [line for line in text.splitlines() if grep.lower() in line.lower()]
            text = "\n".join(filtered)
...
def sherlock_diagnose_job_failure(
    *,
    ...
    combined = "\n".join(part for part in [stdout_text or "", stderr_text or "", script_text or ""] if part)
    ...
    if "invalid qos" in lowered or "invalid qos specification" in lowered:
        ...
        next_steps.append("Switch the job to a valid Sherlock qos for your partition/account.")
    ...
    elif "out of memory" in lowered or "oom" in lowered or "oom-kill" in lowered or state.startswith("OUT_OF_MEMORY"):
        likely_cause = "oom"
        confidence = "high"
        ...
        next_steps.append("Check file ownership, log destinations, and dataset permissions.")
    ...
    if likely_cause == "unknown":
        next_steps.append("Inspect stdout/stderr and Slurm reason codes before changing resources.")
