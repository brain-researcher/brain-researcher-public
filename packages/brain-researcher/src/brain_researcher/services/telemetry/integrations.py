...
class NeuroKGTelemetry(TelemetryIntegration):
    """Telemetry integration for BR-KG service."""
    
    def __init__(self, collector: Optional[TelemetryCollector] = None, config: Optional[TelemetryConfiguration] = None):
        super().__init__(ServiceType.NEUROKG, collector, config)
...
def create_neurokg_telemetry(config: Optional[TelemetryConfiguration] = None) -> NeuroKGTelemetry:
    """Create telemetry integration for BR-KG service."""
    return NeuroKGTelemetry(config=config)
