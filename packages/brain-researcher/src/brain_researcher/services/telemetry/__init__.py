...
class TelemetrySystem:
    ...
    def get_service_integration(self, service: ServiceType) -> TelemetryIntegration:
        ...
        if service not in self.integrations:
            ...
            elif service == ServiceType.NEUROKG:
                self.integrations[service] = NeuroKGTelemetry(self.collector, self.config)
...
def get_sentry_integration() -> Optional[SentryIntegration]:
...
def create_service_telemetry(service: ServiceType,
    ...
    if service == ServiceType.AGENT:
        ...
        return create_neurokg_telemetry(config)
    elif service == ServiceType.WEB_UI:
