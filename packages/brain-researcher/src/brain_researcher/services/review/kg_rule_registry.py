...
def get_implemented_kg_review_rule_mappings(
    *,
    ...
    The default path uses the project NeuroKG client and fail-opens to no
...
def get_kg_review_implementation_rule_ids(
    *,
    ...
    """Return review-gate implementation rule ids represented in NeuroKG."""
...
def evaluate_kg_review_registry(
    ...
    *,
    ...
    """Evaluate implemented rules selected from the NeuroKG rule registry."""
    ...
    for finding in findings:
        ...
        if kg_rule_ids:
            ...
            evidence = [
                (
                    "NeuroKG review implementation catalog selected configured rule "
                    f"{finding.rule_id}."
                )
            ]
        enriched.append(
