...
from .manager import TenantUser, TenantConfiguration
...
class DataIsolationManager:
    ...
    def __init__(self, neo4j_db):
        ...
        self._current_user: Optional[TenantUser] = None
    ...
    @contextmanager
    def tenant_context(self, tenant_id: str, user: Optional[TenantUser] = None):
    ...
    def execute_isolated_query(
        ...
        try:
            ...
            self._log_audit_event('query_execution_failed', {
                'tenant_id': self._current_tenant_id,
                'error': str(e),
                'query': cypher_query[:200] + '...' if len(cypher_query) > 200 else cypher_query
            })
            raise
    ...
    def _apply_tenant_isolation(
        ...
        if allow_cross_tenant:
            ...
            if not self._is_admin_user():
                raise IsolationViolationError("Cross-tenant access not allowed for this user")
            return query, params
    ...
    def _isolate_read_query(self, query: str) -> str:
        ...
        match_pattern = r'MATCH\s+([^W]*?)(?=\s+WHERE|\s+RETURN|\s+WITH|\s+OPTIONAL|\s+MATCH|$)'
        ...
        modified_query = re.sub(match_pattern, add_tenant_filter, modified_query, flags=re.IGNORECASE)
    ...
    def _isolate_write_query(self, query: str) -> str:
        ...
        def add_tenant_property(match):
            ...
            if ':' in node_pattern and '{' in node_pattern:
                ...
                if properties_part == '{}':
                    ...
                    properties_part = properties_part[:-1] + ', _tenant_id: $_isolation_tenant_id}'
                ...
                if '{' in node_pattern:
                    # Has properties
                    node_pattern = node_pattern.replace('}', ', _tenant_id: $_isolation_tenant_id}')
                else:
                    # No properties
        ...
        modified_query = re.sub(create_pattern, add_tenant_property, modified_query, flags=re.IGNORECASE)
    ...
    def _isolate_set_operation(self, query: str) -> str:
        ...
        if 'WHERE' not in query.upper():
            ...
            query = query[:set_pos] + 'WHERE n._tenant_id = $_isolation_tenant_id ' + query[set_pos:]
    ...
    def _is_read_query(self, query: str) -> bool:
        ...
        return any(query_upper.startswith(keyword) for keyword in [
            'MATCH', 'OPTIONAL', 'WITH', 'UNWIND', 'CALL'
        ])
    
    def _is_write_query(self, query: str) -> bool:
        """Check if query is a write operation"""
        query_upper = query.upper()
        return any(keyword in query_upper for keyword in [
    ...
    def _is_admin_user(self) -> bool:
        ...
        return (
            ...
            (self._current_user.role == 'admin' or 'admin' in self._current_user.permissions)
        )
    ...
    def create_isolated_relationship(
        ...
        if not self._verify_node_access(start_node) or not self._verify_node_access(end_node):
            raise IsolationViolationError("Cannot create relationship across tenant boundaries")
        
        # Add tenant ID to relationship properties
        isolated_properties = (properties or {}).copy()
        isolated_properties['_tenant_id'] = self._current_tenant_id
        
        return self.neo4j_db.create_relationship(start_node, end_node, rel_type, isolated_properties)
    ...
    def validate_tenant_data_integrity(self, tenant_id: str) -> Dict[str, Any]:
        ...
        """
        result = self.neo4j_db._run(query).single()
        integrity_report['relationships_without_tenant_id'] = result['count'] if result else 0
        ...
        """
        result = self.neo4j_db._run(query).single()
        integrity_report['cross_tenant_relationships'] = result['count'] if result else 0
    ...
    def fix_tenant_data_isolation(self, tenant_id: str) -> Dict[str, Any]:
        ...
        fix_report['actions_taken'].append("Analysis completed - manual intervention required")
    ...
    def get_tenant_access_log(
        ...
        return [
            event for event in self.audit_events[-limit:] 
    ...
    def _log_query_execution(self, original_query: str, isolated_query: str):
        ...
        if original_query != isolated_query:
            self._log_audit_event('query_isolated', {
                'tenant_id': self._current_tenant_id,
    ...
    def _log_audit_event(self, event_type: str, details: Dict[str, Any]):
        ...
        logger.info("Audit event: %s for tenant %s", event_type, self._current_tenant_id)