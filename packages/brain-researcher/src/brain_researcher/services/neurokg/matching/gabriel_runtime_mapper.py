...
from brain_researcher.services.neurokg.etl.evaluation import gabriel_onvoc_map as _gom
...
class GabrielRuntimeMapper:
    ...
    def from_env(cls) -> GabrielRuntimeMapper:
        ...
        return cls(
            enabled=_env_flag("NEUROKG_GABRIEL_RUNTIME_ENABLED", True),
            embedding_enabled=_env_flag("NEUROKG_GABRIEL_RUNTIME_EMBEDDING_ENABLED", False),
            embedding_backend=os.environ.get("NEUROKG_GABRIEL_RUNTIME_EMBEDDING_BACKEND", "gemini"),
            embedding_model=os.environ.get(
                "NEUROKG_GABRIEL_RUNTIME_EMBEDDING_MODEL", "gemini-embedding-001"
            ),
            embedding_batch_size=_env_int(
                "NEUROKG_GABRIEL_RUNTIME_EMBEDDING_BATCH_SIZE",
                32,
                lower=1,
                upper=256,
            ),
            embedding_timeout_sec=_env_float(
                "NEUROKG_GABRIEL_RUNTIME_EMBEDDING_TIMEOUT_SEC",
                8.0,
                lower=1.0,
                upper=120.0,
            ),
            candidate_top_k=_env_int(
                "NEUROKG_GABRIEL_RUNTIME_CANDIDATE_TOP_K",
                40,
                lower=1,
                upper=200,
            ),
            margin_min=_env_float(
                "NEUROKG_GABRIEL_RUNTIME_MARGIN_MIN",
                0.04,
                lower=0.0,
                upper=1.0,
            ),
            crosswalk_path=os.environ.get("NEUROKG_GABRIEL_RUNTIME_CROSSWALK_PATH"),
            tree_path=os.environ.get("NEUROKG_GABRIEL_RUNTIME_TREE_PATH"),
        )
