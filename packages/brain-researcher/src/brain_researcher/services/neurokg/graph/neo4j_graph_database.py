...
class Neo4jGraphDB:
    def __init__(
        ...
        *,
        ...
        self._run_supports_timeout: bool | None = None
    ...
    def _run(
        ...
        def _run_with_optional_timeout(runner):
            if effective_timeout_s is None:
                return runner(cypher, run_params)
            if self._run_supports_timeout is False:
                return runner(cypher, run_params)
            try:
                result_obj = runner(cypher, run_params, timeout=effective_timeout_s)
                self._run_supports_timeout = True
                return result_obj
            except TypeError:
                # Some driver versions do not accept run(..., timeout=...).
                self._run_supports_timeout = False
                return runner(cypher, run_params)

        if self._tx is not None:
            return _run_with_optional_timeout(self._tx.run)
        session = self._driver.session(database=self._database)
        result = _run_with_optional_timeout(session.run)
