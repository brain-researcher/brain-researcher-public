...
class DualEvidenceGraph(NeuroKGGraphDB):
    """Extended BR-KG graph with dual evidence support."""
    
    def __init__(self, db_path: str = "neurokg_dual_evidence.db"):
    ...
    def get_dual_evidence_stats(self) -> Dict[str, Any]:
        ...
        source_counts = {}
        for node_id, node_data in self.find_nodes("Evidence"):
            source_type = node_data.get("source_type", "unknown")
            source_counts[source_type] = source_counts.get(source_type, 0) + 1
        
        # Count conflicts by type
        conflict_types = {}
        for node_id, node_data in self.find_nodes("ConflictRecord"):
