...
class PlanCache:
    ...
    def get(self, cache_key: str) -> Optional[KnowledgePlan]:
...
class KnowledgePlanner:
    ...
    def _get_entities(
        self, qur: Optional["QueryUnderstandingResult"]
    ) -> List[str]:
    ...
    def _heuristic_plan(
        self,
        query: str,
        qur: Optional["QueryUnderstandingResult"],
        ...
        kg_items = [i for i in evidence.items if i.source_id == "neurokg"]

        # Heuristic 1: Explicit explanation request
        explanation_keywords = ["what is", "explain", "tell me about", "describe", "define"]
        ...
        dataset_keywords = ["find dataset", "search dataset", "which dataset", "looking for data"]
        if any(kw in query_lower for kw in dataset_keywords):
            return KnowledgePlan(
                intent=PlanIntent.DATASET_SELECTION,
                recommended_datasets=[d.metadata.get("dataset_id", d.id) for d in dataset_items[:5]],
        ...
        analysis_keywords = ["analyze", "run", "process", "pipeline", "compute", "glm", "preprocess"]
        ...
        if has_analysis_intent and dataset_items:
            ...
            if resolved_datasets:
                return KnowledgePlan(
                    ...
                    recommended_tools=[t.metadata.get("tool_name", t.title) for t in tool_items[:5]],
    ...
    def _build_llm_prompt(
        ...
        if qur:
            ...
            for ent in qur.entities[:10]:
                if isinstance(ent, dict):
                    prompt_parts.append(f"- {ent.get('text', '')} ({ent.get('entity_type', '')})")
            prompt_parts.append("")
        ...
        for source_id, items in evidence_by_source.items():
            ...
            for item in items[:5]:
                prompt_parts.append(f"- [{item.id}] {item.title} (score: {item.score:.2f})")
    ...
    async def _call_llm(self, prompt: str) -> Optional[KnowledgePlan]:
        """Call LLM to generate a plan."""
        try:
            # Lazy import router
            if self._router is None:
                from brain_researcher.services.agent.router import route_chat
