...
from brain_researcher.services.neurokg.migrations import MigrationManager
...
def create(name: str, migrations_dir: str):
    ...
    Example:
        neurokg-migrate create add_users_table
    """
...
@click.option('--db', '-d', 'db_path', default='neurokg_graph.db',
              help='Database path')
...
def migrate(target: Optional[str], db_path: str, migrations_dir: str, dry_run: bool):
    ...
    Examples:
        neurokg-migrate migrate                    # Run all pending
        neurokg-migrate migrate --target 001       # Migrate to version 001
        neurokg-migrate migrate --dry-run          # Preview changes
    """
...
def rollback(steps: int, db_path: str, migrations_dir: str, force: bool):
    ...
    Examples:
        neurokg-migrate rollback              # Rollback last migration
        neurokg-migrate rollback --steps 3    # Rollback last 3 migrations
    """
    manager = MigrationManager(migrations_dir=migrations_dir, db_path=db_path)
...
def status(db_path: str, migrations_dir: str, as_json: bool):
    ...
    Examples:
        neurokg-migrate status
        neurokg-migrate status --json
    """
    manager = MigrationManager(migrations_dir=migrations_dir, db_path=db_path)
    status = manager.status()
