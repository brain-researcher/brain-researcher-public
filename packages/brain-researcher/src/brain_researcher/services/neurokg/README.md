...
```bash
# Load a single file
python -m brain_researcher.services.neurokg.bulk_loader data.ndjson

# Load with options
python -m brain_researcher.services.neurokg.bulk_loader \
  data.ndjson \
  --batch-size 1000 \
  --workers 4 \
  --verbose
...
python -m brain_researcher.services.neurokg.bulk_loader data_dir/
```
...
```bash
# Run all BR-KG tests
pytest tests/services/neurokg/ -v

# Specific test suites
pytest tests/services/neurokg/test_graphql_comprehensive.py
pytest tests/services/neurokg/test_bulk_loader.py
pytest tests/services/neurokg/test_persisted_queries.py
pytest tests/services/neurokg/test_integration.py
```
