...
class OntologyMapper:
    ...
    def map_concept(
        ...
        if concept_type in self.all_mappings:
            ...
            if concept_lower in mapping_dict:
                for bio2rdf_uri, bio2rdf_label, mapping_type in mapping_dict[concept_lower]:
                    namespace = self._extract_namespace(bio2rdf_uri)
                    mappings.append(ConceptMapping(
                        neurokg_id=f"neurokg:{concept_lower}",
                        neurokg_label=concept,
                        neurokg_type=concept_type,
    ...
    def generate_mapping_sparql(
        ...
        for mapping in mappings:
            neurokg_uri = f"<{mapping.neurokg_id}>"
            bio2rdf_uri = f"<{mapping.bio2rdf_uri}>"
            
            construct_patterns.append(
                f"{neurokg_uri} owl:sameAs {bio2rdf_uri} ."
            )
            construct_patterns.append(
                f"{neurokg_uri} skos:exactMatch {bio2rdf_uri} ."
            )
