...
class SPARQLEndpoint:
    ...
    def __init__(
        self, 
        neo4j_db,
        base_uri: str = "https://neurokg.org/",
        ...
        self.federation_handler = FederationQueryHandler() if enable_federation else None
    ...
    def _handle_sparql_request(self) -> Response:
        ...
        try:
            ...
            if request.method == 'GET':
                ...
                accept_format = request.args.get('format', 
                ...
                accept_format = request.form.get('format', 
                                               request.headers.get('Accept', 'application/sparql-results+json'))
            ...
            result = self._execute_sparql_query(
                query, 
                default_graphs=default_graph,
                named_graphs=named_graph
            )
    ...
    def _execute_sparql_query(
        ...
        try:
            ...
            if self.enable_federation and self._requires_federation(parsed_query):
                ...
                cypher_query, cypher_params = self.translator.translate_query(parsed_query)
    ...
    def _execute_cypher_query(self, cypher_query: str, params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _convert_neo4j_to_sparql_result(
        self, 
        neo4j_result: List[Dict[str, Any]], 
        query_type: str,
        parsed_query
    ) -> Dict[str, Any]:
        """Convert Neo4j result to SPARQL result format"""
    ...
    def _format_select_result(self, neo4j_result: List[Dict[str, Any]], parsed_query) -> Dict[str, Any]:
    ...
    def _format_construct_result(self, neo4j_result: List[Dict[str, Any]], parsed_query) -> Dict[str, Any]:
    ...
    def _format_describe_result(self, neo4j_result: List[Dict[str, Any]], parsed_query) -> Dict[str, Any]:
    ...
    def _format_sparql_value(self, value: Any) -> Dict[str, Any]:
        ...
        if isinstance(value, str):
            ...
            return {"type": "literal", "value": str(value), "datatype": "http://www.w3.org/2001/XMLSchema#decimal"}
        elif isinstance(value, bool):
            return {"type": "literal", "value": str(value).lower(), "datatype": "http://www.w3.org/2001/XMLSchema#boolean"}
        else:
            return {"type": "literal", "value": str(value)}
    
    def _format_sparql_result(self, result: Dict[str, Any], accept_format: str) -> Response:
...
<sparql xmlns="http://www.w3.org/2005/sparql-results#">
    ...
    def _extract_variables(self, parsed_query) -> List[str]:
        ...
        try:
            ...
            logger.warning("Failed to extract variables from SPARQL; defaulting: %s", exc)
    ...
    def _requires_federation(self, parsed_query) -> bool:
        ...
        return 'wikidata' in query_str or 'dbpedia' in query_str or 'service' in query_str
    
    def _get_cache_key(self, query: str, default_graphs: Optional[List[str]], named_graphs: Optional[List[str]]) -> str:
    ...
    def _cache_result(self, cache_key: str, result: Dict[str, Any], execution_time: float):
        """Cache query result"""
