...
DEFAULT_OUTPUT_ROOT = Path("data/neurokg/raw/gabriel")
...
REGION_RULES: list[tuple[str, str, str]] = [
    ...
    ("dlpfc", "Dorsolateral Prefrontal Cortex", "region:dorsolateral_prefrontal_cortex"),
    ("anterior cingulate", "Anterior Cingulate Cortex", "region:anterior_cingulate_cortex"),
...
ASSERTIVE_TOKENS = ("increased", "decreased", "associated", "predicts", "showed", "demonstrated")
...
def _chunked(items: list[PublicationSeed], size: int) -> Iterable[list[PublicationSeed]]:
...
def load_manifest_status(manifest_path: Path | str) -> dict[str, Any]:
    ...
    for shard in shards:
        ...
        shard_path = Path(shard_path_raw).expanduser().resolve() if shard_path_raw else None
    ...
    return {
        ...
        "manifest_ingest_status": (manifest.get("ingest") or {}).get("status", "not_started"),
        "summary": {
            "shards_total": len(shards),
            "shards_ingested": complete_ingest_shards,
            "records_expected": int((manifest.get("counts") or {}).get("records_generated", 0)),
            ...
            "records_heuristic": int((manifest.get("counts") or {}).get("records_heuristic", 0)),
...
class GabrielPipelineGenerator:
    ...
    def generate(
        ...
        *,
        ...
        for shard_index, publication_chunk in enumerate(_chunked(publications, shard_size)):
            ...
            manifest_failure_reasons = manifest["counts"].setdefault("llm_failure_reasons", {})
    ...
    def ingest(
        ...
        *,
        ...
        run_dir = Path(manifest.get("paths", {}).get("run_dir", resolved_manifest.parent))
        checkpoint_path = (
        ...
        per_file_stats = loader_stats.get("per_file", {}) if isinstance(loader_stats, dict) else {}
        ...
        for resolved_path, shard in shard_lookup_by_resolved_path.items():
            ...
            if loader_error:
                ...
                if checkpoint_entry.get("finished_at"):
                    ingest_payload["completed_at"] = str(checkpoint_entry["finished_at"])
    ...
    def _load_publications(
        ...
        *,
        ...
        if publications:
            source_info: dict[str, Any] = {"source": "neo4j", "count": len(publications)}
            if use_cache_fallback:
                publications, enrich_stats = self._enrich_publications_from_cache(publications)
        ...
        *,
        ...
        with text_csv.open("r", encoding="utf-8", newline="") as handle:
            ...
            for row in reader:
                ...
                doi = _normalize_doi(row.get("doi")) or _normalize_doi(metadata.get("doi"))
                title = _clean_text(row.get("title")) or _clean_text(metadata.get("title"))
                ...
                year = (
                    _coerce_int(row.get("publication_year") or row.get("year"))
                ...
                seed = PublicationSeed(
                    ...
                    body=body,
                    coordinate_space=(
                        coordinate_space_by_pmcid.get(pmcid)
                        if pmcid
                        else None
                    ),
                    coordinate_count=(
                        coordinate_counts_by_pmcid.get(pmcid)
                        if pmcid
                        else None
                    ),
                ...
                if current is None or self._publication_richness(seed) > self._publication_richness(
                    current
                ):
        ...
        source_info = _clean_none(
            {
                ...
                "coordinates_csv": str(coordinates_csv) if coordinates_csv.exists() else None,
        ...
        *,
        ...
        for metadata_file in files:
            try:
                ...
                logger.debug("Failed to parse cache file: %s", metadata_file, exc_info=True)
                continue
    ...
    def _publication_from_metadata_record(self, record: dict[str, Any]) -> PublicationSeed | None:
            ...
            for attempt_index in range(1, attempts + 1):
                try:
                    ...
                    if (
                        ...
                        and self._is_retryable_failure(failure_reason)
                    ):
    ...
    def _json_parse_candidates(self, response_text: str) -> list[str]:
        ...
        for base in list(candidates):
            add(re.sub(r"^\s*(?:json|response|output|answer)\s*[:：]\s*", "", base, flags=re.IGNORECASE))
    ...
    def _llm_retry_limit(self) -> int:
        ...
        if (
            not self.model_hint
            and os.environ.get("USE_GEMINI_CLI", "true").lower() in {"1", "true", "yes"}
        ):
            return 2
        return 1
    ...
    def _heuristic_record(self, publication: PublicationSeed) -> dict[str, Any]:
        ...
        section_level_evidence = section in {"abstract", "methods", "results", "discussion"}
        unverifiable_snippet = (
            section == "unknown" and not has_stats and not grounded
        )
    ...
    def _finalize_record(
        ...
        *,
        ...
        signals = self._normalize_signals(
            ...
            publication=publication,
            evidence=evidence,
            mapping_confidence=_clamp01(mapping.get("mapping_confidence"), default=0.55),
            method=method,
        )
    ...
    def _normalize_claim(
        ...
        *,
        ...
        claim_id = _clean_text(payload.get("id")) or _clean_text(payload.get("claim_id"))
        if not claim_id:
    ...
    def _normalize_evidence(
        ...
        *,
        ...
        if not section:
            section = "abstract" if publication.abstract else ("unknown" if publication.body else "title")
        ...
        locatable = _coerce_bool(
            payload.get("locatable"), default=bool(publication.abstract or publication.body)
        )
        ...
        *,
        ...
        return {
            "preregistration": _normalize_method_boolean_block(
                ...
                fallback=signal_payload.get("preregistration")
            ...
            "threshold_correction": _normalize_method_boolean_block(
                ...
                fallback=signal_payload.get("threshold_correction_reported")
    ...
    def _normalize_signals(
        ...
        *,
        ...
        title_hit = _coerce_bool(payload.get("title_hit"), default=label_lower in title_lower)
        ...
        modal_density = _clamp01(
            ...
            default=0.75
            if any(token in full_text for token in UNCERTAIN_TOKENS)
            else 0.25,
        )
        ...
        sample_size_block = method.get("sample_size") if isinstance(method, dict) else {}
        ...
        roi_status = _method_block_status(method.get("roi_definition"), default="unknown")
        ...
        prereg_default = True if prereg_status == "yes" else (False if prereg_status == "no" else None)
        threshold_default = (
            True if threshold_status == "yes" else (False if threshold_status == "no" else None)
        )
        roi_default = True if roi_status == "clear" else (False if roi_status == "unclear" else None)
        operationalization_default = (
            True
            if operationalization_status == "clear"
            else (False if operationalization_status == "unclear" else None)
        )
        open_default = True if open_status == "yes" else (False if open_status == "no" else None)
        ...
        return {
            ...
            "sample_size_adequacy": _clamp01_optional(
                ...
                default=_sample_size_adequacy_from_n(sample_size_n)
            ...
            "method_section": method_section if method_section != "unknown" else evidence.get("section", "unknown"),
        }
    ...
    def _heuristic_method(
        ...
        *,
        ...
        sample_match = SAMPLE_SIZE_PATTERN.search(sample_quote or _publication_blob(publication))
        ...
        return {
            ...
            "roi_definition": {
                "status": "clear" if target_type == "Region" and grounded else "unknown",
                "quote": quote if target_type == "Region" and grounded else None,
                "section": section if target_type == "Region" and grounded else "unknown",
            },
            "operationalization": {
                "status": (
                    "clear"
...
def _publication_blob(publication: PublicationSeed) -> str:
    ...
    return " ".join(chunk.strip() for chunk in chunks if isinstance(chunk, str) and chunk.strip())
...
def _normalize_method_evidence_fields(payload: dict[str, Any]) -> tuple[str | None, str]:
    evidence = payload.get("evidence") if isinstance(payload.get("evidence"), dict) else {}
    quote = _clean_text(payload.get("quote")) or _clean_text(evidence.get("quote"))
    section = _normalize_method_section(payload.get("section") or evidence.get("section"))
...
def _normalize_method_sample_size_block(
    ...
    *,
    ...
    reported_n = _coerce_int(payload.get("reported_n") or payload.get("n") or payload.get("value"))
...
def _infer_open_artifact(text: str | None) -> str:
    lowered = (text or "").lower()
    has_data = any(token in lowered for token in {"open data", "data available", "openneuro", "neurovault"})
    has_code = any(token in lowered for token in {"open code", "code available", "source code", "github", "gitlab"})
