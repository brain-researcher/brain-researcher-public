...
class CognitiveAnnotationFusion:
    ...
    COGNITIVE_TASKS = ['working_memory', 'memory', 'executive', 'attention', 'reasoning', 'cognitive']
    ...
    def _load_mappers(self):
        ...
        try:
            from brain_researcher.services.neurokg.etl.mappers.niclip_task_mapper import get_mapper
            from brain_researcher.services.neurokg.etl.mappers.niclip_spatial_mapper import get_spatial_mapper
            
            self.niclip_task_mapper = get_mapper()
            self.niclip_spatial_mapper = get_spatial_mapper()
            
            self._loaded = (
                self.niclip_task_mapper and self.niclip_task_mapper._loaded and
                self.niclip_spatial_mapper and self.niclip_spatial_mapper._loaded
            )
    ...
    def fuse_annotations(
        ...
        if niclip_result is None and self._loaded and mni_coordinates:
            niclip_result = self._compute_niclip_annotations(
                task_name, mni_coordinates
            )
        
        # Determine task category for weighting
        task_category = self._classify_task(task_name, contrast_name)
        weights = self.config['weights'].get(task_category, self.config['weights']['default'])
        ...
        for construct_id, data in construct_map.items():
            fused = self._fuse_single_construct(
                construct_id, data, weights
            )
        ...
        if validate_with_glm and self.glm_validator and mni_coordinates:
            try:
                ...
                for construct in fused_constructs[:5]:  # Top 5 constructs
                    pred = {
                        'name': construct['name'],
                        'direction': construct.get('evidence', {}).get('llm', {}).get('direction', '+1')
                    }
                ...
                if glm_validation.get('validation_available'):
                    for construct, alignment in zip(fused_constructs[:5], glm_validation['alignments']):
                        construct['glm_alignment'] = {
                            'score': alignment['alignment_score'],
                            'beta_value': alignment['beta_value'],
                            'direction_match': alignment['predicted_direction'] == alignment['actual_direction']
                        }
    ...
    def _compute_niclip_annotations(
        self,
        task_name: str,
        mni_coordinates: List[Tuple[float, float, float]]
    ) -> Dict[str, Any]:
    ...
    def _calculate_fusion_metrics(
        ...
        metrics = {
            ...
            'overlap_ratio': len(llm_ids & niclip_ids) / max(len(llm_ids | niclip_ids), 1)
        }
        
        # Calculate average confidences
        if fused_constructs:
            metrics['avg_confidence'] = np.mean([c['confidence'] for c in fused_constructs])
            metrics['n_conflicts'] = sum(
                1 for c in fused_constructs 
                ...
                if construct.get('evidence', {}).get('conflict', False):
                    ...
                    if conflict_score >= threshold:
                        conflicts.append({
                            ...
                            'niclip_conf': construct['evidence']['niclip']['confidence']
                        })
