...
1. **Database**: Created `neurokg_glmfitlins.db` with:
...
export NEUROKG_GLMFITLINS_DB_PATH=neurokg/data/neurokg/db/neurokg_glmfitlins.db

# Start the server
python neurokg/api/graph_api.py
```
...
```python
from neurokg.examples.glmfitlins_client import GLMFitLinsClient
...
```bash
# 1. Run the pipeline
python neurokg/etl/glmfitlins_ingest/discover_specs.py
python neurokg/etl/glmfitlins_ingest/parse_statsmodel.py
...
python src/brain_researcher/services/neurokg/etl/glmfitlins_ingest/ingest_glm_priors.py --scope all
```
