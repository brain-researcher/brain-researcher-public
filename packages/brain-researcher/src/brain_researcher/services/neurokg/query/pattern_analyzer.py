...
class PatternAnalyzer:
    ...
    def _define_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "cartesian_product": re.compile(r"MATCH\s+\([^)]+\)\s*,\s*\([^)]+\)", re.IGNORECASE),
            ...
            "large_collect": re.compile(r"collect\([^)]+\)(?!.*\[\.\.\d+\])", re.IGNORECASE),
            
            # Optimization opportunities
            "missing_index_hint": re.compile(r"WHERE\s+\w+\.\w+\s*=(?!.*USING INDEX)", re.IGNORECASE),
            "multiple_match": re.compile(r"(MATCH.*){3,}", re.IGNORECASE),
            "expensive_aggregation": re.compile(r"(count|sum|avg|collect)\s*\(\s*\*\s*\)", re.IGNORECASE),
            "redundant_distinct": re.compile(r"DISTINCT.*DISTINCT", re.IGNORECASE),
            "nested_optional": re.compile(r"OPTIONAL MATCH.*OPTIONAL MATCH", re.IGNORECASE),
            
            # Query types
            "simple_lookup": re.compile(r"^MATCH\s+\([^:]+:[^)]+\{[^}]+\}\)\s+RETURN", re.IGNORECASE),
            "traversal": re.compile(r"MATCH.*-\[.*\]-.*RETURN", re.IGNORECASE),
            "aggregation": re.compile(r"(count|sum|avg|min|max|collect)\s*\(", re.IGNORECASE),
            "path_query": re.compile(r"(shortestPath|allShortestPaths|path\s*=)", re.IGNORECASE),
    ...
    def analyze_query(self, query: str, execution_time_ms: float) -> Dict[str, Any]:
        ...
        for pattern_name, pattern_regex in self.patterns.items():
            if pattern_regex.search(query):
                ...
                if pattern_name in ["simple_lookup", "traversal", "aggregation", "path_query", "write_query"]:
                    analysis["query_type"] = pattern_name
                    
        # Suggest optimizations
        analysis["optimizations"] = self._suggest_optimizations(analysis["patterns_detected"])
    ...
    def _update_pattern_stats(self, pattern_name: str, query: str, execution_time_ms: float):
    ...
    def _suggest_optimizations(self, patterns: List[str]) -> List[str]:
        ...
        if "cartesian_product" in patterns:
            suggestions.append("Add relationships between disconnected patterns to avoid cartesian product")
            
        if "missing_where" in patterns:
            suggestions.append("Add WHERE clause to filter results early")
            
        if "unbounded_path" in patterns:
            suggestions.append("Add upper bound to variable-length paths (e.g., [*1..5])")
            
        if "no_limit" in patterns:
            suggestions.append("Add LIMIT clause to restrict result size")
            
        if "large_collect" in patterns:
            suggestions.append("Limit collected items with slice notation (e.g., collect(n)[..100])")
    ...
    def analyze_patterns(self, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
        ...
        slow_queries = [q for q in queries if q["execution_time_ms"] > self.slow_query_threshold_ms]
        report["slow_queries"] = [
            {
                "query": q["query"][:200],
            ...
            for q in sorted(slow_queries, key=lambda x: x["execution_time_ms"], reverse=True)[:10]
        ]
        ...
        problematic = ["cartesian_product", "unbounded_path", "missing_where", "no_limit"]
            ...
            if pattern in self.pattern_stats:
                ...
                if stats.frequency > 0:
                    report["problematic_patterns"].append({
                        ...
                        "impact": "HIGH" if stats.avg_execution_time > 1000 else "MEDIUM"
                    })
                    
        # Optimization opportunities
        optimization_patterns = ["missing_index_hint", "multiple_match", "expensive_aggregation"]
            ...
            if pattern in self.pattern_stats:
                ...
                if stats.frequency > 5:  # Only suggest if pattern is frequent
                    report["optimization_opportunities"].append({
                        "pattern": pattern,
                        "frequency": stats.frequency,
                        "potential_improvement_ms": stats.avg_execution_time * 0.3,  # Estimate 30% improvement
    ...
    def detect_anomalies(self) -> List[Dict[str, Any]]:
        ...
        if total_queries > 0:
            no_limit_ratio = self.pattern_stats.get("no_limit", QueryPattern("", "")).frequency / total_queries
    ...
    def recommend_indexes(self) -> List[Dict[str, str]]:
        ...
        slow_queries = [q for q in self.query_history if q["execution_time_ms"] > self.slow_query_threshold_ms]
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("=" * 60)
        report.append("NEUROKG QUERY PATTERN ANALYSIS REPORT")
        report.append("=" * 60)
        ...
        if analysis.get("problematic_patterns"):
            ...
            for pattern in analysis["problematic_patterns"]:
                report.append(f"  {pattern['pattern']}: {pattern['frequency']} occurrences, avg {pattern['avg_execution_time_ms']:.0f}ms")
                
        # Optimization opportunities
        if analysis.get("optimization_opportunities"):
            report.append(f"\n💡 Optimization Opportunities:")
            for opp in analysis["optimization_opportunities"]:
                report.append(f"  {opp['pattern']}: {opp['frequency']} queries could save ~{opp['potential_improvement_ms']:.0f}ms")
        ...
        if analysis.get("slow_queries"):
            ...
            for sq in analysis["slow_queries"][:3]:
                report.append(f"  {sq['execution_time_ms']:.0f}ms: {sq['query'][:100]}...")
