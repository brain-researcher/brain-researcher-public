...
```json
{
  ...
  "sources": {
    "neurokg": { "n_nodes_scanned": 42, "n_datasets": 1, "n_tasks": 1 },
...
```json
{
  ...
  "sources": {
    "neurokg": { "n_nodes_scanned": 42, "n_datasets": 34, "n_tasks": 120 },
    "literature": { "high_pass": { "n_docs_hit": 5 }, "smoothing_fwhm": { "n_docs_hit": 5 } }
  },
  "spec_family": [
    {
...
**Dev (Flask)**  
```bash
python brain_researcher/services/neurokg/api/graph_api.py
```

**Prod (example)**  
```bash
gunicorn -w 4 -b 0.0.0.0:5000 brain_researcher.services.neurokg.api.graph_api:app
```
