...
def _extract_gt_content(output: dict[str, Any]) -> tuple[dict[str, Any], Any]:
    ...
    for key in ("content", "gt", "ground_truth", "ground_truth_solution", "oracle", "answer", "evaluator", "value"):
...
def _normalize_expected_outputs(raw: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if isinstance(raw_outputs, list):
        for idx, item in enumerate(raw_outputs, start=1):
            ...
            has_gt_keys = any(
                key in normalized
                for key in ("gt", "ground_truth", "ground_truth_solution", "oracle", "answer", "evaluator")
            )
