...
from .magic_link import MagicLinkService, MagicLinkRequest
        ...
        async with httpx.AsyncClient() as client:
            ...
            if provider == 'github' and not user_data.get('email'):
                email_response = await client.get(
                    config.get('userinfo_email_url', 'https://api.github.com/user/emails'),
                    headers=headers
                )
            ...
            logger.info(f"OAuth user authenticated: {user_info.get('email')} (name: {user_info.get('name')}, provider: {provider})")
