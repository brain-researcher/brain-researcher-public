...
from .job_store import JobEvent, JobRecord, JobState, JobStore, GPUSlotManager
...
class MemoryJobStore(JobStore, GPUSlotManager):
    ...
    def __init__(
        ...
        jobs_dict: Optional[Dict[str, JobRecord]] = None
    ):
        """
    ...
    async def enqueue(self, job: JobRecord) -> str:
        ...
        async with self._lock:
            ...
            logger.debug(f"Enqueued job {job.job_id} with priority {job.priority}, state={job.state}")
            return job.job_id
    ...
    async def claim_next(
        ...
        async with self._lock:
            ...
            for job in self._jobs.values():
                ...
                if job.gpu_req > 0:
                    available_slots = self._total_gpu_slots - len(self._gpu_slots_in_use)
            ...
            if job.gpu_req > 0:
                ...
                if len(available) < job.gpu_req:
                    ...
                    logger.warning(f"GPU slot race: job {job.job_id} needs {job.gpu_req} but only {len(available)} available")
                    return None

                # Allocate slots
                assigned = sorted(list(available))[:job.gpu_req]
            ...
            logger.info(f"Worker {worker_id} claimed job {job.job_id} (priority={job.priority}, gpu_req={job.gpu_req})")
            return job
    ...
    async def heartbeat(
        ...
        async with self._lock:
            ...
            if job_id:
                ...
                if job and job.worker_id == worker_id and job.state in (
                    JobState.CLAIMED,
                    JobState.RUNNING,
                ):
                    job.last_heartbeat = now
        ...
        **fields
        ...
        async with self._lock:
            ...
            if state:
                ...
                if state in [JobState.SUCCEEDED, JobState.FAILED, JobState.CANCELLED, JobState.TIMEOUT, JobState.SKIPPED]:
    ...
    async def cancel(
        ...
        async with self._lock:
            ...
            if job.state in [JobState.SUCCEEDED, JobState.FAILED, JobState.CANCELLED, JobState.TIMEOUT, JobState.SKIPPED]:
                logger.debug(f"Job {job_id} already terminal ({job.state}), cannot cancel")
                return False
            ...
            if job.state in [JobState.PENDING, JobState.QUEUED, JobState.CLAIMED]:
                ...
                if job_id in self._gpu_assignments:
                    freed = self._gpu_assignments.pop(job_id)
                    self._gpu_slots_in_use -= set(freed)
                    logger.debug(f"Released GPU slots {freed} for cancelled job {job_id}")
                ...
                logger.warning(f"Unexpected state for cancel: job {job_id} in state {job.state}")
    ...
    async def recover_stale(self, lease_timeout: int = 120) -> Dict[str, int]:
        ...
        async with self._lock:
            now = int(time.time())
            stale_threshold = now - lease_timeout
            ...
            for job in self._jobs.values():
                ...
                # Requeue stale job
                logger.warning(f"Requeuing stale job {job.job_id} (worker={job.worker_id}, lease_expired={job.lease_expires_at})")
            ...
            if jobs_requeued > 0:
                logger.warning(f"Recovery: requeued {jobs_requeued} jobs, freed {gpu_slots_freed} GPU slots")
    ...
    async def append_log(
        ...
        async with self._lock:
            ...
            logger.debug(f"Appended {len(data)} bytes to {stream} for job {job_id} at offset {offset}")
