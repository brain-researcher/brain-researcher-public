...
class StudioSessionRuntime:
    ...
    async def get_session(self, session_id: str) -> StudioSession | None:
        with self._lock:
            with self._connect() as conn:
                row = conn.execute(
    ...
    async def get_runtime_token(self, br_session_id: str) -> str | None:
        """Return the per-pod marimo runtime token for a BR session.

        Server-side only. Must never appear in browser-facing payloads.
        """
        with self._lock:
            with self._connect() as conn:
    ...
    async def get_marimo_runtime_target(
    ...
    async def build_workspace_handoff(
        self,
        owner_user_id: str,
        session_id: str,
    ...
    async def build_hub_handoff(
        ...
        return HubWorkspaceHandoff(
            ...
            initial_focus=request.initial_focus,
            materialize_notebook_if_needed=request.materialize_notebook_if_needed,
            runtime_target_url=target.public_url if target is not None else None,
    ...
    def _resolve_attachable_runtime_locked(
        ...
        *,
        ...
        if runtime is None:
            return None
        if runtime.status not in _ATTACHABLE_SESSION_STATUSES:
            return None
        live = self._runtime_backing_pod_is_live(runtime)
        if live is False:
            self._mark_runtime_and_bound_sessions_stopped_locked(
    ...
    async def reconcile_runtime_sessions_once(self) -> dict[str, int]:
        summary = {
            "scanned": 0,
            "refreshed": 0,
            "stopped": 0,
