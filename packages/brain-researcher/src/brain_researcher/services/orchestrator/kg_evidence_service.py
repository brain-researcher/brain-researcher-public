...
DEFAULT_NEUROKG_URL = os.getenv("NEUROKG_URL", "http://localhost:5000")
...
class KGEvidenceService:
    ...
    def __init__(self, neurokg_url: str = DEFAULT_NEUROKG_URL, timeout: float = 10.0):
        """
        Initialize KG Evidence Service.

        Args:
            neurokg_url: Base URL for BR-KG service
            timeout: HTTP request timeout in seconds
        """
        self.neurokg_url = neurokg_url.rstrip('/')
        self.timeout = timeout
        self.client = httpx.AsyncClient(timeout=timeout)

    def _build_search_url(self) -> str:
        """Return the correct search endpoint for a mounted or direct BR-KG base."""
        if self.neurokg_url.endswith('/api/kg'):
            return f"{self.neurokg_url}/search"
        return f"{self.neurokg_url}/api/kg/search"

    def _build_concept_evidence_url(self, concept_id: str) -> str:
        if self.neurokg_url.endswith('/api/kg'):
            return f"{self.neurokg_url}/concept/{concept_id}/evidence"
        return f"{self.neurokg_url}/api/kg/concept/{concept_id}/evidence"
