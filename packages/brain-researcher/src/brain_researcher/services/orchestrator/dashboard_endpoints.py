...
NEUROKG_DASHBOARD_PATH = "/api/dashboard/metrics"
...
async def _fetch_metrics_direct() -> Dict[str, Any]:
    ...
    url = f"{NEUROKG_URL.rstrip('/')}{NEUROKG_DASHBOARD_PATH}"
    ...
    if service_coordinator:
        try:
            response = await service_coordinator.make_request(
                ServiceType.NEUROKG, "GET", NEUROKG_DASHBOARD_PATH
            )
