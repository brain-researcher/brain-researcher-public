...
split services (web, agent, orchestrator, neurokg). A separate legacy full reverse-proxy gateway lives under `brain_researcher.legacy.api_gateway`.
...
def create_app() -> FastAPI:
    ...
    async def lifespan(app):
        ...
        if orchestrator_app:
            try:
                lifespan_ctx = getattr(orchestrator_app.router, "lifespan_context", None)
    ...
    app = FastAPI(title="Brain Researcher Gateway (Legacy)", version="0.2.0", lifespan=lifespan)
    ...
    # ------------------------------------------------------------------ #
    try:
        from brain_researcher.legacy.gateway.neurokg_proxy import (
            router as neurokg_proxy_router,
        )

        app.include_router(neurokg_proxy_router)
        logger.info("✓ Registered BR-KG proxy router (/api/neurokg/..., /api/kg/...)")
    except Exception as exc:  # pragma: no cover - best-effort import
