...
def fetch_statmaps(session):
    rows = session.run(
        """
        MATCH (s:StatMap {source:'neurovault'})
        RETURN s.id AS id, coalesce(s.contrast_definition, s.name) AS label, s.name AS name, s.contrast_definition AS contrast
        """
    ).data()
    out = []
    for r in rows:
...
def main():
    ...
    with GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD)) as driver:
        with driver.session(database=NEO4J_DB) as session:
            ...
            statmaps = fetch_statmaps(session)
            rows_out = []
            to_link = []
            for sm_id, text in statmaps:
                ...
                for lab, score, _ in matches:
                    if score < CANDIDATE_THRESHOLD:
                        continue
                    cid = label_to_id[lab]
                    rows_out.append([sm_id, cid, lab, score])
                    if APPLY_THRESHOLD and score >= APPLY_THRESHOLD:
                        to_link.append((sm_id, cid, score))

            with OUTPUT.open("w", newline="", encoding="utf-8") as f:
                w = csv.writer(f)
                w.writerow(["statmap_id", "onvoc_id", "onvoc_label", "score"])
                w.writerows(rows_out)
            print(f"Wrote candidates: {len(rows_out)} to {OUTPUT}")

            if APPLY_THRESHOLD:
                created = 0
                for sm_id, cid, score in to_link:
                    session.run(
                        """
                        MATCH (s:StatMap {id:$sm_id})
                        MATCH (o:OnvocClass {id:$cid})
                        MERGE (s)-[r:CLASSIFIED_UNDER]->(o)
                        SET r.score = $score, r.method = $method, r.version = $version
                        """,
                        {"sm_id": sm_id, "cid": cid, "score": score, "method": METHOD, "version": VERSION},
                    )
                    created += 1
                print(f"Auto-linked statmaps (score >= {APPLY_THRESHOLD}): {created}")
