#!/usr/bin/env python3
"""Top up each active account's workflow-runtime credits to a monthly cap.
...
Usage:
  ...
  python scripts/billing/monthly_workflow_credit_allowance.py --month 2026-05 --apply
...
def _preview_top_up(
    ...
    *,
    ...
    balance = store.get_balance(target.workspace_id, target.user_id)
...
async def run_allowance(
    *,
    ...
    for target in targets:
        ...
        result = store.top_up_workflow_monthly_allowance(
            target.workspace_id,
            target.user_id,
            month=month,
            allowance_milli=allowance_milli,
