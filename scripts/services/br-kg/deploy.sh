...
if [ ! -f "app.py" ]; then
    echo "❌ Error: app.py not found. Please run this script from the br-kg directory."
    exit 1
fi

# Check if JSON export exists
if [ ! -f "br_kg_glmfitlins_export.json" ]; then
...
export BR_KG_GLMFITLINS_DB_PATH=br_kg_glmfitlins_export.json
...
echo "   - BR_KG_GLMFITLINS_DB_PATH=br_kg_glmfitlins_export.json"
