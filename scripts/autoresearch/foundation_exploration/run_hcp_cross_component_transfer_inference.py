#!/usr/bin/env python3
"""Prepare, validate, or launch conditional R3 transfer inference."""
...
from brain_researcher.research.predictive import (
    hcp_cross_component_transfer_inference as inference,
)
...
def _parser() -> argparse.ArgumentParser:
    ...
    prepare.add_argument("--source-r3", type=Path, required=True)
...
def _validate(output_dir: Path) -> None:
    contract = inference._read_json(
        output_dir / "transfer_inference_contract.json", label="contract"
    )
    ...
    expected = inference.prepare_contract(
        source_dir=Path(str(source["source_dir"])),
        draws=int(permutation["draws"]),
        permutation_seed=int(permutation["seed"]),
        bootstrap_seed=int(bootstrap["seed"]),
    )
    if contract != expected:
        raise inference.TransferInferenceError(
            "inference contract differs from current R3 source"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    except inference.TransferInferenceError as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 2
    print(f"phase: {result['phase']}")
    print(f"result: {args.output_dir / 'transfer_inference_result.json'}")
    return 0
