#!/usr/bin/env python3
"""Build a policy pack for behavioral phenotype scope-review rows."""
...
STAGE_NAME = "balanced_behavioral_policy"
EXPECTED_SCOPE_LANE = "behavioral_phenotype"
MEASURABLE_PHENOTYPE_TOKENS = (
    "gait speed",
    "consumption",
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--behavioral-rows", type=Path, required=True)
...
def _matches_scope_contract(row: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    if lane and lane != EXPECTED_SCOPE_LANE:
        return False, "skipped_non_behavioral_lane"
...
def classify_behavioral_row(row: dict[str, Any]) -> tuple[str, str, str]:
    label = str(row.get("target_label") or "").strip().lower()
    if any(token in label for token in MEASURABLE_PHENOTYPE_TOKENS):
        return (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    behavioral_rows = args.behavioral_rows.expanduser().resolve()
    ...
    for row in _iter_jsonl(behavioral_rows):
        matches_contract, skip_reason = _matches_scope_contract(row)
        if not matches_contract:
            skipped_counts[str(skip_reason)] += 1
            continue
        bucket, action, bucket_reason = classify_behavioral_row(row)
        payload = _policy_row(
            row,
            source_path=behavioral_rows,
            bucket=bucket,
            action=action,
            bucket_reason=bucket_reason,
        )
    ...
    _write_jsonl(output_dir / "behavioral_policy_pack.jsonl", policy_rows)
    _write_tsv(
        output_dir / "behavioral_policy_pack.tsv",
        policy_rows,
        [
            "policy_bucket",
    ...
    _write_jsonl(
        output_dir / "measurable_behavioral_regeneration.jsonl", regeneration_rows
    )
    _write_jsonl(output_dir / "broad_behavioral_trait_hold.jsonl", hold_rows)

    summary = {
        "generated_at": _utc_now_iso(),
        "behavioral_rows_path": str(behavioral_rows),
        "counts": {
        ...
        "artifacts": {
            "policy_pack_jsonl": str(output_dir / "behavioral_policy_pack.jsonl"),
            "regeneration_rows_jsonl": str(
                output_dir / "measurable_behavioral_regeneration.jsonl"
            ),
            "hold_rows_jsonl": str(
                output_dir / "broad_behavioral_trait_hold.jsonl"
            ),
            "summary_json": str(output_dir / "behavioral_policy_summary.json"),
        },
    }
    (output_dir / "behavioral_policy_summary.json").write_text(
