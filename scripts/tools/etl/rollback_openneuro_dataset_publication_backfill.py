#!/usr/bin/env python3
"""Rollback OpenNeuro Dataset-[:CITED_BY]->Publication backfill by method tag."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "tmp/openneuro_dataset_publication_backfill/"
            "rollback_openneuro_dataset_publication_backfill_report.json"
        ),
    )
...
def _summarize_target_edges(session: Any, *, method_tag: str) -> dict[str, int]:
    query = """
    MATCH (d:Dataset)-[r:CITED_BY]->(p:Publication)
    WHERE r.method_tag = $method_tag
    RETURN
      count(r) AS targeted_edges,
      count(DISTINCT d) AS targeted_datasets,
      count(DISTINCT p) AS targeted_publications
    """
    row = session.run(query, {"method_tag": method_tag}).single()
    return {
        "targeted_edges": int((row or {}).get("targeted_edges") or 0),
        "targeted_datasets": int((row or {}).get("targeted_datasets") or 0),
        "targeted_publications": int((row or {}).get("targeted_publications") or 0),
    }


def _target_publication_element_ids(session: Any, *, method_tag: str) -> list[str]:
    query = """
    MATCH (:Dataset)-[r:CITED_BY]->(p:Publication)
    WHERE r.method_tag = $method_tag
...
def _count_prunable_publications(
    ...
    *,
    ...
    WHERE elementId(p) IN $publication_element_ids
      ...
      AND NOT EXISTS {
        MATCH (p)-[other]-()
        WHERE NOT (type(other) = 'CITED_BY' AND other.method_tag = $method_tag)
      }
...
def _delete_target_edges(session: Any, *, method_tag: str) -> int:
    query = """
    MATCH (:Dataset)-[r:CITED_BY]->(:Publication)
    WHERE r.method_tag = $method_tag
