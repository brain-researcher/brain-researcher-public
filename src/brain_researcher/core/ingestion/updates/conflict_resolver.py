...
class ConflictResolver:
    ...
    def detect_conflicts(self,
        ...
        for key in set(local_data.keys()) & set(remote_data.keys()):
            ...
            if local_value != remote_value:
                conflict = self._analyze_conflict(key, local_value, remote_value, metadata)
                if conflict:
    ...
    def resolve_conflicts(self,
        ...
        resolution_results['statistics'] = {
            ...
            'resolution_rate': len(resolution_results['resolved']) / len(conflicts) if conflicts else 0
        }
        ...
        logger.info(f"Resolved {resolution_results['statistics']['resolved']} of {len(conflicts)} conflicts")
        return resolution_results

    def merge_data(self,
                  local_data: Dict[str, Any],
                  remote_data: Dict[str, Any],
                  resolution_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in original_keys & resolved_keys:
            if type(original_data[key]) != type(resolved_data[key]):
                if not self._is_compatible_type_change(
                    type(original_data[key]),
                    type(resolved_data[key])
                ):
    ...
    def resolve_manual_conflict(self,
                              conflict_id: str,
    ...
    def _check_version_conflict(self, metadata: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _generate_conflict_id(self, key: str, local_value: Any, remote_value: Any) -> str:
        """Generate unique conflict ID."""
        content = f"{key}:{str(local_value)}:{str(remote_value)}:{datetime.now().isoformat()}"
    ...
    def _is_compatible_type_change(self, type1: type, type2: type) -> bool:
        ...
        return (type1, type2) in compatible_changes or (type2, type1) in compatible_changes
