...
class ADNIUnifiedLoader:
    ...
    def load_subject_list(self, diagnosis_filter: Optional[str] = None, demo_mode: bool = False) -> List[str]:
    ...
    def load_clinical_assessments(self, clinical_file: Optional[str] = None, demo_mode: bool = False) -> Dict[str, Any]:
        ...
        logger.info(f"Loaded clinical assessments for {len(self.clinical_data)} subjects")
        return self.clinical_data

    def load_biomarkers(self, biomarker_file: Optional[str] = None, modality: str = 'all', demo_mode: bool = False) -> Dict[str, Any]:
        """Load biomarker data.
                ...
                if subject_id in self.subjects:
                    ...
                    if modality in ['csf', 'all']:
                        biomarkers['csf'] = {
                            ...
                            'tau': row.get('TAU'),
                            'ptau': row.get('PTAU'),
                            'abeta42_tau_ratio': row.get('ABETA') / row.get('TAU') if row.get('TAU') else None,
                            'ptau_abeta42_ratio': row.get('PTAU') / row.get('ABETA') if row.get('ABETA') else None
                        }
                    ...
                    if modality in ['pet', 'all']:
                        biomarkers['pet'] = {
                            ...
                            'amyloid_positive': row.get('AV45_SUVR', 0) > self.biomarker_thresholds['amyloid_pet_suvr']
                        }
        ...
        for subject in self.subjects[:40]:
            ...
            if dx == DiagnosisGroup.AD:
                alleles = np.random.choice(
                    apoe_alleles, 2,
                    p=[0.05, 0.45, 0.50]  # 50% chance of e4
                )
            elif dx in [DiagnosisGroup.LMCI, DiagnosisGroup.EMCI]:
                alleles = np.random.choice(
                    apoe_alleles, 2,
                    p=[0.08, 0.62, 0.30]  # 30% chance of e4
                )
            else:
                alleles = np.random.choice(
                    apoe_alleles, 2,
    ...
    def load_longitudinal_progression(self,
                                     subject_id: str,
        ...
        for i, months in enumerate(visit_months):
            ...
            mmse = max(0, baseline_mmse - decline_rate * months / 12 + np.random.normal(0, 1))
    ...
    def calculate_atrophy_rates(self,
                               subject_id: str,
                               region: str = 'hippocampus') -> float:
    ...
    def _generate_sample_clinical_data(self):
        ...
        for subject in self.subjects[:50]:  # Sample subset
            dx = self.clinical_data.get(subject, {}).get(
                'diagnosis_baseline',
                np.random.choice(list(DiagnosisGroup))
            )
            ...
            self.clinical_data[subject].update({
                ...
                'npi_q': np.random.uniform(0, 5) if dx in [DiagnosisGroup.LMCI, DiagnosisGroup.AD] else 0,
        ...
        for subject in self.subjects[:30]:  # Sample subset
            ...
            if modality in ['pet', 'all']:
                ...
                biomarkers['pet'] = {
                    ...
                    'amyloid_positive': amyloid_suvr > self.biomarker_thresholds['amyloid_pet_suvr']
                }
        ...
        for subject_id in self.subjects[:30]:
            ...
            if subject_id in self.progression_data:
                ...
                nodes.append({
                    ...
                    'properties': {
                        'n_visits': len(self.progression_data[subject_id]['visits']),
                        'conversions': self.progression_data[subject_id]['conversions']
                    }
                })
                edges.append({
                    'source': subject_node_id,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.clinical_data:
            diagnoses = [d.get('diagnosis_baseline', DiagnosisGroup.CN)
                        for d in self.clinical_data.values()]
            stats['diagnosis_distribution'] = {
                dx.value: diagnoses.count(dx)
                for dx in DiagnosisGroup
            }

        # APOE e4 carrier frequency
        if self.genetic_data:
            e4_carriers = sum(
                1 for g in self.genetic_data.values()
                if g.get('apoe_e4_carrier', False)
            )
        ...
        if self.biomarker_data:
            amyloid_positive = sum(
                1 for b in self.biomarker_data.values()
