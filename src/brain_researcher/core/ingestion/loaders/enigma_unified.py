...
class ENIGMAUnifiedLoader:
    ...
    def __init__(self,
                 ...
                 working_groups_dir: Optional[str] = None,
        ...
        self.working_groups_dir = Path(working_groups_dir) if working_groups_dir else None
    ...
    def load_working_groups(self,
        ...
        if demo_mode:
            ...
            for group_name in groups:
                group_dir = self.working_groups_dir / group_name.lower().replace('-', '_')
                if group_dir.exists():
                    working_groups[group_name] = self._load_working_group_data(group_dir)
                else:
    ...
    def load_meta_analysis_results(self,
        ...
        if demo_mode:
            ...
            for result_file in self.results_dir.glob("*.csv"):
                ...
                if not analysis_types or analysis_name in analysis_types:
                    ...
                    meta_analysis_results[analysis_name] = self._process_meta_analysis(results_df)
        else:
            raise ValueError(
        ...
        logger.info(f"Loaded meta-analysis results for {len(meta_analysis_results)} analyses")
        return meta_analysis_results

    def load_brain_measures(self,
        ...
        if demo_mode:
            ...
            for measure_type in measure_types:
                if measure_type in self.brain_measures_config:
                    brain_measures[measure_type] = self._load_brain_measure_data(measure_type)
    ...
    def harmonize_across_cohorts(self,
                                cohorts: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for group_name, group_data in self.working_groups.items():
            if 'cohorts' in group_data:
                ...
                harmonization_results['cohort_mappings'][group_name] = self._map_cohort_variables(group_cohorts)
                harmonization_results['harmonized_measures'][group_name] = self._harmonize_measures(group_cohorts)
                harmonization_results['conversion_factors'][group_name] = self._calculate_conversion_factors(group_cohorts)
                harmonization_results['quality_scores'][group_name] = self._assess_harmonization_quality(group_cohorts)

        logger.info(f"Harmonized data across {len(harmonization_results['cohort_mappings'])} working groups")
        return harmonization_results

    def link_publications(self,
                        pubmed_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        publication_links['impact_metrics'] = {
            ...
            'papers_per_group': len(publication_links['papers']) / len(self.working_groups) if self.working_groups else 0,
            'citation_count': sum(pub.get('citations', 0) for pub in publication_links['papers'])
        }
    ...
    def export_to_knowledge_graph(self) -> Dict[str, Any]:
        ...
        kg_data = {
            ...
            'metadata': {
                ...
                'n_cohorts': sum(len(g.get('cohorts', [])) for g in self.working_groups.values()),
        ...
        for measure_type, measures in self.brain_measures.items():
        ...
        logger.info(f"Exported {len(kg_data['nodes'])} nodes and {len(kg_data['edges'])} edges to knowledge graph")
        return kg_data
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            'n_working_groups': len(self.working_groups),
            'n_cohorts': sum(len(g.get('cohorts', [])) for g in self.working_groups.values()),
            'n_subjects': sum(g.get('total_subjects', 0) for g in self.working_groups.values()),
            'n_brain_measures': sum(
                len(measures.get('measures', [])) for measures in self.brain_measures.values()
            ) if self.brain_measures else 0,
            'n_meta_analyses': len(self.meta_analysis_results),
            'disorders_studied': list(set(
                g.get('disorder', '') for g in self.working_groups.values()
                if g.get('disorder')
            )),
            'quality_summary': {
                ...
                'analyses_with_high_quality': sum(
                    1 for m in self.quality_metrics.values()
                ...
                'publications': [
                    {
                        ...
                        'year': 2020 + i,
                        'journal': np.random.choice(['Nature', 'Science', 'JAMA Psychiatry', 'Molecular Psychiatry']),
    ...
    def _generate_demo_meta_analysis(self) -> Dict[str, Any]:
        ...
        for group_name in list(self.working_groups.keys())[:3]:  # First 3 groups
            ...
            for region in self.brain_measures_config['subcortical'][:5]:
                ...
                result['ci_lower'] = result['cohens_d'] - 1.96 * result['standard_error']
                result['ci_upper'] = result['cohens_d'] + 1.96 * result['standard_error']
    ...
    def _process_meta_analysis(self, results_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'ci_lower' not in results_df.columns:
            results_df['ci_lower'] = results_df['cohens_d'] - 1.96 * results_df['standard_error']
        if 'ci_upper' not in results_df.columns:
            results_df['ci_upper'] = results_df['cohens_d'] + 1.96 * results_df['standard_error']
    ...
    def _assess_data_quality(self, results: Any) -> float:
        ...
        if isinstance(results, pd.DataFrame):
            ...
            if 'cohens_d' in results.columns:
                ...
                outlier_proportion = (z_scores > 3).sum() / len(z_scores) if len(z_scores) > 0 else 0
