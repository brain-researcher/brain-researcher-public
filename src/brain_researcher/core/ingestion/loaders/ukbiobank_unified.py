...
class UKBiobankUnifiedLoader:
    ...
    def __init__(
        ...
        imaging_dir: str | None = None,
        genetic_dir: str | None = None,
    ...
    def load_phenotype_data(
        self, fields: list[str] | None = None, demo_mode: bool = False
    ...
    def load_imaging_metrics(
        self, modalities: list[str] | None = None, demo_mode: bool = False
    ) -> dict[str, Any]:
    ...
    def load_genetic_markers(
        self, marker_types: list[str] | None = None, demo_mode: bool = False
    ) -> dict[str, Any]:
            ...
            kg_data["nodes"].append(
                {
                    ...
                    "properties": {
                        "cohort": "UK_Biobank",
                        "has_imaging": subject_id
                        in self.imaging_metrics.get("T1_structural", {}).get(
                            "subjects", []
                        ),
                        "has_genetics": subject_id
                        in self.genetic_markers.get("snps", {}).get("subjects", []),
                    },
                }
            )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "phenotype_fields": sum(
                len(df.columns) - 1
                for df in self.phenotype_data.values()
        ...
        demographics = pd.DataFrame(
            {
                "eid": (
                    self.subjects[:n]
                    if self.subjects
                    else self._generate_demo_subjects(n)
                ),
    ...
    def _generate_demo_genetics(self) -> dict[str, Any]:
        ...
        genetic_markers = {
            ...
            "ancestry": {
                ...
                "ancestry_group": np.random.choice(
                    ["EUR", "AFR", "EAS", "SAS"], n, p=[0.85, 0.05, 0.05, 0.05]
                ),
            },
        }
    ...
    def _extract_demographics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        demo_cols = ["eid"]
        for field_id, _name in self.field_mappings.items():
    ...
    def _load_modality_data(self, modality_dir: Path) -> dict[str, Any]:
        ...
        modality_data = {"subjects": [], "metrics": {}}
    ...
    def _load_snp_data(self) -> dict[str, Any]:
        ...
        snp_data = {"subjects": [], "markers": [], "genotypes": {}}
    ...
    def _load_ancestry_data(self) -> dict[str, Any]:
        ...
        if ancestry_file.exists():
            ...
            if "ancestry_group" in ancestry_df.columns:
                ancestry_data["ancestry_groups"] = ancestry_df[
                    "ancestry_group"
                ].to_dict()
