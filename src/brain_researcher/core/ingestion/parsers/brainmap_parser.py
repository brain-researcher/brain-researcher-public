...
class BrainMapParser:
    ...
    def __init__(self):
        ...
        self.behavioral_domains_taxonomy = {
            'action': ['execution', 'imagination', 'inhibition', 'observation', 'preparation'],
            ...
            'perception': ['audition', 'gustation', 'olfaction', 'somesthesis', 'vision']
        }
    ...
    def parse_workspace(self, workspace_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if root.tag.startswith('{'):
                ...
                for exp in root.findall('.//bm:Experiment', ns) + root.findall('.//bm:experiment', ns):
                    experiment = self._parse_xml_experiment(exp, ns)
                    if experiment:
                        experiments.append(experiment)
            else:
                # Try both lowercase and uppercase tags without namespace
                for exp in root.findall('.//experiment') + root.findall('.//Experiment'):
    ...
    def _parse_xml_experiment(self, exp_element: ET.Element, ns: Dict[str, str] = None) -> Dict[str, Any]:
        ...
        if ns:
            # With namespace
            experiment['title'] = exp_element.findtext('bm:Title', namespaces=ns) or exp_element.findtext('bm:title', '', namespaces=ns)
            experiment['authors'] = exp_element.findtext('bm:Authors', namespaces=ns) or exp_element.findtext('bm:authors', '', namespaces=ns)
            experiment['year'] = exp_element.findtext('bm:Year', namespaces=ns) or exp_element.findtext('bm:year', '', namespaces=ns)
            experiment['journal'] = exp_element.findtext('bm:Journal', namespaces=ns) or exp_element.findtext('bm:journal', '', namespaces=ns)
            experiment['pmid'] = exp_element.findtext('bm:PMID', namespaces=ns) or exp_element.findtext('bm:pmid', '', namespaces=ns)
            experiment['modality'] = exp_element.findtext('bm:Modality', namespaces=ns) or exp_element.findtext('bm:modality', '', namespaces=ns)
            experiment['paradigm'] = exp_element.findtext('bm:Paradigm', namespaces=ns) or exp_element.findtext('bm:paradigm', '', namespaces=ns)
        else:
            # Without namespace
            experiment['title'] = exp_element.findtext('Title') or exp_element.findtext('title', '')
            experiment['authors'] = exp_element.findtext('Authors') or exp_element.findtext('authors', '')
            experiment['year'] = exp_element.findtext('Year') or exp_element.findtext('year', '')
            experiment['journal'] = exp_element.findtext('Journal') or exp_element.findtext('journal', '')
            experiment['pmid'] = exp_element.findtext('PMID') or exp_element.findtext('pmid', '')
            experiment['modality'] = exp_element.findtext('Modality') or exp_element.findtext('modality', '')
            experiment['paradigm'] = exp_element.findtext('Paradigm') or exp_element.findtext('paradigm', '')
        ...
        if paper is not None:
            experiment['paper'] = {
                ...
                'authors': paper.findtext('authors', '') or paper.findtext('Authors', ''),
        ...
        if ns:
            contrasts_container = exp_element.find('bm:Contrasts', ns) or exp_element.find('bm:contrasts', ns)
            if contrasts_container:
                for contrast in contrasts_container.findall('bm:Contrast', ns) + contrasts_container.findall('bm:contrast', ns):
                    contrast_data = {
                        'id': contrast.get('id', ''),
                        'name': contrast.findtext('bm:Name', namespaces=ns) or contrast.findtext('bm:name', '', namespaces=ns),
                        'type': contrast.findtext('bm:Type', namespaces=ns) or contrast.findtext('bm:type', '', namespaces=ns),
                        'description': contrast.findtext('bm:Description', namespaces=ns) or contrast.findtext('bm:description', '', namespaces=ns),
                        'coordinates': []
                    }

                    # Parse coordinates within contrast
                    coords_container = contrast.find('bm:Coordinates', ns) or contrast.find('bm:coordinates', ns)
                    if coords_container:
                        for coord in coords_container.findall('bm:Coordinate', ns) + coords_container.findall('bm:coordinate', ns):
                            coord_data = self._parse_coordinate(coord, ns)
                            if coord_data:
                                contrast_data['coordinates'].append(coord_data)
                                experiment['coordinates'].append(coord_data)  # Also add to global coordinates

                    experiment['contrasts'].append(contrast_data)
        else:
            contrasts_container = exp_element.find('Contrasts') or exp_element.find('contrasts')
            if contrasts_container:
                for contrast in contrasts_container.findall('Contrast') + contrasts_container.findall('contrast'):
                    contrast_data = {
                        'id': contrast.get('id', ''),
                        'name': contrast.findtext('Name') or contrast.findtext('name', ''),
                        'type': contrast.findtext('Type') or contrast.findtext('type', ''),
                        'description': contrast.findtext('Description') or contrast.findtext('description', ''),
                        'coordinates': []
                    }

                    # Parse coordinates within contrast
                    coords_container = contrast.find('Coordinates') or contrast.find('coordinates')
                    if coords_container:
                        for coord in coords_container.findall('Coordinate') + coords_container.findall('coordinate'):
        ...
        for coord in exp_element.findall('.//coordinate') + exp_element.findall('.//Coordinate'):
        ...
        domain_text = exp_element.findtext('BehavioralDomain') or exp_element.findtext('behavioral_domain')
        if domain_text:
            experiment['behavioral_domains'].append(domain_text)
        for domain in exp_element.findall('.//behavioral_domain') + exp_element.findall('.//BehavioralDomain'):
            if domain.text:
                experiment['behavioral_domains'].append(domain.text)

        # Parse paradigm classes
        for paradigm in exp_element.findall('.//paradigm_class') + exp_element.findall('.//ParadigmClass'):
            if paradigm.text:
    ...
    def _parse_coordinate(self, coord_element, ns: Dict[str, str] = None) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(coord_element, ET.Element):
            if ns:
                # Handle namespace
                x = float(coord_element.findtext('bm:X', namespaces=ns) or coord_element.findtext('bm:x', 0, namespaces=ns))
                y = float(coord_element.findtext('bm:Y', namespaces=ns) or coord_element.findtext('bm:y', 0, namespaces=ns))
                z = float(coord_element.findtext('bm:Z', namespaces=ns) or coord_element.findtext('bm:z', 0, namespaces=ns))
                space = coord_element.findtext('bm:Space', namespaces=ns) or coord_element.findtext('bm:space', 'MNI', namespaces=ns)
            else:
                ...
                space = coord_element.findtext('Space') or coord_element.findtext('space', 'MNI')
    ...
    def _parse_coordinate_string(self, coord_str: str) -> Optional[Dict[str, Any]]:
    ...
    def convert_coordinate_space(self, coord: Dict[str, Any],
                                target_space: str = 'MNI') -> Dict[str, Any]:
    ...
    def parse_contrast_details(self, contrast_element) -> Dict[str, Any]:
        ...
        if isinstance(contrast_element, ET.Element):
            ...
            if stats is not None:
                contrast['statistical_threshold'] = float(stats.findtext('threshold', 0))
                contrast['correction_method'] = stats.findtext('correction', 'uncorrected')

            # Parse subject info
            contrast['n_subjects'] = int(contrast_element.findtext('n_subjects', 0))

        elif isinstance(contrast_element, dict):
            contrast.update({k: v for k, v in contrast_element.items() if k in contrast})
    ...
    def parse_study_metadata(self, study_element) -> Dict[str, Any]:
        ...
        if isinstance(study_element, ET.Element):
            ...
            if scanner is not None:
                metadata['field_strength'] = float(scanner.findtext('field_strength', 3.0))
    ...
    def parse_experiment_full(self, exp_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'behavioral_domains' in experiment:
            ...
            for domain in experiment['behavioral_domains']:
                hierarchical_domains.append(self.parse_behavioral_domain_hierarchy(domain))
            experiment['behavioral_domain_hierarchy'] = hierarchical_domains

        # Add study metadata if available
        if 'study_info' in experiment:
            experiment['study_metadata'] = self.parse_study_metadata(experiment['study_info'])
