...
(`data/br-kg/raw/cognitive_tasks.json`) plus custom synonyms listed in
...
``data/br-kg/raw/cognitive_tasks.json`` and additional synonyms from
...
class TaskMatcher:
    ...
    def _load_vocabulary(self) -> tuple[list[str], dict[str, str]]:
        ...
        ca_path = Path("data/br-kg/raw/cognitive_tasks.json")
        if ca_path.exists():
