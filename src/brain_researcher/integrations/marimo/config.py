...
Prefer notebook code that uses `import brain_researcher.sdk as br` and `br.search()`, `br.recipe()`, `br.execute()`, or `br.call()` before raw `nilearn` or external HTTP APIs when Brain Researcher can express the task.
...
def build_marimo_user_config(
    ...
    *,
    ...
    # Auto-run cells when the hosted quickstart notebook is opened. marimo's
    # packaged DEFAULT_CONFIG ships runtime.auto_instantiate=False (cells sit
    # stale on open, so the user sees code with no rendered output until they
    # click "run all"). For the hosted quickstart the friendlier behavior is to
    # render the examples immediately, so we opt the managed runtime into
    # auto-instantiate. (on_cell_change already defaults to "autorun", so
    # downstream cells cascade once ancestors run.)
    runtime_config = dict(merged.get("runtime") or {})
    runtime_config.setdefault("auto_instantiate", True)
    merged["runtime"] = runtime_config
