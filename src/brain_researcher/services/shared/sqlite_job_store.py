...
from .job_store import JobEvent, JobRecord, JobState, JobStore, LogChunk
...
class SqliteJobStore(JobStore):
    ...
    def __init__(
        self, db_path: str, total_gpu_slots: int = 2, busy_timeout_ms: int = 5000
    ):
    ...
    async def initialize(self) -> None:
        ...
        try:
            async with self._connect() as db:
                ...
                await db.execute(
                    "DELETE FROM job_audit WHERE job_id = ?", ("_writability_test",)
                )
                await db.commit()
        except (OSError, PermissionError) as e:
            raise RuntimeError(
        ...
        async with db.execute("SELECT COUNT(*) FROM gpu_slots") as cursor:
            ...
            if row and row[0] == 0:
                ...
                for slot_id in range(1, self._total_gpu_slots + 1):
                    await db.execute(
                        "INSERT INTO gpu_slots (id, in_use) VALUES (?, 0)", (slot_id,)
                    )
    ...
    async def _ensure_projects_table_and_backfill(
    ...
    async def enqueue(self, job: JobRecord) -> str:
        ...
        async with self._connect() as db:
            ...
            await self._audit_log(
                db, job.job_id, "enqueued", {"priority": job.priority}
            )
            await db.commit()
            ...
            async with self._connect() as db:
                ...
                try:
                    ...
                    await self._audit_log(
                        ...
                        job_id_to_claim,
                        "claimed",
                        {"worker_id": worker_id, "gpu_req": gpu_required},
                    )
                    await db.commit()
                    ...
                    db.row_factory = self._row_factory()
                    async with db.execute(
                        "SELECT * FROM jobs WHERE job_id = ?", (job_id_to_claim,)
                    ) as cursor:
                        row = await cursor.fetchone()
                        if row:
    ...
    async def heartbeat(
        self, worker_id: str, job_id: str | None = None, lease_ttl: int = 60
    ) -> int:
        """
        ...
        async with self._connect() as db:
            ...
            if updated > 0:
                logger.debug(
                    f"Heartbeat from {worker_id}: extended lease for {updated} job(s)"
                )
            return int(updated or 0)

    async def recover_stale_jobs(self, now_ts: int | None = None) -> dict[str, int]:
        """
        ...
        async with self._connect() as db:
            ...
            try:
                ...
                """,
                    (JobState.CLAIMED, JobState.RUNNING, now_ts),
                ) as cursor:
                ...
                sum(row[3] for row in stale_jobs)
                ...
                # Audit log
                for job_id, worker_id, state, _gpu_req in stale_jobs:
                    await self._audit_log(
                        db,
                        job_id,
                        "recovered",
                ...
                return {"recovered": recovered_count, "gpus_freed": gpus_freed}
    ...
    async def update_state(
        ...
        async with self._connect() as db:
            ...
            if updated:
                ...
                if state is not None:
                    state_label = (
                        state.value if isinstance(state, JobState) else str(state)
                    )
                audit_type = (
    ...
    async def release_gpus(self, job_id: str) -> None:
        ...
        async with self._connect() as db:
            ...
            try:
                ...
                if db.total_changes > 0:
                    logger.debug(
                        f"Released {db.total_changes} GPU slot(s) for job {job_id}"
                    )
            except Exception as e:
                await db.rollback()
        ...
        async with self._connect() as db:
            ...
            async with db.execute(
                ...
                try:
                    ...
                    logger.warning(
                        f"Unknown job state {current_state_value} for job {job_id}; cannot cancel"
                    )
                    return False
                ...
                if current_state in terminal_states:
                    logger.warning(
                        f"Cannot cancel job {job_id} in terminal state {current_state.value}"
                    )
                    return False
            ...
            if project_id:
                if project_id == "default":
                    where_clauses.append(
                        "(project_id = ? OR project_id IS NULL OR TRIM(project_id) = '')"
                    )
                    params.append(project_id)
                else:
    ...
    async def count_by_state(self, state: str, user_id: str | None = None) -> int:
        async with self._connect() as db:
            await self._configure_connection(db)
    ...
    async def create_project(
        ...
        if project is None:
            raise RuntimeError(
                f"Failed to read newly created project '{normalized_project_id}'"
            )
        return project
    ...
    def _row_to_record(self, row: aiosqlite.Row) -> JobRecord:
        ...
        return JobRecord(
            ...
            walltime_minutes=(
                row["walltime_minutes"] if "walltime_minutes" in row.keys() else 60
            ),
...
class DualJobStore(JobStore):
    ...
    def __init__(self, primary: JobStore, secondary: JobStore):
        ...
        logger.info(
            f"DualJobStore initialized: primary={type(primary).__name__}, secondary={type(secondary).__name__}"
        )
    ...
    async def _call_heartbeat(
        self, store, worker_id: str, job_id: str | None, lease_ttl: int
    ) -> int | None:
    ...
    async def enqueue(self, job: JobRecord) -> str:
        ...
        try:
            ...
            if secondary_id != job_id:
                logger.warning(
                    f"Discrepancy in enqueue: primary={job_id}, secondary={secondary_id}"
                )
    ...
    async def update_state(
        ...
        try:
            secondary_result = await self.secondary.update_state(
                job_id, state, **kwargs
            )
    ...
    async def recover_stale_jobs(self, now_ts: int | None = None) -> dict[str, int]:
        ...
        primary_recovered = primary_stats.get(
            "recovered", primary_stats.get("jobs_requeued", 0)
        )
        ...
        try:
            if hasattr(self.secondary, "recover_stale_jobs"):
                ...
                secondary_recovered = secondary_stats.get(
                    "recovered", secondary_stats.get("jobs_requeued", 0) or 0
                )
    ...
    def _compare_jobs(
        self,
        job_id: str,
        primary_job: JobRecord | None,
        secondary_job: JobRecord | None,
    ) -> None:
        ...
        if primary_job is None:
            logger.warning(
                f"Discrepancy for {job_id}: exists in secondary but not primary"
            )
            self._discrepancy_count += 1
            return
        ...
        if secondary_job is None:
            logger.warning(
                f"Discrepancy for {job_id}: exists in primary but not secondary"
            )
            self._discrepancy_count += 1
            return
