...
def _toolspec_metadata(tool_id: str) -> tuple[list[str], str]:
    # Resolve via the dependency-inversion seam instead of importing the tools
    # layer (keeps services/shared free of a shared -> tools back-edge). The
    # tools layer registers the lookup at import time; if unavailable, fall back
    # to the conservative defaults below.
    from .toolspec_lookup import get_toolspec_lookup

    result: tuple[list[str], str] | None = None
    lookup = get_toolspec_lookup()
    if lookup is not None:
        try:
            result = lookup(tool_id)
        except Exception:
            result = None
    ...
    allowed_phases, approval_level = result
    return list(allowed_phases or []), str(approval_level or "none")
