...
def _kg_evidence_context(card: Mapping[str, Any], *, origin_pattern: str) -> str:
    provenance = (
        card.get("provenance") if isinstance(card.get("provenance"), Mapping) else {}
    )
    verification = (
    ...
    query_relevance = _safe_float(
        card.get("query_relevance_score") or provenance.get("query_relevance_score"),
        0.0,
    )
    ...
    if isinstance(supporting_titles, Sequence) and not isinstance(
        supporting_titles, str
    ):
        first_titles = [
        ...
        if first_titles:
            return (
                f"{base_context} Supporting papers include: {'; '.join(first_titles)}."
            )
    return base_context
...
def generate_novelty_calibration_questions(
    ...
    *,
    ...
    card_titles = [
        _claim_subject(card) for card in selected_cards[:3] if _claim_subject(card)
    ]
    combination_focus = (
        ", ".join(card_titles) if card_titles else "the proposed package"
    )
    questions.append(
        {
