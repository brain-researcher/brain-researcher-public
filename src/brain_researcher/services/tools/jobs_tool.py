...
class ListUserJobsTool(NeuroToolWrapper):
    ...
    def _run(
        ...
        try:
            from brain_researcher.services.shared.job_store_factory import (
                get_job_store,
            )
            ...
            async def _list_jobs():
                if state:
                    jobs = await store.list_by_state(
                        state, user_id=user_id, limit=limit, offset=offset
                    )
                else:
...
class SubmitJobTool(NeuroToolWrapper):
    ...
    def _run(
        ...
        try:
            ...
            from brain_researcher.services.shared.job_models import (
                JobRecord,
                JobState,
            )
            from brain_researcher.services.shared.job_store_factory import (
...
class CancelJobTool(NeuroToolWrapper):
    ...
    def _run(self, job_id: str, reason: str | None = None) -> ToolResult:
        try:
            from brain_researcher.services.shared.job_store_factory import (
                get_job_store,
            )
