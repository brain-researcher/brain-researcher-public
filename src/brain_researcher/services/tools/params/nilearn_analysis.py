...
def run_connectivity_matrix(params: ConnectivityMatrixParameters) -> dict[str, Any]:
    ...
    # Value-domain gate (record-or-raise, lenient). The connectivity matrix is
    # treated downstream as a covariance and inverted to a precision matrix, so
    # it must be finite and well-conditioned. Violations are recorded into a
    # sidecar (strict=False) instead of raising, so the run still succeeds and
    # the review-gate detector
    # (checks.value_domain.value_domain_contract_violation_check) surfaces a
    # blocking finding. finite is the always-on stage-boundary guard;
    # well_conditioned is selected via the declarative router for
    # covariance/precision methods and applied to each per-subject square
    # matrix. Vectorized output is not a square matrix, so conditioning is
    # skipped there.
    value_domain_sink: list[dict[str, Any]] = []
    matrix_label = f"{params.kind}_connectivity_matrix"
    evaluate_value_domain(
        "finite", matrix, matrix_label, strict=False, sink=value_domain_sink
    )
    selected_contracts = contracts_for(f"connectivity_matrix_{params.kind}")
    if not params.vectorize and matrix.ndim >= 2:
        for subject_idx in range(matrix.shape[0]):
            square = matrix[subject_idx]
            ...
            for contract in selected_contracts:
                evaluate_value_domain(
                    contract,
                    square,
                    f"{matrix_label}[subject={subject_idx}]",
                    strict=False,
                    sink=value_domain_sink,
                )
    value_domain_path = str(
        write_value_domain_diagnostics(value_domain_sink, output_dir)
    )
    ...
    outputs: dict[str, str] = {
        ...
        "value_domain_diagnostics": value_domain_path,
    }
