...
class MNEPreprocessingArgs(BaseModel):
    ...
    filter_method: str = Field(
        default="fir", description="Filter method: 'fir' or 'iir'"
    )
    ...
    # Resampling
    sfreq: float | None = Field(
        default=None, description="New sampling frequency (Hz). None = no resampling"
    )

    # Bad channel handling
    detect_bad_channels: bool = Field(
        default=True, description="Automatically detect bad channels"
    )
    bad_channels: list[str] | None = Field(
        default=None, description="List of bad channel names to mark"
    )
    interpolate_bads: bool = Field(default=True, description="Interpolate bad channels")
    ...
    reference_channels: list[str] | None = Field(
        default=None, description="Specific channels to use as reference"
    )

    # Epoching parameters (optional)
    create_epochs: bool = Field(
        default=False, description="Create epochs from continuous data"
    )
    epoch_tmin: float = Field(
        default=-0.2, description="Start time of epochs (seconds)"
    )
    epoch_tmax: float = Field(default=0.8, description="End time of epochs (seconds)")
    event_id: dict[str, int] | None = Field(
        default=None, description="Event ID mapping for epoching"
    )

    # Baseline correction
    baseline: tuple[float, float] | None = Field(
        default=(None, 0), description="Baseline correction interval (tmin, tmax)"
    )
    ...
    flat: dict[str, float] | None = Field(
        default=None, description="Flat channel thresholds by channel type"
    )

    # Line noise removal
    notch_freq: float | list[float] | None = Field(
        default=None,
        description="Notch filter frequency (Hz), e.g., 50 or 60 for line noise",
    )

    # Montage
    set_montage: str | None = Field(
        default=None,
    ...
    save_format: str = Field(
        default="fif", description="Output format: 'fif', 'edf', 'bdf'"
    )
...
class MNEPreprocessingTool(NeuroToolWrapper):
    ...
    def _apply_reference(
        self, raw, reference: str, reference_channels: list[str] | None = None
    ):
        ...
        **kwargs,
        ...
        try:
            ...
            if l_freq is not None or h_freq is not None:
                raw.filter(
                    l_freq, h_freq, method=filter_method, filter_length=filter_length
                )
    ...
    def batch_preprocess(
        self, input_files: list[str], output_dir: str, **kwargs
    ) -> ToolResult:
        ...
        try:
            ...
            for i, input_file in enumerate(input_files):
                ...
                result = self._run(
                    raw_file=input_file, output_dir=str(subject_dir), **kwargs
                )

                if result.status == "success":
                    results.append(
                        {"input": input_file, "outputs": result.data["outputs"]}
                    )
                else:
                    failed.append({"input": input_file, "error": result.error})
