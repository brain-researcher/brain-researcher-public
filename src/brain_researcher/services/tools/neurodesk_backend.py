...
from brain_researcher.services.shared.r2toolsagent_base_backend import (
    BackendCapacity,
    BackendConfigError,
...
from brain_researcher.services.shared.r2toolsagent_slurm_helpers import (
    parse_sacct_output,
    parse_squeue_output,
)
...
class NeurodeskBackend(BaseBackend):
    ...
    def __init__(self, name: str, config: dict[str, Any]) -> None:
        ...
        if self.mode not in ("local", "remote"):
            raise BackendConfigError(
                f"NeurodeskBackend mode must be 'local' or 'remote', got {self.mode!r}"
            )

        self.run_dir = Path(config.get("run_dir", "/tmp/brain_researcher_neurodesk"))
        self.partition: str | None = config.get("partition")
        self.account: str | None = config.get("account")
        self.qos: str | None = config.get("qos")
    ...
    async def get_job_status(self, job_id: str) -> JobStatus:
        ...
        if state is None:
            ...
            state, started_at, completed_at, exit_code_val = parse_sacct_output(
                slurm_id, sacct_out
            )
            if state is None:
    ...
    async def check_health(self) -> bool:
        try:
            if self.mode == "local":
                proc = subprocess.run(
                    ["sinfo", "--version"], capture_output=True, timeout=10
                )
