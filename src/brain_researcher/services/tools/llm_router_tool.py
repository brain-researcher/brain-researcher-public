...
from brain_researcher.services.llm_gateway.codegen.fs_context import (
    GeminiCliFsClient,
    build_fs_context_for_task_sync,
)
from brain_researcher.services.llm_gateway.codegen.loop import CodegenLoop
from brain_researcher.services.llm_gateway.codegen.render import render_result_for_chat
...
class LLMRouterChatArgs(BaseModel):
    ...
    model_hint: str | None = Field(None, description="Preferred model/provider hint")
...
class CodingAgentArgs(BaseModel):
    ...
    code_context: str | None = Field(None, description="Code snippet or error text")
    ...
    repo_root: str | None = Field(None, description="Repository root for FS search")
    ...
    prefetched_files: list[dict] | None = Field(
...
class CodingAgentTool(NeuroToolWrapper):
    ...
    def _run(
        ...
        prefetched_files: list[dict] | None = None,
        apply: bool = False,
        dry_run: bool = False,
        max_iters: int = 3,
        **kwargs: Any,
        ...
        try:
            ...
            if apply and loop_result.status == "success" and loop_result.patches:
                try:
                    if not dry_run:
                        from brain_researcher.services.llm_gateway.codegen.workspace import (
            ...
            return ToolResult(
                ...
                data={
                    ...
                    "exec_stdout": (
                        loop_result.exec_result.stdout
                        if loop_result.exec_result
                        else None
                    ),
                    "exec_stderr": (
                        loop_result.exec_result.stderr
                        if loop_result.exec_result
                        else None
                    ),
