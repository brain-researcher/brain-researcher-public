...
from brain_researcher.services.shared.novelty_calibration_questions import (
    build_novelty_calibration_context,
    generate_novelty_calibration_questions,
)
from brain_researcher.services.shared.param_norm import coerce_enum

# frontier_mode synonyms -> canonical {off, frontier}; unknown -> off. Mirrors
# the categorical-arg contract in services/shared/param_norm.py. Never raises.
...
def rewrite_candidate_cards(
    ...
    *,
    ...
    for idx, raw_card in enumerate(cards, start=1):
        ...
        gap = _classify_verification_gap(
            kg_verification=(
                kg_verification if isinstance(kg_verification, Mapping) else None
            ),
            relation_hint=relation_hint or source_stage,
            query=query,
            statement=raw_hypothesis,
...
def _extract_principle_controller_meta(
    ...
    return {
        ...
        or None,
        "active_principle": (
            dict(active_principle) if isinstance(active_principle, Mapping) else None
        ),
        "active_principle_id": principle_id or None,
        "active_principle_label": principle_label or None,
        "principle_posterior": (
            dict(posterior) if isinstance(posterior, Mapping) else {}
        ),
...
def _extract_verification_lookup(
    ...
    for item in tested:
        ...
        lookup[key] = {
            ...
            "entity_hints_used": (
                list(item.get("entity_hints_used") or [])
                if isinstance(item.get("entity_hints_used"), list)
                else []
            ),
            "kg_verification": (
                dict(item.get("kg_verification"))
...
def build_candidate_cards_from_workflow_result(
    ...
    *,
    ...
    for idx, row in enumerate(hypotheses, start=1):
        ...
        provenance = {
            ...
            "principle_controller": {
                ...
                "active_principle": (
                    dict(active_principle)
                    if isinstance(active_principle, Mapping)
                    else None
                ),
                "posterior": (
                    row.get("principle_posterior")
        ...
        if (
            not card.get("claim_memory_priority")
            or card.get("claim_memory_priority") == "none"
        ):
    ...
    cards.sort(
        key=lambda c: _priority_order.get(c.get("claim_memory_priority", "none"), 3)
    )
...
def synthesize_candidate_cards_payload(
    *,
    ...
    frontier_cards: list[dict[str, Any]] = []
    if frontier_mode_norm == "frontier":
    ...
    summary["frontier_mode"] = frontier_mode_norm
