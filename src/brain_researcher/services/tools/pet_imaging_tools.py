...
class PETInput(BaseModel):
    ...
    pet_image: str | np.ndarray | None = Field(
        None, description="PET image file or array"
    )
    time_activity_curve: np.ndarray | None = Field(
        None, description="Time activity curve data"
    )
    reference_region: str | None = Field(
        None, description="Reference region for analysis"
    )
    tracer: str | None = Field(None, description="PET tracer type")
    scan_times: list[float] | None = Field(None, description="Scan time points")
...
class SUVCalculationTool(NeuroToolWrapper):
    ...
    def _run(
        self,
        pet_image: str | np.ndarray | None = None,
    ...
    def _load_pet_image(self, pet_image: str | np.ndarray) -> np.ndarray:
        """Load PET image."""
...
class KineticModelingTool(NeuroToolWrapper):
    ...
    def _run(
        self,
        time_activity_curve: np.ndarray | None = None,
        input_function: np.ndarray | None = None,
        scan_times: list[float] | None = None,
    ...
    def _generate_aif(self, times: np.ndarray | None = None) -> np.ndarray:
...
class TracerUptakeAnalysisTool(NeuroToolWrapper):
    ...
    def _analyze_binding(
        self, pet_data: np.ndarray, reference_region: str | None, tracer_props: dict
    ) -> dict[str, float]:
...
class MultiTracerComparisonTool(NeuroToolWrapper):
    ...
    def _run(
        self,
        tracer_images: dict[str, np.ndarray] | None = None,
...
class ParametricMappingTool(NeuroToolWrapper):
    ...
    def _run(
        self,
        dynamic_pet: np.ndarray | None = None,  # 4D: x,y,z,time
        scan_times: list[float] | None = None,
        input_function: np.ndarray | None = None,
