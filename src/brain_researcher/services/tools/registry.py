...
def _normalize_phase_filters(phases: list[str] | None) -> list[str]:
    normalized: list[str] = []
...
def _tool_matches_phases(spec: ToolSpec, phases: list[str] | None) -> bool:
...
def _literature_tools() -> list[StructuredTool]:
    ...
    try:
        ...
        tools = [
            t.as_langchain_tool()
            for t in FixedHrfLiteratureScopingTools.get_all_tools()
        ]
...
def _mcp_tools() -> list[StructuredTool]:
    ...
    except Exception as e:  # pragma: no cover - shared bridge should import
        logger.warning("Failed to load MCP runtime bridge tools: %s", e)
        return []
    ...
    class ToolSearchArgs(BaseModel):
        ...
        kind: Kind | None = Field(default=None, description="Optional tool kind filter")
    ...
    class SherlockGuideArgs(BaseModel):
        ...
        sunet: str | None = Field(
            default=None, description="SUNet ID for user-specific commands"
        )
        pi_group: str = Field(default="russpold", description="PI group/account name")
        partition: str | None = Field(
            default=None, description="Optional partition override"
        )
        qos: str | None = Field(default=None, description="Optional qos override")
        time: str | None = Field(default=None, description="Optional walltime override")
        cpus: int | None = Field(
        ...
        log_path: str | None = Field(default=None, description="Explicit log path")
    ...
    class SherlockSlurmArgs(BaseModel):
        ...
        cluster_profile: str = Field(
            default="sherlock_russpold", description="Cluster profile defaults to apply"
        )
        ...
        ntasks_per_node: int | None = Field(default=None, description="Tasks per node")
        gpus: int | None = Field(
        ...
        array_concurrency: int | None = Field(
            default=None, description="Optional array concurrency cap"
        )
        output: str | None = Field(default=None, description="Stdout pattern")
        error: str | None = Field(default=None, description="Stderr pattern")
        mail_user: str | None = Field(default=None, description="Mail recipient")
        ...
        module_lines: list[str] | None = Field(
            default=None, description="Module lines to inject"
        )
        env_lines: list[str] | None = Field(
            default=None, description="Environment setup lines to inject"
        )
        command: str | None = Field(default=None, description="Main command body")
        task_file: str | None = Field(
        ...
        job_id: str | None = Field(default=None, description="Slurm job ID")
        ...
        stdout_text: str | None = Field(default=None, description="Captured stdout")
        stderr_text: str | None = Field(default=None, description="Captured stderr")
        sacct_state: str | None = Field(
            default=None, description="Known sacct state if already available"
        )

    def _server_info() -> dict:
        return call_mcp_tool("server_info")
    ...
    def _sherlock_slurm(
        action: str,
        cluster_profile: str = "sherlock_russpold",
    ...
    for tool_name, description in [
        ...
        def _make_mcp_runtime_tool(name: str):
            def _runtime_tool(**kwargs: Any) -> dict:
                return call_mcp_tool(name, **kwargs)

            _runtime_tool.__name__ = name
        ...
        tools.append(
            StructuredTool.from_function(
                ...
                func=_make_mcp_runtime_tool(tool_name),
...
class UnifiedToolRegistry:
    ...
    def __init__(self):
        self._cache: list[StructuredTool] | None = None
        self._toolspecs_exposed: list[ToolSpec] | None = None
        self._toolspecs_all: list[ToolSpec] | None = None
        self._toolspecs_all_with_workflows: list[ToolSpec] | None = None
    ...
    def get_toolspec_by_name(self, name: str) -> ToolSpec | None:
    ...
    def resolve_toolspec(self, name: str) -> ToolSpec | None:
