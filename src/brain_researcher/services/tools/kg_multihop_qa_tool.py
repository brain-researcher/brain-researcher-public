...
class KGMultihopQATool(NeuroToolWrapper):
    ...
    @staticmethod
    def _resolve_max_seed_lookup_terms() -> int:
        raw = os.getenv(
            "BR_KG_MULTIHOP_MAX_SEED_LOOKUP_TERMS",
            os.getenv("BR_KG_MULTIHOP_MAX_SEARCH_TERMS", "6"),
        )
        try:
            value = int(raw)
        except (TypeError, ValueError):
    ...
    def _run(self, **kwargs) -> ToolResult:
        ...
        if remaining_s <= 0:
            ...
            if seed_timeout_s is None:
                ...
                max_seed_lookup_terms = self._resolve_max_seed_lookup_terms()
                try:
                    seed_resolution = self.query_service.resolve_multihop_seed_terms(
                        search_terms,
                        max_seed_entities=max_seed_entities,
                        max_seed_terms=max_seed_lookup_terms,
                        max_aliases_per_term=6,
                        hits_per_term=6,
                    ...
                    seed_resolution = {
                        "seed_entities": [],
                        "seed_hits_by_term": [],
                        "warnings": [f"Batched seed lookup failed: {exc}"],
                        "budget_exhausted": _remaining_budget_s() <= 0,
                    }
                for message in seed_resolution.get("warnings") or []:
                    warnings.append(str(message))
                resolved_terms = seed_resolution.get("resolved_terms")
                if isinstance(resolved_terms, list) and resolved_terms:
                    search_terms = [str(term) for term in resolved_terms]
                seed_hits = seed_resolution.get("seed_hits_by_term")
                if isinstance(seed_hits, list):
                    seed_hits_by_term = [
                        hit for hit in seed_hits if isinstance(hit, dict)
                    ]
                for hit in seed_resolution.get("seed_entities") or []:
                    if not getattr(hit, "kg_id", None) or hit.kg_id in seen_seed_ids:
                        continue
                    seed_entities.append(self._seed_from_node(hit))
                    seen_seed_ids.add(hit.kg_id)
                    if len(seed_entities) >= max_seed_entities:
                        break
                if seed_resolution.get("budget_exhausted"):
                    if _remaining_budget_s() <= 0:
                        _mark_budget_exhausted("seed_lookup")
                    else:
                        seed_budget_reached = True
