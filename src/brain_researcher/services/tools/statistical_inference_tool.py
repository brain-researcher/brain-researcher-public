...
class StatisticalInferenceArgs(BaseModel):
    ...
    labels_file: Optional[str] = Field(default=None, description="Optional labels/groups file")
    covariates_file: Optional[str] = Field(default=None, description="Optional covariates file")
    ...
    bootstrap_method: str = Field(default="percentile", description="Bootstrap CI method")
    confidence_level: float = Field(default=0.95, description="Confidence level for intervals")
    prior_type: str = Field(default="uninformative", description="Prior family for Bayesian modes")
    n_mcmc: int = Field(default=5000, description="MCMC samples for Bayesian estimation")
    burn_in: int = Field(default=1000, description="Burn-in samples for MCMC")
    robust_method: str = Field(default="trimmed_mean", description="Robust estimator variant")
    trim_proportion: float = Field(default=0.1, description="Trim proportion for robust methods")
    resampling_method: str = Field(default="permutation", description="Resampling approach")
    n_resamples: int = Field(default=1000, description="Number of resamples for auxiliary metrics")
    ...
    target_effect_size: Optional[float] = Field(default=None, description="Target effect size for power estimation")
    correct_multiple: bool = Field(default=True, description="Apply multiple testing correction")
    correction_method: str = Field(default="fdr", description="Correction method")
    compute_confidence_regions: bool = Field(default=True, description="Generate multivariate confidence region")
    region_method: str = Field(default="ellipse", description="Confidence region approach")
...
class StatisticalInferenceTool(NeuroToolWrapper):
    ...
    def _run(self, **kwargs) -> ToolResult:
        try:
            ...
            params: StatisticalInferenceParameters = statistical_inference_from_payload(payload)
