...
class ConnectivityMatrixArgs(BaseModel):
    ...
    output_file: str | None = Field(None, description="Save matrix to file")
...
class SeedBasedConnectivityArgs(BaseModel):
    ...
    seed_mask: str | None = Field(None, description="Path to seed mask image")
    radius: float = Field(default=8.0, description="Radius for spherical seed in mm")
    mask_img: str | None = Field(None, description="Brain mask for analysis")
    ...
    output_file: str | None = Field(None, description="Save connectivity map")
