...
try:
    ...
    def initialize_telemetry_system(*args, **kwargs):
        return None
...
from brain_researcher.services.agent.planner import (  # noqa: E402
    choose_tool,
)

# Import preflight checks and planner
from brain_researcher.services.agent.preflight import (  # noqa: E402
    PreflightMode,
    run_preflight,
)
from brain_researcher.services.orchestrator.background_tasks import (  # noqa: E402
    start_sqlite_sweeper,
)
from brain_researcher.services.orchestrator.job_adapter import (  # noqa: E402
    JobStoreAdapter,
)

# Import JobStore
from brain_researcher.services.orchestrator.job_store_factory import (  # noqa: E402
    get_job_store,
...
from brain_researcher.services.shared.log_scrubber import (  # noqa: E402
    scrub_data,
    scrub_text,
)
...
from brain_researcher.services.shared.retry_timeout import (  # noqa: E402
    load_retry_config,
    load_timeout_config,
)
...
def _load_allowed_origins() -> list[str]:
    raw_origins = os.getenv("ORCHESTRATOR_ALLOWED_ORIGINS") or os.getenv(
        "CORS_ALLOW_ORIGINS"
    )
...
async def _resolve_job_attachments(job: Job, tool_name: str = "job") -> dict[str, str]:
    ...
    try:
        ...
        logger.warning(
            "Attachment resolver unavailable; skipping resolution for %s", tool_name
        )
        return {}
...
from .user_store import user_store as _user_store  # noqa: E402
...
class NotificationManager:
    ...
    async def get_user_notifications(
        ...
        if cursor:
            cursor_idx = next(
                (i for i, n in enumerate(notifications) if n.id == cursor), 0
            )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if retry_settings.enabled:
            print(
                f"Retry system enabled: max_attempts={retry_settings.max_attempts}, "
                f"base_delay={retry_settings.base_delay}s, taxonomy_loaded={retry_settings.taxonomy is not None}"
            )
            if retry_settings.taxonomy:
                print(
                    f"  Categories: {', '.join(retry_settings.taxonomy.categories.keys())}"
                )
        else:
    ...
    if run_backend == "inprocess":
        ...
        print(
            "Skipping in-process job queue processor (BR_RUN_EXECUTION_BACKEND=jobstore)"
        )
    ...
    backend = backend_env or (
        "sqlite" if runtime_env in {"prod", "production"} else "memory"
    )
    ...
    if existing_store is not None:
        ...
        try:
            job_store = get_job_store(
                backend=backend, db_path=db_path, total_gpu_slots=total_gpu_slots
            )
    ...
    if cache_enabled:
        try:
            ...
            if cache_backend == "sqlite":
                ...
                cache_db_path = Path(
                    os.getenv("BR_CACHE_DB_PATH", "/tmp/brain_researcher_cache.db")
                )
    ...
    set_initialized_cache_store(cache_store)
    ...
    sweeper_task = await start_sqlite_sweeper(
        job_store=job_store, backend=backend, stop_event=sweeper_stop_event
    )
    ...
    if create_parallel_orchestrator and set_orchestrator:
        try:
            ...
            orchestrator = create_parallel_orchestrator(
                max_workers=4, resource_limits=resource_limits, enable_adaptive=True
            )
    ...
    if (
        StudioExecutionRuntime is not None
    ...
    if (
        StudioNotebookRuntime is not None
        and app.state.studio_session_runtime is not None
    ):
        try:
    ...
    # Bound concurrent hub-session provisioning so a burst sheds load with a fast
    # 429 (see endpoints/hub_sessions create) instead of piling onto the event
    # loop / thread pool and stalling probes. Created on the running loop so the
    # Semaphore binds to the correct loop.
    try:
        _max_hub_create = int(os.getenv("BR_HUB_SESSION_CREATE_MAX_CONCURRENCY", "8"))
    except (TypeError, ValueError):
        _max_hub_create = 8
    app.state.hub_create_semaphore = asyncio.Semaphore(max(1, _max_hub_create))

    # Startup finished: /readyz now reports ready.
...
async def api_chat(payload: ChatIn):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            response = await client.post(
                f"{AGENT_URL}/chat", json={"query": payload.message}
            )
        ...
        raise HTTPException(  # noqa: B904
            status_code=502, detail=f"Agent service error: {str(e)}"
        )
    except Exception as e:
        raise HTTPException(  # noqa: B904
    ...
    if not text:
        raise HTTPException(status_code=500, detail="Empty response from agent service")
    ...
    run_card = RunCardV1(
        ...
        title="Direct Chat",
        description=(
            payload.message[:200] if len(payload.message) > 200 else payload.message
        ),
        outputs={
            "text": text,
    ...
    return ChatOut(
        message={"id": str(uuid.uuid4()), "role": "assistant", "content": text},
        runCard=run_card,
            ...
            if store is not None:
                ...
                if job_record is not None:
                    if job_record.job_id not in jobs_db:
                        ...
                        try:
                            created_at = (
                                datetime.utcfromtimestamp(job_record.created_at)
...
class EnhancedJobManager:
    ...
    async def create_job(
        ...
        if job_id and EnhancedJobManager._flag_true(
            (request.parameters or {}).get("demo_seed")
        ):
                ...
                if scenario.prompt.lower() in request.prompt.lower():
                    ...
                    demo_job = Job(
                        ...
                        timing=TimingInfo(
                            ...
                            - timedelta(seconds=scenario.execution_time_seconds),
                            end_time=datetime.utcnow(),
                            duration_ms=scenario.execution_time_seconds * 1000,
                        ),
                        metadata={"demo_mode": True, "scenario_id": scenario.id},
        ...
        job = Job(
            ...
            status=(
                ...
                else JobStatus.PENDING if use_inprocess_queue else JobStatus.QUEUED
            ),
            prompt=request.prompt,
            ...
            metadata={
                ...
                "demo_seed": EnhancedJobManager._flag_true(
                    job_parameters.get("demo_seed")
                ),
                "demo_mode": bool(request.demo_mode),
                "copilot": request.copilot,
                "thread_id": request.thread_id,
                "checkpoint_id": (
                    str(effective_checkpoint_id) if effective_checkpoint_id else None
                ),
    ...
    async def _maybe_complete_demo_job(job: Job, request: RunRequest) -> bool:
        ...
        if (
            request.pipeline
            not in (PipelineType.DEMO, PipelineType.CHAT, PipelineType.CUSTOM)
            and not request.demo_mode
        ):
            return False
        ...
        if not scenario:
            logger.debug(
                "Demo scenario %s not found; falling back to normal execution",
                normalized_demo_id,
            )
            return False
    ...
    async def _complete_demo_seed_job(job: Job, request: RunRequest) -> None:
        ...
        await notify_job_update(
            job.id, {"type": "status", "status": JobStatus.COMPLETED}
        )
    ...
    async def _complete_demo_job(job: Job, scenario: DemoScenario) -> None:
        ...
        step = JobStep(
            ...
            provenance=ProvenanceInfo(
                tool_version="demo",
                parameters_hash=hashlib.md5(scenario.id.encode()).hexdigest(),
            ),
        )
        job.steps = [step]
    ...
    async def add_provenance(
        job_id: str, step_id: str, tool_version: str, parameters: dict
    ):
    ...
    async def generate_run_card(job_id: str) -> RunCard:
        ...
        if job is None:
            ...
            if (
                not job.metadata.get("branch_events")
                or not job.metadata.get("planner_events")
                or not job.metadata.get("planner_state")
            ):
                await _load_payload_from_store()
                if payload_metadata:
                    if not job.metadata.get("branch_events") and payload_metadata.get(
                        "branch_events"
                    ):
                        job.metadata["branch_events"] = payload_metadata.get(
                            "branch_events"
                        )
                    if not job.metadata.get("planner_events") and payload_metadata.get(
                        "planner_events"
                    ):
                        job.metadata["planner_events"] = payload_metadata.get(
                            "planner_events"
                        )
                    if not job.metadata.get("planner_state") and payload_metadata.get(
                        "planner_state"
                    ):
                        job.metadata["planner_state"] = payload_metadata.get(
                            "planner_state"
                        )
                if plan_payload:
                    if not job.metadata.get("planner_events") and plan_payload.get(
                        "planner_events"
                    ):
                        job.metadata["planner_events"] = plan_payload.get(
                            "planner_events"
                        )
                    if not job.metadata.get("planner_state") and plan_payload.get(
                        "planner_state"
                    ):
                        job.metadata["planner_state"] = plan_payload.get(
                            "planner_state"
                        )
        ...
        parameters = (
            metadata.get("parameters", {}) if isinstance(metadata, dict) else {}
        )
        ...
        if job is not None:
            ...
            execution_steps = EnhancedJobManager._extract_execution_steps(
        ...
        for step in execution_steps:
            ...
            if tool_name == "fsl":
                citations.append(
                    {"title": "FSL Software", "doi": "10.1016/j.neuroimage.2011.09.015"}
                )
            elif tool_name == "nilearn":
                citations.append(
                    {"title": "Nilearn", "doi": "10.3389/fninf.2014.00014"}
                )
        ...
        description_source = (
            job.prompt if job is not None else payload.get("prompt", "")
        )
        pipeline_name = (
            metadata.get("pipeline", "custom")
        ...
        run_card = RunCard(
            ...
            inputs={
                "parameters": parameters,
                "datasets": (
                    metadata.get("datasets", []) if isinstance(metadata, dict) else []
                ),
                "attachments": (
                    [att.model_dump() for att in job.attachments]
                    if job and job.attachments
                    else []
                ),
            },
            execution={
                "duration_seconds": (
                    (job.timing.end_time - job.timing.start_time).total_seconds()
                    if job and job.timing.end_time
                    else 0
                ),
            ...
            outputs={
                "text": text_content,
                "artifacts": (
                    [art.model_dump() for art in job.artifacts if art.type != "text"]
        ...
        try:
            ...
            datasets_payload = []
            if isinstance(run_card.inputs, dict) and isinstance(
                run_card.inputs.get("datasets"), list
            ):
                datasets_payload = [
                    d for d in run_card.inputs.get("datasets") if isinstance(d, dict)
                ]

            artifacts_payload = []
            if isinstance(run_card.outputs, dict) and isinstance(
                run_card.outputs.get("artifacts"), list
            ):
                artifacts_payload = [
                    a for a in run_card.outputs.get("artifacts") if isinstance(a, dict)
                ]

            repro = compute_reproducibility_v1(
                run_dir=(
                    run_dir_path if run_dir_path and run_dir_path.exists() else None
                ),
    ...
    if job.progress:
        ...
        if job_id in job_updates:
            await job_updates[job_id].put(
                {"type": "progress", "progress": job.progress.model_dump()}
            )
...
class ThreadManager:
    ...
    async def add_message(
        thread_id: str, request: MessageRequest, role: str = "user"
    ) -> Message:
    ...
    async def get_messages(
        thread_id: str, limit: int = 50, before: str | None = None
    ) -> MessageHistory:
        ...
        if before:
            ...
            cursor_idx = next(
                (i for i, m in enumerate(messages) if m.id == before), len(messages)
            )
    ...
    async def execute_query(prompt: str, parameters: dict | None = None) -> dict:
        ...
        async def _execute():
            async with httpx.AsyncClient(
                ...
                response = await client.post(
                    f"{AGENT_URL}/chat", json={"query": prompt, **(parameters or {})}
                )
                response.raise_for_status()
                data = response.json()
...
class EnhancedBRKGClient:
    ...
    async def search_datasets(
        ...
        try:
            async with httpx.AsyncClient(
                timeout=default_timeout_config.dataset_timeout_ms / 1000
            ) as client:
                params = {
                    "q": query,
                    "page": page,
                ...
                response = await client.get(f"{BR_KG_URL}/api/datasets", params=params)
...
def generate_mock_datasets(
    query: str | None, filters: dict | None, page: int, limit: int
) -> DatasetSearchResponse:
...
async def _run_agent_plan_for_job(job: Job) -> bool:
    ...
    try:
        ...
        claims = verify_por_token_from_env(
            token=por_token, plan_id=plan.plan_id, version=plan.version
        )
        if claims is None:
            logger.warning(
    ...
    try:
        ...
        if fallback_tool and not started_plan_steps and completed == 0:
            ...
            return False
        logger.error(
            "Agent plan execution failed for job %s: %s", job.id, exc, exc_info=True
        )
        raise
    ...
    if completed < total:
        await update_job_progress(
            job.id, completed, (completed / total) * 100, total - completed
        )
    ...
    if failed_events or failed_plan:
        first_failure = (
            ...
            else failed_plan if isinstance(failed_plan, dict) else {}
        )
        message = (
...
async def execute_job(job_id: str):
    ...
    ENABLE_DEMO_PIPELINES = (
        os.getenv("BR_ENABLE_DEMO_PIPELINES", "false").lower() == "true"
    )
    ...
    try:
        ...
        if not plan_executed:
            ...
            if demo_pipelines_enabled and pipeline == PipelineType.GLM.value:
                ...
                demo_pipelines_enabled
                and pipeline == PipelineType.PIPELINE_BUILDER.value
            ):
        ...
        if job.status in {JobStatus.FAILED, JobStatus.CANCELLED, JobStatus.TIMEOUT}:
            ...
            raise RuntimeError(
                existing_error or "job execution ended in non-success state"
            )
        ...
        await notify_job_update(
            job_id, {"type": "status", "status": JobStatus.COMPLETED}
        )
        await EnhancedJobManager._sync_job_in_store(job)
            ...
            record_telemetry_event(
                {
                    "job_id": job_id,
                    "artifact_id": (
                        artifact.id if hasattr(artifact, "id") else str(artifact)
                    ),
                    "artifact_type": (
                        artifact.type if hasattr(artifact, "type") else "unknown"
                    ),
                    "artifact_size": artifact.size if hasattr(artifact, "size") else 0,
                    "artifact_checksum": (
                        artifact.checksum if hasattr(artifact, "checksum") else None
                    ),
                },
        ...
        job.status = JobStatus.FAILED
        job.error = ErrorResponse.create(ErrorCode.PROCESSING_ERROR, str(e))
        job.timing.end_time = datetime.utcnow()
        try:
            job.completed_at = job.timing.end_time
        except Exception:
        ...
        record_telemetry_event(
            {
                ...
                "steps_completed": len(
                    [
                        s
                        for s in job.steps
                        if hasattr(s, "status") and s.status == "completed"
                    ]
                ),
            },
        ...
        await notify_job_update(
            job_id, {"type": "status", "status": JobStatus.FAILED, "error": str(e)}
        )
        await EnhancedJobManager._sync_job_in_store(job)
...
async def execute_glm_pipeline(job_id: str):
    ...
    step1 = JobStep(
        ...
        tool="br_kg",
        args={
            "dataset_id": job.metadata.get("dataset_id"),
            "_resolved_files": resolved_files,
        },
        status=StepStatus.RUNNING,
    )
    ...
    step2 = JobStep(
        ...
        tool="fsl",
        args={
            **(job.metadata.get("parameters", {}) or {}),
            "_resolved_files": resolved_files,
        },
        status=StepStatus.RUNNING,
    )
    ...
    artifact1 = JobArtifact(
        ...
        provenance=ProvenanceInfo(generated_by="step_2", tool_version="FSL 6.0.5"),
    )
    job.artifacts.append(artifact1)
    await notify_job_update(
        job_id, {"type": "artifact", "artifact": artifact1.model_dump()}
    )
...
async def execute_connectivity_pipeline(job_id: str):
    ...
    await EnhancedJobManager.add_provenance(
        job_id, "step_2", "Nilearn 0.10", step2.args
    )
    await update_job_progress(job_id, 2, 90.0, 5)
    ...
    await notify_job_update(
        job_id, {"type": "artifact", "artifact": artifact.model_dump()}
    )
...
def _normalize_generic_tool_output_args(
    ...
    *,
    ...
    if (
        isinstance(output_dir_value, str)
        and output_dir_value.strip()
        and "://" not in output_dir_value
    ):
    ...
    for key in output_path_keys:
        ...
        if (
            force_job_output_dir
            and key not in output_dir_keys
...
def _derive_cross_pod_tool_run_dir(run_dir: str | None) -> str | None:
    ...
    source_root = os.getenv(
        "BR_GENERIC_TOOL_SOURCE_ROOT", "/app/data/shared/runs"
    ).strip()
...
def _extract_plan_output_dir(job: Any, events: list[dict[str, Any]]) -> str | None:
    ...
    try:
        common = Path(
            os.path.commonpath([str(path.resolve()) for path in output_paths])
        )
    except Exception:
        return None
    return str(common)
...
def _attach_plan_execution_artifacts(
    job: Any, job_id: str, events: list[dict[str, Any]]
) -> None:
    ...
    if source_output_dir and str(Path(source_output_dir).expanduser()) != str(
        ...
        try:
            ...
            source_root = Path(source_output_dir).expanduser().resolve()
            if not run_root.exists() and source_root.exists() and source_root.is_dir():
                job.run_dir = str(source_root)
                run_dir = str(source_root)
        except Exception:
            pass
    ...
    try:
        ...
        if explicit_tool:
            ...
            if job.attachments:
                ...
                attachment_dicts = [att.model_dump() for att in job.attachments]
                resolved_paths = await resolve_attachments_for_tool(
                    attachment_dicts, "agent"
                )
                ...
                for att in job.attachments:
                    if att.id in resolved_paths:
                        ...
                        att.storage = "local"
                agent_params["attachments"] = [
                    att.model_dump() for att in job.attachments
                ]
            scenario_payload = get_chat_scenario_payload(
                job.metadata.get("scenario_id")
            )
            if scenario_payload:
                ...
                agent_params.setdefault(
                    "system_prompt", scenario_payload.get("system_prompt")
                )
                if scenario_payload.get("planner_hints"):
                    agent_params.setdefault(
                        "planner_hints", scenario_payload["planner_hints"]
                    )
...
async def execute_builder_pipeline(job_id: str):
    ...
    for index, step_data in enumerate(plan):
        ...
        try:
            ...
            await update_job_progress(
                job_id, index + 1, max(5.0, start_progress), max(0, total_steps - index)
            )
            ...
            job_step.preview = (
                step_data.get("summary") or f"{tool_name} completed successfully"
            )
            ...
            # Emit step_completed telemetry event
            builder_step_duration_ms = (
                time.perf_counter_ns() - builder_step_start_ns
            ) // 1_000_000
            record_telemetry_event(
                {
                    "job_id": job_id,
        ...
        if node_id and node_id in resource_snapshot:
            ...
            resource_snapshot[node_id]["resources"] = {
                "cpu": 18,
                "memory": 32,
                "gpu": 0,
            }
        ...
        artifact = JobArtifact(
            ...
            meta={"node_id": node_id, "summary": job_step.preview, "parameters": args},
        )
        job.artifacts.append(artifact)
        await notify_job_update(
            job_id, {"type": "artifact", "artifact": artifact.model_dump()}
...
async def execute_chat_pipeline(job_id: str):
    ...
    TIMEOUT = httpx.Timeout(connect=10.0, read=110.0, write=30.0, pool=10.0)
    ...
    try:
        ...
        if use_tools:
            ...
            request_data.update(
                {
                    "tool_mode": job.metadata.get(
                        "tool_mode", params_meta.get("tool_mode", "auto")
                    ),
        ...
        # Call agent service
        async with httpx.AsyncClient(timeout=TIMEOUT) as client:
            response = await client.post(f"{AGENT_URL}{endpoint}", json=request_data)
            response.raise_for_status()
            data = response.json()
        ...
        artifact = JobArtifact(
            ...
            meta={
                "content": text,
                "model": data.get("runCard", {})
                .get("execution", {})
                .get("model", "deepseek"),
                "latency_ms": data.get("runCard", {})
                .get("execution", {})
                .get("latency_ms"),
            },
        )
        ...
        if use_tools and data.get("tool_calls"):
            for i, tool_call in enumerate(data["tool_calls"]):
                if tool_call.get("status") == "ok":
                    ...
                    await notify_job_update(
                        job_id, {"type": "step", "step": tool_step.model_dump()}
                    )
        ...
        job.status = JobStatus.FAILED
        job.error = ErrorResponse.create(
            ErrorCode.PROCESSING_ERROR, f"Failed to get AI response: {str(e)}"
        )
    ...
    await notify_job_update(
        job_id,
        {
            ...
            "status": step.status,
            "preview": (
                step.preview if step.status == StepStatus.COMPLETED else step.error
            ),
        },
    )
...
async def login(request: Request, login_data: LoginRequest):
    ...
    if not verify_password(login_data.password, user.hashed_password):
        ...
        raise HTTPException(
            ...
            detail=(
                await create_error_response(
                    request, ErrorCode.UNAUTHORIZED, "Invalid username or password"
                )
            ).model_dump(),
        )
...
async def oauth_login(
    request: Request, provider: OAuthProvider, oauth_data: OAuthRequest
):
...
async def get_user_profile(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        if current_user.id in notifications_db:
            unread_count = sum(
                1 for n in notifications_db[current_user.id] if not n.read
            )
...
async def websocket_notifications(websocket: WebSocket, token: str = Query(...)):
    ...
    try:
        ...
        await websocket.send_json(
            {"type": "unread_count", "count": notifications.unread_count}
        )
...
async def health_check():
    ...
    services["orchestrator"] = ServiceHealth(
        name="orchestrator", status="healthy", latency_ms=0
    )
    ...
    try:
        ...
        services["agent"] = ServiceHealth(
            name="agent", status="unavailable", error=str(e)
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=2.0) as client:
            response = await client.get(f"{BR_KG_URL}/health")
            latency = int((datetime.utcnow() - start).total_seconds() * 1000)
            services["br_kg"] = ServiceHealth(
                name="br_kg",
                status="healthy" if response.status_code == 200 else "degraded",
                latency_ms=latency,
            )
    except Exception:
        services["br_kg"] = ServiceHealth(name="br_kg", status="unavailable")
...
async def agent_plan(
    ...
    mode: str | None = Body(
        None,
        embed=True,
        description="Planner mode. Active runtime only supports 'catalog'.",
    ),
    user_id: str | None = Body(
        None, embed=True, description="Optional user ID for plan attribution"
    ),
    workspace_id: str | None = Body(
        None, embed=True, description="Optional workspace/org ID for plan attribution"
    ),
...
def _build_plan_request_from_run(
    ...
    if "fmri" in modality_list:
        inputs["fmri_img"] = str(
            parameters.get("fmri_img") or parameters.get("input") or "bold.nii.gz"
        )
    ...
    if "eeg" in modality_list:
        ...
        inputs["montage_name"] = str(parameters.get("montage_name") or "standard_1020")
    ...
    if allowlist:
        ...
        constraint = ConstraintSpec(
            tool_allowlist=[str(t) for t in allowlist if t], max_steps=3
        )
...
def _resolve_effective_tool_allowlist(
    ...
    if constraint and constraint.tool_allowlist:
        ...
        effective = (
            request_allowlist if effective is None else effective & request_allowlist
        )
...
def _apply_env_tool_allowlist(plan_request: PlanRequest) -> PlanRequest:
    ...
    if constraint.tool_allowlist:
        constraint.tool_allowlist = [
            tool for tool in constraint.tool_allowlist if tool in env_allowlist
        ]
    else:
...
async def create_run(
    ...
    accepts = (
        (http_request.headers.get("accept") if http_request else "") or ""
    ).lower()
    wants_sse = False
    if http_request:
    ...
    requested_job_id = (
        ...
        if isinstance(request.requested_job_id, str)
        and request.requested_job_id.strip()
        else None
    )
    ...
    pipeline_value = (
        pipeline_hint.value
        if isinstance(pipeline_hint, PipelineType)
        else pipeline_hint
    )
    ...
    if pipeline_value in {
        ...
        selection = select_tool(
            ...
            current_pipeline=(
                pipeline_hint if isinstance(pipeline_hint, PipelineType) else None
            ),
        ...
        logger.info(
            ...
            selection.profile,
            (
                selection.pipeline.value
                if isinstance(selection.pipeline, PipelineType)
                else selection.pipeline
            ),
            selection.tool,
    ...
    if tool_selection and tool_selection.profile == "code":
        ...
        job.status = JobStatus.RUNNING
        await notify_job_update(
            job.id, {"type": "step", "step": plan_step.model_dump()}
        )
        ...
        if wants_sse:
            ...
            accepted_payload = json.dumps(
                (
                    ...
                    if hasattr(job_response, "model_dump")
                    else job_response.dict()
                ),
                default=str,
            )
    ...
    if cache_enabled and not trusted_demo_seed_request:
        try:
            ...
            if cache_entry:
                if cache_entry.state == "completed":
                    ...
                    if hasattr(app.state, "metrics"):
                        app.state.metrics.record_cache_operation(
                            operation="lookup", result="hit"
                        )
                    return JSONResponse(
                        {
            ...
            if canonical_params:
                ...
                pending_meta["params_preview"] = {
                    k: str(v)[:80] for k, v in preview_items
                }
    ...
    resource_planner_enabled = (
        os.getenv("BR_RESOURCE_PLANNER_ENABLED", "false").lower() == "true"
    )
    ...
    if forwarded_client_plan_payload or forwarded_plan_payload or forwarded_tags:
        parameters["_client_metadata"] = {
            ...
            **({"normalized_tags": forwarded_tags} if forwarded_tags else {}),
        }
    ...
    if job.status != JobStatus.COMPLETED:  # Not a demo job
        run_backend = (
            getattr(app.state, "run_execution_backend", None)
            or os.getenv("BR_RUN_EXECUTION_BACKEND", "inprocess").strip().lower()
        )
        if run_backend in {"job_store", "job-store"}:
            run_backend = "jobstore"
...
async def resolve_run_by_cache_key(
    ...
    if not entry or entry.state != "completed":
        raise HTTPException(
            status_code=404, detail="Cache entry not found or incomplete"
        )
...
async def get_job_provenance(job_id: str):
    ...
    for step in job.steps:
        nodes.append(
            {
                ...
                "type": "process",
                "label": step.name,
                "metadata": {"tool": step.tool, "status": step.status.value},
            }
        )
        ...
        if job.metadata.get("dataset_id"):
            edges.append(
                {"source": "dataset", "target": step.id, "relationship": "used_by"}
            )
        ...
        nodes.append(
            {
                ...
                "type": "data",
                "label": artifact.name,
                "metadata": {"type": artifact.type.value},
            }
        )
    ...
    return {
        "job_id": job_id,
        "provenance_graph": {"nodes": nodes, "edges": edges},
        "run_card": (
            job.run_card.model_dump()
            if job.run_card
            else await EnhancedJobManager.generate_run_card(job_id)
        ),
    }
...
async def annotate_artifact(
    job_id: str, artifact_id: str, annotations: list[dict] = Body(...)
):
    ...
    raise HTTPException(
        status_code=404,
        detail=ErrorResponse.create(
            ErrorCode.NOT_FOUND, "Artifact not found"
        ).model_dump(),
    )
...
async def get_thread_messages(
    thread_id: str, limit: int = Query(50, ge=1, le=200), before: str | None = None
):
    ...
    if normalized_thread_id not in messages_db:
        return JSONResponse(
            ...
            content=ErrorResponse.create(
                ErrorCode.NOT_FOUND, "Thread not found"
            ).model_dump(),
        )
...
async def add_thread_message(thread_id: str, request: MessageRequest):
    ...
    normalized_thread_id = _normalize_thread_id(thread_id)
    if (
        normalized_thread_id not in threads_db
        and normalized_thread_id != "thread_default"
    ):
        return JSONResponse(
            status_code=404,
            content=ErrorResponse.create(
    ...
    threads_db.get(normalized_thread_id)
...
async def upload_file(
    ...
    try:
        ...
        try:
            FileUploadRequest(
                filename=file.filename, content_type=content_type, size=file_size
            )
...
async def get_file_info(
    file_id: str, credentials: HTTPAuthorizationCredentials | None = Depends(security)
):
...
async def download_file(
    ...
    return FileResponse(
        path=file_path, filename=filename, media_type=file_info["content_type"]
    )
...
def _topological_sort_pipeline(request: PipelineExecutionRequest) -> list[str]:
    ...
    if len(ordered) != len(request.nodes):
        raise ValueError(
            "Pipeline contains cycles. Please remove circular dependencies."
        )
...
) -> (list[PipelineExecutionStep], dict[str, PipelineResourceSnapshot]):
...
async def execute_visual_pipeline(payload: PipelineExecutionRequest, request: Request):
    ...
    estimated_duration_seconds = max(
        5, sum(step.estimated_duration_ms for step in plan_steps) // 1000
    )
    ...
    job.metadata["resource_snapshot"] = {
        node_id: snapshot.model_dump(mode="json")
        for node_id, snapshot in resource_snapshot.items()
    }
    ...
    await notify_job_update(
        job.id, {"type": "resources", "snapshot": job.metadata["resource_snapshot"]}
    )
...
async def list_datasets(
    ...
    return await EnhancedBRKGClient.search_datasets(
        q, filters, page, limit, sort, order
    )
...
async def search_datasets(request: DatasetSearchRequest):
    ...
    # Perform search
    result = await EnhancedBRKGClient.search_datasets(text, filters)
...
async def get_dataset(dataset_id: str):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get(f"{BR_KG_URL}/api/datasets/{dataset_id}")
            if response.status_code == 200:
    ...
    if dataset_id == "motor-task-sample":
        return Dataset(
            ...
            has_derivatives=True,
            bids_version="1.8.0",
            metadata=DatasetMetadata(authors=["Demo Author"], license="CC0"),
            statistics=DatasetStatistics(
                mean_age=28.5, age_range=[21, 45], sex_distribution={"M": 10, "F": 10}
            ),
            last_updated=datetime.utcnow(),
            quality_score=0.85,
        )
    ...
    return JSONResponse(
        status_code=404,
        content=ErrorResponse.create(
            ErrorCode.NOT_FOUND, "Dataset not found"
        ).model_dump(),
    )
...
async def list_tools():
    ...
    return ToolsResponse(tools=tools, categories=categories, total_count=len(tools))
...
async def graphql_query(query: str = Body(...), variables: dict | None = Body(None)):
    ...
    try:
        ...
        raise HTTPException(  # noqa: B904
            status_code=503,
            detail=ErrorResponse.create(
                ErrorCode.SERVICE_ERROR, f"GraphQL query failed: {str(e)}"
            ).model_dump(),
        )
...
async def run_demo_scenario(scenario_id: str):
    ...
    if scenario_id not in demo_scenarios:
        raise HTTPException(
            status_code=404,
            detail=ErrorResponse.create(
                ErrorCode.NOT_FOUND, "Demo scenario not found"
            ).model_dump(),
        )
...
DEMO_DATA_SOURCES = {
    ...
    "motor_glm": Path(os.environ.get("BR_DATA_ROOT", "/app/data"))
    / "openneuro_glmfitlins/stat_maps/ds000009/task-balloonanalogrisktask/node-runLevel",
...
def _demo_root_or_404(demo_id: str) -> Path:
    ...
    if not root or not root.exists():
        raise HTTPException(
            status_code=404, detail=f"Demo '{demo_id}' data not available on this host"
        )
    return root
        ...
        if normalized_demo_id == "glm_motor":
            ...
            return {
                ...
                "key_findings": [
                    ...
                    (
                        f"Peak activation in Primary Motor Cortex (Z = {cluster_data[0]['Max_Z']})"
                        if cluster_data
                        else "Peak activation detected"
                    ),
            ...
            if results_file.exists():
                with open(results_file) as f:
                    reader = csv.DictReader(f)
                    classification_data = [
                        row for row in reader if not row["Fold"].startswith("#")
                    ]

            # Calculate mean accuracy
            accuracies = [
                float(row["Accuracy"]) for row in classification_data if row["Accuracy"]
            ]
            ...
            return {
                ...
                "key_findings": [
                    ...
                    f"Analyzed {len({row['Class'] for row in classification_data})} motor imagery classes",
    ...
    if normalized_demo_id == "glm_motor":
        ...
        evidence_items = [
            ...
            },
            {
                ...
                "metadata": {
                    "toolbox": "BCT",
                    "metrics": "efficiency, modularity, centrality",
                },
            },
        ]
    elif normalized_demo_id in ("ml_decoding", "decoding"):
...
async def get_demo_results(demo_id: str):
    ...
    if summary is None:
        raise HTTPException(
            status_code=404,
            detail=ErrorResponse.create(
                ErrorCode.NOT_FOUND, "Demo results not found"
            ).model_dump(),
        )
...
async def get_demo_artifacts(
    ...
    try:
        ...
        return {
            ...
            "artifacts": paginated,
            "total_count": total,
            "index_stats": {"by_statistic": {}, "by_subject": {}},
        }
    except HTTPException:
        ...
        if scenario is None:
            raise HTTPException(  # noqa: B904
                status_code=404,
                detail=ErrorResponse.create(
                    ErrorCode.NOT_FOUND, "Demo not found"
                ).model_dump(),
            )
...
async def create_demo_share_link(
    request: DemoShareRequest, current_user: User | None = Depends(get_current_user)
):
...
async def get_demo_citations(demo_id: str):
    ...
    for citation in citations:
        categories[citation.citation_type] = (
            categories.get(citation.citation_type, 0) + 1
        )
...
async def get_demo_provenance(demo_id: str):
    ...
    try:
        ...
        raise HTTPException(  # noqa: B904
            status_code=404, detail="Provenance data not available for this demo"
        )
...
async def download_real_artifact(demo_id: str, artifact_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="Artifact not found")  # noqa: B904
    ...
    return FileResponse(
        str(file_path), media_type="application/gzip", filename=file_path.name
    )
