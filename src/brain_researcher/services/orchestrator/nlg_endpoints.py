...
from ..agent.language_templates import ExplanationLevel, Language
...
class NLGRequest(BaseModel):
    ...
    response_type: str = Field(
        default="analysis_result", description="Type of response"
    )
    explanation_level: str = Field(
        default="structured", description="Level of explanation"
    )
    ...
    expertise_level: str = Field(
        default="intermediate", description="User expertise level"
    )

    # Generation options
    include_alternatives: bool = Field(
        default=True, description="Include alternative explanations"
    )
    include_citations: bool = Field(default=True, description="Include citations")
    include_visualizations: bool = Field(
        default=False, description="Include visualization descriptions"
    )
    time_preference: str | None = Field(
        default=None, description="Time preference: brief, standard, comprehensive"
    )
...
class ExplanationRequest(BaseModel):
    ...
    analysis_result: dict[str, Any] = Field(
        ..., description="Analysis result to explain"
    )

    # Explanation configuration
    explanation_levels: list[str] = Field(
        default=["technical", "layman"], description="Explanation levels to generate"
    )
    languages: list[str] = Field(default=["en"], description="Languages to generate")

    # User context
    user_profile: dict[str, Any] | None = Field(
        default=None, description="User profile"
    )
    domain_focus: list[str] = Field(default=[], description="Domain focus areas")

    # Output options
    structured_output: bool = Field(
        default=True, description="Include structured explanation"
    )
    confidence_analysis: bool = Field(
        default=True, description="Include confidence analysis"
    )
...
class UserProfileUpdate(BaseModel):
    ...
    preferred_explanation_level: str | None = Field(
        default=None, description="Preferred explanation level"
    )
    adaptation_strategy: str | None = Field(
        default=None, description="Adaptation strategy"
    )

    # Preferences
    detailed_methodology: bool | None = Field(
        default=None, description="Include detailed methodology"
    )
    include_citations: bool | None = Field(
        default=None, description="Include citations"
    )
    visual_descriptions: bool | None = Field(
        default=None, description="Include visual descriptions"
    )
    statistical_details: bool | None = Field(
        default=None, description="Include statistical details"
    )

    # Domain focus
    domain_focus: list[str] | None = Field(
        default=None, description="Domain focus areas"
    )
    time_preferences: str | None = Field(default=None, description="Time preferences")
...
class FeedbackRequest(BaseModel):
    ...
    usefulness_score: float = Field(
        ..., description="Usefulness score (0-1)", ge=0, le=1
    )

    # Optional feedback
    comments: str | None = Field(default=None, description="Additional comments")
    suggested_improvements: list[str] | None = Field(
        default=None, description="Suggested improvements"
    )
    confusion_points: list[str] | None = Field(
        default=None, description="Points of confusion"
    )
...
class TranslationRequest(BaseModel):
    ...
    preserve_technical_terms: bool = Field(
        default=True, description="Preserve technical terminology"
    )
...
class NLGResponse(BaseModel):
    ...
    structured_explanation: dict[str, str] | None = None
    citations: list[str] = []
...
async def generate_response(request: NLGRequest):
    ...
    try:
        ...
        return NLGResponse(
            ...
            alternative_texts=(
                nlg_response.alternative_texts if request.include_alternatives else []
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Response generation failed: {str(e)}"
        )
    ...
    try:
        ...
        for level_str in request.explanation_levels:
            ...
            for lang_str in request.languages:
                ...
                explanation_result = (
                    nlg_engine.explanation_generator.generate_explanation(
        ...
        if request.structured_output:
            ...
            structured = (
                nlg_engine.explanation_generator.generate_structured_explanation(
                    request.analysis_result, context
                )
            )
...
async def get_language_support():
    ...
    try:
        supported_languages = [
            {"code": lang.value, "name": _get_language_name(lang)} for lang in Language
        ]
...
async def update_user_profile(user_id: str, request: UserProfileUpdate):
    ...
    try:
        ...
        if request.preferred_explanation_level:
            profile.preferred_explanation_level = ExplanationLevel(
                request.preferred_explanation_level
            )
        if request.adaptation_strategy:
            profile.adaptation_strategy = AdaptationStrategy(
...
async def submit_feedback(request: FeedbackRequest):
    ...
    try:
        ...
        overall_score = (
            request.quality_score + request.clarity_score + request.usefulness_score
        ) / 3
        ...
        logger.info(
            f"Feedback received for user {request.user_id}: "
...
async def get_quality_analytics():
    ...
    try:
        ...
        for _user_id, profile in nlg_engine.user_profiles.items():
            if profile.feedback_scores:
                all_feedback_scores.extend(profile.feedback_scores)
                user_count += 1

        if all_feedback_scores:
            analytics["average_user_satisfaction"] = sum(all_feedback_scores) / len(
                all_feedback_scores
            )
        ...
        analytics["cache_hit_rate"] = len(nlg_engine.response_cache) / max(
            user_count, 1
        )

        # Response type distribution
        response_type_counts = {}
        for _response in nlg_engine.response_cache.values():
...
def _analyze_confidence_factors(
    ...
    if n_subjects > 50:
        factors.append(
            {"factor": "sample_size", "strength": "high", "value": n_subjects}
        )
    elif n_subjects > 20:
        factors.append(
            {"factor": "sample_size", "strength": "moderate", "value": n_subjects}
        )
    else:
        factors.append(
            {"factor": "sample_size", "strength": "low", "value": n_subjects}
        )
    ...
    if effect_size is not None:
        if effect_size > 0.8:
            factors.append(
                {"factor": "effect_size", "strength": "large", "value": effect_size}
            )
        elif effect_size > 0.5:
            factors.append(
                {"factor": "effect_size", "strength": "medium", "value": effect_size}
            )
        else:
            factors.append(
                {"factor": "effect_size", "strength": "small", "value": effect_size}
            )
...
def _recommend_best_variant(variants: list[dict], profile: UserProfile) -> dict:
    ...
    for variant in variants:
        ...
        if profile.time_preferences == "brief" and variant["reading_time"] < 30:
            ...
            profile.time_preferences == "comprehensive" and variant["reading_time"] > 60
        ):
            score += 1
        ...
        if (
            ...
            and variant["complexity"] > 0.7
        ):
            score += 1
        elif (
            profile.expertise_level == ExpertiseLevel.NOVICE
            and variant["complexity"] < 0.4
        ):
            score += 1
