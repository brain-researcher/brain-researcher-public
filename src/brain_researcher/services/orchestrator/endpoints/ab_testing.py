...
async def track_ab_conversion(
    event: ABTestEvent, session_id: str | None = Cookie(None)
):
...
async def track_analytics_events(
    batch: AnalyticsBatch, session_id: str | None = Cookie(None)
):
...
async def get_analytics_stats(
    ...
    # Get unique sessions
    unique_sessions = {e.session_id for e in filtered_events if e.session_id}
    unique_users = {e.user_id for e in filtered_events if e.user_id}
...
async def get_funnel_analytics(funnel_name: str, session_id: str | None = Query(None)):
...
async def track_error_event(
    error_data: dict[str, Any] = Body(...), session_id: str | None = Cookie(None)
):
