...
# Load-shedding for session creation: each create kicks off blocking k8s
# provisioning (offloaded to a thread). The lifespan installs an asyncio
# Semaphore on app.state.hub_create_semaphore bounding how many run concurrently;
# when it is saturated, the excess gets a fast, retryable 429 instead of hanging
# to the client's request timeout. Tunable via env.
try:
    _HUB_CREATE_ACQUIRE_TIMEOUT_S = float(
        os.getenv("BR_HUB_SESSION_CREATE_ACQUIRE_TIMEOUT_S", "2.0")
    )
...
async def create_or_attach_hub_session(
    ...
    # Shed load under provisioning bursts: bound concurrent creates, fast 429 on
    # saturation rather than a hang to the client timeout.
    sem = getattr(request.app.state, "hub_create_semaphore", None)
    acquired = False
    if sem is not None:
        try:
            ...
            raise HTTPException(
                status_code=429,
                detail={"error": "overloaded", "reason": "session_create_capacity"},
                headers={"Retry-After": str(_HUB_CREATE_RETRY_AFTER_S)},
            )
    try:
...
def _resolve_marimo_internal_base_url(target: Any) -> str | None:
    """Pick the cluster-internal marimo API base the orchestrator should call.

    marimo is launched with ``--base-url <path>`` (the public proxy prefix, e.g.
    ``/hub/br-marimo-rt-xxx``), so EVERY marimo route — ``/api/sessions``,
    ``/api/document/transaction`` — is served under that prefix. ``internal_url``
    is the bare cluster service root (``http://br-marimo-rt-xxx:2718``) with no
    prefix, so calls against it 404 unless we append the base path derived from
    ``public_url``. (This is why the server-side cell-append path 404'd until
    now.)
    """
    public = getattr(target, "public_url", None)
    base_path = ""
    if isinstance(public, str) and public.strip():
        base_path = urlparse(public.strip()).path.rstrip("/")

    internal = getattr(target, "internal_url", None)
    if isinstance(internal, str) and internal.strip():
        internal = internal.strip().rstrip("/")
        if base_path and not internal.endswith(base_path):
            return f"{internal}{base_path}"
        return internal
...
def _select_marimo_session_id(
    sessions: dict[str, Any], path_hint: str | None
) -> str | None:
    """Pick the live marimo session id from ``GET /api/sessions``.

    marimo now OWNS its session id (it mints its own ``s_xxxxxx`` instead of
    accepting the studio id), so the orchestrator must DISCOVER it before
    proxying a cell append. The singleuser runtime serves exactly one notebook,
    so there is normally a single session; if several ever exist we prefer one
    whose notebook path/filename matches ``path_hint``, else fall back to the
    first entry.
    """
    if not isinstance(sessions, dict) or not sessions:
        return None

    ids = [sid for sid in sessions.keys() if isinstance(sid, str) and sid]
    if not ids:
        return None
    if len(ids) == 1:
        return ids[0]
    ...
    if path_hint:
        hint = path_hint.rsplit("/", 1)[-1]
        for sid in ids:
            info = sessions.get(sid) or {}
            for key in ("path", "filename"):
                value = info.get(key) if isinstance(info, dict) else None
                if isinstance(value, str) and value.rsplit("/", 1)[-1] == hint:
                    return sid
...
async def _discover_marimo_session_id(
    client: httpx.AsyncClient,
    base_url: str,
    headers: dict[str, str],
    path_hint: str | None,
) -> str | None:
    """Fetch marimo's live session id via its discovery endpoint."""
    response = await client.get(f"{base_url}/api/sessions", headers=headers)
    if response.status_code != 200:
        logger.warning(
            "marimo discovery GET %s/api/sessions -> %s body=%r",
            base_url,
    ...
    try:
        ...
        return None
    return _select_marimo_session_id(payload, path_hint)
...
async def append_marimo_cell(
    ...
    skew_token = await runtime.get_runtime_skew_token(session_id)
    ...
    # marimo gates /api/document/transaction on TWO independent checks:
    #  - skew protection: Marimo-Server-Token must equal the server's
    #    skew_protection_token (pinned per-pod via BR_MARIMO_SKEW_PROTECTION_TOKEN),
    #  - auth: the request must carry the runtime auth token.
    # The auth token must NOT be sent as Marimo-Server-Token (that is the skew slot).
    if skew_token:
        headers["Marimo-Server-Token"] = skew_token
    if token:
        headers["Authorization"] = f"Bearer {token}"

    path_hint = getattr(runtime_session, "working_directory", None)

    try:
        async with httpx.AsyncClient(timeout=_CELL_INJECT_TIMEOUT_SECONDS) as client:
            # marimo OWNS its session id (mints its own s_xxxxxx). The studio
            # session_id is no longer imposed on the iframe URL, so we must
            # discover marimo's live id before targeting the transaction —
            # sending the studio id would yield "session-not-found".
            marimo_session_id = await _discover_marimo_session_id(
                client, base_url, headers, path_hint
            )
            if not marimo_session_id:
                raise HTTPException(
                    status_code=503,
                    detail={
            ...
            response = await client.post(
                ...
                headers={**headers, "Marimo-Session-Id": marimo_session_id},
            )
