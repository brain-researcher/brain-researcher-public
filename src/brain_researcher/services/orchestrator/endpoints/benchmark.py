...
from brain_researcher.services.shared import benchmark_db as _benchmark_db
from brain_researcher.services.shared.benchmark_importer import (
...
def _resolve_db_path() -> Path:
    global _DB_PATH
    _benchmark_db._DB_PATH = _DB_PATH
    _DB_PATH = _benchmark_db._resolve_db_path()
    return _DB_PATH


def _get_conn() -> sqlite3.Connection:
    _benchmark_db._DB_PATH = _resolve_db_path()
    return _benchmark_db._get_conn()


def _ensure_tables(conn: sqlite3.Connection) -> None:
    """Create benchmark tables if they don't already exist."""

    _benchmark_db._ensure_tables(conn)
...
def _ensure_governance_columns(conn: sqlite3.Connection) -> None:
    ...
    _benchmark_db._ensure_governance_columns(conn)
...
async def import_benchmark(req: ImportRequest):
    ...
    now = int(time.time())
    dataset_id = req.dataset_id or req.url.rstrip("/").split("/")[-1].replace(
        ".json", ""
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status_code, detail=f"Import failed: {exc}"
        ) from exc
    finally:
        conn.close()
...
async def list_datasets(
    ...
    try:
        ...
        total = conn.execute(
            "SELECT COUNT(*) FROM benchmark_datasets"
...
async def list_tasks(
    ...
    sort: str = Query(
        "updated_at",
        pattern="^(task_id|source_category|source_difficulty|created_at|updated_at)$",
    ),
