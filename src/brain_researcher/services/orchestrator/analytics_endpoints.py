...
class ErrorReport(BaseModel):
    ...
    code: ErrorCode | None = ErrorCode.UNKNOWN
    message: str
...
async def get_analytics_metrics(
    ...
    unique_sessions = len({e.session_id for e in filtered_events})
    unique_users = len({e.user_id for e in filtered_events if e.user_id})

    # Calculate average session duration
    session_durations = []
    for session_id in {e.session_id for e in filtered_events}:
...
async def get_recent_errors(limit: int = 100, severity: ErrorSeverity | None = None):
...
class AlertConfig(BaseModel):
    ...
    enabled: bool
    recipients: list[str]
    lastTriggered: datetime | None = None
...
async def get_usage_metrics(
    ...
    segment: str | None = Query(None, description="User segment filter"),
):
