...
class ReasoningRequest(BaseModel):
    ...
    context: dict[str, Any] | None = Field(
        default=None, description="Additional context for reasoning"
    )
    max_steps: int | None = Field(
        default=10, description="Maximum number of reasoning steps", ge=1, le=20
    )
...
class ReasoningStepsRequest(BaseModel):
    ...
    step_range: tuple[int, int] | None = Field(
        default=None, description="Range of steps to retrieve (start, end)"
    )
...
@reasoning_router.post(
    "/trace", response_model=ReasoningResponse, status_code=status.HTTP_201_CREATED
)
async def create_reasoning_trace(
    request: ReasoningRequest, background_tasks: BackgroundTasks
) -> ReasoningResponse:
    """
    ...
    try:
        ...
        background_tasks.add_task(_validate_reasoning_trace_background, trace.trace_id)
...
@reasoning_router.get(
    "/trace/{trace_id}/steps", response_model=list[ReasoningStepResponse]
)
async def get_reasoning_steps(
    trace_id: str, start: int | None = None, end: int | None = None
...
@reasoning_router.post(
    "/trace/{trace_id}/validate", response_model=ReasoningValidationResponse
)
async def validate_reasoning_trace(trace_id: str) -> ReasoningValidationResponse:
    ...
    if trace.overall_confidence >= 0.8:
        ...
        confidence_assessment = (
            "Medium - reasoning is generally sound with minor concerns"
        )
    else:
        confidence_assessment = (
            "Low - reasoning has significant gaps or inconsistencies"
        )
...
async def list_reasoning_traces(
    limit: int = 10, offset: int = 0, query_filter: str | None = None
) -> list[dict[str, Any]]:
    """
...
async def _validate_reasoning_trace_background(trace_id: str):
    ...
    try:
        if trace_id in _reasoning_traces:
            ...
            if issues:
                logger.warning(
                    f"Reasoning trace {trace_id} has validation issues: {issues}"
                )
            else:
