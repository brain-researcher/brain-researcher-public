...
class JobSpecificationRequest(BaseModel):
    ...
    # Optional metadata
    deadline: str | None = Field(default=None, description="Job deadline (ISO format)")
...
class SpotOptimizationRequest(BaseModel):
    ...
    budget: float | None = Field(default=None, description="Budget constraint")
...
async def get_cost_recommendations(job_type: str, budget: float | None = None):
