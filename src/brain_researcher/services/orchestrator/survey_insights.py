...
class SurveyInsightsEngine:
    ...
    async def get_survey_insights(
        self, survey_id: str, insight_type: str | None, db: Session
    ) -> list[dict[str, Any]]:
    ...
    async def _analyze_sentiment(
        ...
        text_responses = []
        for response in responses:
            for _question_id, answer in response.responses.items():
    ...
    async def _assess_response_quality(
        ...
        low_quality_count = 0
        for score, issues in zip(quality_scores, per_response_issues, strict=False):
