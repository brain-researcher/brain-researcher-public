...
class FileAttachment(BaseModel):
    ...
    upload_progress: float | None = Field(
        None, ge=0, le=100, description="Upload progress percentage"
    )
    ...
    checksum: str | None = Field(None, description="SHA256 checksum of file contents")
    ...
    def validate_filename(cls, v):
        ...
        if not re.match(r"^[a-zA-Z0-9._-]+$", filename):
            raise ValueError(
                "Filename contains invalid characters. Only alphanumeric characters, dots, underscores, and hyphens are allowed."
            )
    ...
    def validate_demo_mode(self):
        ...
        if self.demo_mode and not self.cache_key:
            self.cache_key = hashlib.md5(
                json.dumps(
                    {"prompt": self.prompt, "pipeline": self.pipeline}, sort_keys=True
                ).encode()
            ).hexdigest()
        return self
...
class FileUploadRequest(BaseModel):
    ...
    def validate_content_type(cls, v):
        ...
        if v not in allowed_types:
            raise ValueError(
                f"File type {v} not allowed. Allowed types: {allowed_types}"
            )
        return v
...
class MessageRequest(BaseModel):
    ...
    metadata: dict[str, Any] = Field(default_factory=dict)
    scenario_id: str | None = Field(
        default=None, description="Optional chat scenario identifier for this turn."
    )
...
class JobLogEntry(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.utcnow)
    step_id: str | None = Field(default=None, pattern="^step_[a-zA-Z0-9_]+$")
...
class User(BaseModel):
    ...
    auth_provider: str | None = Field(
        None, description="OAuth provider: google, microsoft, github, email, password"
    )
...
class ErrorResponse(BaseModel):
        ...
        "json_schema_extra": {
            "example": {
                "error": {
                    ...
                    "context": {"request_id": "req_abc123", "endpoint": "/run"},
                    "suggestions": [
    ...
    def create(
        ...
        if code in [ErrorCode.VALIDATION_ERROR, ErrorCode.INVALID_PARAMETER]:
            error_data["documentation_url"] = (
                "https://docs.brain-researcher.ai/api/validation"
            )
        elif code in [ErrorCode.SERVICE_UNAVAILABLE, ErrorCode.SERVICE_TIMEOUT]:
            error_data["documentation_url"] = (
                "https://docs.brain-researcher.ai/api/services"
            )
    ...
    def validate_widget_positions(cls, v):
        ...
        for widget in v:
            ...
            for existing_pos in positions:
                if not (
                    pos.x >= existing_pos["x"] + existing_pos["w"]
                    or existing_pos["x"] >= pos.x + pos.w
                    or pos.y >= existing_pos["y"] + existing_pos["h"]
                    or existing_pos["y"] >= pos.y + pos.h
                ):
