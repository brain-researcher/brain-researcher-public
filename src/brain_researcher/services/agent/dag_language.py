...
class LoopConfig:
    ...
    items: str | None = None  # Parameter name for foreach
...
class DAGDefinition:
            ...
            if node.type == NodeType.CONDITIONAL:
                for branch_node in node.true_branch + node.false_branch:
                    if branch_node not in self.nodes:
                        errors.append(
                            f"Conditional node {node_id} references non-existent node {branch_node}"
                        )
            ...
            if node.type == NodeType.LOOP and node.loop_config:
                for body_node in node.loop_config.body:
                    if body_node not in self.nodes:
                        errors.append(
                            f"Loop node {node_id} references non-existent node {body_node}"
                        )
            ...
            if node.type == NodeType.PARALLEL:
                for parallel_node in node.parallel_nodes:
                    if parallel_node not in self.nodes:
                        errors.append(
                            f"Parallel node {node_id} references non-existent node {parallel_node}"
                        )

        # Validate parameters
        for param_name, param_value in self.parameters.items():
            if isinstance(param_value, str) and self._has_circular_parameter_reference(
    ...
    def _has_circular_parameter_reference(
    ...
    @classmethod
    def from_file(cls, file_path: str | Path) -> "DAGDefinition":
    ...
    def _from_dict(cls, data: dict[str, Any]) -> "DAGDefinition":
        ...
        if "global_retry_policy" in data:
            dag.global_retry_policy = cls._parse_retry_policy(
    ...
    def _parse_node(node_data: dict[str, Any]) -> DAGNode:
        ...
        if "retry_policy" in node_data:
            node.retry_policy = DAGDefinition._parse_retry_policy(
                node_data["retry_policy"]
            )
...
class ParameterResolver:
    ...
    def resolve_parameters(value: Any, context: dict[str, Any]) -> Any:
        ...
        if isinstance(value, str):
            ...
            return {
                k: ParameterResolver.resolve_parameters(v, context)
                for k, v in value.items()
            }
        elif isinstance(value, list):
            return [
                ParameterResolver.resolve_parameters(item, context) for item in value
            ]
        else:
            return value
