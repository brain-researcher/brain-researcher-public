...
class KubernetesBackend(BaseBackend):
    ...
    def _create_job_manifest(self, job_spec: JobSpecification, job_name: str) -> Dict[str, Any]:
        ...
        pod_spec = {
            ...
            'containers': [{
                ...
                'volumeMounts': [
                    {
                        'name': 'workspace',
                        'mountPath': job_spec.working_dir
                    },
                    {
                        'name': 'outputs',
                        'mountPath': job_spec.output_path
                    }
                ]
            }],
            'volumes': [
    ...
    async def get_capacity(self) -> BackendCapacity:
        ...
        try:
            ...
            queue_depth = len([job for job in jobs.items if not job.status.completion_time])
    ...
    def supports_requirements(self, requirements: ResourceRequirements) -> bool:
        ...
        return (requirements.cpu <= 64 and
                requirements.memory_gb <= 512 and
                requirements.gpu <= 8)
    ...
    def estimate_queue_time(self, requirements: ResourceRequirements) -> int:
        ...
        try:
            jobs = self.batch_api.list_namespaced_job(namespace=self.namespace)
            pending_jobs = len([job for job in jobs.items
