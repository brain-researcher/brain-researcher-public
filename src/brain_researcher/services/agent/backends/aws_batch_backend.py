...
class AWSBatchBackend(BaseBackend):
    ...
    def _generate_job_name(self, base_name: str) -> str:
        ...
        safe_name = "".join(
            c if c.isalnum() or c == "-" else "-" for c in base_name.lower()
        )
    ...
    def _create_job_definition(self, job_spec: JobSpecification) -> Dict[str, Any]:
        ...
        environment = [{"name": k, "value": v} for k, v in job_spec.environment.items()]

        # Resource requirements
        resource_requirements = []
        if job_spec.resources.gpu > 0:
            resource_requirements.append(
                {"type": "GPU", "value": str(job_spec.resources.gpu)}
            )
        ...
        if self.subnets or self.security_groups:
            container_properties["networkConfiguration"] = {"assignPublicIp": "ENABLED"}
    ...
    async def cancel_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            # Cancel the job
            self.batch_client.cancel_job(jobId=aws_job_id, reason="Cancelled by user")
            ...
            if e.response["Error"]["Code"] == "InvalidParameterValue":
                raise JobNotFoundError(f"Job {job_id} not found")
            logger.error(
                f"Failed to cancel job {job_id}: {e.response['Error']['Message']}"
            )
            return False
        except JobNotFoundError:
            raise
    ...
    async def get_logs(self, job_id: str) -> str:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "InvalidParameterValue":
                raise JobNotFoundError(f"Job {job_id} not found")
            logger.error(
                f"Error getting logs for job {job_id}: {e.response['Error']['Message']}"
            )
            return f"Error retrieving logs: {e.response['Error']['Message']}"
        except JobNotFoundError:
            raise
    ...
    async def check_health(self) -> bool:
        ...
        try:
            ...
            response = self.batch_client.describe_job_queues(jobQueues=[self.job_queue])
            ...
            logger.error(
                f"AWS Batch health check failed: {e.response['Error']['Message']}"
            )
            return False
        except Exception as e:
    ...
    async def get_capacity(self) -> BackendCapacity:
        ...
        try:
            ...
            for ce_order in compute_envs:
                ...
                if ce_response["computeEnvironments"]:
                    ...
                    if ce["type"] == "MANAGED":
                        ...
                        instance_types = compute_resources.get(
            ...
            jobs_response = self.batch_client.list_jobs(
                jobQueue=self.job_queue, jobStatus="RUNNING"
            )
            running_jobs = len(jobs_response["jobList"])

            jobs_response = self.batch_client.list_jobs(
                jobQueue=self.job_queue, jobStatus="PENDING"
            )
    ...
    def supports_requirements(self, requirements: ResourceRequirements) -> bool:
        ...
        return (
            ...
            and requirements.memory_gb <= 30000  # 30TB limit in some instances
            and requirements.gpu <= 8
            and requirements.walltime_minutes <= 24 * 60
        )  # 24 hours
    ...
    def get_cost_estimate(self, requirements: ResourceRequirements) -> float:
        ...
        gpu_cost = (
            gpu_cost_per_hour * requirements.gpu * hours if requirements.gpu > 0 else 0
        )

        # Add storage cost (EBS)
        storage_cost = (
            0.10 * requirements.storage_gb * hours / (24 * 30)
        )  # Monthly rate
