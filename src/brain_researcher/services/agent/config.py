...
def resolve_br_kg_url() -> str:
    ...
    env_url = os.getenv("BR_KG_API_URL") or os.getenv("BR_KG_URL")
    if env_url:
    ...
    raise RuntimeError(
        ...
        "Set BR_KG_API_URL (or BR_KG_URL) or ensure the service is reachable on port 5000."
    )
...
def get_config() -> dict:
    ...
    if not _config:
        ...
        try:
            _config["BR_KG_API_URL"] = resolve_br_kg_url()
        except Exception as e:
            logger.warning(f"Failed to resolve BR-KG URL: {e}")
            _config["BR_KG_API_URL"] = None
