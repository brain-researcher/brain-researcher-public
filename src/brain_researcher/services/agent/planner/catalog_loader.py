...
def _set_catalog_status(
    *,
    ...
    """Record the effective planner catalog mode for health/status surfaces."""
...
def load_tools_catalog_json(path: Optional[Path] = None) -> Dict[str, ToolSpec]:
    ...
    if is_legacy_schema:
        for entry in entries:
            # Resilience: a single malformed entry (e.g. an invalid `domain`
            # enum) must not abort the whole catalog. Aborting here propagates
            # up to _get_capability_index, whose blanket except returns an empty
            # CapabilityIndex -> the unified planner silently degrades to legacy
            # mode. Skip+log the bad row instead, mirroring the merged-schema path.
            try:
                spec = ToolSpec(**entry)
            except Exception as exc:
                logger.warning(
                    "Skipping invalid tools_catalog.json entry %r: %s",
...
def _get_capability_index(include_local_first: bool = False) -> CapabilityIndex:
    ...
    if source == "catalog":
        ...
        index = build_indexes(merged)
        tool_count = len(index.by_id)
        _set_catalog_status(
            status="ok" if tool_count else "degraded",
            planner_mode="catalog",
            loaded=True,
            tool_count=tool_count,
            source="catalog_assets",
            reason=None if tool_count else "empty_index",
        )
        return index
    else:
        ...
        _set_catalog_status(
            ...
            reason=f"BR_PLANNER_SOURCE={source}",
        )
        return CapabilityIndex()
...
def _clear_capability_index_cache() -> None:
    _get_capability_index.cache_clear()
    _set_catalog_status(
        status="unknown",
        planner_mode="unknown",
        loaded=False,
