...
class QueryAnalyzer:
    ...
    def _extract_coordinates(self, query: str) -> tuple[float, float, float] | None:
...
class GraphTraversal:
    ...
    def find_shortest_path(
        self, start_id: str, end_id: str, relation_types: list[str] | None = None
    ) -> dict[str, Any] | None:
...
class HybridSearchEngine:
    ...
    def _graph_search(
        ...
        if analysis.coordinates:
            ...
            for _i, vector_result in enumerate(vector_results[:k]):
                scored_result = ScoredResult(
