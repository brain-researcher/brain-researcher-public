...
class ToolConnector(SyncWrapperConnector):
    ...
    _registry: ToolRegistryView | None = None
    ...
    def _get_registry(self):
        ...
        if self._registry is None:
            ...
            try:
                ...
                self.__class__._registry = get_default_tool_registry(
                    auto_discover=True,
                    light_mode=True,
                )
            except Exception as e:
                raise ConnectorError(
                    self.source, f"Failed to load ToolRegistry: {e}", e
                ) from e
    ...
    async def search(
        ...
        *,
        ...
        try:
            ...
            raise ConnectorError(self.source, f"Failed to get registry: {e}", e) from e

        try:
            tools = await self._run_sync(registry.get_tools_for_task, query, k=limit)
        except Exception as e:
            raise ConnectorError(self.source, f"Search failed: {e}", e) from e
