...
def get_contrast_constructs(contrast_id):
    ...
    try:
        ...
        if not contrast_nodes:
            ...
            for node_id, _node_data in db.graph.nodes(data=True):
        ...
        result = []
        for _start, end, edge_data in relationships:
            # Get construct node
...
def get_glm_priors():
    ...
    try:
        ...
        if result.get("source") == "br_kg":
            sources.setdefault("br_kg", support)
            ...
            return jsonify(
                {
                    ...
                    "source": result.get("source", "br_kg"),
                    "support": support,
                    "coverage": coverage,
                    "literature_support": result.get("literature_support"),
