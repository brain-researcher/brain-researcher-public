...
try:
    ...
    # If cognitiveatlas package is not installed, we'll use fallback data
    get_concept = None
    get_task = None
...
def _fetch_concepts_with_cognitiveatlas(sample_size: int) -> list[dict]:
    ...
    if get_concept is None:
        raise CognitiveAtlasError("cognitiveatlas package not installed")
    ...
    try:
        ...
        logger.info(f"📊 Retrieved {len(concepts_df)} concepts from API")
        ...
        for _, row in concepts_df.iterrows():
            ...
            try:
                ...
                concept = {
                    ...
                    "url": f"https://www.cognitiveatlas.org/term/id/{row.get('id', '')}/",
                    "category": _categorize_concept(
                        str(row.get("name", "")), str(row.get("definition", ""))
                    ),
                }
...
def _fetch_tasks_with_cognitiveatlas(sample_size: int) -> list[dict]:
    ...
    try:
        ...
        logger.info(f"📊 Retrieved {len(tasks_df)} tasks from API")
        ...
        for _, row in tasks_df.iterrows():
            ...
            try:
                ...
                task = {
                    ...
                    "creation_time": row.get("creation_time"),
                    "source": "cognitive_atlas",
                    "url": f"https://www.cognitiveatlas.org/task/id/{row.get('id', '')}/",
...
class CognitiveAtlasLoader:
    """Loader for Cognitive Atlas data into BR-KG graph database"""
    ...
    def load_concepts_from_json(self, json_path: str) -> int:
        """
        Load concepts from JSON file into graph database

        Args:
            json_path: Path to concepts JSON file
        ...
        for concept in concepts:
            try:
                ...
                logger.warning(
                    f"Failed to load concept {concept.get('name', 'unknown')}: {e}"
                )
    ...
    def load_tasks_from_json(self, json_path: str) -> int:
        """
        Load tasks (TaskDef nodes) from JSON file into graph database
        ...
        for task in tasks:
            try:
                ...
                logger.warning(
                    f"Failed to load task {task.get('name', 'unknown')}: {e}"
                )

        return count

    def fetch_cognitive_atlas_data(
        self, output_dir: str, sample_size: int = 500
    ) -> dict[str, str]:
        """
        Fetch data from Cognitive Atlas API or use fallback

        Args:
            output_dir: Directory to save fetched data
            sample_size: Maximum number of items to fetch

        Returns:
            Dictionary mapping data type to file path
        """
        return fetch_cognitive_atlas_data(output_dir, sample_size)
