...
Usage (CLI):
    python -m brain_researcher.services.br_kg.etl.loaders.dataset_index_loader \\
...
class DatasetIndexLoader:
    ...
    def _resolve_storage(
        ...
        for key, path_str in local_paths.items():
            if not isinstance(path_str, str):
                continue
            if key.endswith("_dir") or key in ("cache", "processed", "bids", "br_kg"):
                continue
            key_norm = normalize_text(key)
            if key_norm in (normalize_text(dataset_id), dataset_name_norm):
                path_obj = Path(path_str).expanduser()
