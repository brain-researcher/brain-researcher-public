...
Usage:
    python -m brain_researcher.services.br_kg.etl.load_all --full
    python -m brain_researcher.services.br_kg.etl.load_all --sources ca pm
    python -m brain_researcher.services.br_kg.etl.load_all --config config.json
...
from brain_researcher.services.br_kg.etl.adapters import (
    AllenHBAAdapter,
    NeuroQueryAdapter,
    NeuroscoutAdapter,
    NiMAREAdapter,
...
from brain_researcher.services.br_kg.etl.linkers.neurostore_task_linker import (
    ConstructManager,
    NeurostoreTaskLinker,
)
from brain_researcher.services.br_kg.etl.linkers.taxonomy_linker import (
    TaxonomyLinker,
)
...
from brain_researcher.services.br_kg.etl.loaders.nidm_results_loader import (
    NIDMResultsLoader,
)
...
class MasterDataLoader:
    ...
    def __init__(
        ...
        *,
        ...
        default_db_path = db_path or os.getenv(
            "BR_KG_INGEST_CACHE_PATH", "data/br-kg/cache/ingestion_cache.db"
        )
    ...
    def _link_neurostore_metadata(self, link_tasks: bool = True) -> Dict[str, int]:
        ...
        if link_tasks:
            try:
                from brain_researcher.services.br_kg.utils.node_label_linker import (
                    NodeLabelLinker,
                )
    ...
    def load_neurosynth(
        ...
        try:
            ...
            if load_coordinates and not coordinates_df.empty:
                env_batch = os.environ.get("BR_KG_COORDINATE_BATCH_SIZE")
                ...
                if env_batch:
                    try:
                        ...
                        logger.warning(
                            "Invalid BR_KG_COORDINATE_BATCH_SIZE=%s; using %s",
                            env_batch,
                            batch_size,
                        )
    ...
    def load_neurobagel(
        ...
        try:
            ...
            if ingest_mode == "public":
                ...
                if not tsv_path:
                    cache_dir = config.get("cache_dir", "data/br-kg/raw/neurobagel")
    ...
    def load_nilearn_atlases(
        ...
        try:
            data_dir = config.get("data_dir", "data/br-kg/raw/nilearn_atlases")
