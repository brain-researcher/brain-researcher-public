...
class FusionEvaluator:
    ...
    def _calculate_confidence_metrics(self, results: List[Dict]) -> Dict[str, float]:
        ...
        return {
            ...
            'niclip_mean_confidence': np.mean(niclip_confidences) if niclip_confidences else 0,
            'confidence_spread': np.max(confidences) - np.min(confidences) if confidences else 0,
    ...
    def _calculate_coverage_metrics(self, results: List[Dict]) -> Dict[str, float]:
        ...
        concept_entropy = entropy(list(concept_counts.values())) if concept_counts else 0
        process_entropy = entropy(list(process_counts.values())) if process_counts else 0
        ...
        return {
            ...
            'avg_concepts_per_task': np.mean([len(concepts) for concepts in task_coverage.values()]) if task_coverage else 0,
    ...
    def _calculate_consistency_metrics(self, results: List[Dict]) -> Dict[str, float]:
        ...
        for task, task_results in task_groups.items():
            ...
            for result in task_results:
                concepts = set(c['name'] for c in result.get('constructs', [])[:5])
            ...
            for i in range(len(concept_sets)):
                for j in range(i+1, len(concept_sets)):
                    if concept_sets[i] or concept_sets[j]:
                        jaccard = len(concept_sets[i] & concept_sets[j]) / len(concept_sets[i] | concept_sets[j])
                        semantic_consistency.append(jaccard)

        return {
            'mean_semantic_consistency': np.mean(semantic_consistency) if semantic_consistency else 0,
            'std_semantic_consistency': np.std(semantic_consistency) if semantic_consistency else 0,
    ...
    def _calculate_validation_metrics(self, results: List[Dict]) -> Dict[str, float]:
        ...
        return {
            ...
            'direction_accuracy': direction_matches / total_validated if total_validated > 0 else 0,
        ...
        for result in results:
            ...
            if metrics.get('n_conflicts', 0) > 2:
                misalignments['high_conflict'].append({
                    ...
                    'constructs': [c['name'] for c in result.get('constructs', [])[:3]]
                })
    ...
    def _calculate_summary_scores(self, metrics: Dict[str, Dict]) -> Dict[str, float]:
        ...
        confidence_score = 1 - abs(metrics['confidence']['mean_confidence'] - 0.7)  # Optimal around 0.7
        coverage_score = min(metrics['coverage']['n_unique_concepts'] / 100, 1.0)  # Normalize
    ...
    def _generate_evaluation_plots(self, metrics: Dict, timestamp: str):
        ...
        ax = axes[0, 0]
        alignment_data = [m['metrics']['alignment']['mean_alignment'] for m in self.evaluation_results[-10:] if 'metrics' in m]
        if alignment_data:
        ...
        ax.set_ylabel('Score')
        ax.set_ylim(0, 1)
        ax.axhline(y=summary['overall_score'], color='r', linestyle='--', label='Overall')
        ax.legend()
