...
class CognitiveAnnotationFusion:
    ...
    def fuse_annotations(
        ...
        niclip_result: dict[str, Any] | None = None,
        mni_coordinates: list[tuple[float, float, float]] | None = None,
            ...
            try:
                ...
                if glm_validation.get("validation_available"):
                    for construct, alignment in zip(
                        fused_constructs[:5], glm_validation["alignments"], strict=False
                    ):
...
def get_fusion_module(config: dict[str, Any] | None = None):
