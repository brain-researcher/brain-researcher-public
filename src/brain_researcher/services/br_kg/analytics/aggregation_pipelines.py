...
class GroupBySpec:
    ...
    hierarchy: list[str] | None = None  # For rollup operations
    custom_sets: list[list[str]] | None = None  # For grouping sets
...
class StatisticalAnalyzer:
    ...
    def _calculate_correlation(x: list[float], y: list[float]) -> float:
        ...
        x_clean, y_clean = zip(*pairs, strict=False)
...
class AggregationPipeline:
        ...
        try:
            with self.neo4j_db.session() as session:
                ...
                analytics_results["summary"] = {
                    ...
                    "edge_types": list({edge["edge_type"] for edge in edges}),
    ...
    def _execute_node_aggregation_stage(
        self, stage: PipelineStage, filters: dict[str, Any] | None
    ) -> dict[str, Any]:
    ...
    def _sum_aggregation(self, values: list[int | float]) -> int | float:
        """Sum numeric values."""
        numeric_values = [v for v in values if isinstance(v, int | float)]
        return sum(numeric_values)

    def _avg_aggregation(self, values: list[int | float]) -> float:
        """Average of numeric values."""
        numeric_values = [v for v in values if isinstance(v, int | float)]
        return sum(numeric_values) / len(numeric_values) if numeric_values else 0.0

    def _min_aggregation(self, values: list[int | float]) -> int | float:
        """Minimum of numeric values."""
        numeric_values = [v for v in values if isinstance(v, int | float)]
        return min(numeric_values) if numeric_values else 0

    def _max_aggregation(self, values: list[int | float]) -> int | float:
        """Maximum of numeric values."""
        numeric_values = [v for v in values if isinstance(v, int | float)]
        return max(numeric_values) if numeric_values else 0

    def _stddev_aggregation(self, values: list[int | float]) -> float:
    ...
    def _median_aggregation(self, values: list[int | float]) -> float:
    ...
    def _get_cached_result(self, pipeline_id: str) -> AggregationResult | None:
