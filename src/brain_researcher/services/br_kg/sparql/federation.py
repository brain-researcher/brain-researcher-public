...
class FederationQueryHandler:
    ...
    def execute_federated_query(
        ...
        try:
            ...
            logger.info(
                "Federated query executed successfully in %.2fs", execution_time
            )
    ...
    def _create_federation_plan(self, parsed_query) -> dict[str, Any]:
        ...
        local_query = self._extract_local_query_part(
            query_str, plan["external_queries"]
        )
        if local_query:
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                for endpoint_name, query in query_plan["external_queries"].items():
                    future = executor.submit(
                        self._execute_external_query, endpoint_name, query
                    )
                ...
                for future in as_completed(future_to_endpoint):
                    ...
                    try:
                        ...
                        logger.error(
                            "External query failed for %s: %s", endpoint_name, str(e)
                        )
        ...
        for attempt in range(endpoint_config["max_retries"]):
            try:
                result = self._execute_sparql_request(
                    endpoint_config["url"], full_query, endpoint_config["timeout"]
                )
                ...
                logger.warning(
                    "Attempt %d failed for %s: %s", attempt + 1, endpoint_name, str(e)
                )
    ...
    def _execute_sparql_request(
        self, endpoint_url: str, query: str, timeout: int
    ) -> dict[str, Any]:
    ...
    def _merge_federated_results(
        self, results: dict[str, Any], query_plan: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _join_results(
        self, results: dict[str, Any], join_variables: set[str]
    ) -> dict[str, Any]:
        ...
        for i in range(1, len(result_list)):
            joined_bindings = self._join_binding_sets(
                joined_bindings, result_list[i].get("bindings", []), join_variables
            )
    ...
    def _extract_wikidata_patterns(self, query: str) -> str | None:
    ...
    def _extract_dbpedia_patterns(self, query: str) -> str | None:
    ...
    def _extract_local_query_part(
        self, query: str, external_queries: dict[str, str]
    ) -> str | None:
        ...
        # Clean up the query
        local_query = re.sub(
            r"SERVICE\s*<[^>]+>\s*\{[^}]*\}", "", local_query, flags=re.IGNORECASE
        )
    ...
    # Utility methods
    def _url_to_endpoint_name(self, url: str) -> str | None:
    ...
    def _get_cached_result(self, cache_key: str, ttl: int) -> dict[str, Any] | None:
    ...
    def _cache_result(self, cache_key: str, result: dict[str, Any]):
        """Cache query result"""
        self.federation_cache[cache_key] = {"result": result, "timestamp": time.time()}
    ...
    def _check_rate_limit(self, endpoint_name: str):
        ...
        self.query_count[endpoint_name] = [
            query_time
            for query_time in self.query_count[endpoint_name]
        ...
        if len(self.query_count[endpoint_name]) >= rate_limit:
            ...
            if sleep_time > 0:
                logger.info(
                    "Rate limit reached for %s, sleeping for %.1fs",
                    endpoint_name,
                    sleep_time,
                )
