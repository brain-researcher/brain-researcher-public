...
from brain_researcher.services.llm_gateway.codegen.context import FileSnippet
...
class GeminiFsClient(Protocol):
    ...
    async def search_text(
        self, query: str, root: str, max_results: int = 200
    ) -> Sequence[dict]: ...

    async def read_file(
        self, path: str, max_bytes: int = 8000, offset: int = 0
    ) -> str: ...
...
async def build_fs_context_for_task(
    ...
    root = repo_root or "."
    try:
        hits = await fs_client.search_text(
            query=query, root=root, max_results=max_files * 6
        )
    except Exception:
        return []
