...
from brain_researcher.services.llm_gateway.codegen.constitution import (
...
def build_prompt(
    context: CodegenContext, mode: Literal["fresh", "repair"] = "fresh"
) -> str:
    ...
    if context.code_context:
        sections.append(
            _format_section(
                "Current Code Context", _truncate(context.code_context, 4000)
            )
        )
    ...
    if context.plan_steps:
        plan_lines = [
            ...
            for i, s in enumerate(context.plan_steps)
        ]
    ...
    if context.kg_info:
        sections.append(
            _format_section("Knowledge Graph", _truncate(context.kg_info, 2000))
        )
    ...
    if constraints.get("max_iters"):
        constraint_lines.append(
            f"- Max iterations remaining: {constraints['max_iters']}"
        )
