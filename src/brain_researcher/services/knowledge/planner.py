...
class EvidenceAggregator:
    ...
    async def gather(
        ...
        ) -> tuple[EvidenceSourceType, list[EvidenceResult], str | None]:
    ...
    def _add_to_bundle(self, bundle: EvidenceBundle, item: EvidenceResult) -> None:
        ...
        if item.source == EvidenceSourceType.KNOWLEDGE_GRAPH:
            ...
            if (
                item.payload.get("is_brain_region")
                or item.payload.get("node_type") == "BrainRegion"
            ):
...
class KnowledgePlanner:
    ...
    async def _generate_dataset_plan(
        ...
        try:
            ...
            return KnowledgePlan(
                ...
                dataset_scores=dict.fromkeys(fallback_datasets, 0.5),
                citations=self._build_citations(bundle),
                confidence=0.4,
                evidence_bundle=bundle,
            )
    ...
    async def build_plan(
        ...
        force_intent: DecisionType | None = None,
        use_llm: bool = True,
    ) -> KnowledgePlan:
