...
def launch_agent_service(
    ...
    # Check for BR-KG API
    br_kg_url = env.get("BR_KG_API_URL", "http://localhost:5000")
    try:
        import urllib.request

        with urllib.request.urlopen(f"{br_kg_url}/health", timeout=2) as response:
            if response.status == 200:
                console.print(f"[green]✓ BR-KG API found at {br_kg_url}[/green]")
    except Exception:
        console.print(f"[yellow]⚠ BR-KG API not found at {br_kg_url}[/yellow]")
        console.print(
