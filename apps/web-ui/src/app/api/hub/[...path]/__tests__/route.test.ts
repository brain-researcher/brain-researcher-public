...
describe('/api/hub/[...path] route', () => {
  afterEach(() => {
    vi.restoreAllMocks()
  })

  it('proxies hub session create requests to orchestrator', async () => {
    ...
    const request = new NextRequest('http://localhost/api/hub/sessions', {
      method: 'POST',
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'http://orchestrator/api/hub/sessions',
  ...
  it('preserves query strings for hub session reads', async () => {
    ...
    const request = new NextRequest(
      'http://localhost/api/hub/sessions?project_id=proj_demo&limit=5',
    )
    const response = await GET(request, { params: { path: ['sessions'] } })

    expect(response.status).toBe(200)
    expect(fetchMock).toHaveBeenCalledWith(
      'http://orchestrator/api/hub/sessions?project_id=proj_demo&limit=5',
      expect.objectContaining({
        method: 'GET',
        cache: 'no-store',
        headers: expect.any(Headers),
  ...
  it('returns 502 when the hub gateway is unreachable', async () => {
    ...
    expect(await response.json()).toEqual({ detail: 'Hub gateway temporarily unavailable' })
