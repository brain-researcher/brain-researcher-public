...
describe('/api/config route', () => {
  ...
  it('preserves explicit public overrides when configured', async () => {
    ...
    vi.stubEnv('NEXT_PUBLIC_ORCHESTRATOR_URL', 'https://${PUBLIC_HOSTNAME}')
    ...
    const data = await response.json()
    expect(data.services.agent).toBe('https://${PUBLIC_HOSTNAME}/internal-agent')
    expect(data.services.orchestrator).toBe('https://${PUBLIC_HOSTNAME}')
    expect(data.services.websocket).toBe('wss://${PUBLIC_HOSTNAME}/ws')
    expect(data.health.orchestrator).toBe('https://${PUBLIC_HOSTNAME}/health')
  })
