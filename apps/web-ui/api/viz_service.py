...
def get_mni_template(resolution: str = "2mm") -> nib.Nifti1Image:
    ...
    if cache_key not in _template_cache:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to load MNI152 template: {e}"
            )
...
async def get_mni152_template_endpoint(
    ...
    try:
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=mni152_{resolution}.nii.gz"
            },
        )
    except Exception as e:
    ...
    try:
        ...
        if request.threshold_min is not None or request.threshold_max is not None:
            ...
            processed_img = nib.Nifti1Image(
                data, processed_img.affine, processed_img.header
            )
                ...
                try:
                    ...
                    if len(voxel_sizes) == 3:
                        processed_img = image.resample_img(
                            processed_img, target_affine=np.diag(voxel_sizes + [1])
                        )
...
async def create_statistical_map(
    ...
    try:
        ...
        return StreamingResponse(
            ...
            media_type="application/octet-stream",
            headers={
                "Content-Disposition": f"attachment; filename=stat_map_{file.filename}"
            },
        )
...
async def export_animation(
    ...
    if not OPENCV_AVAILABLE:
        raise HTTPException(
            status_code=503, detail="OpenCV not available for video export"
        )
    ...
    try:
        ...
        if len(files) == 1:
            ...
            if img.ndim == 4:
                ...
                raise HTTPException(
                    status_code=400, detail="Single file must be 4D volume"
                )
        else:
        ...
        try:
            ...
            return StreamingResponse(
                ...
                headers={
                    "Content-Disposition": f"attachment; filename=brain_animation.{request.format}"
                },
            )
...
async def get_volume_info(file: UploadFile = File(...)):
    ...
    try:
        ...
        info = {
            ...
            "coordinate_system": (
                "RAS+" if nib.aff2axcodes(img.affine) == ("R", "A", "S") else "other"
            ),
        }
