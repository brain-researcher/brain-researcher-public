#!/usr/bin/env python3
"""Portable reproduction verifier for Brain Researcher reproducibility packs.
...
synthetic schema exemplar) — reported as ``indeterminate``, never a failure.

Exit codes: 0 reproduced · 1 mismatch/missing · 2 cannot verify (no manifest).
...
def _verify_manifest(pack_dir: Path) -> dict:
    ...
    if not manifest_path.exists():
        return {"reproduced": None, "reason": "no manifest.json", "artifacts": []}
    ...
    for e in entries:
        ...
        if e.get("schema_only") or expected is None:
            rows.append({"path": rel, "status": "indeterminate"})
            n_indet += 1
            continue
    ...
    reproduced = n_mismatch == 0 and n_missing == 0 and n_match > 0
    return {
...
def main(argv: list[str] | None = None) -> int:
    ...
    if result.get("reproduced") is True:
        return 0
    if result.get("reproduced") is None:
        return 2
    return 1
