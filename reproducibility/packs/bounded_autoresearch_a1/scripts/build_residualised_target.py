...
# This step needs HCP-YA behavioral data, which is NOT redistributed here.
# Stage it yourself under the HCP Open Access Data Use Terms (a click-through
# agreement — https://db.humanconnectome.org): a Liu-style component behavior
# CSV (``liu_component_behavior.csv``, subject-keyed 5-component targets,
# rebuildable with brain_researcher.research.predictive.liu_component_projection)
# and the raw HCP-YA subjects export. Point --behavior-dir / $A1_BEHAVIOR_DIR at
# the directory containing both. Running the pack's shipped, de-identified target
# does NOT require this step — see scripts/run_prediction.py.
_BEHAVIOR_DIR = os.environ.get("A1_BEHAVIOR_DIR", "")
_COMPONENT_CSV_NAME = os.environ.get("A1_COMPONENT_CSV", "liu_component_behavior.csv")
_HCP_CSV_NAME = os.environ.get("A1_HCP_SUBJECTS_CSV", "HCP_YA_subjects.csv")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Rebuild the intelligence-residualised Cognition target from staged "
            "HCP-YA behavior (Open Access). Optional deeper-provenance step; the "
            "pack ships the de-identified result so run_prediction.py needs no HCP data."
        )
    )
    ap.add_argument(
        "--behavior-dir",
        default=_BEHAVIOR_DIR or None,
        required=not _BEHAVIOR_DIR,
        help="Directory holding the staged HCP-YA CSVs "
        f"($A1_BEHAVIOR_DIR; expects {_COMPONENT_CSV_NAME} and {_HCP_CSV_NAME}).",
    )
    ap.add_argument(
        "--component-csv",
        default=_COMPONENT_CSV_NAME,
        help="Component behavior CSV filename.",
    )
    ap.add_argument(
        "--hcp-csv", default=_HCP_CSV_NAME, help="Raw HCP-YA subjects CSV filename."
    )
    ap.add_argument(
        "--out-dir",
        default=str(HERE.parent / "artifacts"),
        help="Where to write the residualised CSV + provenance JSON (default: pack artifacts/).",
    )
    args = ap.parse_args()
    behavior_dir = Path(args.behavior_dir)
    global BEHAVIOR_CSV, HCP_CSV, OUT_CSV, OUT_JSON
    BEHAVIOR_CSV = behavior_dir / args.component_csv
    HCP_CSV = behavior_dir / args.hcp_csv
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    OUT_CSV = out_dir / "liu_component_behavior_residualised_cognition.csv"
    OUT_JSON = out_dir / "residualised_target_provenance.json"
